		...
		struct pinctrl_map **map, u32 *num_maps)
{
	...
	*map = kzalloc(temp_32, GFP_KERNEL);
	...
	group = devm_kstrdup(pctl->dev, np->name, GFP_KERNEL);

	prop_ret = of_find_property(np, "telechips,pin-function", NULL);
	if (prop_ret != NULL) {
		function = devm_kstrdup(pctl->dev, np->name, GFP_KERNEL);
...
#else
		s32 cmp_ret = strncmp("gpk", bank->name, 3);
		...
		struct tcc_pinctrl *pctl)
{
	...
	for_each_child_of_node(np, (child)) {
		if(of_device_is_available(np) == true) {
...
static int tcc_gpio_set_drive_strength_axon(void __iomem *base, u32 offset, int ds)
{
	void __iomem *reg = base + GPIO_DRIVE_STRENGTH + ((offset / 8) << 2U);
	u32 mask, shift;
	s32 ds_value = ds;
	u32 data;
...
static int tcc_gpio_get_drive_strength_axon(void __iomem *base, u32 offset)
{
	void __iomem *reg = base + GPIO_DRIVE_STRENGTH + ((offset / 8) << 2U);
	u32 shift;
	u32 data;
	...
	data = (readl(reg) >> shift) & 0xfU;
	return (s32)data;
}
#else
#if defined(CONFIG_ARCH_TCC807X)
#define TCC_GPIO_DS_MAX 6
#else
#define TCC_GPIO_DS_MAX 3
#endif
					   ...
					   int value)
{
	...
	reg = reg - base_offset_sc;
	ret = request_gpio_to_sc((phys_addr_t)reg, (ulong)bit_num, 2U, (ulong)value);

#if defined(CONFIG_ARCH_TCC807X)
	if (ret == 0) {
		reg = base + GPIO_DRIVE_STRENGTH2;
		reg = reg - base_offset_sc;
		ret = request_gpio_to_sc((phys_addr_t)reg, offset, 1U, (((ulong)value >> 2UL) & 1UL));
	}
#endif
	return ret;
#else
...
static int tcc_gpio_get_drive_strength(void __iomem *base, u32 offset)
{
	...
	ds2_data = readl(reg);
	ds2_data = (ds2_data >> offset) & GPIO_DS2_MASK;
	data |= (ds2_data << 2);
#endif
...
#define TCC_MFIOSEL			(0x600U)
#define TCC_MFIO_MAX_NUM	(41U)
int tcc_gpio_set_mfio(void __iomem *base, u32 offset, s32 value, struct tcc_pinctrl *pctl)
{
	void __iomem *reg = pctl->base + TCC_MFIOSEL + ((value/4U)*0x4U);
	...
	if (value > TCC_MFIO_MAX_NUM) {
		return -EINVAL;
		/* comment for QAC, codesonar, kernel coding style */
	}
	for (i = 0; i < pctl->nbanks ; i++) {
		if (bank->reg_base == port) {
			if (bank->mfio_num == 0xffU) {
						(void)pr_err("[ERROR][ECLK] %s: %s is not supported for mfio\n", __func__, bank->name);
				return -EINVAL;
			}
			for (j = 0; j < bank->mfio_num; j++) {
				if (((UINT_MAX) - bank->mfio_offset_base[j])
						< bank->mfio_range[j]) {
					continue;
				}
				if ((offset >= bank->mfio_offset_base[j])
					&& (offset <
						(bank->mfio_offset_base[j]
						+ bank->mfio_range[j]))) {
	...
	if (pin_valid == 0U) {
		(void)pr_err(
				"[ERROR][MFIO] %s: %d(%d) is out of range of pin number of %s group\n"
				, __func__, offset, idx, bank->name);
		return -EINVAL;
	}

	data = readl(reg);
	data &= ~((u32)0xFFU << ((u32)value % 4U)*8);
	data |= (((u32)0xFFU & idx) << ((u32)value % 4U)*8);
	writel(data, reg);
...
#elif defined(CONFIG_ARCH_TCN100X)
	unsigned int slew_reg = GPIO_SLEW_RATE + ((offset / 8) << 2U);
	unsigned int slew_offset = ((offset % 8) << 2U);
	tcc_gpio_pinconf_extra(base, slew_offset, value, slew_reg);
#endif
}
...
#if defined(CONFIG_ARCH_TCN100X)
		ret = tcc_gpio_get_drive_strength_axon(base, offset);
#else
...
#if defined(CONFIG_ARCH_TCC750X)
#define TCC_ECLKSEL		(0x660U)
#elif defined(CONFIG_ARCH_TCN100X)
#define TCC_ECLKSEL		(0x600U)
#else
...
int tcc_gpio_set_eclk_sel(void __iomem *base, u32 offset, s32 value, struct tcc_pinctrl *pctl)
{
	...
	u32 j;
	u32 pin_valid = 0U;
#if !defined(CONFIG_PINCTRL_TCC_SCFW) && !defined(CONFIG_ARCH_TCN100X)
	u32 data;
#endif
			...
			int config, struct tcc_pinctrl *pctl)
{
	...
	case TCC_PINCONF_PARAM_DRIVE_STRENGTH:
#if defined(CONFIG_ARCH_TCN100X)
		if (tcc_gpio_set_drive_strength_axon(base, offset, config) < 0)
#else
	...
	case TCC_PINCONF_PARAM_ECLK_SEL:
		...
		break;
#if defined(CONFIG_ARCH_TCC750X)
	case TCC_PINCONF_PARAM_MFIO:
		(void)tcc_gpio_set_mfio(base, offset, config, pctl);
		break;
#endif
	}
	return 0;
...
#if defined(CONFIG_ARCH_TCC750X)
	{"telechips,mfio", TCC_PINCONF_PARAM_MFIO},
#endif
};
...
static int tcc_pinctrl_probe(struct platform_device *pdev)
{
	...
	pctl->pin_configs = tcc_pin_configs;
	pctl->nconfigs = ARRAY_SIZE(tcc_pin_configs);
...
#if defined(CONFIG_ARCH_TCC750X)
			ret = of_property_read_u32_index(np, "mfio-info", 0,
					&bank->mfio_num);

			if (ret > 0) {
				dev_err(&(pdev->dev),
						"[ERROR][PINCTRL] failed to get mfio section\n"
					   );
				return -EINVAL;
			}
			if (bank->mfio_num != 0xffU) {
				bank->mfio_offset_base = devm_kzalloc(&pdev->dev, sizeof(u32), GFP_KERNEL);
				bank->mfio_base = devm_kzalloc(&pdev->dev, sizeof(u32), GFP_KERNEL);
				bank->mfio_range = devm_kzalloc(&pdev->dev, sizeof(u32), GFP_KERNEL);
				for (i = 0U; i < bank->mfio_num; i++) {
					ret = of_property_read_u32_index(np,
							"mfio-info", ((i * 3) + 1),
							&bank->mfio_offset_base[i]);
					if (ret > 0) {

						dev_err(&(pdev->dev),
								"[ERROR][PINCTRL] failed to get mfio offset base\n"
							   );
						return -EINVAL;
					}
					...
					if (ret > 0) {
						dev_err(&(pdev->dev),
								"[ERROR][PINCTRL] failed to get mfio base\n"
							   );
						return -EINVAL;
					}
					...
					if (ret > 0) {
						dev_err(&(pdev->dev),
								"[ERROR][PINCTRL] failed to get mfio range\n"
							   );
						return -EINVAL;
					}
				}
