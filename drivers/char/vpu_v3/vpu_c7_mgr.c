...
static const int VPU_C7_NUM_OF_BITSTREAM_BUFFERS = 2;
...
static vpu_mgr_t* vpu_c7_mgr_ctx = NULL;
...
static int vmgr_c7_dec_init(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	dlog_vpuc7("[id:%u] dbg type:%d, mask:%d", drv_id, dbg_type, dbg_mask);

	if (dbg_type == VPU_ALWAYS_PRT_DBG)
	{
		vetc_memset(&ip_param->dec_log, 0x00, sizeof(vpu_dec_ctrl_log_status_t), 0);
		...
		dlog_vpuc7("[id:%u] VPU_CTRL_LOG_STATUS", drv_id);
		ret = tcc_vpu_dec_l(vpu_ap, VPU_CTRL_LOG_STATUS, (codec_handle_t *)NULL, (void*)&ip_param->dec_log, NULL);
		dlog_vpuc7("[id:%u] VPU_CTRL_LOG_STATUS ret:%d", drv_id, ret);
	}
...
static int vmgr_c7_dec_decode(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	else
	{
		...
		if((drv_info->enabled_ringbuffer_mode == 0U) && (drv_info->is_interlace_video == 1))
		{
			//In the case of interlace, only the first field is stored, with the bitstream size inputted as 0
			arg_decode_out->out_info.decoded_status = VPU_DEC_STAT_SUCCESS_FIELD_PICTURE;
			arg_decode_out->out_info.display_idx = -1;
			arg_decode_out->out_info.decoded_idx = -1;
			arg_decode_out->out_info.pic_type = VPU_PICTURE_UNKNOWN;
...
static int vmgr_c7_enc_init(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	if (dbg_type == VPU_ALWAYS_PRT_DBG)
	{
		vetc_memset(&ip_param->enc_log, 0x00, sizeof(vpu_dec_ctrl_log_status_t), 0);
		...
		dlog_vpuc7("[id:%u] VPU_CTRL_LOG_STATUS", drv_id);
		ret = tcc_vpu_enc_l(vpu_ap, VPU_CTRL_LOG_STATUS, (codec_handle_t *)NULL, (void*)&ip_param->enc_log, NULL);
		dlog_vpuc7("[id:%u] VPU_CTRL_LOG_STATUS ret:%d", drv_id, ret);
	}
...
static int vmgr_c7_get_buffer_size(void* vpu_private, enum vmgr_buffer_type buf_type, vpu_drv_info_t* drv_info)
{
	...
	switch(buf_type)
	{
		...
		{
			// what does it mean ?
			size = 17 * 4 * ((drv_info->initial_info.pic_width * drv_info->initial_info.pic_height) >> 8);
...
static vpu_ip_module_t vpu_c7_module = {
	...
	.dec_capa = {{VCODEC_ID_AVC, CODEC_NAME_AVC, "high", "4.2", 1920, 1080, 60},
				...
				{VCODEC_ID_DIVX, CODEC_NAME_DIVX, "DivX Plus HD profile (v7.4)", NULL, 1920, 1080, 60},
...
int vmgr_c7_remove(struct platform_device *pdev)
{
