...
enum vpu_ip_type vmgr_find_ip_type(unsigned int drv_id, const enum vpu_codec_id codec_id, enum vpu_op_type op_type)
{
	int mgr_idx;
	int pcap_idx;
				...
				for(pcap_idx=0; pcap_idx<MAX_SUPPORT_CODEC; pcap_idx++)
				{
					...
					if(codec_name != NULL)
					{
						if(strstr(pcap[pcap_idx].support_codec, codec_name) != NULL)
						{
							...
							detail_vmctx("%s:%u, found codec with type:%s(%d) in IP:%s(%d), pixel_product:%lu, min_pixel_product:%lu",
