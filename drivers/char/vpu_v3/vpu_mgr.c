...
static vpu_accesspoint_t* vmgr_accesspoint_get_addr_mem(enum vpu_ip_type ip_type)
{
	...
	if(accesspoint != NULL)
	{
		va = ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * addr_offset), SHARD_POINT_GAP);
...
vpu_cmd_t* vmgr_list_manager_show_result(vpu_mgr_t* mgr_ctx, const enum vpu_op_type op_type, const unsigned drv_id)
{
	vpu_cmd_t* cmd = NULL;
...
vpu_cmd_t* vmgr_list_manager_get_result(vpu_mgr_t* mgr_ctx, const enum vpu_op_type op_type, const unsigned drv_id)
{
	vpu_cmd_t* cmd = NULL;
...
int vmgr_list_manager_result_remain(vpu_mgr_t* mgr_ctx, const enum vpu_op_type op_type, const unsigned drv_id)
{
	int count = 0;
...
vpu_cmd_t* vmgr_get_pending_escapable(vpu_mgr_t* mgr_ctx, vpu_drv_info_t* drv_info)
{
	vpu_cmd_t* cmd = NULL;
...
void vmgr_wakeup_user_poll(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd)
{
...
vpu_cmd_t* vmgr_cmd_delay_for_interlace(vpu_mgr_t* mgr_ctx)
{
	vpu_cmd_t* cmd = NULL;
	vmgr_comm_t* cmdQueue = &mgr_ctx->comm_data;
...
int vmgr_deregister(vpu_mgr_t* mgr_ctx, vpu_drv_info_t* drv_info)
{
	...
	if((mgr_ctx != NULL) && (drv_info != NULL))
	{
		...
		if (atomic_read(&mgr_ctx->dev_opened) == 0)
		{
			...
			(void)atomic_set(&mgr_ctx->oper_intr, 0);
...
#if defined(ENABLE_INTERLACE_DELAY_PROCESS)
		if (drv_info->enable_avoid_pending == 1)
		{
			drv_info->enable_avoid_pending = 0U;
			...
			vpu_dllist_destroy(drv_info->delay_queue); //queue node에 argument 가 alloc 되어 있으면 해지를 해야 함
...
int vmgr_init(vpu_mgr_t* mgr_ctx, const char* mgr_name)
{
	int ret = 0;
...
int vmgr_deinit(vpu_mgr_t* mgr_ctx)
{
	int ret = 0;
	vmgr_clock_t* clock_ctrl = mgr_ctx->each_ip->clock_ctrl;
