...
#include "vpu_dbg_info.h" //DBG_INFO
...
static struct vrm *vrm_find_vinfo_by_vidx(s32 idx)
{
	...
	if ((idx >= (s32)video) && (idx < (s32)MAX_VRM_VMEM)) {
		...
		if (entry->info.base != VRM_FREED) {
			...
			V_DBG(VPU_DBG_MEMORY, "Invaild vpu info.");
		}
	} else {
...
#else
	if ((codec >= STD_AVC) && (codec <= STD_HEVC_ENC)) { //STD_AVC 0 , STD_HEVC_ENC2 18
#endif
		switch (codec) {
#ifdef CONFIG_SUPPORT_TCC_WAVE420L_VPU_HEVC_ENC
		case STD_HEVC_ENC:
			idx = wave420l;
			V_DBG(VPU_DBG_DETAIL, "codec:%d(STD_HEVC_ENC), ret index:%d", codec, idx);
			break;
#endif
#ifdef CONFIG_SUPPORT_TCC_WAVE420L_2ND_VPU_HEVC_ENC
		case STD_HEVC_ENC2:
...
EXPORT_SYMBOL(vrm_check_index_availability);
...
void vrm_clear_instance(s32 idx)
{
	...
	if ((idx >= 0) && (idx < imax_vrm_vmem)) {
		...
		if (info != NULL) {
			if (info->rc != 0U) {
				V_DBG(VPU_DBG_ERROR,
				"vpu failed to clear instance-#%d.",
				idx);
			}
			info->reserved = 0U;
...
EXPORT_SYMBOL(vmem_is_index_in_use);
...
int vmem_set_instance(int idx)
{
	...
	(void)mutex_lock(&vrm_mutex);
	ret = vrm_set_instance(idx);
	(void)mutex_unlock(&vrm_mutex);

	return ret;
}
EXPORT_SYMBOL(vmem_set_instance);
...
EXPORT_SYMBOL(vmem_get_instance);
...
EXPORT_SYMBOL(mem_check_instance_available);
...
EXPORT_SYMBOL(vmem_clear_instance);
