...
const char* vdec_ioctl_name(const int cmd)
{
...
int vdec_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	{
		err_vdec("dec_id:%u, %s :: mmap :: vm_address_range failed : start_addr(0x%u), end_addr(0x%u)", vdata->info.drv_id, vdata->dec_shared->misc.name, vma->vm_start, vma->vm_end);
		ret = (int)-EAGAIN;
	}
	else
	{
...
static int vdec_register_info(vpu_dec_drv_t *vdata)
{
	int ret = 0;

	vetc_mutex_lock(&vdata->dec_shared->shared_mutex);

	vdata->info.ip_type = vmgr_find_ip_type(vdata->info.drv_id, vdata->info.codec_id, vdata->info.op_type);
	dlog_vdec("dec_id:%u, codec_id:%d, ip_type:%s(%d)", vdata->info.drv_id, vdata->info.codec_id, vmgr_get_ip_name(vdata->info.ip_type), vdata->info.ip_type);
...
int vdec_open(struct inode *pinode, struct file *filp)
{
	int ret = 0;
...
int vdec_release(struct inode *pinode, struct file *filp)
{
	int dec_id = 0;
	vpu_dec_drv_t *vdata = (vpu_dec_drv_t *)filp->private_data;
