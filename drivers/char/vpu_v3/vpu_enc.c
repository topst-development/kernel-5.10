...
const char* venc_ioctl_name(const int cmd)
{
...
int venc_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	{
		err_venc("enc_id:%u, %s :: mmap :: vm_address_range failed : start_addr(0x%u), end_addr(0x%u)", vdata->info.drv_id, vdata->enc_shared->misc.name, vma->vm_start, vma->vm_end);
		ret = (int)-EAGAIN;
	}
	else
	{
...
static int venc_register_info(vpu_enc_drv_t *vdata)
{
	int ret = 0;
	...
	dlog_venc("enc_id:%u,codec_id:%d, ip_type:%s(%d)", vdata->info.drv_id, vdata->info.codec_id, vmgr_get_ip_name(vdata->info.ip_type), vdata->info.ip_type);
