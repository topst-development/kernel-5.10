...
#define dlog_henc(msg...)  		V_DBG(VPU_DBG_INFO, "[HEVC_ENC][INFO]: " msg)
#define detail_henc(msg...)  	V_DBG(VPU_DBG_DETAIL, "[HEVC_ENC][DETAIL]:" msg)
#define seq_henc(msg...)     	V_DBG(VPU_DBG_SEQUENCE, "[HEVC_ENC][SEQ]: " msg)
#define err_henc(msg...)      	V_DBG(VPU_DBG_ERROR, "[HEVC_ENC][ERR]: " msg)
...
typedef struct vpu_hevc_enc_papam_t
{
...
} vpu_hevc_enc_papam_t;

static vpu_mgr_t* vpu_hevc_enc_mgr_ctx = NULL;

//static unsigned int cntInt_vpu_he;	// = 0;
static void __iomem *hevc_enc_vidsys_conf_reg = NULL;
...
#define HEVC_ENC_ACCESSPOINT_PATH	 "/proc/hevc_enc"

#if !defined(USE_ACCESS_POINT)
#if DEFINED_CONFIG_VENC
extern int tcc_vpu_hevc_enc(int Op, codec_handle_t *pHandle, void *pParam1, void *pParam2);
#endif
...
static int tcc_vpu_hevc_enc_l(vpu_accesspoint_t* vpu_ap, int Op, codec_handle_t *pHandle, void *pParam1, void *pParam2) VPU_NO_SANITIZE_CFI
{
...
static int vmgr_hevc_enc_internal_handler(void)
{
	...
	vpu_mgr_t* mgr_ctx = vpu_hevc_enc_mgr_ctx;
...
static int vmgr_hevc_enc_init(vpu_mgr_t* mgr_ctx,vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	...
	vpu_hevc_enc_papam_t* ip_param = (vpu_hevc_enc_papam_t*)drv_info->ip_param;
	...
	dlog_henc("[id:%u] VPU_ENC_INIT start", drv_id);
	...
	dlog_henc("[id:%u] Init In =>Memcpy(0x%px),Memset(0x%px),Interrupt(0x%px),remap(0x%px),unmap(0x%px),read(0x%px),write(0x%px),sleep(0x%px)||workbuff(0x%px/0x%px),Reg(0x%px/0x%px), format(%d),W:H(%d:%d),Fps(%d),Bps(%d),Keyi(%d),Stream(0x%px/0x%px, %d), interleave:%d",
		drv_id,
		pEncInit->m_Memcpy,
		pEncInit->m_Memset,
	...
	dlog_henc("[id:%u] dbg type:%d, mask:%d", drv_id, dbg_type, dbg_mask);
	...
	if (dbg_type == VPU_ALWAYS_PRT_DBG)
	{
		...
		dlog_henc("[id:%u] VPU_HEVC_ENC_CTRL_LOG_STATUS", drv_id);
		ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_HEVC_ENC_CTRL_LOG_STATUS, (codec_handle_t *)NULL, (void*)&ip_param->enc_log, NULL);
		dlog_henc("[id:%u] VPU_HEVC_ENC_CTRL_LOG_STATUS ret:%d", drv_id, ret);
	}

	ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_ENC_INIT, (codec_handle_t *)&encHandle, (void *)pEncInit, (void *)pEncInitialInfo);
	if (ret != RETCODE_SUCCESS)
	{
		err_henc("[id:%u] Init failed with ret(0x%x)", drv_id, ret);
	...
	dlog_henc("[id:%u] init Done Handle(0x%x)", drv_id, encHandle);

	return ret;
}

static int vmgr_hevc_enc_register_framebuffer(vpu_mgr_t* mgr_ctx,vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	unsigned int drv_id = drv_info->drv_id;
	...
	dlog_henc("[id:%u] Register a frame buffer w PA(0x%px)/VA(0x%px)",
		drv_id,
		pEncBuffer->m_FrameBufferStartAddr[PA],
		pEncBuffer->m_FrameBufferStartAddr[VA]);

	ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_ENC_REG_FRAME_BUFFER, (codec_handle_t *) &pHandle, (void *)pEncBuffer, (void *)NULL);

	return ret;
}

static int vmgr_hevc_enc_put_header(vpu_mgr_t* mgr_ctx,vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	unsigned int drv_id = drv_info->drv_id;
	long pHandle = drv_info->handle;
	...
	dlog_henc("[id:%u] VPU_ENC_PUT_HEADER In, codec id:%d, bitstream addr:%x/%x, size:%d, header type:0x%x",
				drv_id,
	...
	if(drv_info->codec_id == VCODEC_ID_HEVC)
	{
		if((arg_putheader->header_type & VPU_HEADER_HEVC_VPS) != 0)
		{
			...
			dlog_henc("[id:%u] add VPS Header", drv_id);
		}

		if((arg_putheader->header_type & VPU_HEADER_HEVC_SPS) != 0)
		{
			pEncHeader->m_iHeaderType |= HEVC_ENC_CODEOPT_ENC_SPS;
			dlog_henc("[id:%u] add SPS Header", drv_id);
		}

		if((arg_putheader->header_type & VPU_HEADER_HEVC_PPS) != 0)
		{
			pEncHeader->m_iHeaderType |= HEVC_ENC_CODEOPT_ENC_PPS;
			dlog_henc("[id:%u] add PPS Header", drv_id);
		}
	}
	else
	{
		err_henc("[id:%u] not support codec header, codec id:%d, header type:0x%x", drv_id, drv_info->codec_id, pEncHeader->m_iHeaderType);
		ret = -1;
	}
	...
	if(ret == 0)
	{
		...
		ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_ENC_PUT_HEADER, (codec_handle_t *) &pHandle, (void *)pEncHeader, (void *)NULL);
		if(ret == RETCODE_SUCCESS)
		{
			...
			dlog_henc("[id:%u] VPU_ENC_PUT_HEADER out, bitstream addr:%x/%x, size:%d",
				drv_id,
				arg_putheader->bitstream_buffer_addr[VPU_PA],
				arg_putheader->bitstream_buffer_addr[VPU_KVA],
				arg_putheader->bitstream_buffer_size);
		...
		dlog_henc("[id:%u] VPU_ENC_PUT_HEADER done, ret:%d", drv_id, ret);
	}

	return ret;
}

static int vmgr_hevc_enc_encode(vpu_mgr_t* mgr_ctx,vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	unsigned int drv_id = drv_info->drv_id;
	long pHandle = drv_info->handle;
	...
	detail_henc("[id:%u] VPU_ENC_ENCODE in", drv_id, ret);

	detail_henc("[id:%u] encode input, pic_y_addr:0x%x, pic_cb_addr:0x%x, pic_cr_addr:0x%x", drv_id, arg_init_in->pic_y_addr, arg_init_in->pic_cb_addr, arg_init_in->pic_cr_addr);
	detail_henc("[id:%u] force_i_picture:%d, skip_picture:%d, quant_param:%d", drv_id, arg_init_in->force_i_picture, arg_init_in->skip_picture, arg_init_in->quant_param);
	detail_henc("[id:%u] bitstream_buffer_addr:0x%x/0x%x, bitstream_buffer_size:%d", drv_id, arg_init_in->bitstream_buffer_addr[VPU_PA], arg_init_in->bitstream_buffer_addr[VPU_KVA], arg_init_in->bitstream_buffer_size);
	detail_henc("[id:%u] change_rc_param_flag:%d, change_target_kbps:%d, change_framerate:%d, change_key_interval:%d", drv_id, arg_init_in->change_rc_param_flag, arg_init_in->change_target_kbps, arg_init_in->change_framerate, arg_init_in->change_key_interval);
	...
	ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_ENC_ENCODE, (codec_handle_t *)&pHandle, (void *)pEncInput, (void *)pEncOutput);
	if(ret == RETCODE_SUCCESS)
	{
		...
		detail_henc("[id:%u] VPU_ENC_ENCODE success, pic type:%d, addr:%x/%x, size:%d",
	...
	detail_henc("[id:%u] VPU_ENC_ENCODE done, ret:%d", drv_id, ret);

	return ret;
}

static int vmgr_hevc_enc_close(vpu_mgr_t* mgr_ctx,vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	unsigned int drv_id = drv_info->drv_id;
	long pHandle = drv_info->handle;
	...
	dlog_henc("[id:%u] VPU_ENC_CLOSE in", drv_id, ret);
	ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_ENC_CLOSE, (codec_handle_t *)&pHandle, (void *)NULL, (void *)NULL);

	dlog_henc("[id:%u] VPU_ENC_CLOSE done, ret:%d", drv_id, ret);

	return ret;
}

static int vmgr_hevc_enc_encode_process(void* vpu_private, enum vpu_cmd_type cmd, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	int ret = 0;
	unsigned int drv_id = drv_info->drv_id;
	vpu_mgr_t* mgr_ctx = (vpu_mgr_t*)vpu_private;

	dlog_henc("[id:%u] %s(%d)/start mgr_ctx:%p, drv_id:%d", drv_id, vmgr_cmd_name(cmd), cmd, mgr_ctx, drv_id);

	switch (cmd)
	{
		case VPU_CMD_ENC_INIT:
		{
			ret = vmgr_hevc_enc_init(mgr_ctx, cmd_info, drv_info);
		}
		break;

		case VPU_CMD_ENC_REG_FRAME_BUFFER:
		{
			ret = vmgr_hevc_enc_register_framebuffer(mgr_ctx, cmd_info, drv_info);
		}
		break;

		case VPU_CMD_ENC_PUT_HEADER:
		{
			ret = vmgr_hevc_enc_put_header(mgr_ctx, cmd_info, drv_info);
		}
		break;

		case VPU_CMD_ENC_ENCODE:
		{
			ret = vmgr_hevc_enc_encode(mgr_ctx, cmd_info, drv_info);
		}
		break;

		case VPU_CMD_ENC_CLOSE:
		{
			ret = vmgr_hevc_enc_close(mgr_ctx, cmd_info, drv_info);
		}
		break;

		default:
		{
			err_henc("[id:%u] unknown command(%d)", drv_id, cmd);
			ret = -1;
		}
	}
...
static int vmgr_hevc_enc_get_buffer_size(void* vpu_private, enum vmgr_buffer_type buf_type, vpu_drv_info_t* drv_info)
{
	int size = 0;
...
static irqreturn_t vmgr_hevc_enc_isr_handler(int irq, void *vpu_private)
{
	vpu_mgr_t* mgr_ctx = (vpu_mgr_t*)vpu_private;
...
extern vmgr_clock_t vpu_hevc_enc_clock;

static vpu_ip_module_t vpu_hevc_enc_module = {
	.ip_type = VPU_IP_HEVC_ENC,
	.cq_type = VPU_CQ_LEGACY,
	.cq_depth = 1,
	.buffer_mode = VPU_BS_MODE_LINEARBUFFR,
	.internal_timeout_ms = 200,
	.enc_param_size = sizeof(vpu_hevc_enc_papam_t),
	.dec_param_size = 0,
	.internal_handler = vmgr_hevc_enc_internal_handler,
	...
	.clock_ctrl = &vpu_hevc_enc_clock,
	.proc_encode = vmgr_hevc_enc_encode_process,
	.proc_decode = NULL,
	.proc_get_buffer_size = vmgr_hevc_enc_get_buffer_size,
	.isr_handler = vmgr_hevc_enc_isr_handler,
	.cq_func = NULL,
	.ip_private = NULL,
	.access_point_path = HEVC_ENC_ACCESSPOINT_PATH,
};
...
int vmgr_hevc_enc_probe(struct platform_device *pdev)
{
	...
	mgr_ctx = vmgr_alloc(&vpu_hevc_enc_module);
	if(mgr_ctx != NULL)
	{
		hevc_enc_vidsys_conf_reg = (void __iomem *)of_iomap(pdev->dev.of_node, 1);
		if (hevc_enc_vidsys_conf_reg == NULL)
		{
			err_henc("hevc_enc_vidsys_conf_reg: NULL");
		}
		else
		{
			vetc_reg_write((void *)hevc_enc_vidsys_conf_reg, 0x84, 0x9c9a3000);
			detail_henc("Video sub-system cfg reg (0x%px) : Sec. AXI (0x%x)", hevc_enc_vidsys_conf_reg, vetc_reg_read((void *) hevc_enc_vidsys_conf_reg, 0x84));
		}
		...
		ret = vmgr_probe(mgr_ctx, pdev, VPU_HEVC_ENC_MGR_NAME);
		if(ret == 0)
		{
			//assigning VPU manager context to avoid mutex race condition in interrupt handler
			vpu_hevc_enc_mgr_ctx = (vpu_mgr_t*)vmgr_get_context(VPU_IP_HEVC_ENC);
		}
	}
