...
static vpu_mgr_t *vpu_hevc_enc_mgr_ctx = INITIAL_NULL;

static void __iomem *hevc_enc_vidsys_conf_reg = INITIAL_NULL;
...
static int vmgr_hevc_enc_init(vpu_mgr_t *mgr_ctx, vpu_cmd_t *cmd_info, vpu_drv_info_t *drv_info)
{
	...
	if ((vpulib_dbg_param & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
		...
		V_DBG(VPU_DBG_ERROR, "[VPU_HEVC_ENC] codec_ip: %d", codec_ip);

		// Check if codec_ip matches desired value
		if (codec_ip == VPU_IP_HEVC_ENC) {
			// Extract log_mask (BBB part)
			unsigned int log_mask = (vpulib_dbg_param & 0x000FFFU);

			V_DBG(VPU_DBG_ERROR, "[VPU_HEVC_ENC] log_mask: %d (%x)", log_mask, log_mask);
			...
			ret = tcc_vpu_hevc_enc_l(vpu_ap, VPU_HEVC_ENC_CTRL_LOG_STATUS, NULL, (void *)(&ip_param->enc_log), (void *)NULL);
		}
	}
