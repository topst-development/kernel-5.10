...
static vpu_mgr_t* jpu_mgr_ctx = NULL;
...
static int jmgr_enc_init(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	if(ret == 0)
	{
		...
		dlog_jpu("[id:%u] dbg type:%d, mask:%d", drv_id, dbg_type, dbg_mask);

		if (dbg_type == VPU_ALWAYS_PRT_DBG)
		{
			vetc_memset(&ip_param->enc_log, 0x00, sizeof(jpu_ctrl_log_status_t), 0);
			...
			dlog_jpu("[id:%u] JPU_CTRL_LOG_STATUS", drv_id);
			retEnc = tcc_jpu_enc_l(vpu_ap, JPU_CTRL_LOG_STATUS, (codec_handle_t *)NULL, (void*)&ip_param->enc_log, NULL);
			dlog_jpu("[id:%u] JPU_CTRL_LOG_STATUS ret:%d", drv_id, retEnc);
		}
...
static int jmgr_dec_init(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	if (dbg_type == VPU_ALWAYS_PRT_DBG)
	{
		vetc_memset(&ip_param->dec_log, 0x00, sizeof(jpu_ctrl_log_status_t), 0);
		...
		dlog_jpu("[id:%u] JPU_CTRL_LOG_STATUS", drv_id);
		retDec = tcc_jpu_dec_l(vpu_ap, JPU_CTRL_LOG_STATUS, (codec_handle_t *)NULL, (void*)&ip_param->dec_log, NULL);
		dlog_jpu("[id:%u] JPU_CTRL_LOG_STATUS ret:%d", drv_id, retDec);
	}
...
static int jmgr_dec_user_framebuffer(vpu_mgr_t* mgr_ctx, vpu_cmd_t* cmd_info, vpu_drv_info_t* drv_info)
{
	...
	for (buffer_idx=0; buffer_idx<input->frame_buffer_count; buffer_idx++)
	{
		...
		vaddr = (void *) ioremap_wc((phys_addr_t) paddr, PAGE_ALIGN(size));
...
static int jmgr_get_buffer_size(void* vpu_private, enum vmgr_buffer_type buf_type, vpu_drv_info_t* drv_info)
{
	...
	V_DBG(VPU_DBG_INFO, "[%s][%s][id:%u]: buf_type:%d", vmgr_get_ip_name(drv_info->ip_type), vmgr_get_optype_name(drv_info->op_type), drv_info->drv_id, buf_type);
	...
	V_DBG(VPU_DBG_INFO, "[%s][%s][id:%u]: buf_type:%d, size:%d", vmgr_get_ip_name(drv_info->ip_type), vmgr_get_optype_name(drv_info->op_type), drv_info->drv_id, buf_type, size);
	return size;
}
