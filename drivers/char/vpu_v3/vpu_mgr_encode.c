...
static int vmgr_encode_alloc_init_buffer(vpu_mgr_t* mgr_ctx, vpu_drv_info_t* drv_info, venc_v3_init_t* cmd_init)
{
	...
	if(buf_size > 0)
	{
		...
		if(ret == 0)
		{
			...
			dlog_info("alloc success bitstream_buf, PA:0x%x, KVA:0x%x, size:%d",
	...
	switch (cmd->cmd_type)
	{
		case VPU_CMD_ENC_INIT:
		{
			...
			if (cmd->result == VPU_RETCODE_SUCCESS)
			{
				...
				dlog_info("Init Done with ret(0x%x), bitstream pa:0x%x, kva:0x%x, size:%d",
