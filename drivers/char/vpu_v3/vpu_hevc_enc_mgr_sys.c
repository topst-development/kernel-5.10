...
#define dlog_hencs(msg...)  	V_DBG(VPU_DBG_INFO, "[HEVC_ENC_MGR_SYS][LOG]:" msg)
#define detail_hencs(msg...)  	V_DBG(VPU_DBG_DETAIL, "[HEVC_ENC_MGR_SYS][DETIAL]:" msg)
#define seq_hencs(msg...)     	V_DBG(VPU_DBG_SEQUENCE, "[HEVC_ENC_MGR_SYS][SEQ]:" msg)
#define err_hencs(msg...)      	V_DBG(VPU_DBG_ERROR, "[HEVC_ENC_MGR_SYS][ERR]:" msg)
...
static void vmgr_hevc_enc_enable_clock(vmgr_clock_t *vmgr_clk, int vbus_no_ctrl)
{
...
static void vmgr_hevc_enc_disable_clock(vmgr_clock_t *vmgr_clk, int vbus_no_ctrl)
{
...
static void vmgr_hevc_enc_get_clock(vmgr_clock_t *vmgr_clk, struct device_node *node)
{
	if (node == NULL) {
...
static void vmgr_hevc_enc_put_clock(vmgr_clock_t *vmgr_clk)
{
	if (vmgr_clk->vpu_clk[FBUS_CHEVCENC_CLK] != NULL) {
...
static void vmgr_hevc_enc_change_clock(vmgr_clock_t *vmgr_clk, unsigned int width, unsigned int height)
{
...
static void vmgr_hevc_enc_get_reset(vmgr_clock_t *vmgr_clk, struct device_node *node)
{
...
static void vmgr_hevc_enc_put_reset(vmgr_clock_t *vmgr_clk)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
...
static int vmgr_hevc_enc_get_reset_register(vmgr_clock_t *vmgr_clk)
{
...
static void vmgr_hevc_enc_hw_assert(vmgr_clock_t *vmgr_clk)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	...
	if (vmgr_clk->bus_reset[VBUS_HEVC_ENC_RESET] != NULL) {
		V_DBG(VPU_DBG_RSTCLK, "Video bus hevc encoder reset: assert (rsr:0x%x)",
			vmgr_hevc_enc_get_reset_register(vmgr_clk));
		(void)reset_control_assert(vmgr_clk->bus_reset[VBUS_HEVC_ENC_RESET]);
	}

	V_DBG(VPU_DBG_RSTCLK, "out!! (rsr:0x%x)", vmgr_hevc_enc_get_reset_register(vmgr_clk));
#endif
}

static void vmgr_hevc_enc_hw_deassert(vmgr_clock_t *vmgr_clk)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	V_DBG(VPU_DBG_RSTCLK, "enter");
...
static void vmgr_hevc_enc_hw_reset(vmgr_clock_t *vmgr_clk)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	V_DBG(VPU_DBG_RSTCLK, "enter");
	...
	vmgr_hevc_enc_hw_assert(vmgr_clk);

	udelay(1000); //1ms

	vmgr_hevc_enc_hw_deassert(vmgr_clk);
...
static void vmgr_hevc_enc_restore_clock(vmgr_clock_t *vmgr_clk, int vbus_no_ctrl, int opened_cnt)
{
	...
	while (opened_count > 0) {
		vmgr_hevc_enc_disable_clock(vmgr_clk, vbus_no_ctrl);
		opened_count--;
	}
	...
	while (opened_count > 0) {
		vmgr_hevc_enc_enable_clock(vmgr_clk, vbus_no_ctrl);
		opened_count--;
	}
...
vmgr_clock_t vpu_hevc_enc_clock = {
	.vpu_clk = {NULL, },
	.bus_reset = {NULL, },
	.enable_clock = vmgr_hevc_enc_enable_clock,
	.disable_clock = vmgr_hevc_enc_disable_clock,
	.get_clock = vmgr_hevc_enc_get_clock,
	.put_clock = vmgr_hevc_enc_put_clock,
	.change_clock = vmgr_hevc_enc_change_clock,
	.get_reset = vmgr_hevc_enc_get_reset,
	.put_reset = vmgr_hevc_enc_put_reset,
	.get_reset_register = vmgr_hevc_enc_get_reset_register,
	.hw_assert = vmgr_hevc_enc_hw_assert,
	.hw_deassert = vmgr_hevc_enc_hw_deassert,
	.hw_reset = vmgr_hevc_enc_hw_reset,
	.restore_clock = vmgr_hevc_enc_restore_clock
};
