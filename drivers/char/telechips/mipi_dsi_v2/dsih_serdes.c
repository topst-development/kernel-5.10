...
static struct max9xxxx_reg_data stSerMAX96789_MAX96851[] = {
	...
	{MAX96789_SER_ADDR, 0x2C5, 0xAA, 2}, // SER GPIO_TX_ID  : 0xA (  to GPIO18)
	{MAX96878_DES_ADDR, 0x238, 0x6A, 2}, // DES GPIO_RX_ID  : 0xA (from GPIO2)
	{MAX96878_DES_ADDR, 0x236, 0x85, 2 }, // GPIO18 Des setting
	...
	{MAX96789_SER_ADDR, 0x2C8, 0xAB, 2}, // SER GPIO_TX_ID  : 0xB   (to GPIO17)
	{MAX96878_DES_ADDR, 0x235, 0x6B, 2}, // DES GPIO_RX_ID  : 0xB (from GPIO3)
	{MAX96878_DES_ADDR, 0x233, 0x85, 2}, // GPIO17 Des setting
	{MAX96789_SER_ADDR, 0x2C7, 0x83, 2}, // GPIO3 Ser setting
	{MAX96878_DES_ADDR, 0x233, 0x84, 2}, // GPIO17 Des setting

	// MFP7 (GPIO07) - MFP2 (GPIO2) // BL_EN
	{MAX96789_SER_ADDR, 0x2D4, 0xAC, 2}, // SER GPIO_TX_ID  : 0xC (  to GPIO2)
	{MAX96878_DES_ADDR, 0x208, 0x6C, 2}, // DES GPIO_RX_ID  : 0xC (from GPIO7)
	{MAX96878_DES_ADDR, 0x206, 0x85, 2}, // GPIO2 Des setting
	{MAX96789_SER_ADDR, 0x2D3, 0x83, 2}, // GPIO7 Ser setting
	{MAX96878_DES_ADDR, 0x206, 0x84, 2}, // GPIO2 Des setting

	// MFP5 (GPIO5) // MIPI DET LED_ON
	{MAX96878_DES_ADDR, 0x20F, 0x90, 2}, // LED ON
...
static int max9XXXX_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(ret == 0) {
        ...
        pr_info("flag: %x,  chnum: %d \n", (client->flags)<<1, i2c_adapter_id(client->adapter));
        pr_info("name: %s, addr: 0x%x, client: 0x%p\n",
