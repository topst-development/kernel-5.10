...
static void camipc_cmd_status_handle(struct camipc_device *p_camipc,
				     packet *msg)
{
	return;
}

static void camipc_receive_message(struct mbox_client *client, void *p_msg)
{
	struct camipc_device *p_camipc =
		container_of(client, struct camipc_device, mbox_cl);

	packet *msg = (packet *)p_msg;
	uint32_t command = (uint32_t)((msg->cmd[1] >> 16) & 0xFFFFU);
	int ret = 0;

	ret = camipc_check_rx_seqence(p_camipc, msg);

	if (ret == 0) {
		switch (command) {
		case CAMIPC_CMD_OVP:
			camipc_cmd_ovp_handle(p_camipc, msg);
			break;
		case CAMIPC_CMD_POS:
			break;
		...
		case CAMIPC_CMD_READY:
			camipc_cmd_ready_handle(p_camipc, msg);
			break;
		case CAMIPC_CMD_STATUS:
			camipc_cmd_status_handle(p_camipc, msg);
			break;
		default:
...
int camipc_send_message(struct camipc_device *p_camipc, packet *msg)
{
	...
	if (msg == NULL) {
		...
		if (p_camipc != NULL) {
			ret = mbox_send_message(p_camipc->mbox_ch, msg);
			mbox_client_txdone(p_camipc->mbox_ch, ret);
...
int camipc_request_channel(struct camipc_device *p_camipc, const char *name)
{
	...
	p_camipc->mbox_cl.dev = &p_camipc->m_pdev->dev;
	p_camipc->mbox_cl.tx_block = (bool)false;
	p_camipc->mbox_cl.tx_done = NULL;
	p_camipc->mbox_cl.tx_tout = 500;
	p_camipc->mbox_cl.knows_txdone = (bool)false;
	p_camipc->mbox_cl.rx_callback = camipc_receive_message;
	p_camipc->mbox_ch =
		mbox_request_channel_byname(&p_camipc->mbox_cl, name);
	if (IS_ERR(p_camipc->mbox_ch)) {
		ret = (int)PTR_ERR(p_camipc->mbox_ch);
		loge("Fail camipc_request_channel(%d)\n", ret);
	}
	return ret;
}
...
static int camipc_probe_f_parse_dt(struct platform_device *p_pdev)
{
	...
	ret = (of_property_read_string(p_pdev->dev.of_node, "device-name",
				       &p_camipc->device_name) == 0) &&
	      (of_property_read_string(p_pdev->dev.of_node, "mbox-names",
				       &p_camipc->mbox_name) == 0);
...
static int camipc_probe_f_init_txrx(struct platform_device *p_pdev)
{
	...
	ret = camipc_request_channel(p_camipc, p_camipc->mbox_name);
...
int camipc_probe(struct platform_device *p_pdev)
{
	...
	if (ret == 0) {
		...
		camipc_generate_meessage(p_camipc, &msg, CAMIPC_CMD_READY,
					 CAMIPC_SEND, NULL, 0);
		ret = camipc_send_message(p_camipc, &msg);
		...
		loge("Fail initialize camipc driver\n");
	}
