...
#include "camipc_mailbox_client.h"
...
static int camipc_receive_message(struct tcc_mbox_client_msg *p_msg,
				  struct platform_device *pdev)
{
	struct camipc_device *p_camipc = platform_get_drvdata(pdev);
...
int camipc_send_message(struct camipc_device *p_camipc, packet *msg)
{
	...
	if (msg == NULL) {
		...
		if (p_camipc != NULL) {
			ret = tcc_mbox_client_send_data(p_camipc->mbox_client,
							msg,
							p_camipc->mbox_index);
...
int camipc_request_channel(struct camipc_device *p_camipc, const char *name)
{
	int ret = 0;
	int32_t mbox_index;
	struct tcc_mbox_register_dat mbox_prot_dat = {
		0,
	};

	memcpy(&(mbox_prot_dat.id[0]), name, MBOX_CLIENT_PROTOCOL_ID_MAX_LEN);
	mbox_prot_dat.rx_callback = camipc_receive_message;
	mbox_prot_dat.pdev = p_camipc->m_pdev;

	mbox_index =
		tcc_mbox_client_register(p_camipc->mbox_client, &mbox_prot_dat);
	if (mbox_index < 0) {
		ret = -1;
	} else {
		p_camipc->mbox_index = mbox_index;
	}
...
static int camipc_probe_f_parse_dt(struct platform_device *p_pdev)
{
	...
	p_camipc->mbox_client =
		of_parse_phandle(p_pdev->dev.of_node, "tcc_mbox_client", 0);
...
static int camipc_probe_f_init_txrx(struct platform_device *p_pdev)
{
	...
	ret = camipc_request_channel(p_camipc, "CAMIPC");
...
int camipc_probe(struct platform_device *p_pdev)
{
	...
	uint32_t msg_cmd[MBOX_MSG_CMD_MAX_LEN] = {
		0,
	};
	int ret = 0;
	int step;
