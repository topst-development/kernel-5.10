...
#define TCC_THSM_REQUIRED_DAEMON_VER_MAJOR (1u)
#define TCC_THSM_REQUIRED_DAEMON_VER_MINOR (6u)

#define TCC_THSM_DRIVER_VER_MAJOR (2u)
#define TCC_THSM_DRIVER_VER_MINOR (3u)
/****************************************************************************
...
static struct tcc_thsm_ioctl_version_param gVerParam;
/****************************************************************************
...
static long tcc_thsm_ioctl_get_ta_version(unsigned long arg)
{
	struct tcc_thsm_ioctl_version_param param = { 0 };
	long ret = -EFAULT;

	ret = tcc_thsm_cmd_get_ta_version(device_id, &param.major,
					  &param.minor);
	if (ret != 0) {
...
static long tcc_thsm_ioctl_get_ca_version(unsigned long arg)
{
	struct tcc_thsm_ioctl_version_param param = { 0 };
	long ret = -EFAULT;

	ret = tcc_thsm_cmd_get_ca_version(device_id, &param.major,
					  &param.minor);
	if (ret != 0) {
		ELOG("failed to get version from multi device\n");
		return ret;
...
static long tcc_thsm_ioctl_get_daemon_version(unsigned long arg)
{
	struct tcc_thsm_ioctl_version_param param = { 0 };
	long ret = -EFAULT;

	ret = tcc_thsm_cmd_get_daemon_version(device_id, &param.major,
					      &param.minor);
	if (ret != 0) {
		ELOG("failed to get version from multi device\n");
		return ret;
...
static long tcc_thsm_ioctl_get_driver_version(unsigned long arg)
{
	struct tcc_thsm_ioctl_version_param param = { 0 };
	long ret = 0;

	param.major = TCC_THSM_DRIVER_VER_MAJOR;
	param.minor = TCC_THSM_DRIVER_VER_MINOR;
...
static long tcc_thsm_ioctl_set_mode(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_set_mdoe_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_set_mode(device_id, param.key_index, param.algorithm,
				    param.op_mode, param.key_size);
...
static long tcc_thsm_ioctl_set_key(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_set_key_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user((void *)key1, (const void *)param.key1,
			   param.key1_size)) {
	...
	ret = tcc_thsm_cmd_set_key(device_id, param.key_index, key1,
				   param.key1_size, key2, param.key2_size, key3,
				   param.key3_size);

	return ret;
}

static long tcc_thsm_ioctl_set_key_v2(unsigned long arg)
{
	struct tcc_thsm_ioctl_set_key_v2_param param = { 0 };
	struct TEETHSMKey key = { 0 };
	long ret = -EFAULT;

	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_set_key_v2_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	if (copy_from_user(&key, (const struct TEETHSMKey *)param.buffer,
			   sizeof(key))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_set_key_v2(device_id, param.key_index, &key);
...
static long tcc_thsm_ioctl_set_key_from_storage(unsigned long arg)
{
	...
	if (copy_from_user(
		    &param,
		    (const struct tcc_thsm_ioctl_set_key_storage_param *)arg,
		    sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user(obj_id, (const uint8_t *)param.obj_id,
			   param.obj_id_len)) {
		ELOG("copy_from_user failed(%d)\n", param.obj_id_len);
		return ret;
	}

	ret = tcc_thsm_cmd_set_key_from_storage(device_id, param.key_index,
						obj_id, param.obj_id_len);
...
static long tcc_thsm_ioctl_set_key_from_otp(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_set_key_otp_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_set_key_from_otp(device_id, param.key_index,
					    param.otp_addr, param.key_size);
...
static long tcc_thsm_ioctl_set_iv_symmetric(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_set_iv_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_set_iv_symmetric(device_id, param.key_index, iv,
					    param.iv_size);
...
static long tcc_thsm_ioctl_run_cipher(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_cipher_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user(dma_buf->srcVir, (const uint8_t *)param.src_addr,
			   param.src_size)) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_run_cipher_by_dma(device_id, param.key_index,
					     (uint32_t)dma_buf->srcPhy,
					     param.src_size,
					     (uint32_t)dma_buf->dstPhy,
					     &dst_size, param.flag);

	dma_sync_single_for_cpu(dma_buf->dev, dma_buf->dstPhy, param.src_size,
				DMA_FROM_DEVICE);

	if (copy_to_user(param.dst_addr, (const uint8_t *)dma_buf->dstVir,
			 param.src_size)) {
		ELOG("copy_to_user failed\n");
		return ret;
	}
...
static long tcc_thsm_ioctl_run_cipher_by_dma(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_cipher_dma_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_run_cipher_by_dma(device_id, param.key_index,
					     (uint32_t)param.src_addr,
					     param.src_size,
					     (uint32_t)param.dst_addr,
					     &dst_size, param.flag);
...
static long tcc_thsm_ioctl_run_digest(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_run_digest_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user(dma_buf->srcVir, (const uint8_t *)param.chunk,
			   param.chunk_len)) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_run_digest_by_dma(device_id, param.key_index,
					     (uint32_t)dma_buf->srcPhy,
					     param.chunk_len, hash, &hash_len,
					     param.flag);

	dma_sync_single_for_cpu(dma_buf->dev, dma_buf->dstPhy, param.chunk_len,
				DMA_FROM_DEVICE);

	if (hash_len > sizeof(hash)) {
		ELOG("The hash_len(0x%x) should not exceed 0x%x bytes\n",
		     hash_len, (uint32_t)sizeof(hash));
		return ret;
	}
...
static long tcc_thsm_ioctl_run_digest_by_dma(unsigned long arg)
{
	...
	if (copy_from_user(
		    &param,
		    (const struct tcc_thsm_ioctl_run_digest_dma_param *)arg,
		    sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
...
static long tcc_thsm_ioctl_set_iv_mac(unsigned long arg)
{
	...
	if (param.iv_size > sizeof(iv)) {
		ELOG("The iv_size(0x%x) should not exceed 0x%x bytes\n",
		     param.iv_size, (uint32_t)sizeof(iv));
		return ret;
	}
	...
	ret = tcc_thsm_cmd_set_iv_mac(device_id, param.key_index, iv,
				      param.iv_size);
...
static long tcc_thsm_ioctl_compute_mac(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_compute_mac_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user(dma_buf->srcVir, (const uint8_t *)param.message,
			   param.message_len)) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_compute_mac_by_dma(device_id, param.key_index,
					      (uint32_t)dma_buf->srcPhy,
					      param.message_len, mac, &mac_len,
					      param.flag);

	dma_sync_single_for_cpu(dma_buf->dev, dma_buf->dstPhy,
				param.message_len, DMA_FROM_DEVICE);

	if (mac_len > sizeof(mac)) {
		ELOG("The mac_size(0x%x) should not exceed 0x%x bytes\n",
		     mac_len, (uint32_t)sizeof(mac));
		return ret;
	}
...
static long tcc_thsm_ioctl_compute_mac_by_dma(unsigned long arg)
{
	...
	if (copy_from_user(
		    &param,
		    (const struct tcc_thsm_ioctl_compute_mac_dma_param *)arg,
		    sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
...
static long tcc_thsm_ioctl_compare_mac(unsigned long arg)
{
	...
	if (param.mac_len > sizeof(mac)) {
		ELOG("The mac_size(0x%x) should not exceed 0x%x bytes\n",
		     param.mac_len, (uint32_t)sizeof(mac));
		return ret;
	}
	...
	ret = tcc_thsm_cmd_compare_mac_by_dma(device_id, param.key_index,
					      (uint32_t)dma_buf->srcPhy,
					      param.message_len, mac,
...
static long tcc_thsm_ioctl_get_rand(unsigned long arg)
{
	...
	if (copy_from_user(&param, (const struct tcc_thsm_ioctl_rng_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_get_rand(device_id, (uint32_t *)dma_buf->dstVir,
				    param.size);

	if (copy_to_user(param.rng, (const uint8_t *)dma_buf->dstVir,
			 param.size)) {
		ELOG("copy_to_user failed\n");
		return ret;
	}
...
static long tcc_thsm_ioctl_gen_key_ss(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_gen_key_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (copy_from_user(obj_id, (const int8_t *)param.obj_id,
			   param.obj_len)) {
		ELOG("copy_from_user failed(%d)\n", param.obj_len);
		return ret;
	}

	ret = tcc_thsm_cmd_gen_key_ss(device_id, obj_id, param.obj_len,
				      param.algorithm, param.key_size);
...
static long tcc_thsm_ioctl_del_key_ss(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_del_key_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
...
static long tcc_thsm_ioctl_write_key_ss(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_write_key_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_write_key_ss(device_id, obj_id, param.obj_len,
					(uint8_t *)buffer, param.size);

	return ret;
}

static long tcc_thsm_ioctl_write_key_ss_v2(unsigned long arg)
{
	struct tcc_thsm_ioctl_write_key_v2_param param = { 0 };
	int8_t obj_id[TCCTHSM_OBJ_ID_MAX] = { 0 };
	struct TEETHSMKey key = { 0 };
	long ret = -EFAULT;

	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_write_key_v2_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if ((param.obj_len > TCCTHSM_OBJ_ID_MAX)) {
		ELOG("Invalid param(obj_len=0x%x)\n", param.obj_len);
		return ret;
	}
	...
	ret = tcc_thsm_cmd_write_key_ss_v2(device_id, obj_id, param.obj_len,
					   &key);

	return ret;
}

static long tcc_thsm_ioctl_ecdh_phaseI(unsigned long arg)
{
	struct tcc_thsm_ioctl_ecdh_param param = { 0 };
	long ret = -EFAULT;

	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_ecdh_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_ecdh_phaseI(device_id, param.algorithm, param.pubkey,
				       param.pubkey_size, param.prikey,
				       param.prikey_size);
	if (ret != 0) {
		ELOG("tcc_thsm_cmd_ecdh_phaseI failed\n");
		ret = -EFAULT;
	}
...
static long tcc_thsm_ioctl_ecdh_phaseII(unsigned long arg)
{
	struct tcc_thsm_ioctl_ecdh_phaseII_param param = { 0 };
	long ret = -EFAULT;

	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_ecdh_phaseII_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_ecdh_phaseII(device_id, param.algorithm,
					param.pubkey, param.pubkey_size,
					param.prikey, param.prikey_size,
					param.seckey, param.seckey_size);
	if (ret != 0) {
		ELOG("tcc_thsm_cmd_ecdh_phaseII failed\n");
		ret = -EFAULT;
	}
...
static long tcc_thsm_ioctl_ecdh_compute_pubkey(unsigned long arg)
{
	struct tcc_thsm_ioctl_ecdh_param param = { 0 };
	long ret = -EFAULT;
	...
	ret = tcc_thsm_cmd_ecdh_compute_pubkey(device_id, param.algorithm,
					       param.pubkey, param.pubkey_size,
					       param.prikey, param.prikey_size);
	if (ret != 0) {
		ELOG("tcc_thsm_cmd_ecdh_compute_pubkey failed\n");
		ret = -EFAULT;
	}
...
static long tcc_thsm_ioctl_kdf(unsigned long arg)
{
	struct tcc_thsm_ioctl_kdf_param param = { 0 };
	long ret = -EFAULT;

	if (copy_from_user(&param, (const struct tcc_thsm_ioctl_kdf_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_kdf(device_id, param.algorithm, param.key,
			       param.key_size, param.salt, param.salt_size,
			       param.info, param.info_size, param.sessionkey,
			       param.session_size, param.type);
	if (ret != 0) {
		ELOG("tcc_thsm_cmd_kdf failed\n");
		ret = -EFAULT;
	}
	return ret;
}
...
static long tcc_thsm_ioctl_write_otp(unsigned long arg)
{
	...
	if (copy_from_user(&param, (const struct tcc_thsm_ioctl_otp_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (param.size > sizeof(buffer)) {
		ELOG("The param_size(0x%x) should not exceed 0x%x bytes\n",
		     param.size, (uint32_t)sizeof(buffer));
		return ret;
	}
	...
	ret = tcc_thsm_cmd_write_otp(device_id, param.otp_addr, buffer,
				     param.size);
...
static long tcc_thsm_ioctl_write_otpimage(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_otpimage_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}

	ret = tcc_thsm_cmd_write_otpimage(device_id, param.otp_addr,
					  param.size);
...
static long tcc_thsm_ioctl_asym_enc_dec(unsigned long arg)
{
	...
	if (copy_from_user(&param,
			   (const struct tcc_thsm_ioctl_asym_enc_dec_param *)arg,
			   sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_asym_enc_dec_by_dma(device_id, param.key_index,
					       (uint32_t)dma_buf->srcPhy,
					       param.src_size,
					       (uint32_t)dma_buf->dstPhy,
					       &dst_size, param.enc);
...
static long tcc_thsm_ioctl_asym_enc_dec_by_dma(unsigned long arg)
{
	...
	if (copy_from_user(
		    &param,
		    (const struct tcc_thsm_ioctl_asym_enc_dec_dma_param *)arg,
		    sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	ret = tcc_thsm_cmd_asym_enc_dec_by_dma(
		device_id, param.key_index, (uint32_t)param.src_addr,
		param.src_size, (uint32_t)param.dst_addr, &dst_size, param.enc);
...
static long tcc_thsm_ioctl_asym_sign_digest(unsigned long arg)
{
	...
	if (copy_from_user(
		    &param,
		    (const struct tcc_thsm_ioctl_asym_sign_digest_param *)arg,
		    sizeof(param))) {
		ELOG("copy_from_user failed\n");
		return ret;
	}
	...
	if (param.dig_size > sizeof(dig)) {
		ELOG("The digSize(0x%x) should not exceed 0x%x bytes\n",
		     param.dig_size, (uint32_t)sizeof(dig));
		return ret;
	}
	...
	ret = tcc_thsm_cmd_asym_sign_digest(device_id, param.key_index, dig,
					    param.dig_size, sig, &sig_size);

	if (sig_size > sizeof(sig)) {
		ELOG("The sigSize(0x%x) should not exceed 0x%x bytes\n",
		     sig_size, (uint32_t)sizeof(sig));
		return ret;
	}
...
static long tcc_thsm_ioctl_asym_verify_digest(unsigned long arg)
{
	...
	ret = tcc_thsm_cmd_asym_verify_digest(device_id, param.key_index, dig,
					      param.dig_size, sig,
					      param.sig_size);

	dma_sync_single_for_cpu(dma_buf->dev, dma_buf->dstPhy, param.dig_size,
				DMA_FROM_DEVICE);

	return ret;
}

static long tcc_thsm_ioctl(struct file *file, uint32_t cmd, unsigned long arg)
{
	long ret = -EFAULT;

	/* gVerParam is initialized in tcc_thsm_open
	 * Do not run ioctl cmd If HSM F/W version is lower than the required
	 * version */
	if ((gVerParam.major != TCC_THSM_REQUIRED_DAEMON_VER_MAJOR) ||
	    (gVerParam.minor < TCC_THSM_REQUIRED_DAEMON_VER_MINOR)) {
		ELOG("DAEMON verison(%d.%d) must be higher than equal to %d.%d\n",
		     gVerParam.major, gVerParam.minor,
		     TCC_THSM_REQUIRED_DAEMON_VER_MAJOR,
		     TCC_THSM_REQUIRED_DAEMON_VER_MINOR);
		ret = -EPERM;
		return ret;
	}
	...
	case TCCTHSM_IOCTL_GET_TA_VERSION:
		ret = tcc_thsm_ioctl_get_ta_version(arg);
		break;

	case TCCTHSM_IOCTL_GET_CA_VERSION:
		ret = tcc_thsm_ioctl_get_ca_version(arg);
		break;

	case TCCTHSM_IOCTL_GET_DAEMON_VERSION:
		ret = tcc_thsm_ioctl_get_daemon_version(arg);
		break;

	case TCCTHSM_IOCTL_GET_DRIVER_VERSION:
		ret = tcc_thsm_ioctl_get_driver_version(arg);
		break;
	...
	case TCCTHSM_IOCTL_SET_KEY_V2:
		ret = tcc_thsm_ioctl_set_key_v2(arg);
		break;
	...
	case TCCTHSM_IOCTL_WRITE_KEY_SS_V2:
		ret = tcc_thsm_ioctl_write_key_ss_v2(arg);
		break;

	case TCCTHSM_IOCTL_ECDH_PHASE_I:
		ret = tcc_thsm_ioctl_ecdh_phaseI(arg);
		;
		break;

	case TCCTHSM_IOCTL_ECDH_PHASE_II:
		ret = tcc_thsm_ioctl_ecdh_phaseII(arg);
		break;

	case TCCTHSM_IOCTL_ECDH_COMPUTE_PUBKEY:
		ret = tcc_thsm_ioctl_ecdh_compute_pubkey(arg);
		break;

	case TCCTHSM_IOCTL_KDF:
		ret = tcc_thsm_ioctl_kdf(arg);
		break;
...
int tcc_thsm_open(struct inode *inode, struct file *filp)
{
	...
	/*Get DAEMON version to check version when running ioctl  */
	if (tcc_thsm_cmd_get_daemon_version(MBOX_DEV_A53, &gVerParam.major,
					    &gVerParam.minor) != 0) {
...
static int tcc_thsm_probe(struct platform_device *pdev)
{
	...
	dma_buf = devm_kzalloc(&pdev->dev, sizeof(struct tcc_thsm_dma_buf),
			       GFP_KERNEL);
	if (dma_buf == NULL) {
	...
	dma_buf->srcVir = dma_alloc_coherent(&pdev->dev, TCC_THSM_DMA_BUF_SIZE,
	...
	dma_buf->dstVir = dma_alloc_coherent(&pdev->dev, TCC_THSM_DMA_BUF_SIZE,
					     &dma_buf->dstPhy, GFP_KERNEL);
	if (dma_buf->dstVir == NULL) {
		DLOG("failed to allocate dma_buf->dstVir\n");
		dma_free_coherent(&pdev->dev, TCC_THSM_DMA_BUF_SIZE,
...
static int tcc_thsm_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(&pdev->dev, TCC_THSM_DMA_BUF_SIZE, dma_buf->srcVir,
			  dma_buf->srcPhy);
	dma_buf->srcVir = NULL;

	dma_free_coherent(&pdev->dev, TCC_THSM_DMA_BUF_SIZE, dma_buf->dstVir,
			  dma_buf->dstPhy);
	dma_buf->dstVir = NULL;
...
#ifdef CONFIG_OF
static const struct of_device_id thsm_of_match[] = {
