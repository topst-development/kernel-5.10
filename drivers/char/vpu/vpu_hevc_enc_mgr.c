...
#endif
// Control only once!!
static struct mgr_data_t vmgr_hevc_enc_data;
...
static st_hevc_enc_func stHENCFuncBase = {0, NULL, 0, NULL, 0, NULL, NULL, 0};
#endif

static st_hevc_enc_func *stHENCFunc = INITIAL_NULL;

static int check_vpu_hevc_enc_access_addr_valid(void)
{
	int ret = -1;

	if (((CHECK_CODE_01 | stHENCFunc->check_code1) == CHECK_CODE_01) &&
			((CHECK_CODE_02 | stHENCFunc->check_code2) == CHECK_CODE_02) &&
			((CHECK_CODE_03 | stHENCFunc->check_code3) == CHECK_CODE_03) &&
			((CHECK_CODE_04 | stHENCFunc->check_code4) == CHECK_CODE_04)) {
		ret = 0;
	} else {
		V_DBG(VPU_DBG_ERROR, "HEVC Enc CheckCode %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
				GET_FOURCC_1(stHENCFunc->check_code1),
				GET_FOURCC_2(stHENCFunc->check_code1),
				GET_FOURCC_3(stHENCFunc->check_code1),
				GET_FOURCC_4(stHENCFunc->check_code1),
				GET_FOURCC_1(stHENCFunc->check_code2),
				GET_FOURCC_2(stHENCFunc->check_code2),
				GET_FOURCC_3(stHENCFunc->check_code2),
				GET_FOURCC_4(stHENCFunc->check_code2),
				GET_FOURCC_1(stHENCFunc->check_code3),
				GET_FOURCC_2(stHENCFunc->check_code3),
				GET_FOURCC_3(stHENCFunc->check_code3),
				GET_FOURCC_4(stHENCFunc->check_code3),
				GET_FOURCC_1(stHENCFunc->check_code4),
				GET_FOURCC_2(stHENCFunc->check_code4),
				GET_FOURCC_3(stHENCFunc->check_code4),
				GET_FOURCC_4(stHENCFunc->check_code4)
			  );
	}
	return ret;
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
static int get_vpu_hevc_enc_access_addr_file(void)
{
	int ret = 0;
	...
	filp = filp_open("/proc/hevc_enc", O_RDONLY, 0x1A4); //0644
	...
	if (IS_ERR(tTmpPtr)) {
		...
		(void)memmove((void *)&stHENCFunc, (void *)&res, sizeof(unsigned long));
...
static int get_vpu_hevc_enc_access_addr_mem(void)
{
	int ret = 0;
	void *va = NULL;

	va = vetc_ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * SHARE_POINT_ORDER_HEVC_ENC), SHARD_POINT_GAP);

	if (va == NULL) {
		V_DBG(VPU_DBG_ERROR, "ioremap failed");
		ret = -ENOMEM;
	} else {
		memcpy(&stHENCFuncBase, va, sizeof(st_hevc_enc_func));
		stHENCFunc = &stHENCFuncBase;

		V_DBG(VPU_DBG_INFO, "remap (PA : 0x%08x / VA : 0x%p) Dec ADDR : 0x%p",
				SHARE_POINT_ADDR, va,
				stHENCFunc->tccfp_vpu_hevc_enc);
...
static int get_vpu_hevc_enc_access_addr(void)
{
	int ret = 0;

#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
	ret = get_vpu_hevc_enc_access_addr_file();
#else
	ret = get_vpu_hevc_enc_access_addr_mem();
#endif
	...
	if (ret == 0) {
		...
		if (ret == 0) {
			tcc_vpu_hevc_enc = (tccfp_vpu_hevc_enc_t)stHENCFunc->tccfp_vpu_hevc_enc;
			tcc_vpu_hevc_enc_esc = (tccfp_vpu_hevc_enc_esc_t)stHENCFunc->tccfp_vpu_hevc_enc_esc;
			tcc_vpu_hevc_enc_ext = (tccfp_vpu_hevc_enc_ext_t)stHENCFunc->tccfp_vpu_hevc_enc_ext;
		} else {
			ret = -1;
		}
	}
...
extern int tcc_vpu_hevc_enc(int Op, vcodec_handle_t *pHandle,
					void *pParam1, void *pParam2);
extern int tcc_vpu_hevc_enc_esc(int Op, vcodec_handle_t *pHandle,
					void *pParam1, void *pParam2);
extern int tcc_vpu_hevc_enc_ext(int Op, vcodec_handle_t *pHandle,
					void *pParam1, void *pParam2);
#endif //#if defined(USE_ACCESS_POINT)


#if DEFINED_CONFIG_VENC_CNT_1to16
static int tcc_vpu_hevc_enc_l(int Op, vcodec_handle_t *pHandle, void *pParam1, void *pParam2) VPU_NO_SANITIZE_CFI
{
	return tcc_vpu_hevc_enc(Op, pHandle, pParam1, pParam2);
}
#endif
	...
	mutex_lock(&vmgr_hevc_enc_data.comm_data.list_mutex);
	{
		...
		case LIST_ADD:
			*oper_data->vpu_result |= RET1;
			...
			if (vmgr_hevc_enc_data.cmd_queued > INT_MAX_U) {
				V_DBG(VPU_DBG_ERROR, "Cmd queued is already FULL");
			} else {
				vmgr_hevc_enc_data.cmd_queued++;
			}

			if (vmgr_hevc_enc_data.comm_data.thread_intr > INT_MAX_U) {
		...
		case LIST_DEL:
			...
			if (vmgr_hevc_enc_data.cmd_queued > 0U) {
				vmgr_hevc_enc_data.cmd_queued--;
			}
			break;
		case LIST_IS_EMPTY:
			if (list_empty(&vmgr_hevc_enc_data.comm_data.main_list) != 0) {
...
static void vmgr_hevc_enc_wait_process(int wait_ms)
{
...
static irqreturn_t vmgr_hevc_enc_isr_handler(int irq, void *dev_id)
{
...
static int vmgr_hevc_enc_cmd_open(char *str)
{
	int ret = 0;

	V_DBG(VPU_DBG_SEQUENCE, "======> _vmgr_hevc_enc_%s_open enter!! %d'th",
		str, atomic_read(&vmgr_hevc_enc_data.opened));

	vmgr_hevc_enc_enable_clock(0);

	if (atomic_read(&vmgr_hevc_enc_data.opened) == 0) {
	#ifdef FORCED_ERROR
	...
	atomic_inc(&vmgr_hevc_enc_data.opened);
...
{
	return atomic_read(&vmgr_hevc_enc_data.opened);
}
...
static void vmgr_hevc_enc_close_all(int bfreemem)
{
#if DEFINED_CONFIG_VENC_CNT_1to16
					...
					int Op, int *result)
{
	...
	if (atomic_read(&vmgr_hevc_enc_data.opened) == 0) {
		...
		if (vmgr_hevc_enc_get_close(type) == 0) {
			cmd_list->type = (unsigned int)type;
			cmd_list->cmd_type = Op;
			cmd_list->handle
			= vmgr_hevc_enc_data.handle[(unsigned int)type];
			cmd_list->args = NULL;
			cmd_list->comm_data = NULL;
			cmd_list->vpu_result = result;

			(void)vmgr_hevc_enc_list_manager(cmd_list,
...
static int vmgr_hevc_enc_proc_exit_by_external(struct VpuList *list,
					int *result, unsigned int type)
{
	int ret = 0;
	if ((type < (unsigned int)VPU_MAX) &&
		(vmgr_hevc_enc_get_close((vputype)type) == 0) &&
		(vmgr_hevc_enc_data.handle[(unsigned int)type] != 0L)) {
		list->type = type;
		...
		(void)vmgr_hevc_enc_list_manager(list, (unsigned int)LIST_ADD);
...
static int vmgr_hevc_enc_external_all_close(int wait_ms)
{
	int type = 0;
	int max_count = 0;
	int ret;
	...
	for (type = (int)VPU_ENC; type < (int)VPU_HEVC_ENC_MAX; type++) {
		if (vmgr_hevc_enc_proc_exit_by_external(
...
static int vmgr_hevc_enc_cmd_release(char *str)
{
	V_DBG(VPU_DBG_CLOSE, "======> _vmgr_hevc_enc_%s_release In!! %d'th",
		str, atomic_read(&vmgr_hevc_enc_data.opened));

	if (atomic_read(&vmgr_hevc_enc_data.opened) > 0) {
		atomic_dec(&vmgr_hevc_enc_data.opened);
	}
	...
	if (atomic_read(&vmgr_hevc_enc_data.opened) == 0) {
		...
		if (!vmgr_hevc_enc_data.bVpu_already_proc_force_closed) {
			...
			(void)vmgr_hevc_enc_external_all_close(200);
			vmgr_hevc_enc_data.external_proc = 0;
		}
		vmgr_hevc_enc_data.bVpu_already_proc_force_closed = (bool)false;
		...
		vmgr_hevc_enc_data.cmd_processing = 0U;

		vmgr_hevc_enc_close_all(1);

		vmgr_hevc_enc_disable_irq(vmgr_hevc_enc_data.irq);
		(void)vmgr_hevc_enc_BusPrioritySetting(BUS_FOR_NORMAL, 0);
	...
	if (vmgr_hevc_enc_data.nOpened_Count < (unsigned int)(UINT_MAX-1)) {
	...
	"======> _vmgr_hevc_enc_%s_release Out!! %d'th, total = %d  - ENC(%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d)",
		str,
		atomic_read(&vmgr_hevc_enc_data.opened),
		vmgr_hevc_enc_data.nOpened_Count,
...
static int vmgr_hevc_enc_open(struct inode *pinode, struct file *filp)
{
	if (vmgr_hevc_enc_data.irq_reged == 0U) {
	...
	(void)vmgr_hevc_enc_cmd_open(str_file_hevc_enc);
...
static int vmgr_hevc_enc_release(struct inode *pinode, struct file *filp)
{
#ifdef USE_DEV_OPEN_CLOSE_IOCTL
	V_DBG(VPU_DBG_CLOSE,
		"enter!! %d'th",
	...
	mutex_lock(&vmgr_hevc_enc_data.comm_data.file_mutex);
	(void)vmgr_hevc_enc_cmd_release(str_file_hevc_enc);
	mutex_unlock(&vmgr_hevc_enc_data.comm_data.file_mutex);
...
#if DEFINED_CONFIG_VENC_CNT_1to16
static int vmgr_hevc_enc_internal_handler(void)
{
	...
	if (vmgr_hevc_enc_data.check_interrupt_detection) {
		if (atomic_read(&vmgr_hevc_enc_data.oper_intr) > 0) {
			...
			"Success-1: vpu hevc enc operation!!");
			...
			if (atomic_read(&vmgr_hevc_enc_data.oper_intr) > 0) {
				V_DBG(VPU_DBG_INTERRUPT,
				"Success-2: vpu hevc enc operation!!");
				...
				vetc_dump_reg_all(
					(char *)vmgr_hevc_enc_data.base_addr,
					str_cmd_time_out);
				ret_code = RETCODE_CODEC_EXIT;
			}
		}
		atomic_set(&vmgr_hevc_enc_data.oper_intr, 0);
		vmgr_hevc_enc_status_clear(
			(const unsigned int *)vmgr_hevc_enc_data.base_addr);
	}
...
static int vmgr_hevc_enc_process(vputype type, int cmd, long pHandle,
					 void *args)
{
	int ret = 0;
	...
	if (type <= VPU_HEVC_ENC_MAX) {
		...
		case VPU_ENC_INIT:
		{
			...
			= (int (*) (void)) vmgr_hevc_enc_internal_handler;
			...
			if ((vpu_lib_dbg & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
				...
				V_DBG(VPU_DBG_ERROR, "[HEVC_ENC] codec_ip: %d", codec_ip);
				...
				if (codec_ip == 3) {
					...
					V_DBG(VPU_DBG_ERROR, "[HEVC_ENC] log_mask: %d (%x)", log_mask, log_mask);
					...
					ret = tcc_vpu_hevc_enc_l(VPU_HEVC_ENC_CTRL_LOG_STATUS, NULL, (void *)(&enc_log), (void *)NULL);
				}
			}

			ret = tcc_vpu_hevc_enc_l(cmd,
	...
	if (cmd == VPU_ENC_INIT) {
		...
		printMeasurementTime((void *)&vmgr_hevc_enc_data, type, 0, time_gap_us);
	}
#endif
#endif
...
static int vmgr_hevc_enc_operation(void)
{
	int oper_finished;
	struct VpuList *oper_data = NULL;

	while (vmgr_hevc_enc_list_manager(NULL, (unsigned int)LIST_IS_EMPTY) == NULL) {
		...
		*oper_data->vpu_result |= RET2;
		...
		if ((oper_data->type >= (unsigned int)VPU_ENC)
			...
			*oper_data->vpu_result
			= vmgr_hevc_enc_process((vputype)oper_data->type,
				oper_data->cmd_type,
				oper_data->handle,
				oper_data->args);
			oper_finished = 1;
		...
		if (oper_finished != 0) {
			int opened = atomic_read(&vmgr_hevc_enc_data.opened);
		...
		(void)vmgr_hevc_enc_list_manager(oper_data, (unsigned int)LIST_DEL);
...
static int vmgr_hevc_enc_thread(void *kthread)
{
	unsigned long jtimeout;
	...
	do {
		if (vmgr_hevc_enc_list_manager(NULL, (unsigned int)LIST_IS_EMPTY) != NULL) {
			vmgr_hevc_enc_data.cmd_processing = 0;

			(void)wait_event_interruptible_timeout(
				vmgr_hevc_enc_data.comm_data.thread_wq,
				(vmgr_hevc_enc_data.comm_data.thread_intr > 0U),
				(long)jtimeout);

			vmgr_hevc_enc_data.comm_data.thread_intr = 0;
		} else {
			if ((atomic_read(&vmgr_hevc_enc_data.opened) != 0)
				|| (vmgr_hevc_enc_data.external_proc != 0U)) {
				(void)vmgr_hevc_enc_operation();
			} else {
				struct VpuList *oper_data = NULL;
				...
				= vmgr_hevc_enc_list_manager(NULL,
					(unsigned int)LIST_GET_ENTRY);
				if (oper_data != NULL) {
					(void)vmgr_hevc_enc_list_manager(
...
static long vmgr_hevc_enc_ioctl(struct file *filep,
			unsigned int cmd, unsigned long arg)
{
	...
	case VPU_CHECK_CODEC_STATUS_KERNEL:
		if (cmd == (unsigned int)VPU_CHECK_CODEC_STATUS_KERNEL) {
			...
			if (copy_to_user((int *)uarg.pi_data,
				vmgr_hevc_enc_data.closed,
				sizeof(vmgr_hevc_enc_data.closed)) != 0U) {
				ret = -EFAULT;
			} else {
				ret = 0;
			}
	...
	case VPU_TRY_OPEN_DEV_KERNEL:
		(void)vmgr_hevc_enc_cmd_open("cmd");
		break;

	case VPU_TRY_CLOSE_DEV:
	case VPU_TRY_CLOSE_DEV_KERNEL:
		(void)vmgr_hevc_enc_cmd_release("cmd");
		break;
#endif
...
#ifdef CONFIG_COMPAT
static long vmgr_hevc_enc_compat_ioctl(struct file *filep,
					unsigned int cmd, unsigned long arg)
{
	unsigned int iarg;
	...
	return vmgr_hevc_enc_ioctl(filep, cmd, (unsigned long)compat_ptr(iarg));

}
#endif

static int vmgr_hevc_enc_mmap(struct file *filep, struct vm_area_struct *vma)
{
	int ret;
...
static const struct file_operations vmgr_hevc_enc_fops = {
	.open			= vmgr_hevc_enc_open,
	.release		= vmgr_hevc_enc_release,
	.mmap			= vmgr_hevc_enc_mmap,
	.unlocked_ioctl		= vmgr_hevc_enc_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl		= vmgr_hevc_enc_compat_ioctl,
#endif
};

static struct miscdevice vmgr_hevc_enc_misc_device = {
	MISC_DYNAMIC_MINOR,
...
int vmgr_hevc_enc_probe(struct platform_device *pdev)
{
	...
	(void)memset(&vmgr_hevc_enc_data, 0, sizeof(struct mgr_data_t));
	for (type = (int)VPU_ENC; type < (int)VPU_HEVC_ENC_MAX; type++) {
	...
	vmgr_hevc_enc_data.nOpened_Count = 0U;
	...
	irqv = (long)vmgr_hevc_enc_data.irq;
	if (irqv < 32) {
		irqv = 0;
	} else {
		irqv -= 32;
...
#if defined(USE_ACCESS_POINT)
	if (stHENCFunc == NULL) {
		ret = get_vpu_hevc_enc_access_addr();
		if (ret != 0) {
	...
	kidle_task_hevce = kthread_run(vmgr_hevc_enc_thread, NULL, "vHEVC_ENC_th");
	...
	ret = 0;
	if (misc_register(&vmgr_hevc_enc_misc_device) != 0) {
		V_DBG(VPU_DBG_ERROR,
...
VREMOVE_RET_TYPE vmgr_hevc_enc_remove(struct platform_device *pdev)
{
	...
	misc_deregister(&vmgr_hevc_enc_misc_device);
...
int vmgr_hevc_enc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (atomic_read(&vmgr_hevc_enc_data.opened) != 0) {
		...
		open_count = atomic_read(&vmgr_hevc_enc_data.opened);
