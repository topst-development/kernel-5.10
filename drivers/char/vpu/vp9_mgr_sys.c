...
static int cache_droped_vp9d;	// = 0;
...
void vp9mgr_enable_clock(int vbus_no_ctrl)
{
	...
	if (vbus_vp9_clk) {
		(void)clk_prepare_enable(vbus_vp9_clk);
	}
}
...
void vp9mgr_get_clock(struct device_node *pnode)
{
	if (pnode == NULL) {
	...
	vbus_vp9_clk = of_clk_get(pnode, 1);
	VPU_BUG_ON(vbus_vp9_clk);
}
...
void vp9mgr_get_reset(struct device_node *pnode)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	if (pnode == NULL) {
		err("device node is null");
	...
	V_DBG(VPU_DBG_RSTCLK,
		"hmgr_get_reset");
	vbus_vp9_reset = of_reset_control_get_by_index(pnode, 0);
	VPU_BUG_ON(vbus_vp9_reset);
#endif
}
...
void vp9mgr_hw_assert(void)
{
	...
	if (vbus_vp9_reset) {
		(void)reset_control_assert(vbus_vp9_reset);
	}
...
void vp9mgr_hw_deassert(void)
{
	...
	V_DBG(VPU_DBG_RSTCLK, "enter");
	if (vbus_vp9_reset) {
		(void)reset_control_deassert(vbus_vp9_reset);
	}
...
{
	LOG_COVERITY("%d", cache_droped_vp9d);
}
