...
static st_jpu_func *stJPUFunc = INITIAL_NULL;
...
static int get_jpu_access_addr_mem(void)
{
	...
	va = vetc_ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * SHARE_POINT_ORDER_JPU), SHARD_POINT_GAP);
...
static int jmgr_process(vputype type, int cmd, long pHandle, void *args)
{
	...
	if (type < VPU_ENC) {
		...
		case V2D_IP_DRV_INI:
		{
			...
			if ((vpu_lib_dbg & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
				...
				if (codec_ip == 5) {
					...
					ret = gs_fpTccJpuDec(JPU_CTRL_LOG_STATUS, NULL, (void *)(&dec_log), (void *)NULL);
				}
			}
			#endif
		...
		{
			JPU_SET_BUFFER3_t *arg = (JPU_SET_BUFFER3_t *)args;
			int retDec;

#if defined(JPU_C6)
			{
				int ii;
				jpu_dec_buffer3_t *pDecBuffer3 = &arg->gsJpuDecBuffer3;

				dprintk_jpu("JPU_DEC_REG_FRAME_BUFFER3 :: cnt = %d", pDecBuffer3->m_iFrameBufferCount);
				for (ii = 0; ii < pDecBuffer3->m_iFrameBufferCount; ii++) {
					dprintk_jpu("[%d] addrY:0x%x, addrCb:0x%x, addrCr:0x%x",
			...
			retDec = gs_fpTccJpuDec(
				JPU_DEC_REG_FRAME_BUFFER3,
				(vcodec_handle_t *)&pHandle,
				(void *)(&arg->gsJpuDecBuffer3),
				(void *)NULL);
			ret = jmgr_convert_returnType(retDec);
			dprintk_jpu(
			"Dec :: JPU_DEC_REG_FRAME_BUFFER3 out\n");
		}
		break;
		...
		case VPU_ENC_INIT:
		{
			...
			if ((vpu_lib_dbg & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
				...
				if (codec_ip == 5) {
					...
					ret = tcc_jpu_enc_l(JPU_CTRL_LOG_STATUS, NULL, (void *)(&enc_log), (void *)NULL);
				}
			}
			#endif
