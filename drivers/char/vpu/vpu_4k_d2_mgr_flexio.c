...
////////////////////////////////////////////////////////////////////////////
/* flex array converter for v1 struct: VPU_4K_D2_INIT_t */
////////////////////////////////////////////////////////////////////////////
/* bitstream buffer */
static void v24kd2mgr_assign_hwbuf_bs(struct vpu_decoder_data *vdata,
								  struct v2hw_flex_io *fli)
{
	...
	if (res > 0) {
		...
		mai.request_size = ALIGNED_BUFF(WAVE5_STREAM_BUF_SIZE, 1024u);
	}
...
/* userdata buffer */
static void v24kd2mgr_assign_hwbuf_ud(struct vpu_decoder_data *vdata,
								  struct v2hw_flex_io *fli)
{
	int32_t res = 0;
	MEM_ALLOC_INFO_t mai = {};
	...
	if (res > 0) {
		...
		mai.request_size = ALIGNED_BUFF(WAVE5_USERDATA_BUF_SIZE, 4096u);
	}
...
/* bit work buffer */
static void v24kd2mgr_assign_hwbuf_bw(struct vpu_decoder_data *vdata,
								  struct v2hw_flex_io *fli)
{
	int ret;
	MEM_ALLOC_INFO_t mai = {};
	mai.request_size = ALIGNED_BUFF(WAVE5_WORK_CODE_BUF_SIZE, 4096u);
...
void *v24kd2mgr_unmarshal_ip_inidata(void *args)
{
	...
	VPU_4K_D2_INIT_t *init_handle;
	vpu_4K_D2_dec_init_t *ini_info;
	...
	init_handle = &vdata->gsV4kd2DecInit_Info;
	ini_info = &init_handle->gsV4kd2DecInit;

	/* reset fields unused/unassigned by user space client */
	init_handle->result = 0;
	init_handle->gsV4kd2DecHandle = 0;
	...
	v24kd2mgr_assign_hwbuf_bs(vdata, fli);
	v24kd2mgr_assign_hwbuf_bw(vdata, fli);
	if (V2_FLEXIP_B_CHECK(fli, INI_B_USRDAT)) {
		v24kd2mgr_assign_hwbuf_ud(vdata, fli);
	}
...
#if 0
	printk("[%s][bs] PA = %#llx, VA = %#llx", __func__,
...
#if 0
		printk("[%s][ud] PA = %#llx, VA = %#llx", __func__,
	...
	if (flx_field > 0) {
		...
		ini_info->m_uiDecOptFlags |= WAVE5_WTL_ENABLE; // (1 << 0u)
	}
	...
	if (flx_field > 0) {
		...
		ini_info->m_uiDecOptFlags |= WAVE5_10BITS_DISABLE; // (1u << 3u)
	...
	if (max_fb_mode > 0) {
		//printk("[%s] max fb mode", __func__);
		ini_info->m_uiDecOptFlags |= (unsigned int)(0x10000); //(1u << 16u);
	}

	/* option: cq depth */
	if (WAVE5_COMMAND_QUEUE_DEPTH > 0) {
		(void)pr_info("[%s] cq-2 setting", __func__);
		// [FIXME] Risky attemption: flx_field could overwrite the bits after 17u
		ini_info->m_uiDecOptFlags |= ((uint32_t)WAVE5_COMMAND_QUEUE_DEPTH << 17u);
	}
...
void v24kd2mgr_marshal_op_inidata(void *args)
{
	...
	/* [KO:INI:F:08] */
	V2_FLEXOP_SET(flo, INI_CQ_CAPS, WAVE5_COMMAND_QUEUE_DEPTH);

#if 0
	(void)printk(V2_FLXMETA_PRINTFMT(flo));
	for (index = 0; index < flo->maxfields; index++) {
		printk("[%s][%zu] %#x", __func__, index, flo->fields[index]);
	}
#endif
}

////////////////////////////////////////////////////////////////////////////
/* flex array converter for v1 struct: VPU_4K_D2_SEQ_HEADER_t */
////////////////////////////////////////////////////////////////////////////
void *v24kd2mgr_unmarshal_ip_seqdata(void *args)
{
	...
	VPU_4K_D2_SEQ_HEADER_t *seq_handle;
	...
	seq_handle = &vdata->gsV4kd2DecSeqHeader_Info;
...
void v24kd2mgr_marshal_op_seqdata(void *args)
{
	...
	VPU_4K_D2_SEQ_HEADER_t *seq_handle;
	vpu_4K_D2_dec_initial_info_t *seq_info;
	...
	seq_handle = &vdata->gsV4kd2DecSeqHeader_Info;
	seq_info = &seq_handle->gsV4kd2DecInitialInfo;
	...
	V2_FLEXOP_SET(flo, DECSEQ_BTDP, seq_info->m_iBitDepth);
...
int32_t v24kd2mgr_register_hwbuf_fb(void *args, long handle, tccfp_vpu_proc_t tcc_vpu_4k_d2_dec)
{
	...
	res = vmem_proc_alloc_memory(vdata->gsCodecType, &mai, vdata->gsDecType);
	if (res == 0) {
		VPU_4K_D2_SET_BUFFER_t *set_handle = &vdata->gsV4kd2DecBuffer_Info;
		vpu_4K_D2_dec_buffer_t *set_info   = &set_handle->gsV4kd2DecBuffer;
		...
		res = tcc_vpu_4k_d2_dec(VPU_DEC_REG_FRAME_BUFFER,
...
////////////////////////////////////////////////////////////////////////////
/* flex array converter for v1 struct: VPU_4K_D2_DECODE_t */
////////////////////////////////////////////////////////////////////////////
void *v24kd2mgr_unmarshal_ip_drndata(void *args)
{
	...
	frm_handle = &vdata->gsV4kd2DecInOut_Info;
	dec_input = &frm_handle->gsV4kd2DecInput;
...
void *v24kd2mgr_unmarshal_ip_frmdata(void *args)
{
	...
	/* ==== set v1 boolean field  ==== */
	/* [V2:KI:DECFRM:B:00] */
	if (V2_FLEXIP_B_CHECK(fli, DECFRM_B_RA_RORDR)) {
		dec_input->m_iSkipFrameMode |= (1u << 4u);
	}
	/* [V2:KI:DECFRM:B:01] */
	if (V2_FLEXIP_B_CHECK(fli, DECFRM_B_SPRFRM_X)) {
		dec_input->m_Reserved[20] = 20u;
	}
...
void v24kd2mgr_marshal_op_frmdata(void *args, bool updated)
{
	...
	if (updated) {
		VPU_4K_D2_DECODE_t *frm_handle = &vdata->gsV4kd2DecInOut_Info;
		vpu_4K_D2_dec_output_t *dec_output = &frm_handle->gsV4kd2DecOutput;
		vpu_4K_D2_dec_output_info_t	*info = &dec_output->m_DecOutInfo;
		vpu_4K_D2_dec_MapConv_info_t *mc = &info->m_DispMapConvInfo;
		...
		/* [V2:KO:DECFRM:F:05]
		 * [FIXME] m_iInterlace is located in vpu_4K_D2_dec_initial_info_t (DECSEQ) */
	...
	#if 0
		(void)pr_info("[%08d] pic: %d, dec: %02d, %04d x %04d, "
					  "disp: %02d: %04d x %04d", op_frm_num, info->m_iPicType,
				info->m_iDecodedIdx, info->m_iDecodedWidth, info->m_iDecodedHeight,
				info->m_iDispOutIdx, info->m_iDisplayWidth, info->m_iDisplayHeight);
		op_frm_num++;
	#endif
		...
		if (V2_FLEXIP_B_CHECK(fli, DECFRM_B_MAPDAT_O)) {
			...
			/* [V2:KO:DECFRM:CSI:23] 16: VP9, else: HEVC */
...
////////////////////////////////////////////////////////////////////////////
/* flex array converter for v1 struct: VPU_4K_D2_RINGBUF_SETBUF_PTRONLY_t */
////////////////////////////////////////////////////////////////////////////
void *v24kd2mgr_unmarshal_ip_setpos(void *args)
{
	...
	VPU_4K_D2_RINGBUF_SETBUF_PTRONLY_t *setpos_handle;
	...
	setpos_handle = &vdata->gsV4kd2DecUpdateWP;
...
////////////////////////////////////////////////////////////////////////////
/* flex array converter for v1 struct: VPU_4K_D2_RINGBUF_GETINFO_t */
////////////////////////////////////////////////////////////////////////////
void *v24kd2mgr_unmarshal_ip_getpos(void *args)
{
	...
	VPU_4K_D2_RINGBUF_GETINFO_t *getpos_handle;
	...
	getpos_handle = &vdata->gsV4kd2DecBufStatus;
...
void v24kd2mgr_marshal_op_getpos(void *args)
{
	...
	VPU_4K_D2_RINGBUF_GETINFO_t *getpos_handle;
	vpu_4K_D2_dec_ring_buffer_status_out_t *getpos_info;
	...
	getpos_handle = &vdata->gsV4kd2DecBufStatus;
	getpos_info = &getpos_handle->gsV4kd2DecRingStatus;
	...
	(void)printk(V2_FLXMETA_PRINTFMT(flo));
	for (index = 0; index < flo->maxfields; index++) {
		printk("[%s][%zu] %d", __func__, index, flo->fields[index]);
	}
#endif
}

#endif //#ifdef CONFIG_SUPPORT_TCC_WAVE512_4K_D2
