...
static st_vpu_func *stVPUFunc = NULL;
...
static int get_access_addr_mem(void)
{
	...
	va = ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * SHARE_POINT_ORDER_VPU), SHARD_POINT_GAP);
	...
	if (type < VPU_ENC) {
		...
		case V2D_IP_DEC_FRMDATA:
		{
			...
			if (arg == NULL) {
				...
				vmgr_data.check_interrupt_detection = 1;
				ret =
					tcc_vpu_dec_l(cmd & ~VPU_BASE_OP_KERNEL,
						(vcodec_handle_t *)&pHandle,
						...
						if (vmgr_data.vpu_ctrl.pending_vtype == -1) {
							...
							for (i = 0u; i < 2u; i++) {
								if (dio->ip_state[i] == V2_DIO_DELAY) {
									(void)pr_info("flush: reset delayed cq2 index %ld", i);
...
static long vmgr_ioctl_l(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case VPU_DEBUG_ISR:
		...
		if (copy_to_user
			((void *)uarg.pv_data, &vpu_isr_param_debug,
			sizeof(struct debug_vpu_k_isr)) != 0U) {
			ret = -EFAULT;
		}
		break;
#endif
...
static irqreturn_t vmgr_isr_handler(int irq, void *dev_id)
{
	...
	if ((cntInt_vpu > INT_MAX_U)  //FIXME : need to set valid check number
#ifdef DEBUG_VPU_K
		|| (cntwk_vpu > INT_MAX_U) //FIXME : need to set valid check number
#endif
	) {
		ret = IRQ_NONE; // FIXME : what is error value???
	} else {
		cntInt_vpu++;

		if ((cntInt_vpu % 30U) == 0U) {
	...
	if (timed_out || timed_off) {
		...
		if (oper_data != NULL) {
			...
			if (found) {
				...
				(void)pr_info("[%s] decode pending field (ip:%ld, op:%d, to: %d, fa: %d)",
						vpu_vputype_to_string(vmgr_data.vpu_ctrl.pending_vtype),
