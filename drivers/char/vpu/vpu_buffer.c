...
/*
 * The header to be used depending
 on
 * whether or not to use the helper module for pmap.
 */
...
#define VPU_ENC_EXT_MAX (VPU_MAX - VPU_ENC)	//VPU_ENC_MAX_CNT
static struct pmap pmap_enc_ext[VPU_ENC_EXT_MAX];
static phys_addr_t ptr_enc_ext_addr_mem[VPU_ENC_EXT_MAX];
static unsigned int sz_enc_ext_used_mem[VPU_ENC_EXT_MAX];
static unsigned int sz_enc_ext_remained_mem[VPU_ENC_EXT_MAX];
#endif
...
#define MAX_VDEC_IDX (5U)
#define MAX_VENC_IDX (16U)

static int vdec_used[MAX_VDEC_IDX];
static int venc_used[VPU_ENC_MAX_CNT];

int vmem_get_vdec_used (int type)
{
	int in_use = -1;
	s32 num_left = (s32) (MAX_VDEC_IDX - 1);
	if (type >= 0 && type < VPU_INST_MAX)
		in_use = vdec_used[num_left - type];
	return in_use;
}

int vmem_set_vdec_used (int type, int value)
{
	s32 num_left = (s32) (MAX_VDEC_IDX - 1);
	if (type >= 0 && type < VPU_INST_MAX) {
		vdec_used[num_left - type] = value;
		return 0;
	}
	return -1;
}

int vmem_get_venc_used (int type)
{
	int in_use = -1;
	s32 num_left = (s32) (MAX_VENC_IDX - 1);
	if (type >= 0 && type < VPU_ENC_MAX_CNT)
		in_use = venc_used[num_left - type];
	return in_use;
}

int vmem_set_venc_used (int type, int value)
{
	s32 num_left = (s32) (MAX_VENC_IDX - 1);
	if (type >= 0 && type < VPU_ENC_MAX_CNT) {
		venc_used[num_left - type] = value;
		return 0;
	}
	return -1;
}
...
static void vmem_set_page_type(phys_addr_t uiAddress, int iProperty)
{
	if (iProperty != 0) {
		int idx = 0;
		...
		if (idx < PAGE_TYPE_MAX) {
			...
			if (g_iMmapPropCnt < INT_MAX)
				g_iMmapPropCnt++;
			else {
				V_DBG(VPU_DBG_ERROR,
				"g_iMmapPropCnt_Count is already MAX count, can't increase.");
			}
...
static int vmem_get_page_type(unsigned long ulPageOffset)
{
	unsigned int idx = 0;
	int prop = 0;
	...
	if (idx < PAGE_TYPE_MAX) {
		...
		if (g_iMmapPropCnt > INT_MIN) {
			g_iMmapPropCnt--;
		} else {
			V_DBG(VPU_DBG_ERROR,
				"g_iMmapPropCnt_Count is already MIN count, can't decrease.");
		}
	} else {
		V_DBG(VPU_DBG_MEM_SEQ, " Page Offset (%ld) - FAILED",
			ulPageOffset);
	}
...
pgprot_t vmem_get_pgprot(pgprot_t ulOldProt, unsigned long ulPageOffset)
{
	...
	if (g_iMmapPropCnt > 0) {
		int prop = vmem_get_page_type(ulPageOffset);
...
static void *vmem_get_virtaddr(unsigned int start_phyaddr, unsigned int length)
{
	void *cma_virt_address = NULL;

	V_DBG(VPU_DBG_MEMORY, "physical region [0x%lx - 0x%lx]!!",
		(unsigned long)start_phyaddr,
		(unsigned long)(start_phyaddr + length));

	if (vmem_is_cma_allocated_phy_region(start_phyaddr, length))
		cma_virt_address =
...
static void vmem_release_virtaddr(void *target_virtaddr,
...
#if DEFINED_CONFIG_VDEC_CNT_345
static phys_addr_t vmem_request_phyaddr_dec_ext23(unsigned int request_size,
							vputype type)
{
...
static int vmem_release_phyaddr_dec_ext23(phys_addr_t phyaddr,
						unsigned int size, vputype type)
{
	...
	if (type == VPU_DEC_EXT2) {
		...
		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_ext_rear_addr_mem = 0x%x -> 0x%lx(0x%x + 0x%x)!!",
			type, ptr_ext_rear_addr_mem,
			((unsigned long)ptr_ext_rear_addr_mem + size), ptr_ext_rear_addr_mem,
			size);
...
#if DEFINED_CONFIG_VDEC_CNT_5
static phys_addr_t vmem_request_phyaddr_dec_ext4(unsigned int request_size,
							vputype type)
{
	phys_addr_t curr_phyaddr = 0;
...
static int vmem_release_phyaddr_dec_ext4(phys_addr_t phyaddr,
					  unsigned int size, vputype type)
{
	//calc remained_size and ptr_addr_mem
	if (type == VPU_DEC_EXT4) {
...
#if DEFINED_CONFIG_VENC_CNT_2to16
static phys_addr_t vmem_request_phyaddr_enc_ext(unsigned int request_size,
						 vputype type)
{
	phys_addr_t curr_phyaddr = 0;
...
static int vmem_release_phyaddr_enc_ext(phys_addr_t phyaddr, unsigned int size,
					 vputype type)
{
	unsigned int enc_idx = type - VPU_ENC_EXT;
...
static int vmem_check_allocation_available(char check_type,
						...
						vputype type)
{
	...
	case 1:		// Check encoder memory.
		if ((unsigned long)sz_enc_mem <
			((unsigned long)sz_rear_used_mem + request_size)) {
			V_DBG(VPU_DBG_ERROR,
	...
	case 2:		// Check remainning except encoder memory.
		if ((pmap_video.size) <
			((unsigned long)sz_ext_used_mem + sz_front_used_mem
			+ request_size + sz_enc_mem + VPU_SW_ACCESS_REGION_SIZE)) {
			V_DBG(VPU_DBG_ERROR,
				"type[%u] : insufficient memory : total(0x%llx(%llu) ) except encoder, allocated(0x%lx(%lu) ), request(0x%x(%u) )",
...
static phys_addr_t vmem_request_phyaddr(unsigned int request_size,
					 vputype type)
{
	phys_addr_t curr_phyaddr = 0;

#ifdef CONFIG_VPU_ALLOC_MEM_IN_SPECIFIC_SEQUENCE
	if (vmem_check_allocation_available(0, request_size, type))
		return 0;
...
#else
	if (only_decmode) {
		if (vmem_check_allocation_available(
			0, request_size, type))
			return 0;
	} else {
		if (type == VPU_ENC) {
	...
	if (type == VPU_DEC) {
		...
		if (((unsigned long)ptr_front_addr_mem
			+ (unsigned long)request_size) > UINT_MAX) {
			V_DBG(VPU_DBG_ERROR,
				"ptr_front_addr_mem_range failed : ptr_front_addr_mem=0x%lx, request_size=0x%lx(%ul)",
				ptr_front_addr_mem, request_size, request_size);
			ptr_front_addr_mem = UINT_MAX;
		} else
			ptr_front_addr_mem += request_size;

		if (((unsigned long)sz_front_used_mem
		  + (unsigned long)request_size) > UINT_MAX) {
			V_DBG(VPU_DBG_ERROR,
				"sz_front_used_mem_range failed : ptr_ext_addr_mem=0x%lx, request_size=0x%lx(%ul)",
				sz_front_used_mem, request_size, request_size);
			sz_front_used_mem = UINT_MAX;
		} else
			sz_front_used_mem += request_size;
		...
		if (only_decmode) {
			if (ptr_rear_addr_mem < request_size) {
				V_DBG(VPU_DBG_ERROR,
					"ptr_rear_addr_mem_range failed : ptr_rear_addr_mem=0x%lx, request_size=0x%lx(%ul)",
					ptr_rear_addr_mem, request_size, request_size);
				ptr_rear_addr_mem = 0;
			} else
				ptr_rear_addr_mem -= request_size;
			...
			if (sz_rear_used_mem < request_size) {
				sz_rear_used_mem = 0;
				V_DBG(VPU_DBG_ERROR,
					"sz_rear_used_mem_range failed : sz_rear_used_mem=0x%lx, request_size=0x%lx(%ul)",
					sz_rear_used_mem, request_size, request_size);
			} else
				sz_rear_used_mem -= request_size;

			V_DBG(VPU_DBG_MEM_USAGE,
				"type[%d] : alloc = 0x%lx ~ 0x%lx, 0x%x(%u)!!, used 0x%x(%u)",
				type, (unsigned long)(ptr_rear_addr_mem + request_size),
				(unsigned long)curr_phyaddr, request_size, request_size,
				sz_rear_used_mem, sz_rear_used_mem);
		} else {
			if (ptr_ext_addr_mem < request_size) {
				V_DBG(VPU_DBG_ERROR,
					"ptr_ext_addr_mem_range failed : ptr_ext_addr_mem=0x%lx, request_size=0x%lx(%ul)",
					ptr_ext_addr_mem, request_size, request_size);
				ptr_ext_addr_mem = 0;
			} else
			...
			if (((unsigned long)sz_ext_used_mem
				+ (unsigned long)request_size) > UINT_MAX) {
				V_DBG(VPU_DBG_ERROR,
					"sz_ext_used_mem_range failed : sz_ext_used_mem=0x%lx, request_size=0x%lx(%ul)",
					sz_ext_used_mem, request_size, request_size);
				sz_ext_used_mem = UINT_MAX;
			} else
				sz_ext_used_mem += request_size;

			V_DBG(VPU_DBG_MEM_USAGE,
				"type[%d] : alloc = 0x%lx ~ 0x%lx, 0x%x(%u)!!, used 0x%x(%u)",
				type, (unsigned long)(ptr_ext_addr_mem + request_size),
				(unsigned long)curr_phyaddr, request_size, request_size,
		...
		if (((unsigned long)sz_rear_used_mem
			+ (unsigned long)request_size) > UINT_MAX) {
			V_DBG(VPU_DBG_ERROR,
				"sz_rear_used_mem_range failed : sz_rear_used_mem=0x%lx, request_size=0x%lx(%ul)",
				sz_rear_used_mem, request_size, request_size);
			sz_rear_used_mem = UINT_MAX;
		} else
			sz_rear_used_mem += request_size;

		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : alloc = 0x%lx ~ 0x%lx, 0x%x(%u)!!, used 0x%x(%u)",
				type, ((unsigned long)ptr_rear_addr_mem + request_size), (unsigned long)curr_phyaddr,
				request_size, request_size, sz_rear_used_mem,
				sz_rear_used_mem);
	}
#endif
	V_DBG(VPU_DBG_MEM_USAGE,
		"type[%d] : mem usage = 0x%lx/0x%llx = Dec(f:0x%x(%u) + ext:0x%x(%u) ) + Dec(rear(0x%x(%u) ) !!",
		type, ((unsigned long)sz_front_used_mem + sz_rear_used_mem + sz_ext_used_mem),
	...
	if (sz_remained_mem < request_size) {
		V_DBG(VPU_DBG_ERROR,
			"sz_remained_mem_range failed : sz_remained_mem=0x%lx, request_size=0x%lx(%ul)",
			sz_remained_mem, request_size, request_size);
		sz_remained_mem = 0;
	} else
...
static int vmem_release_phyaddr(phys_addr_t phyaddr, unsigned int size,
				vputype type)
{
#ifdef CONFIG_VPU_ALLOC_MEM_IN_SPECIFIC_SEQUENCE
	if (type == VPU_DEC || type == VPU_ENC) {
		...
		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_rear_addr_mem = 0x%x -> 0x%lx(0x%x + 0x%x(%u) ) !!",
			type, ptr_rear_addr_mem, ((unsigned long)ptr_rear_addr_mem + size),
	...
	if (type == VPU_DEC) {
		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_front_addr_mem = 0x%x size = 0x%x(%u) !!",
			type, ptr_front_addr_mem, size, size);

		if (ptr_front_addr_mem < size)
			ptr_front_addr_mem = 0;
		else
			ptr_front_addr_mem -= size;

		if (sz_front_used_mem < size)
			sz_front_used_mem = 0;
		else
			sz_front_used_mem -= size;

		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_front_addr_mem = 0x%x, sz_front_used_mem = 0x%x, size=0x%x(%u) !!",
			type, ptr_front_addr_mem, sz_front_used_mem, size, size);
		...
		if (only_decmode) {
			V_DBG(VPU_DBG_MEM_USAGE,
				...
				((unsigned long)ptr_rear_addr_mem + (unsigned long)size), ptr_rear_addr_mem,
				size, size);
			...
			if (((unsigned long)ptr_rear_addr_mem
				+ (unsigned long)size) > UINT_MAX) {
				V_DBG(VPU_DBG_ERROR,
					"ptr_rear_addr_mem_range failed : ptr_rear_addr_mem=0x%lx, size=0x%lx(%ul)",
					ptr_rear_addr_mem, size, size);
				ptr_rear_addr_mem = UINT_MAX;
			} else
				ptr_rear_addr_mem += size;

			if(sz_rear_used_mem < size) {
				V_DBG(VPU_DBG_ERROR, "type[%d] : release sz_rear_used_mem 0x%x undeflow -> size = 0x%x(%u)!!",
					type, sz_rear_used_mem, size, size);
				sz_rear_used_mem = 0;
			} else {
				sz_rear_used_mem -= size;
			}
			...
			V_DBG(VPU_DBG_MEM_USAGE,
				"type[%d] : release ptr_ext_addr_mem = 0x%x + size = 0x%x(%u)!!",
				type, ptr_ext_addr_mem, size, size);
			...
			if (((unsigned long)ptr_ext_addr_mem
				 + (unsigned long)size) > UINT_MAX) {
				V_DBG(VPU_DBG_ERROR,
					"ptr_ext_addr_mem_range failed : ptr_ext_addr_mem=0x%lx, size=0x%lx(%ul)",
					ptr_ext_addr_mem, size, size);
				ptr_ext_addr_mem = UINT_MAX;
			} else {
				ptr_ext_addr_mem += size;
			}

			if (sz_ext_used_mem < size)
				sz_ext_used_mem = 0;
			else
				sz_ext_used_mem -= size;

			V_DBG(VPU_DBG_MEM_USAGE,
				"type[%d] : release ptr_ext_addr_mem = 0x%x, sz_ext_used_mem = 0x%x, size = 0x%x(%u), !!",
				type, ptr_ext_addr_mem, sz_ext_used_mem, size, size);
		}
	} else {		/*VPU_ENC */

		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_rear_addr_mem = 0x%x size = 0x%x(%u)!!",
			type, ptr_rear_addr_mem, size, size);
		...
		V_DBG(VPU_DBG_MEM_USAGE,
			"type[%d] : release ptr_rear_addr_mem = 0x%x, sz_rear_used_mem = 0x%x, szie = 0x%x(%u)!!",
			type, ptr_rear_addr_mem, sz_rear_used_mem, size, size);

	}
#endif

	if (((unsigned long)sz_remained_mem
		+ (unsigned long)size) > UINT_MAX) {
		V_DBG(VPU_DBG_ERROR,
			"sz_remained_mem_range failed : sz_remained_mem=0x%lx, size=0x%lx(%ul)",
			sz_remained_mem, size, size);
		sz_remained_mem = UINT_MAX;
	} else
...
int vmem_alloc_dedicated_buffer(void)
{
	int ret = 0;
	unsigned int type = 0;
	...
	if (gsVpu4KD2Work_memInfo.kernel_remap_addr == 0) {
		...
		if (gsVpu4KD2Work_memInfo.request_size) {
			...
			lsize = ptr_sw_addr_mem;
			lsize += vpu_4k_d2_alloc_size;
			lsize += vpu_hevc_enc_alloc_size;
			lsize += hevc_alloc_size;
			lsize += jpu_alloc_size;
			lsize += vpu_alloc_size;
			lsize += vp9_alloc_size;
			lsize += seq_alloc_size;
			lsize += user_alloc_size;
			if (lsize > UINT_MAX){
				V_DBG(VPU_DBG_ERROR,
				"gsVpu4KD2Work_memInfo.phy_addr_range failed = 0x%lx(%ul)", type, lsize, lsize);
			} else {
				gsVpu4KD2Work_memInfo.phy_addr = lsize;
			}
	...
	if (gsVpuWork_memInfo.kernel_remap_addr == 0) {
		...
		if (gsVpuWork_memInfo.request_size) {
			...
			if (lsize > UINT_MAX){
				V_DBG(VPU_DBG_ERROR,
				"gsVpuWork_memInfo.phy_addr_range failed = 0x%lx(%ul)", type, lsize, lsize);
			} else {
				gsVpuWork_memInfo.phy_addr = lsize;
			}
	...
	for (type = 0; type < VPU_ENC_MAX_CNT; type++) {
		...
		// SEQ-HEADER BUFFER FOR ENCODER
			gsVpuEncSeqheader_memInfo[type].request_size
			  = PAGE_ALIGN(VPU_ENC_HEADER_BUF_SIZE);
			if (gsVpuEncSeqheader_memInfo[type].request_size) {
				...
				if (lsize > UINT_MAX){
					V_DBG(VPU_DBG_ERROR,
						"gsVpuEncSeqheader_memInfo[%d].phy_addr_range failed = 0x%lx(%ul)", type, lsize, lsize);
					gsVpuEncSeqheader_memInfo[type].phy_addr = UINT_MAX;
				} else {
					gsVpuEncSeqheader_memInfo[type].phy_addr = lsize;
				}
	...
	for (type = 0; type < VPU_INST_MAX; type++) {
		if (gsVpuUserData_memInfo[type].kernel_remap_addr == 0) {
			...
			if (gsVpuUserData_memInfo[type].request_size) {
				...
				if (lsize > UINT_MAX){
					V_DBG(VPU_DBG_ERROR,
					"gsVpuUserData_memInfo[%d].phy_addr_range failed = 0x%lx(%ul)", type, lsize, lsize);
				} else {
					gsVpuUserData_memInfo[type].phy_addr = lsize;
				}
				...
				lsize = (unsigned long)user_alloc_size + (unsigned long)gsVpuUserData_memInfo[type].request_size;
				if (lsize < UINT_MAX)
					user_alloc_size = lsize;
				else {
					V_DBG(VPU_DBG_ERROR,
						"user_alloc_size_range failed : user_alloc_size=0x%u, request_size=0x%u",
...
void vmem_free_dedicated_buffer(void)
{
	int type = 0;
...
static phys_addr_t vmem_request_workbuff_phyaddr(int codec_type,
						  void **remapped_addr)
{
...
static phys_addr_t vmem_request_seqheader_buff_phyaddr(int type,
						 void **remapped_addr,
						 unsigned int *request_size)
{
...
static phys_addr_t vmem_request_userdata_buff_phyaddr(int type,
						   void **remapped_addr,
						 unsigned int *request_size)
{
	if (type < 0 || type >= VPU_INST_MAX)
...
#if defined(CONFIG_TEST_VPU_DRAM_INTLV)
static phys_addr_t vmem_request_ps_buff_phyaddr(int type,
						  void **remapped_addr,
						  unsigned int *request_size)
{
	if (type < 0 || type >= VPU_INST_MAX)
...
static phys_addr_t vmem_request_slice_buff_phyaddr(int type,
						void **remapped_addr,
						unsigned int *request_size)
{
	if (type < 0 || type >= VPU_INST_MAX)
...
static phys_addr_t vmem_request_stream_buff_phyaddr(int type,
						 void **remapped_addr,
						 unsigned int *request_size)
{
	if (type < 0 || type >= VPU_INST_MAX)
...
int vmem_init_memory_info(void)
{
	int ret = 0;
	...
	if (pmap_video.base > UINT_MAX) {
		ret = -24;
		V_DBG(VPU_DBG_ERROR,"pmap_video.base_range failed (%llu)", pmap_video.base);
		goto Error;
	}
	ptr_front_addr_mem = (phys_addr_t)pmap_video.base;

	if (pmap_video.size > UINT_MAX) {
		ret = -23;
		V_DBG(VPU_DBG_ERROR,"pmap_video.size_range failed (%llu)", pmap_video.size);
		goto Error;
	}
	...
	if (pmap_video_sw.base > UINT_MAX) {
		ret = -22;
		V_DBG(VPU_DBG_ERROR,"pmap_video_sw.base_range failed (%llu)", pmap_video_sw.base);
		goto Error;
	}
...
#if DEFINED_CONFIG_VENC_CNT_1to16
	{
		...
		if (pmap_enc.size > UINT_MAX) {
			ret = -21;
			V_DBG(VPU_DBG_ERROR,"pmap_enc.size_range failed (%llu)", pmap_enc.size);
			goto Error;
		}
	...
	if (ptr_rear_addr_mem < sz_enc_mem) {
		ret = -20;
		goto Error;
	}
...
int vmem_deinit_memory_info(void)
{
	V_DBG(VPU_DBG_MEM_SEQ, "enter");
					  ...
					  unsigned int length)
{
	...
	if (lsize < UINT_MAX) {
		...
		V_DBG(VPU_DBG_ERROR,
			"end_phyaddr_range failed : start_addr=0x%u, length=0x%u", start_phyaddr, length);
		return 0;
	}
			   ...
			   vputype type)
{
	...
	case BUFFER_SLICE:
		alloc_info->phy_addr
		 = vmem_request_slice_buff_phyaddr(
			type,
			&(alloc_info->kernel_remap_addr),
			&(alloc_info->request_size));
		if (alloc_info->phy_addr != 0)
	...
	case BUFFER_STREAM:
		alloc_info->phy_addr
		 = vmem_request_stream_buff_phyaddr(
			type,
			&(alloc_info->kernel_remap_addr),
			&(alloc_info->request_size));
		if (alloc_info->phy_addr != 0)
	...
	if (alloc_info->buffer_type == BUFFER_WORK) {
		...
		alloc_info->phy_addr
		 = vmem_request_userdata_buff_phyaddr(
			(int)type,
			&(alloc_info->kernel_remap_addr),
			&(alloc_info->request_size));
		if (alloc_info->phy_addr == 0) {
		...
		alloc_info->phy_addr =
		 = vmem_request_seqheader_buff_phyaddr((int)type,
				&(alloc_info->kernel_remap_addr),
				&(alloc_info->request_size));
		if (alloc_info->phy_addr == 0) {
			V_DBG(VPU_DBG_ERROR,
...
#if DEFINED_CONFIG_VDEC_CNT_345
			alloc_info->phy_addr
			 = vmem_request_phyaddr_dec_ext23(
			 alloc_info->request_size, type);
			if (alloc_info->phy_addr == 0) {
				V_DBG(VPU_DBG_ERROR,
					"type[%d]-buffer[%d] : [0x%x] ALLOC_MEMORY failed.",
		...
		if (alloc_info->buffer_type != BUFFER_FRAMEBUFFER) {
			alloc_info->kernel_remap_addr =
				vmem_get_virtaddr(alloc_info->phy_addr,
					alloc_info->request_size);
			if (alloc_info->kernel_remap_addr == 0) {
				...
				if (vmem_allocated_count[type] < INT_MAX) {
					vmem_allocated_count[type] += 1;
				} else {
					V_DBG(VPU_DBG_ERROR, "vmem_allocated_count[%d] is already FULL", type);
				}
				goto Error;
			}
		}
...
Success:
	...
	if (alloc_info->buffer_type == BUFFER_STREAM)
		vmem_set_page_type(alloc_info->phy_addr, 3);
...
int vmem_proc_free_memory(vputype vtype)
{
	mutex_lock(&mem_mutex);
	if ((vtype>=0) && (vtype<VPU_MAX))
	{
		int i;
		unsigned int j, type;
		...
		type = (unsigned int)vtype;
		for (i = vmem_allocated_count[type]; i > 0; i--) {
			j = i-1;
			if (j >= 20) {
				V_DBG(VPU_DBG_ERROR,"vmem_allocated_count_range failed");
				break;
			}

			if (vmem_alloc_info[type][j].kernel_remap_addr != NULL)
				vmem_release_virtaddr(
				((void *)
				vmem_alloc_info[type][j].kernel_remap_addr),
				vmem_alloc_info[type][j].phy_addr,
				vmem_alloc_info[type][j].request_size);
		#ifdef CONFIG_VPU_ALLOC_MEM_IN_SPECIFIC_SEQUENCE
			vmem_release_phyaddr(
				vmem_alloc_info[type][j].phy_addr,
				vmem_alloc_info[type][j].request_size,
				vtype);
		#else
			if (type == VPU_DEC_EXT4) {
				...
				|| type == VPU_DEC_EXT3) {
			#if DEFINED_CONFIG_VDEC_CNT_345
				vmem_release_phyaddr_dec_ext23(
				vmem_alloc_info[type][j].phy_addr,
				vmem_alloc_info[type][j].request_size,
				vtype);
			#endif
		...
		V_DBG(VPU_DBG_ERROR,"vputype_range failed (%d)", vtype);
	}
	mutex_unlock(&mem_mutex);
	...
	if (type == VPU_DEC) {
		if (vmgr_get_close(VPU_DEC_EXT) && vmgr_get_close(VPU_ENC)
			...
			if (only_decmode)
				...
				lsize = (unsigned long)sz_enc_mem;
				lsize += sz_ext_used_mem;
				lsize += sz_front_used_mem;
				lsize += VPU_SW_ACCESS_REGION_SIZE;
				if ((unsigned long)pmap_video.size < lsize)
					szFreeMem = 0;
				else
					szFreeMem =
						pmap_video.size - lsize;
			}
		}
	} else if (type == VPU_DEC_EXT) {
		if (only_decmode)
			...
			if ((unsigned long)pmap_video.size < lsize)
				...
				szFreeMem = pmap_video.size - lsize;
		}
	} else if (type == VPU_ENC) {
		if (sz_enc_mem > sz_rear_used_mem)
...
{
	if (type <= VPU_DEC_EXT4) {
		unsigned int vsz_ext_remained_mem = 0;
		unsigned int vsz_ext2_remained_mem = 0;
		unsigned int vsz_ext_front_used_mem = 0;
		unsigned int vsz_ext_rear_used_mem = 0;
		unsigned int vsz_ext2_front_used_mem = 0;

	#if DEFINED_CONFIG_VDEC_CNT_345
		vsz_ext_remained_mem = sz_ext_remained_mem;
		vsz_ext_front_used_mem = sz_ext_front_used_mem;
		vsz_ext_rear_used_mem = sz_ext_rear_used_mem;
	#endif
	#if DEFINED_CONFIG_VDEC_CNT_5
		vsz_ext2_remained_mem = sz_ext2_remained_mem;
		vsz_ext2_front_used_mem = sz_ext2_front_used_mem;
	#endif

		V_DBG(VPU_DBG_MEM_SEQ,
			  "type[%d] mem info for vpu :: remain(0x%x : 0x%x : 0x%x), used mem(0x%x/0x%x, 0x%x : 0x%x/0x%x : 0x%x)",
			  type, sz_remained_mem,
			  vsz_ext_remained_mem,
			  vsz_ext2_remained_mem,
			  sz_front_used_mem, sz_ext_used_mem, sz_rear_used_mem,
			  vsz_ext_front_used_mem, vsz_ext_rear_used_mem,
			  vsz_ext2_front_used_mem
			);
	} else if (type >= VPU_ENC && type < VPU_MAX) {
		unsigned int remaiedsize;

		if (sz_enc_mem < sz_rear_used_mem) {
			V_DBG(VPU_DBG_ERROR, "mem_info :: remain(%u) < used mem(%u)", sz_enc_mem, sz_rear_used_mem);
			remaiedsize = 0;
		} else {
			remaiedsize = sz_enc_mem - sz_rear_used_mem;
		}
	...
	mutex_lock(&mem_mutex);
	{
		if ((vmgr_opened() == 0)
			...
			if (!atomic_read(&cntMem_Reference)) {
				...
				ret = vmem_init_memory_info();
				if (ret < 0) {
					V_DBG(VPU_DBG_ERROR,
						  "vmem_init_memory_info() is failure with %d error",
						  ret);
					goto Error1;
				}
				ret = vmem_alloc_dedicated_buffer();
				if (ret < 0) {
					V_DBG(VPU_DBG_ERROR,
						  "vmem_alloc_dedicated_buffer() is failure with %d error",
						  ret);
					goto Error0;
				}
			}
...
Error0:
		if (ret < 0)
			vmem_free_dedicated_buffer();
Error1:
		if (ret < 0)
			vmem_deinit_memory_info();
...
void vdec_get_instance(int *nIdx)
{
	mutex_lock(&mem_mutex);
	{
		...
		if (*nIdx >= 0 && *nIdx < VPU_INST_MAX) {
			if (vdec_used[*nIdx] == 0
				&& (vmem_get_free_memory((vputype)*nIdx + VPU_DEC) > 0))
				nInstance = *nIdx;
		}
		...
		if ((nInstance >= 0) && (nInstance < VPU_INST_MAX)) {
			vdec_used[(unsigned int)nInstance] = 1;
		} else {
			V_DBG(VPU_DBG_ERROR,
...
void vdec_check_instance_available(unsigned int *nAvailable_Inst)
{
	mutex_lock(&mem_mutex);
	{
		...
		for (i = 0; i < VPU_INST_MAX; i++) {
			if (vdec_used[i] == 0
				&& (vmem_get_free_memory((vputype)i + VPU_DEC) > 0)
				&& (nAvailable_Instance < UINT_MAX)) {
				nAvailable_Instance++;
			}
		}

		V_DBG(VPU_DBG_INSTANCE, "Available Instance is #%d",
			nAvailable_Instance);
...
void vdec_clear_instance(int nIdx)
{
	mutex_lock(&mem_mutex);
	{
		...
		V_DBG(VPU_DBG_INSTANCE, "Instance-#%d is cleared", nIdx);
	}
	mutex_unlock(&mem_mutex);
}
...
void venc_get_instance(int *nIdx)
{
	mutex_lock(&mem_mutex);
	{
		...
		if (*nIdx >= 0 && *nIdx < VPU_INST_MAX) {
			if (venc_used[*nIdx] == 0
				&& (vmem_get_free_memory((vputype)*nIdx + VPU_ENC) > 0))
				nInstance = *nIdx;
		}
		...
		if ((nInstance >= 0) && (nInstance < VPU_INST_MAX)) {
			venc_used[(unsigned int)nInstance] = 1;
		} else {
			V_DBG(VPU_DBG_ERROR,
...
void venc_check_instance_available(unsigned int *nAvailable_Inst)
{
	mutex_lock(&mem_mutex);
	{
		...
		for (i = 0; i < VPU_INST_MAX; i++) {
			if (venc_used[i] == 0
				&& (vmem_get_free_memory((vputype)i + VPU_ENC) > 0)
				&& (nAvailable_Instance < UINT_MAX)) {
				nAvailable_Instance++;
			}
		}
