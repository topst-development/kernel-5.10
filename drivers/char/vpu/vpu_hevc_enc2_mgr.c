...
static st_hevc_enc_func *stHENC2Func = INITIAL_NULL;
...
static int get_vpu_hevc_enc2_access_addr_mem(void)
{
	...
	va = vetc_ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * SHARE_POINT_ORDER_HEVC_ENC2), SHARD_POINT_GAP);
	...
	if (type <= VPU_HEVC_ENC_MAX) {
		...
		case VPU_ENC_INIT:
		{
			...
			if ((vpu_lib_dbg & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
				...
				V_DBG(VPU_DBG_ERROR, "[HEVC_ENC2] codec_ip: %d", codec_ip);
				...
				if (codec_ip == 4) {
					...
					V_DBG(VPU_DBG_ERROR, "[HEVC_ENC2] log_mask: %d (%x)", log_mask, log_mask);
					...
					ret = tcc_vpu_hevc_enc2_l(VPU_HEVC_ENC_CTRL_LOG_STATUS, NULL, (void *)(&enc_log), (void *)NULL);
				}
			}
