...
#define dprintk_hevcds(msg...)  V_DBG(VPU_DBG_INFO,  "TCC_HEVC_MGR_SYS: " msg)
#define detailk_hevcds(msg...)  V_DBG(VPU_DBG_INFO,  "TCC_HEVC_MGR_SYS: " msg)
#define err_hevcds(msg...)      V_DBG(VPU_DBG_ERROR, "TCC_HEVC_MGR_SYS[Err]: " msg)
#define info_hevcds(msg...)     V_DBG(VPU_DBG_INFO,  "TCC_HEVC_MGR_SYS[Info]: " msg)
...
#ifdef CONFIG_ARCH_TCC898X
	if (fbus_vhevc_clk != NULL) {
		(void)clk_prepare_enable(fbus_vhevc_clk);
	}
#endif
...
void hmgr_get_clock(struct device_node *pnode)
{
	int i = 0;
	...
	i = 0;
	fbus_vbus_clk = of_clk_get(pnode, i);
	VPU_BUG_ON(fbus_vbus_clk);

	i += 1;
	fbus_chevc_clk = of_clk_get(pnode, i);
	VPU_BUG_ON(fbus_chevc_clk);

#ifdef CONFIG_ARCH_TCC898X
	i += 1;
	fbus_vhevc_clk = of_clk_get(pnode, i);
	VPU_BUG_ON(fbus_vhevc_clk);
#endif

	i += 1;
	fbus_bhevc_clk = of_clk_get(pnode, i);
	VPU_BUG_ON(fbus_bhevc_clk);

	i += 1;
	vbus_hevc_bus_clk = of_clk_get(pnode, i);
	VPU_BUG_ON(vbus_hevc_bus_clk);

#if defined(VBUS_CODA_CORE_CLK_CTRL)
	i += 1;
	vbus_hevc_core_clk = of_clk_get(pnode, i);
...
void hmgr_get_reset(struct device_node *pnode)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	if (pnode == NULL) {
		err_hevcds("device node is null");
...
int hmgr_request_irq(unsigned int irq,
			irqreturn_t (*handler)(int irq, void *dev_idh),
...
{
	LOG_COVERITY("%d", cache_droped_hevcd);
}
