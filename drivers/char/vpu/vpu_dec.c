...
static int vdec_proc_reg_framebuffer3(struct vpu_decoder_data *vdata,
					void *arg, bool fromKernel)
{
	int ret = 0;
	void *pArgs = NULL;
	...
	if (vdata->gsCodecType == STD_MJPG) {
		if (fromKernel) {
			(void)memcpy(&vdata->gsJpuDecBuffer3_Info, arg,
				sizeof(JPU_SET_BUFFER3_t));
		} else {
			if (copy_from_user(&vdata->gsJpuDecBuffer3_Info, arg,
				sizeof(JPU_SET_BUFFER3_t)) != 0U) {
				return -EFAULT;
			}
		}
		pArgs = (void *)&vdata->gsJpuDecBuffer3_Info;

		{
			int ii;
			jpu_dec_buffer3_t *pDecBuffer3 = &vdata->gsJpuDecBuffer3_Info.gsJpuDecBuffer3;

			V_DBG(VPU_DBG_INFO, "%s :: jpu_proc_reg_framebuffer3 :: cnt = %d", vdata->misc->name, pDecBuffer3->m_iFrameBufferCount);
			for (ii = 0; ii < pDecBuffer3->m_iFrameBufferCount; ii++) {
				V_DBG(VPU_DBG_INFO, "[%d] addrY:0x%x, addrCb:0x%x, addrCr:0x%x",
					ii, pDecBuffer3->m_addrFrameBuffer[0][ii][0], pDecBuffer3->m_addrFrameBuffer[0][ii][1], pDecBuffer3->m_addrFrameBuffer[0][ii][2]);
			}
		}
	}
#endif
	...
	if (vdata->gsCodecType == STD_HEVC
			|| vdata->gsCodecType == STD_VP9) {
		if (fromKernel) {
			(void)memcpy(&vdata->gsV4kd2DecBuffer3_Info, arg,
				sizeof(VPU_4K_D2_SET_BUFFER3_t));
		} else {
			if (copy_from_user(&vdata->gsV4kd2DecBuffer3_Info, arg,
				sizeof(VPU_4K_D2_SET_BUFFER3_t)) != 0U) {
				return -EFAULT;
			}
		}
		pArgs = (void *)&vdata->gsV4kd2DecBuffer3_Info;

		{
			int ii;
			vpu_4K_D2_dec_buffer3_t *pDecBuffer3 = &vdata->gsV4kd2DecBuffer3_Info.gsV4kd2DecBuffer3;

			V_DBG(VPU_DBG_INFO, "%s :: vpu-4k-d2_proc_reg_framebuffer3 :: cnt = %d", vdata->misc->name, pDecBuffer3->m_iFrameBufferCount);
			for (ii = 0; ii < pDecBuffer3->m_iFrameBufferCount; ii++) {
				V_DBG(VPU_DBG_INFO, "[%d] addrY:0x%x, addrCb:0x%x, addrCr:0x%x, mvcol:0x%x, fbcY:0x%x, fbcC:0x%x",
					ii, pDecBuffer3->m_addrFrameBuffer[0][ii][0], pDecBuffer3->m_addrFrameBuffer[0][ii][1], pDecBuffer3->m_addrFrameBuffer[0][ii][2],
					pDecBuffer3->m_addrFrameBuffer[0][ii][3], pDecBuffer3->m_addrFrameBuffer[0][ii][4], pDecBuffer3->m_addrFrameBuffer[0][ii][5]);
			}
		}
...
#ifdef CONFIG_SUPPORT_TCC_WAVE410_HEVC
	if (vdata->gsCodecType == STD_HEVC) {
		err_dec("%s ::hevc dec not support this !!", vdata->misc->name);
		ret = -0x999;
	}
#endif
#ifdef CONFIG_SUPPORT_TCC_VPU
	if (isVPUDecSupportCodec(vdata->gsCodecType) == 1) {
		if (fromKernel) {
			(void)memcpy(&vdata->gsVpuDecBuffer3_Info, arg,
				sizeof(VDEC_SET_BUFFER3_t));
		} else {
			if (copy_from_user(&vdata->gsVpuDecBuffer3_Info, arg,
				sizeof(VDEC_SET_BUFFER3_t)) != 0U) {
				return -EFAULT;
			}
		}
		pArgs = (void *)&vdata->gsVpuDecBuffer3_Info;

		{
			int ii;
			dec_buffer3_t *pDecBuffer3 = &vdata->gsVpuDecBuffer3_Info.gsVpuDecBuffer3;

			V_DBG(VPU_DBG_INFO, "%s :: vpu_proc_reg_framebuffer3 :: cnt = %d", vdata->misc->name, pDecBuffer3->m_ulFrameBufferCount);
			for (ii = 0; ii < pDecBuffer3->m_ulFrameBufferCount; ii++) {
				V_DBG(VPU_DBG_INFO, "[%d] addrY:0x%x, addrCb:0x%x, addrCr:0x%x, mvcol:0x%x",
			...
			V_DBG(VPU_DBG_INFO, "avcSliceSave:0x%x, vp8mbdata:0x%x", pDecBuffer3->m_AvcSliceSaveBufferAddr, pDecBuffer3->m_Vp8MbDataSaveBufferAddr);
		}
	}
#endif

	if (pArgs != NULL) {
		vdec_inter_add_list(vdata, VPU_DEC_REG_FRAME_BUFFER3, pArgs);
	}
...
int vdec_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (vma->vm_end < vma->vm_start) {
		V_DBG(VPU_DBG_ERROR,
		  "%s :: mmap :: vm_address_range failed : start_addr(%u), end_addr(%u)",
...
static long vdec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case V_DEC_REG_FRAME_BUFFER_KERNEL:
		...
		break;
	case V_DEC_REG_FRAME_BUFFER3:
	case V_DEC_REG_FRAME_BUFFER3_KERNEL:
		lret = (long)vdec_proc_reg_framebuffer3(vdata, uarg.pv_data,
				cmd == (unsigned int)V_DEC_REG_FRAME_BUFFER3_KERNEL);
		break;
	case V_DEC_DECODE:
