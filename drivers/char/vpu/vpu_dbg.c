...
unsigned int module_param_vdbg_lib = (unsigned int)INITIAL_ZERO;
unsigned int module_param_vdbg_drv = (unsigned int)INITIAL_ZERO;
...
static int gs_iUseCustomLog = INITIAL_ZERO;
...
static int gs_iYuvDumpCount = INITIAL_ZERO;
static int gs_iYuvDumpRequestedCount = INITIAL_ZERO;
static spinlock_t gs_yuvThreadInfoLock;
static bool gs_bThreadDumpRequested = (bool)INITIAL_ZERO;
struct task_struct *gs_pYuvThreadTask = INITIAL_NULL;
...

			{
				...
				if (p_src_y == 0 || p_src_cb == 0 || p_src_cr == 0) {
					...
					// Y
					{
						...
						p_src_y_virt = vetc_ioremap(p_src_y, save_size);
					...
					if (yuv_interleaved) { //NV12
						...
						p_src_cbcr_virt = vetc_ioremap(p_src_cb, save_size);
						...
						// U
						{
							void *p_src_cb_virt = vetc_ioremap(p_src_cb, save_size);
						...
						// V
						{
							void *p_src_cr_virt = vetc_ioremap(p_src_cb, save_size);
...
static int gs_iUseCustomLogYuv = INITIAL_ZERO;
