	...
	if (ready) {
		...
		if (dio != NULL) {
			if (dio->op_state != V2_DIO_READY) {
				(void)pr_info("[%s][vtype:%d] output is delayed (pop: %ld, push: %ld, op state: %d)",
...
// vpu v2: op - clear fb slot(s)
static long v2dec_ioctl_op_clrdata(struct vpu_decoder_data *vdata, void *arg)
{
	long ret;
	...
	/* get access point of flex handle for 'clrdata' input (not output) process */
	flx_index = V2D_FINDEX(V2D_IP_FRM_CLEAR);
	flo = vdata->flx_list[flx_index];
	if (flo == NULL) {
		pr_err("[%s] failed to get ap: flo (max. %u)", __func__, V2D_FI_CLEAR_MAX);
		return -EFAULT;
	}
...
// vpu v2: op - clear fb slot(s)
static long v2dec_ioctl_op_fludata(struct vpu_decoder_data *vdata, void *arg)
{
	long ret;
	struct v2hw_flex_io *flo;
	size_t flx_index;
	...
	flo = vdata->flx_list[flx_index];
	if (flo == NULL) {
		pr_err("[%s] failed to get ap: flo (max. %u)", __func__, V2D_FI_FLUSH_MAX);
		return -EFAULT;
	}
...
static long v2dec_ioctl_ip_drndata(struct vpu_decoder_data *vdata, void *arg)
{
	...
	if (input_exists) {
		...
		(void)pr_info("[%s][vtype:%d] pop delayed flx frame (index:%ld)",
...
// vpu v2: op - reset
//
static int v2dec_ioctl_op_rstdata(struct vpu_decoder_data *vdata, void *arg)
{
	int32_t ret;
	struct v2hw_flex_io flx_meta;
	struct v2hw_flex_io *flo = &flx_meta;
...
long vdec_ioctl_flexio(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case V2D_OP_FRM_CLEAR:
		lret = v2dec_ioctl_op_clrdata(vdata, uarg.pv_data);
		break;
	...
	case V2D_OP_FRM_FLUSH:
		lret = v2dec_ioctl_op_fludata(vdata, uarg.pv_data);
		break;
	...
	case V2D_OP_DRV_RST:
		lret = v2dec_ioctl_op_rstdata(vdata, uarg.pv_data);
		break;
