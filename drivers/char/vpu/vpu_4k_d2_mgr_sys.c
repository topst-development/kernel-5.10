...
#define dprintk_4kd2s(msg...)  V_DBG(VPU_DBG_INFO,  "TCC_4K_D2_VMGR_SYS: " msg)
#define detailk_4kd2s(msg...)  V_DBG(VPU_DBG_INFO,  "TCC_4K_D2_VMGR_SYS: " msg)
#define err_4kd2s(msg...)      V_DBG(VPU_DBG_ERROR, "TCC_4K_D2_VMGR_SYS[Err]: " msg)
#define info_4kd2s(msg...)     V_DBG(VPU_DBG_INFO,  "TCC_4K_D2_VMGR_SYS[Info]: " msg)
...
void vmgr_4k_d2_enable_clock(int vbus_no_ctrl, int only_clk_ctrl)
{
	...
	if ((fbus_vbus_clk != NULL) && (vbus_no_ctrl == 0)) {
		(void)clk_prepare_enable(fbus_vbus_clk);
	}

	if (fbus_chevc_clk != NULL) {
		(void)clk_prepare_enable(fbus_chevc_clk);
	}

	if (fbus_bhevc_clk != NULL) {
		(void)clk_prepare_enable(fbus_bhevc_clk);
	}

	if (vbus_hevc_core_clk != NULL) {
		(void)clk_prepare_enable(vbus_hevc_core_clk);
	}

	if (vbus_hevc_bus_clk != NULL) {
		(void)clk_prepare_enable(vbus_hevc_bus_clk);
	}
	...
	if (only_clk_ctrl == 0) {
		...
		if (ret != 0) {
			err_4kd2s("vpu_optee_open: failed !! (ret: %d)", ret);
		} else {
			ret = vpu_optee_fw_load(TYPE_VPU_4K_D2);
			if (ret != 0) {
				err_4kd2s("vpu_optee_fw_load: failed !! (ret: %d)", ret);
			}
		}
	}
#else
	if (only_clk_ctrl == 0) {
		VPU_DONOTHING(only_clk_ctrl);
	}
#endif
}
...
void vmgr_4k_d2_get_clock(struct device_node *pnode)
{
	if (pnode == NULL) {
		err_4kd2s("device node is null");
	}

	fbus_vbus_clk = of_clk_get(pnode, 0);
	VPU_BUG_ON(fbus_vbus_clk);

	fbus_chevc_clk = of_clk_get(pnode, 1);
	VPU_BUG_ON(fbus_chevc_clk);

	fbus_bhevc_clk = of_clk_get(pnode, 2);
	VPU_BUG_ON(fbus_bhevc_clk);

	vbus_hevc_bus_clk = of_clk_get(pnode, 3);
	VPU_BUG_ON(vbus_hevc_bus_clk);

	vbus_hevc_core_clk = of_clk_get(pnode, 4);
	VPU_BUG_ON(vbus_hevc_core_clk);
}
...
void vmgr_4k_d2_put_clock(void)
{
	...
	if (fbus_vbus_clk != NULL) {
		clk_put(fbus_vbus_clk);
		fbus_vbus_clk = NULL;
	}
}

void vmgr_4k_d2_change_clock(int width, int height)
{
...
// clock_init_early
	...
	if (prev_resolution != curr_resolution) {
		...
		if (fbus_bhevc_clk != NULL) {
			...
			if (err) {
				(void)pr_err("cannot change fbus_bhevc_clk rate to %ld: %d\n", bhevc_clk_value, err);
			} else {
				bclk_changed |= 0x2;
			}
		}
		...
		if (fbus_chevc_clk != NULL) {
			...
			if (err) {
				(void)pr_err("cannot change fbus_chevc_clk rate to %ld: %d\n", chevc_clk_value, err);
			} else {
				bclk_changed |= 0x4;
			}
		}
...
void vmgr_4k_d2_get_reset(struct device_node *pnode)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	if (pnode == NULL) {
		err_4kd2s("device node is null");
	...
	vbus_hevc_bus_reset = of_reset_control_get_by_index(pnode, 0);
	VPU_BUG_ON(vbus_hevc_bus_reset);

	vbus_hevc_core_reset = of_reset_control_get_by_index(pnode, 1);
	VPU_BUG_ON(vbus_hevc_core_reset);
#endif
}
...
void vmgr_4k_d2_hw_assert(void)
{
	...
	if (vbus_hevc_bus_reset != NULL) {
		(void)reset_control_assert(vbus_hevc_bus_reset);
	}

	if (vbus_hevc_core_reset != NULL) {
		(void)reset_control_assert(vbus_hevc_core_reset);
	}
...
void vmgr_4k_d2_hw_deassert(void)
{
	...
	if (vbus_hevc_core_reset != NULL) {
		(void)reset_control_deassert(vbus_hevc_core_reset);
	}

	if (vbus_hevc_bus_reset != NULL) {
		(void)reset_control_deassert(vbus_hevc_bus_reset);
	}
...
{
	LOG_COVERITY("%d", cache_droped_4kd2);
}
