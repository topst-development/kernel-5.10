...
#define dprintk(msg...)  V_DBG(VPU_DBG_INFO, "TCC_VP9_MGR:" msg)
#define detailk(msg...)  V_DBG(VPU_DBG_INFO, "TCC_VP9_MGR:" msg)
#define cmdk(msg...)     V_DBG(VPU_DBG_INFO, "TCC_VP9_MGR [Cmd]:" msg)
#define err(msg...)      V_DBG(VPU_DBG_ERROR, "TCC_VP9_MGR [Err]:" msg)
...
struct VpuList vp9_mgr_vlist;
// Control only once!!
static struct mgr_data_t vp9mgr_data;
static struct task_struct *kidle_task_vp9d;	// = NULL;

extern int tcc_vp9_dec(int Op, vcodec_handle_t *pHandle,
	 void *pParam1, void *pParam2);

int vp9mgr_opened(void)
{
	if (atomic_read(&vp9mgr_data.opened) == 0)
		return 0;
	return 1;
}
...
{
	return atomic_read(&vp9mgr_data.opened);
}
...
static void vp9mgr_close_all(int bfreemem)
{
	(void)vp9mgr_set_close(VPU_DEC, 1, bfreemem);
	(void)vp9mgr_set_close(VPU_DEC_EXT, 1, bfreemem);
	(void)vp9mgr_set_close(VPU_DEC_EXT2, 1, bfreemem);
	(void)vp9mgr_set_close(VPU_DEC_EXT3, 1, bfreemem);
	(void)vp9mgr_set_close(VPU_DEC_EXT4, 1, bfreemem);
}
	 ...
	 int Op, int *result)
{
	...
	if (!vp9mgr_get_close(type)) {
		...
		cmd_list->handle = vp9mgr_data.handle[(unsigned int)type];
		cmd_list->args = NULL;
		cmd_list->comm_data = NULL;
		cmd_list->vpu_result = result;
		vp9mgr_list_manager(cmd_list, (unsigned int)LIST_ADD);
...
static int vp9mgr_internal_handler(void)
{
	...
	if (vp9mgr_data.check_interrupt_detection) {
		if (atomic_read(&vp9mgr_data.oper_intr) > 0) {
			...
			ret =
				...
				vp9mgr_data.oper_wq,
				atomic_read(&vp9mgr_data.oper_intr) > 0,
							(long)jtimeout);
			...
			if (atomic_read(&vp9mgr_data.oper_intr) > 0) {
				...
				if (forced_error_count-- <= 0) {
					...
					vetc_dump_reg_all(vp9mgr_data.base_addr,
						 "vp9mgr_internal_handler force-timed_out");
				} else {
					ret_code = RETCODE_SUCCESS;
				}
#else
				...
				err(
				"[CMD 0x%x][%d]: vp9 timed_out(ref %d msec) => oper_intr[%d]!! [%d]th frame len %d\n",
					vp9mgr_data.current_cmd,
					ret,
					timeout,
					atomic_read(&vp9mgr_data.oper_intr),
					vp9mgr_data.nDecode_Cmd,
					vp9mgr_data.szFrame_Len);
				vetc_dump_reg_all(vp9mgr_data.base_addr,
					"vp9mgr_internal_handler timed_out");
				ret_code = RETCODE_CODEC_EXIT;
			}
		}
...
static int vp9mgr_process(vputype type, int cmd, int pHandle, void *args)
{
	int ret = 0;
	...
	if (type < VPU_ENC) {
		...
		{
		VP9_INIT_t *arg = (VP9_INIT_t *)args;

		vp9mgr_data.handle[type] = 0x00;
		arg->gsVp9DecInit.m_RegBaseVirtualAddr
		 = (codec_addr_t)vp9mgr_data.base_addr;
		arg->gsVp9DecInit.m_Memcpy =
		 (void * (*) (void *, const void *, unsigned int))
		 vetc_memcpy;
		arg->gsVp9DecInit.m_Memset =
		 (void  (*) (void *, int, unsigned int))
		 vetc_memset;
		arg->gsVp9DecInit.m_Interrupt =
		 (int   (*) (void))vp9mgr_internal_handler;
		arg->gsVp9DecInit.m_Ioremap =
		 (void * (*) (phys_addr_t, unsigned int))
		 vetc_ioremap;
		arg->gsVp9DecInit.m_Iounmap =
		 (void  (*) (void *))vetc_iounmap;

		vp9mgr_data.check_interrupt_detection = 1;
		vp9mgr_data.bDiminishInputCopy =
		 (arg->gsVp9DecInit.m_uiDecOptFlags & (1<<26)) ? 1 : 0;

		dprintk(
		"Dec :: Init In => Reg: 0x%p/0x%x, format : %d, Stream(0x%x/0x%x, 0x%x)\n",
			vp9mgr_data.base_addr,
			arg->gsVp9DecInit.m_RegBaseVirtualAddr,
			arg->gsVp9DecInit.m_iBitstreamFormat,
			arg->gsVp9DecInit.m_BitstreamBufAddr[PA],
			arg->gsVp9DecInit.m_BitstreamBufAddr[VA],
			arg->gsVp9DecInit.m_iBitstreamBufSize);
		dprintk(
		"Dec :: Init In => optFlag 0x%x, Entropy(0x%x/0x%x, 0x%x), PlayEn: %d, MaxRes: %d\n",
			arg->gsVp9DecInit.m_uiDecOptFlags,
			arg->gsVp9DecInit.m_EntropySaveBuffer[PA],
			arg->gsVp9DecInit.m_EntropySaveBuffer[VA],
			arg->gsVp9DecInit.m_iEntropySaveBufferSize,
			arg->gsVp9DecInit.m_iFilePlayEnable,
			arg->gsVp9DecInit.m_iMaxResolution);
		...
		ret = tcc_vp9_dec(
				cmd & ~VPU_BASE_OP_KERNEL,
					(void *)(&arg->gsVp9DecHandle),
					(void *)(&arg->gsVp9DecInit),
					(void *)NULL);
		if (ret != RETCODE_SUCCESS) {
			err("Dec :: Init Done with ret(0x%x)\n", ret);
			if (ret != RETCODE_CODEC_EXIT) {
				vetc_dump_reg_all(
					vp9mgr_data.base_addr, "init failure");
			}
		}
		...
		vp9mgr_data.iTime[type].accumulated_proc_time =
		 vp9mgr_data.iTime[type].accumulated_frame_cnt = 0;
	...
	break;
		...
		{
			void *arg = (VP9_SEQ_HEADER_t *)args;
			int iSize;
			...
			vp9mgr_data.szFrame_Len = iSize
			 = vp9mgr_data.bDiminishInputCopy
				? (int)((VP9_DECODE_t *)arg)->gsVp9DecInput
			...
			ret = tcc_vp9_dec(cmd & ~VPU_BASE_OP_KERNEL,
				...
				(vp9mgr_data.bDiminishInputCopy
				? (void *)(&((VP9_DECODE_t *)arg)->gsVp9DecInput)
				: (void *)iSize),
				(void *)gsVp9DecInitialInfo);

			dprintk(
			"Dec :: SEQ_HEADER out 0x%x\n res info. %d - %d - %d, %d - %d - %d\n",
				ret,
			...
			dprintk(
			"Dec :: SEQ_HEADER out(0x%x) MinFrameBuffer(%d/0x%x), TempBuffer: 0x%x, Version: %d, Profile: %d, BitDepth: %d/%d, ErrorReason: %d\n",
				ret,
		...
		{
			VP9_SET_BUFFER_t *arg = (VP9_SET_BUFFER_t *)args;

			dprintk(
			"Dec :: REG_FRAME_BUFFER in :: Frame(0x%x/0x%x, %d), Temp(0x%x/0x%x, 0x%x)\n",
				arg->gsVp9DecBuffer.m_FrameBufferStartAddr[0],
				arg->gsVp9DecBuffer.m_FrameBufferStartAddr[1],
				arg->gsVp9DecBuffer.m_iFrameBufferCount,
				arg->gsVp9DecBuffer.m_TempBufferAddr[0],
				arg->gsVp9DecBuffer.m_TempBufferAddr[1],
				arg->gsVp9DecBuffer.m_iTempBufferSize);

			ret =
				 tcc_vp9_dec(cmd & ~VPU_BASE_OP_KERNEL,
				(vcodec_handle_t *) &pHandle,
				(void *)(&arg->gsVp9DecBuffer),
				(void *)NULL);
		...
		{
			VP9_DECODE_t *arg = (VP9_DECODE_t *)args;
			...
			vp9mgr_data.szFrame_Len =
				arg->gsVp9DecInput.m_iBitstreamDataSize;
			dprintk(
			"Dec :: Dec In => Stream(0x%x/0x%x, 0x%x), FrameSearch: %d\n",
				arg->gsVp9DecInput.m_BitstreamDataAddr[PA],
				arg->gsVp9DecInput.m_BitstreamDataAddr[VA],
				arg->gsVp9DecInput.m_iBitstreamDataSize,
			...
			ret =
				tcc_vp9_dec(cmd & ~VPU_BASE_OP_KERNEL,
				(vcodec_handle_t *) &pHandle,
				(void *)(&arg->gsVp9DecInput),
				(void *)(&arg->gsVp9DecOutput));
			dprintk(
			"Dec :: Dec Out => %d - %d - %d, %d - %d - %d\n",
				arg->gsVp9DecOutput.m_DecOutInfo.m_iWidth,
				arg->gsVp9DecOutput.m_DecOutInfo
				  .m_CropInfo.m_iCropLeft,
				arg->gsVp9DecOutput.m_DecOutInfo
				  .m_CropInfo.m_iCropRight,
				arg->gsVp9DecOutput.m_DecOutInfo.m_iHeight,
				arg->gsVp9DecOutput.m_DecOutInfo
				  .m_CropInfo.m_iCropTop,
				arg->gsVp9DecOutput.m_DecOutInfo
				  .m_CropInfo.m_iCropBottom);

			dprintk(
			"Dec :: Dec Out => ret[%d/%d] !! PicType[%d], OutIdx[%d/%d], OutStatus[%d/%d]\n",
			  ret, -ret,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iPicType,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDispOutIdx,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDecodedIdx,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iOutputStatus,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDecodingStatus);

			dprintk(
			"Dec :: Dec Out => disp_Idx(%d), 0x%x 0x%x 0x%x / 0x%x 0x%x 0x%x\n",
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDispOutIdx,
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[PA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[PA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[PA][2],
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[VA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[VA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_pDispOut[VA][2]);

			dprintk(
			"Dec :: Dec Out => dec_Idx(%d), 0x%x 0x%x 0x%x / 0x%x 0x%x 0x%x\n",
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDecodedIdx,
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[PA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[PA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[PA][2],
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[VA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[VA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_pCurrOut[VA][2]);

			dprintk(
			"Dec :: Dec Out => BitDepth (%d,%d), Stride(%d)\n",
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iBitDepthLuma,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iBitDepthChroma,
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iPicStride);

			dprintk(
			"Dec :: Dec Out => Scale(%d/%d), Stride(%d), 0x%x 0x%x 0x%x / 0x%x 0x%x 0x%x\n",
			  arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_iDscaleWidth,
			  arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_iDscaleHeight,
			  arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_iDscaleStride,
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[PA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[PA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[PA][2],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[VA][0],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[VA][1],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DscaleInfo.m_pDscaleDispOut[VA][2]);

			dprintk(
			"Dec :: Dec Out => Compressed (0x%x/0x%x - 0x%x) / Table(0x%x/0x%x - 0x%x)\n",
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
			   .m_DispCompressedInfo.m_CompressedY[PA],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DispCompressedInfo.m_CompressedCb[PA],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
			   .m_DispCompressedInfo.m_iCompressionTableLumaSize,
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
				.m_DispCompressedInfo.m_CompressionTableLuma[PA],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
			   .m_DispCompressedInfo.m_CompressionTableChroma[PA],
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
			   .m_DispCompressedInfo.m_iCompressionTableChromaSize);
			if (ret < 0)
				ret = -ret;
			...
			dprintk("VD-Out Disp=%d[0x%x]_Dec=%d[0x%x],\n",
			  ...
			  arg->gsVp9DecOutput.m_DecOutInfo.m_iDecodedIdx,
			  (unsigned int)arg->gsVp9DecOutput.m_DecOutInfo
			   .m_CurrCompressedInfo.m_CompressedY[PA]);

			if (arg->gsVp9DecOutput.m_DecOutInfo.m_iDecodingStatus
				== VPU_DEC_BUF_FULL)
				err("Buffer full\n");
		...
		case VPU_DEC_FLUSH_OUTPUT_KERNEL:
		{
			...
			dprintk("Dec :: VP9_DEC_FLUSH_OUTPUT!!\n");
			ret = tcc_vp9_dec(cmd & ~VPU_BASE_OP_KERNEL,
					(vcodec_handle_t *) &pHandle,
					(void *)(&arg->gsVp9DecInput),
					(void *)(&arg->gsVp9DecOutput));
		...
		case GET_RING_BUFFER_STATUS:
		{
			VP9_RINGBUF_GETINFO_t *arg
			 = (VP9_RINGBUF_GETINFO_t *)args;

			vp9mgr_data.check_interrupt_detection = 1;
			ret = tcc_vp9_dec(cmd, (vcodec_handle_t *) &pHandle,
					(void *)NULL,
					(void *)(&arg->gsVp9DecRingStatus));
		}
		break;
		...
		case FILL_RING_BUFFER_AUTO:
		{
			...
			ret = tcc_vp9_dec(cmd, (vcodec_handle_t *) &pHandle,
					(void *)(&arg->gsVp9DecInit),
					(void *)(&arg->gsVp9DecRingFeed));
			dprintk(
			"Dec :: ReadPTR : 0x%08x, WritePTR : 0x%08x\n",
				vetc_reg_read(vp9mgr_data.base_addr, 0x120),
				vetc_reg_read(vp9mgr_data.base_addr, 0x124));
		}
		break;
		...
		case VPU_UPDATE_WRITE_BUFFER_PTR:
		{
			VP9_RINGBUF_SETBUF_PTRONLY_t *arg
			 = (VP9_RINGBUF_SETBUF_PTRONLY_t *)args;
		...
		{
			VP9_SEQ_HEADER_t *arg = (VP9_SEQ_HEADER_t *)args;

			vp9mgr_data.check_interrupt_detection = 1;
			ret = tcc_vp9_dec(cmd, (vcodec_handle_t *) &pHandle,
				(void *)(&arg->gsVp9DecInitialInfo), NULL);
		}
		break;
#endif
		...
		{
			VP9_GET_VERSION_t *arg = (VP9_GET_VERSION_t *)args;
	...
	if (cmd == VPU_DEC_DECODE) {
		printMeasurementTime((void *)&vp9mgr_data, type, 1, time_gap_us);
	}
#endif
...
static int vp9mgr_proc_exit_by_external(struct VpuList *list, int *result,
					 unsigned int type)
{
	if (!vp9mgr_get_close(type) && vp9mgr_data.handle[type] != 0x00) {
		...
		vp9mgr_list_manager(list, (unsigned int)LIST_ADD);
...
static void vp9mgr_wait_process(int wait_ms)
{
...
static int vp9mgr_external_all_close(int wait_ms)
{
	int type = 0;
	int max_count = 0;
	int ret;
	...
	for (type = 0; type < VP9_MAX; type++) {
		if (vp9mgr_proc_exit_by_external
			(&vp9mgr_data.vList[type], &ret, type)) {
...
static int vp9mgr_cmd_open(char *str)
{
	int ret = 0;

	dprintk("======> _vp9mgr_%s_open In!! %d'th\n",
		str, atomic_read(&vp9mgr_data.opened));
	...
	if (atomic_read(&vp9mgr_data.opened) == 0) {
		...
		if (1) {
			void *vbus_cfg =
				...
				HwVIDEOBUSCONFIG_BASE, PAGE_SIZE);

			if (vbus_cfg) {
				// for VP9 fuse dec
				vetc_reg_write(vbus_cfg, 0x60, 0xFFFFFFFF);
	...
	atomic_inc(&vp9mgr_data.opened);

	dprintk("======> _vp9mgr_%s_open Out!! %d'th\n", str,
		atomic_read(&vp9mgr_data.opened));

	return 0;
}

static int vp9mgr_cmd_release(char *str)
{
	dprintk("======> _vp9mgr_%s_release In!! %d'th\n", str,
		atomic_read(&vp9mgr_data.opened));

	if (atomic_read(&vp9mgr_data.opened) > 0) {
		atomic_dec(&vp9mgr_data.opened);
	}
	...
	if (atomic_read(&vp9mgr_data.opened) == 0) {
		...
		if (!vp9mgr_data.bVpu_already_proc_force_closed) {
			...
			vp9mgr_external_all_close(200);
			vp9mgr_data.external_proc = 0;
			vp9mgr_wait_process(200);
		}
		vp9mgr_data.bVpu_already_proc_force_closed = (bool)false;
#endif
		...
		if (alive_cnt) {
			(void)pr_info("VP9 might be cleared by force.\n");
		}
	...
	"======> _vp9mgr_%s_release Out!! %d'th, total = %d  - DEC(%d/%d/%d/%d/%d)\n",
		...
		atomic_read(&vp9mgr_data.opened),
		vp9mgr_data.nOpened_Count,
		vp9mgr_get_close(VPU_DEC),
		vp9mgr_get_close(VPU_DEC_EXT),
		vp9mgr_get_close(VPU_DEC_EXT2),
		vp9mgr_get_close(VPU_DEC_EXT3),
		vp9mgr_get_close(VPU_DEC_EXT4));

	return 0;
}

static long vp9mgr_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
	int ret = 0;
	CONTENTS_INFO info;
	...
	case VPU_CHECK_CODEC_STATUS_KERNEL:
		{
			if (cmd == (unsigned int)VPU_CHECK_CODEC_STATUS_KERNEL) {
				(void)memcpy((int *)arg, vp9mgr_data.closed,
					sizeof(vp9mgr_data .closed));
			} else {
				if (copy_to_user((int *)arg, vp9mgr_data.closed,
					sizeof(vp9mgr_data .closed)) != 0U) {
					ret = -EFAULT;
				}
			}
		}
	...
	case VPU_TRY_FORCE_CLOSE_KERNEL:
		{
			if (!vp9mgr_data.bVpu_already_proc_force_closed) {
				...
				vp9mgr_data.external_proc = 0;
				vp9mgr_wait_process(200);
				vp9mgr_data.bVpu_already_proc_force_closed =
					(bool)true;
			}
		}
		break;
...
#ifdef CONFIG_COMPAT
static long vp9mgr_compat_ioctl(struct file *filep, unsigned int cmd,
				 unsigned long arg)
{
	return vp9mgr_ioctl(filep, cmd, (unsigned long)compat_ptr(arg));
}
#endif

static irqreturn_t vp9mgr_isr_handler(int irq, void *dev_id)
{

	detailk("%s\n", __func__);

	atomic_inc(&vp9mgr_data.oper_intr);
...
static int vp9mgr_open(struct inode *pinode, struct file *filp)
{
	...
	vp9mgr_cmd_open("file");
	mutex_unlock(&vp9mgr_data.comm_data.file_mutex);
#endif
...
static int vp9mgr_release(struct inode *pinode, struct file *filp)
{
#ifdef USE_DEV_OPEN_CLOSE_IOCTL
	dprintk("%s In!! %d'th\n", __func__,
		atomic_read(&vp9mgr_data.dev_file_opened));
...
struct VpuList *vp9mgr_list_manager(struct VpuList *args, unsigned int cmd)
{
	...
	case LIST_ADD:
		*oper_data->vpu_result |= RET1;
		list_add_tail(&oper_data->list,
			&vp9mgr_data.comm_data.main_list);
	...
	case LIST_IS_EMPTY:
		if (list_empty(&vp9mgr_data.comm_data.main_list) != 0) {
			ret = &vp9_mgr_vlist;
		}
		break;

	case LIST_GET_ENTRY:
		ret = list_first_entry(&vp9mgr_data.comm_data.main_list,
				struct VpuList, list);
		break;
	default:
		/* Nothing to do */
...
static int vp9mgr_operation(void)
{
	int oper_finished;
	struct VpuList *oper_data = NULL;

	while (!vp9mgr_list_manager(NULL, (unsigned int)LIST_IS_EMPTY)) {
		vp9mgr_data.cmd_processing = 1;

		oper_finished = 1;
		dprintk("%s :: not empty cmd_queued(%d)\n",
			__func__, vp9mgr_data.cmd_queued);

		oper_data = vp9mgr_list_manager(NULL, (unsigned int)LIST_GET_ENTRY);
		if (oper_data == NULL) {
			err("data is null\n");
			vp9mgr_data.cmd_processing = 0;
			return 0;
		...
		*oper_data->vpu_result |= RET2;
		...
		dprintk(
		 "%s [%d] :: cmd = 0x%x, vp9mgr_data.cmd_queued(%d)\n",
			...
			*oper_data->vpu_result
			...
			if (*oper_data->vpu_result != RETCODE_SUCCESS) {
				if ((*oper_data->vpu_result
					...
					err(
					"vp9mgr_out[0x%x/%d] :: type = %d, vp9mgr_data.handle = 0x%x, cmd = 0x%x, frame_len %d\n",
						*oper_data->vpu_result,
						*oper_data->vpu_result,
						oper_data->type,
						oper_data->handle,
		...
		if (oper_finished != 0) {
			if ((oper_data->comm_data != NULL)
				&& (atomic_read(&vp9mgr_data.opened) != 0)) {
		...
		vp9mgr_list_manager(oper_data, (unsigned int)LIST_DEL);
...
static int vp9mgr_thread(void *kthread)
{
	unsigned long jtimeout;
	...
	do {
		if (vp9mgr_list_manager(NULL, (unsigned int)LIST_IS_EMPTY)) {
			vp9mgr_data.cmd_processing = 0;

			(void)wait_event_interruptible_timeout(
				vp9mgr_data.comm_data.thread_wq,
			...
			if ((atomic_read(&vp9mgr_data.opened) != 0)
				...
				oper_data =
					vp9mgr_list_manager(NULL, (unsigned int)LIST_GET_ENTRY);
				if (oper_data) {
					vp9mgr_list_manager(oper_data,
							(unsigned int)LIST_DEL);
				}
			}
		}
...
static int vp9mgr_mmap(struct file *file, struct vm_area_struct *vma)
{
#if defined(CONFIG_TCC_MEM)
	if (range_is_allowed(vma->vm_pgoff, vma->vm_end - vma->vm_start) < 0) {
		(void)pr_info("%s: this address is not allowed\n", __func__);
		return -EAGAIN;
	}
#endif
	...
	if (remap_pfn_range
		...
		vma->vm_page_prot)) {
		(void)pr_info("%s :: remap_pfn_range failed\n", __func__);
		return -EAGAIN;
	}

	vma->vm_ops = NULL;
	vm_flags_set(vma->vm_flags, VM_IO);
	vm_flags_set(vma->vm_flags, VM_DONTEXPAND | VM_PFNMAP);

	return 0;
}

static const struct file_operations vp9mgr_fops = {
	.open = vp9mgr_open,
	.release = vp9mgr_release,
	.mmap = vp9mgr_mmap,
	.unlocked_ioctl = vp9mgr_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = vp9mgr_compat_ioctl,
#endif
};

static struct miscdevice vp9mgr_misc_device = {
	MISC_DYNAMIC_MINOR,
	VP9MGR_NAME,
	&vp9mgr_fops,
};
...
int vp9mgr_probe(struct platform_device *pdev)
{
	...
	(void)memset(&vp9mgr_data, 0, sizeof(struct mgr_data_t));
	for (type = 0; type < VP9_MAX; type++) {
	...
	dprintk(
	"============> VP9 base address [0x%x -> 0x%p], irq num [%d]\n",
	...
	mutex_init(&(vp9mgr_data.comm_data.file_mutex));
	...
	ret =
		vp9mgr_request_irq(vp9mgr_data.irq, vp9mgr_isr_handler, int_flags,
				VP9MGR_NAME, &vp9mgr_data);
	if (ret) {
	...
	kidle_task_vp9d = kthread_run(vp9mgr_thread, NULL, "vVP9_th");
	VPU_CAST_PT(tTmpPtr, kidle_task_vp9d);
	if (IS_ERR(tTmpPtr)) {
		err("unable to create thread!!\n");
		kidle_task_vp9d = NULL;
		return -1;
	}
	...
	if (misc_register(&vp9mgr_misc_device)) {
		(void)pr_info("VP9 Manager: Couldn't register device.\n");
		return -EBUSY;
	}
...
VREMOVE_RET_TYPE vp9mgr_remove(struct platform_device *pdev)
{
	misc_deregister(&vp9mgr_misc_device);

	if (kidle_task_vp9d == NULL) {
		(void)kthread_stop(kidle_task_vp9d);
		kidle_task_vp9d = NULL;
	}

	devm_iounmap(&pdev->dev, (void __iomem *)vp9mgr_data.base_addr);
	if (vp9mgr_data.irq_reged != 0) {
	...
	(void)pr_info("success :: vp9mgr thread stopped!!\n");
...
int vp9mgr_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (atomic_read(&vp9mgr_data.opened) != 0) {
		...
		open_count = atomic_read(&vp9mgr_data.opened);

		for (i = 0; i < open_count; i++) {
			vp9mgr_disable_clock(0);
		}

		(void)pr_info("vp9: suspend Out DEC(%d/%d/%d/%d/%d)\n\n",
			vp9mgr_get_close(VPU_DEC),
			vp9mgr_get_close(VPU_DEC_EXT),
			vp9mgr_get_close(VPU_DEC_EXT2),
			vp9mgr_get_close(VPU_DEC_EXT3),
