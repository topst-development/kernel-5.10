...
static int tcc_dma_buf_attach(struct dma_buf *buf,
			...
			struct dma_buf_attachment *attach)
{
	LOG_COVERITY("%p,%p,%p", buf, dev, attach);
	return 0;
}
#endif
						...
						enum dma_data_direction dir)
{
	...
	LOG_COVERITY("%p,%p,%d", attach, sgt, (int)dir);
}
...
{
	LOG_COVERITY("%p,%d", buf, (int)direction);
	return 0;
}
...
static void *tcc_dma_buf_kmap(struct dma_buf *buf, unsigned long ulpage)
{
	LOG_COVERITY("%p,%ld", buf, ulpage);
	return NULL;
}
...
static void tcc_dma_buf_kunmap(struct dma_buf *buf,
						...
						void *vaddr)
{
	LOG_COVERITY("%p,%ld,%p", buf, ulpage, vaddr);
}
#endif
...
{
	LOG_COVERITY("%p,%ld", buf, page_num);
	return NULL;
}
#endif
...
{
	unsigned int ret = 0U;
	struct vpu_dec_data_t *vpu_data
			= filp->private_data;

	if (vpu_data == NULL) {
		ret = (unsigned int)POLLERR | (unsigned int)POLLNVAL;
	} else {
		if (vpu_data->count == 0U) {
...
static int vmem_mmap(struct file *filep, struct vm_area_struct *vma)
{
	...
	if (ret == 0) {
		...
		if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff, current_vm_range, vma->vm_page_prot) != 0) {
			...
			vma->vm_ops = NULL;
			vetc_vm_flags_set(vma, (VM_IO | VM_PFNMAP | VM_DONTEXPAND));
		}
	}
