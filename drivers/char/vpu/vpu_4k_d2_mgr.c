...
static st_vpu_4k_d2_func *st4KD2Func = INITIAL_NULL;
...
static int get_vpu_4k_d2_access_addr_mem(void)
{
	...
	va = vetc_ioremap(SHARE_POINT_ADDR + (SHARD_POINT_GAP * SHARE_POINT_ORDER_4KD2), SHARD_POINT_GAP);
	...
	if (type < VPU_ENC) {
		...
		case V2D_IP_DRV_INI:
		{
			...
			if (arg == NULL) {
				...
				if ((vpu_lib_dbg & VLOG_MASK_LIB_WRITE_OUTPUT_TO_YUV)
...
#endif
				// Temporary code
				if ((vpu_lib_dbg & VLOG_MASK_LIB_MEASURE_PERF) == VLOG_MASK_LIB_MEASURE_PERF) {
				...
				if ((vpu_lib_dbg & VPU_DBG_LIB_USE_CB_PRINTK) == VPU_DBG_LIB_USE_CB_PRINTK) {
					...
					if (codec_ip == 2) {
						...
						ret = tcc_vpu_4k_d2_dec_l(VPU_4KD2_CTRL_LOG_STATUS, NULL, (void *)(&dec_log), (void *)NULL);
					}
				}
		...
		case V2D_IP_DEC_SEQDATA:
		{
			...
			if (arg == NULL) {
				...
				dprintk_4kd2("[%s][Out] seqdata out: ret = %#x\n "
						"res info. %d - %d - %d, %d - %d - %d\n"
						"size Linear:%d, %d, Comp:%d, %d, mvcol:%d, Fbc:%d,%d",
						__func__, ret,
						...
						initial_info->m_uiBufSizeLinearLuma, initial_info->m_uiBufSizeLinearChroma,
						initial_info->m_uiBufSizeCompressedLuma, initial_info->m_uiBufSizeCompressedChroma,
						initial_info->m_uiBufSizeMVCol, initial_info->m_uiBufSizeFBCYTable, initial_info->m_uiBufSizeFBCCTable);
		...
		{
			VPU_4K_D2_SET_BUFFER3_t *arg;
			arg = (VPU_4K_D2_SET_BUFFER3_t *) args;

			{
				int ii;
				vpu_4K_D2_dec_buffer3_t *pDecBuffer3 = &arg->gsV4kd2DecBuffer3;

				dprintk_4kd2("VPU_DEC_REG_FRAME_BUFFER3 :: m_iFrameBufferCount:%d",  pDecBuffer3->m_iFrameBufferCount);
				for (ii = 0; ii < pDecBuffer3->m_iFrameBufferCount; ii++) {
					dprintk_4kd2("[%d] addrY:0x%x, addrCb:0x%x, addrCr:0x%x, mvcol:0x%x, fbcY:0x%x, fbcC:0x%x",
			...
			ret =
				tcc_vpu_4k_d2_dec_l(cmd & ~VPU_BASE_OP_KERNEL,
				(vcodec_handle_t *)&pHandle,
				(void *)(&arg->gsV4kd2DecBuffer3),
				(void *)NULL);
			dprintk_4kd2("@@ Dec :: VPU_4K_D2_DEC_REG_FRAME_BUFFER3 out, ret:%d", ret);
		}
		break;
...
{
	unsigned int interrupt_reason = 0;

	VPU_UNUSED_PARAMETER(irq);
	VPU_UNUSED_PARAMETER(dev_id);

	interrupt_reason = vetc_reg_read(vmgr_4k_d2_data.base_addr, 0x004C/*W5_VPU_VINT_REASON*/);

	interrupt_reason &= 0x0000FFFFU; //bit [31:16] is reserved.

	atomic_or((int)interrupt_reason, &vmgr_4k_d2_data.oper_intr);
