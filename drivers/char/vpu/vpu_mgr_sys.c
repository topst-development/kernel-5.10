...
#define dprintk_vpus(msg...)  V_DBG(VPU_DBG_INFO, "TCC_VPU_MGR_SYS: " msg)
#define detailk_vpus(msg...)  V_DBG(VPU_DBG_INFO, "TCC_VPU_MGR_SYS: " msg)
#define err_vpus(msg...)      V_DBG(VPU_DBG_ERROR, "TCC_VPU_MGR_SYS[Err]: "msg)
#define info_vpus(msg...)     V_DBG(VPU_DBG_INFO, "TCC_VPU_MGR_SYS[Info]: "msg)
...
{
	VPU_UNUSED_PARAMETER(only_clk_ctrl);
	...
	if (fbus_xoda_clk != NULL) {
		(void)clk_prepare_enable(fbus_xoda_clk);
	}

#if defined(VBUS_CODA_CORE_CLK_CTRL)
	if (vbus_core_clk != NULL) {
		(void)clk_prepare_enable(vbus_core_clk);
	}
#endif

	if (vbus_xoda_clk != NULL) {
		(void)clk_prepare_enable(vbus_xoda_clk);
	}
	...
	if (only_clk_ctrl == 0) {
		...
		if (ret != 0) {
			err_vpus("vpu_optee_open: failed !! - ret [%d]", ret);
		} else {
			ret = vpu_optee_fw_load(TYPE_VPU_D6);
			if (ret != 0) {
				err_vpus("vpu_optee_fw_load: failed !! (ret: %d)",
					ret);
			}
		}
	}
...
void vmgr_get_clock(struct device_node *pnode)
{
	if (pnode == NULL) {
		err_vpus("device node is null");
	}

	fbus_vbus_clk = of_clk_get(pnode, 0);
	VPU_BUG_ON(fbus_vbus_clk);

	fbus_xoda_clk = of_clk_get(pnode, 1);
	VPU_BUG_ON(fbus_xoda_clk);

	vbus_xoda_clk = of_clk_get(pnode, 2);
	VPU_BUG_ON(vbus_xoda_clk);

#if defined(VBUS_CODA_CORE_CLK_CTRL)
	vbus_core_clk = of_clk_get(pnode, 3);
	VPU_BUG_ON(vbus_core_clk);
#endif
}
...
void vmgr_get_reset(struct device_node *pnode)
{
#if defined(VIDEO_IP_DIRECT_RESET_CTRL)
	if (pnode == NULL) {
		err_vpus("device node is null");
	...
	V_DBG(VPU_DBG_RSTCLK, "enter");
	vbus_xoda_reset = of_reset_control_get_by_index(pnode, 0);
	VPU_BUG_ON(vbus_xoda_reset);

	vbus_core_reset = of_reset_control_get_by_index(pnode, 1);
	VPU_BUG_ON(vbus_core_reset);
#else
...
void vmgr_hw_assert(void)
{
	...
	V_DBG(VPU_DBG_RSTCLK, "enter");
	if (vbus_xoda_reset != NULL) {
		(void)reset_control_assert(vbus_xoda_reset);
	}

	if (vbus_core_reset != NULL) {
		(void)reset_control_assert(vbus_core_reset);
	}
...
void vmgr_hw_deassert(void)
{
	...
	V_DBG(VPU_DBG_RSTCLK, "enter");
	if (vbus_core_reset != NULL) {
		(void)reset_control_deassert(vbus_core_reset);
	}

	if (vbus_xoda_reset != NULL) {
		(void)reset_control_deassert(vbus_xoda_reset);
	}
...
int vmgr_request_irq(unsigned int irq,
			irqreturn_t (*handler)(int irq_num, void *dev_id),
			unsigned long frags, const char *dev, void *dev_id)
{
	return request_irq(irq, handler, frags, dev, dev_id);
}
