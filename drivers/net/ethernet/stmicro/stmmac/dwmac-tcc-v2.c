...
#if defined(CONFIG_ARCH_TCC750X) || defined(CONFIG_ARCH_TCN100X)
...
{
	dev_info(&gmac->dev, "[ECC] %s: Set GMAC ECC soft fault\n", __func__);

	tcc_ecc_write_reg(gmac, GMAC_ECC_PW, GMAC_ECC_PASSWORD);
	tcc_ecc_write_reg(gmac, GMAC_ECC_SFC, 0x10011);
	tcc_ecc_write_reg(gmac, GMAC_ECC_PW, GMAC_ECC_PASSWORD);
	tcc_ecc_write_reg(gmac, GMAC_ECC_SFE0, 0x1);
	tcc_ecc_write_reg(gmac, GMAC_ECC_PW, GMAC_ECC_PASSWORD);
	tcc_ecc_write_reg(gmac, GMAC_ECC_TEEE, 0x0);
}
#endif
...
#if defined(CONFIG_ARCH_TCN100X)
static void tcc_config_bus_id_dt(
	const struct platform_device *plat_dev, struct tcc_dwmac *gmac)
{
	if (of_property_read_u32(
		    plat_dev->dev.of_node, "bus-id", &gmac->bus_id)
	    < 0) {
		gmac->bus_id = 0;
		dev_err(&plat_dev->dev, "bus-id not found.(bus_id=%d)\n",
				gmac->bus_id);
	}
}
#endif
...
#if defined(CONFIG_ARCH_TCN100X)
		tcc_config_bus_id_dt(plat_dev, gmac);
#endif
