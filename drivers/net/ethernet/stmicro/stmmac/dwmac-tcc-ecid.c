...
static void tcc_read_ecid(void __iomem *ecid_block, unsigned int ecid[])
{
	...
	for (i = 0; i < 8U; i++) {
		writel((MODE | CS | (i << ECID_CON_A) | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | TSIGDEV | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | TSIGDEV | PRCHG
			       | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | TSIGDEV | PRCHG | FSET
			       | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | PRCHG | FSET
			       | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | FSET | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
		writel((MODE | CS | (i << ECID_CON_A) | TCC_ECID_CON_SELECT),
		       (pgpio + TCC_ECID_OFFSET_0));
	}
