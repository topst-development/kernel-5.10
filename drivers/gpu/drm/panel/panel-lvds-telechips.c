...
static int panel_lvds_enable(struct drm_panel *panel)
{
	...
	dev_dbg(
		...
		LOG_TAG, lvds->data->name, __func__);
	if (lvds->blk_wait_time > 0u) {
		ms_delta = ktime_ms_delta(ktime_get(), lvds->timestamp);
		if ((ms_delta >= (s64)0) && (ms_delta < (s64)lvds->blk_wait_time)) {
			mdelay(lvds->blk_wait_time - (uint32_t)ms_delta);
		}
	}
#if defined(CONFIG_DRM_TELECHIPS_CTRL_BACKLIGHT)
