...
static int panel_dsi_enable(struct drm_panel *panel)
{
	...
	if (dsi->blk_wait_time > 0u) {
		ms_delta = ktime_ms_delta(ktime_get(), dsi->timestamp);
		if ((ms_delta >= (s64)0) && (ms_delta < (s64)dsi->blk_wait_time)) {
			mdelay(dsi->blk_wait_time - (uint32_t)ms_delta);
		}
	}
#if defined(CONFIG_DRM_TELECHIPS_CTRL_BACKLIGHT)
