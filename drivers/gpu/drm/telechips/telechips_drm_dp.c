...
#if defined(CONFIG_TELECHIPS_DPTX_AUDIO)
#include <linux/of_platform.h>
#include <linux/kernel.h>
#include <sound/hdmi-codec.h>
#include <telechips_dptx_api.h>
#endif

#define DRIVER_DATE "20240309"
...
static int tcc_dpv14_aud_mute(struct device *dev, void *data, bool enable)
{
	int ret = 0;
	struct tccdrm_dptx_context *dp_context;
	struct tccdrm_dp_context *dev_context;
...
static const struct hdmi_codec_ops hdmi_audio_codec_ops = {
	...
	.digital_mute = tcc_dpv14_aud_mute,
	...
	uint64_t *val)
{
	...
	/*
	 * FIX
	 * misra_c_2012_rule_2_7_violation: The parameter connector_state
				 ...
				 int in_flags)
{
	...
	/*
	 * FIX
	 * misra_c_2012_rule_2_7_violation: The parameter in_flags is not used
	 * in the function.
	 */
	(void)in_flags;
...
#if defined(CONFIG_DRM_PANEL_DPV14_TELECHIPS) && \
...
static void tccdrm_dp_make_proc(struct tccdrm_dp_context *dev_context)
{
	...
	if (dev_context->proc_dir != NULL) {
		/* coverity[misra_c_2012_rule_7_1] */
