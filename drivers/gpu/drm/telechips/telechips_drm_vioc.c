...
static enum hrtimer_restart lcd_vtimer_irq_handler(struct hrtimer *timer)
{
	/* coverity[misra_c_2012_rule_8_13] */
	/* coverity[misra_c_2012_rule_11_5] */
	struct tccdrm_vioc_context *dev_context;
	...
	dev_context = container_of(timer, struct tccdrm_vioc_context, vtimer);

	if (!tcc_math_check_uint_plus_uint(1000000000U - 1U, dev_context->vrefresh)) {
...
static int tcc_drm_crtc_enable_timer_vblank(struct tccdrm_vioc_context *dev_context)
{
	...
	if (!tcc_math_check_uint_plus_uint(1000000000U - 1U, dev_context->vrefresh)) {
		...
		/* coverity[misra_c_2012_rule_10_4] */
		nsec = DIV_ROUND_UP(1000000000U, dev_context->vrefresh);
		DRM_DEV_INFO(dev_context->dev, "refresh = %uHz usec = %lluus\r\n",
			dev_context->vrefresh, ktime_to_us(nsec) );
		/* coverity[cert_dcl37_c] */
		/* coverity[misra_c_2012_rule_14_4] */
		spin_lock_irqsave(&dev_context->irq_lock, irqflags);
		...
		if (test_and_set_bit(CRTC_FLAGS_IRQ_BIT,
			...
			dev_context->vtimer.function = lcd_vtimer_irq_handler;
