...
/*
 *
 * (C) COPYRIGHT 2017 Telechips, Inc.
...
static inline unsigned int decideNextStatus(unsigned int utilisation)
{
	int i;
	unsigned int currStep = 0;

	for (i = 0; i < MALI_DVFS_STEPS; i++) {
		if (utilisation <= gpu_dvfs_table[i].up_threshold
		    && utilisation > gpu_dvfs_table[i].down_threshold)
			currStep = i;
	}
	return currStep;
}
			      ...
			      u32 util_gl_share, u32 util_cl_share[2])
{
	...
	if (currStep != nextStep) {
		if (kbdev->clock) {
			...
			if (kbdev->regulator)
				regulator_set_voltage(kbdev->regulator,
					      gpu_dvfs_table[nextStep].uVol,
					      gpu_dvfs_table[nextStep].uVol);
#endif
			clk_set_rate(kbdev->clock,
