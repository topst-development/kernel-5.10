 ...
 */
static int gpu_req_timeout = 100;    //This value has been changed to account for the Telechips subcore env.
...
int kbase_arbiter_pm_early_init(struct kbase_device *kbdev)
{
	...
	if (kbdev->arb.arb_if) {
		/* Telechips : In the intree kernel, the aw message handler of the ptm module should be initialized first */
		int retry = 1000;
		while(retry) {
				kbase_arbif_gpu_request(kbdev);
                dev_err(kbdev->dev, "Waiting for initial GPU assignment...\n");
