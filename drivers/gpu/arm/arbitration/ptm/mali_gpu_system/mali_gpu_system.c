...
static const char *const irq_name_mid[] = {
	"IRQ_GROUP0_WATCHDOG",		"IRQ_GROUP1_WATCHDOG",
	"IRQ_GROUP2_WATCHDOG",		"IRQ_GROUP3_WATCHDOG",
	"IRQ_PARTITION0_LOCKED_ACCESS", "IRQ_PARTITION1_LOCKED_ACCESS",
	"IRQ_PARTITION2_LOCKED_ACCESS", "IRQ_PARTITION3_LOCKED_ACCESS",
	"IRQ_PARTITION0_LOCKED_BIST",	"IRQ_PARTITION1_LOCKED_BIST",
	"IRQ_PARTITION2_LOCKED_BIST",	"IRQ_PARTITION3_LOCKED_BIST",
	"IRQ_PARTITION0_MTCRC",		"IRQ_PARTITION1_MTCRC",
	"IRQ_PARTITION2_MTCRC",		"IRQ_PARTITION3_MTCRC"
};

/* IRQ names, bits 64-79 */
static const char *const irq_name_high[] = { "IRQ_AW0_INVALID_ACCESS",	"IRQ_AW1_INVALID_ACCESS",
 ...
 */
static int process_irq(u32 status, int reg_ind, struct system *system)
{
	char *t;
	uint32_t i = 0;
	int irq_level;
...
/**
 * get_system_irq_masks() - Reads the IRQ masks from the registers
 * @system: Internal system device data
 */
static void get_system_irq_masks(struct system *system)
{
	int i;
	...
	for (i = 0; i < NUM_IRQ_REGISTERS; ++i) {
		system->irq.mask[i] = ioread32(system->base_addr +
...
static void set_system_irq_masks(struct system *system)
{
	...
	for (i = 0; i < NUM_IRQ_REGISTERS; ++i) {
		iowrite32(system->irq.mask[i],
			  system->base_addr + PTM_UNCORRECTED_ERROR_IRQ_MASK + (OFFSET_4B * i));
	}
}
 ...
 */
static void clear_system_irqs(struct system *system, u32 status_bef_clear[NUM_IRQ_REGISTERS])
{
	u32 clear_irqs;
	...
	for (i = 0; i < NUM_IRQ_REGISTERS_PER_TYPE; ++i) {
		...
		iowrite32(clear_irqs, system->base_addr + PTM_IRQ_CLEAR + (OFFSET_4B * i));
	...
	if (set_irq_masks) {
		dev_dbg(system->dev, "The IRQs of the issues above will be masked.\n");
...
static irqreturn_t system_isr(int irq, void *data)
{
	...
	for (i = 0; i < NUM_IRQ_REGISTERS; ++i) {
		irq_status[i] = ioread32(system->base_addr + PTM_UNCORRECTED_ERROR_IRQ_STATUS +
					 (OFFSET_4B * i));
	 ...
	 */
	error_finger_print = ioread32(system->base_addr + PTM_ERROR_FINGER_PRINT);
...
/**
 * set_streamids() - Configure the StreamID values
 * @system:		Internal system device data
 */
static void set_streamids(void __iomem *base_addr)
{
	int aw;
...
/* To avoid affecting the behavior of other OSes during core-reset in Telechips GPU virtualization environment */
#if !defined(CONFIG_TCC_MALI_VZ)
	reset_status = 0;
	...
	for (group = 0; group < NUM_GROUPS; group++) {
		...
		reset_status |= PTM_GROUP_SOFT_RESET << (group * PTM_GROUP_RESET_STATE_BITS);
	}

	iowrite32(value, base_addr + PTM_GROUP_RESET_SET);
	ret = readx_poll_timeout(ioread32, base_addr + PTM_GROUP_RESET_STATE, value,
				 (value == reset_status), REG_POLL_SLEEP_US,
				 REG_POLL_RESET_TIMEOUT_US);
	if (ret) {
	...
	ret = readx_poll_timeout(ioread32, base_addr + PTM_GROUP_RESET_STATE, value, (value == 0),
	...
	set_streamids(base_addr);

	system->irq.line = irq;
	system->irq.flags = irqd_get_trigger_type(irq_get_irq_data(irq));
	system->dev = dev;
	system->res = res;
	...
	/* request IRQ. */
	irq_req = request_irq(system->irq.line, system_isr, system->irq.flags | IRQF_SHARED,
			      dev_name(dev), system);
	if (irq_req) {
...
static int gpu_system_remove(struct platform_device *pdev)
{
	...
	if (system) {
		...
		release_mem_region(system->res->start, resource_size(system->res));
		free_irq(system->irq.line, system);
...
/**
 * gpu_system_device_suspend - Suspend callback from the OS.
 *
 ...
 */
static int gpu_system_device_suspend(struct device *dev)
{
	struct system *system;
	...
	get_system_irq_masks(system);

	return 0;
}

/**
 * gpu_system_device_resume - Resume callback from the OS.
 *
 ...
 * Return: A standard Linux error code
 */
static int gpu_system_device_resume(struct device *dev)
{
	struct system *system;
	...
	set_system_irq_masks(system);
	set_streamids(system->base_addr);
 ...
 */
static const struct dev_pm_ops gpu_system_pm_ops = {
	.suspend = gpu_system_device_suspend,
	.resume = gpu_system_device_resume,
};

/*
 * struct gpu_system_dt_match: Match the platform device with the Device Tree.
 */
static const struct of_device_id gpu_system_dt_match[] = { { .compatible = MALI_SYSTEM_DT_NAME },
							   {} };
...
MODULE_DESCRIPTION("GPU system driver.");
