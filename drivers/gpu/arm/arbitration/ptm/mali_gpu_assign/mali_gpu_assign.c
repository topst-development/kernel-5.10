...
MODULE_PARM_DESC(
	...
	"ptm_config='A:S0:S1:S2:P0:P1:W0:W1,B:S3:P2:W2'\n"
	"This string correspond to the following configuration:\n"
	"- first group assigned to Bus A with slices 0, 1, 2; partition 0, 1 and access windows 0, 1\n"
	"- second group assigned to Bus B with slice 3; partition 2 and access window 2");
...
enum PTM_BUS_AB_ENUM { BUS_A = 0, BUS_B = 1, BUS_COUNT = 2 };

static struct _resource_group {
	bool is_set;
	u32 bus;
} resource_group_defaults = { .is_set = false, .bus = BUS_A };

static struct _access_window {
	bool is_set;
	u32 rg;
} access_window_defaults = { .is_set = false, .rg = 0 };

static struct _partition {
	bool is_set;
	u32 rg;
} partition_defaults = { .is_set = false, .rg = 0 };

static struct _slice {
	bool is_set;
	u32 rg;
	u32 isolation_set;
} slice_defaults = { .is_set = false, .rg = 0, .isolation_set = 0 };
 ...
 * @assign:        Structure holding the parsed Assign configuration.
 ...
 */
static bool init_data(struct device *dev, struct _gpu_assign *assign, void __iomem *base_addr)
{
	int i;
	u32 v;
	u64 cores;
	...
	v = ioread32(base_addr + PTM_UNIT_FEATURES);
	 ...
	 */
	cores = ioread32(base_addr + PTM_SLICE_CORES);
	cores |= (u64)ioread32(base_addr + PTM_SLICE_CORES + 4) << 32;
	for (i = 0; i < MALI_PTM_SLICES_COUNT; ++i) {
		if (((cores >> i * PTM_SLICE_CORES_CORE_BITS) & PTM_SLICE_CORES_CORE_MASK) == 0)
			break;
	}
 ...
 * Return: true for success or false for failure.
 */
static bool set_rg_bus(struct device *dev, struct _gpu_assign *assign, u32 rg, u32 bus)
{
 ...
 * Return: true for success or false for failure.
 */
static bool set_aw_rg(struct device *dev, struct _gpu_assign *assign, u32 aw, u32 rg)
{
 ...
 * Return: true for success or false for failure.
 */
static bool set_partition_rg(struct device *dev, struct _gpu_assign *assign, u32 partition, u32 rg)
{
	if (partition >= assign->partition_count) {
		dev_err(dev, "Partition %d out of range (max %d)\n", partition,
			assign->partition_count - 1);
		return false;
	}
	if (rg >= assign->rg_count) {
	...
	if (!assign->partitions[partition].is_set || assign->partitions[partition].rg == rg) {
		...
		dev_err(dev, "Partition %d already assigned to group %d\n", partition,
			assign->partitions[partition].rg);
		return false;
	}
	return true;
}
 ...
 * Return: true for success or false for failure.
 */
static bool set_slice_rg(struct device *dev, struct _gpu_assign *assign, u32 slice, u32 rg)
{
	if (slice >= MALI_PTM_SLICES_COUNT) {
		dev_err(dev, "Slice %d out of driver range (max %d)\n", slice,
			MALI_PTM_SLICES_COUNT);
		return false;
	}
 ...
 * Return: true for success or false for failure.
 */
static bool to_uint32(struct device *dev, const char *str, u32 *ui, const char *param)
{
 ...
 * from the command line. The string is guaranteed to be NULL terminated and
 * of max length 1024.
 * @dev: pointer to assign device (only used for logging)
 ...
 * false.
 */
static bool get_data_from_command_line(struct device *dev, struct _gpu_assign *assign,
				       const char *str)
{
	bool retval = false;
	u32 grp, i = 0;
	...
	/* The string is guaranteed to be NULL terminated. Invalid strings are
	 * rejected by the Kernel's module command line processing functions.
	 */
	slen = strlen(str);
	...
	while ((grp_tok = strsep(&next_grp_str, ","))) {
		...
		/* Added by Telechips for cases that do not start with RG0
		 * When setting from RG0, unused resources are sometimes set as 
		 * if they were used from RG0 because most registers default to 0.
		 */
...
/* Telechips : To use CONFIG_MALI_DEFAULT_PTM_CONFIG in in-tree kernel */
			...
			case 'C':
				if (*(tok+1) != ':' && *(tok+1) != '\0') {
					dev_err(dev, "Invalid '%c' after bus '%c'\n", *(tok + 1),
						*tok);
					goto parse_fail;
				}
 ...
 *
 */
static void pack_data_into_register_block(struct device *dev, struct _gpu_assign *assign,
					  void __iomem *base_addr)
{
	u32 i;
	u32 value;
	...
	value = 0;
	for (i = 0; i < assign->slice_count; ++i) {
		value |= (assign->slices[i].isolation_set & ISOLATION_MASK) << (i * ISOLATION_BITS);
	}
...
static int gpu_assign_probe(struct platform_device *pdev)
{
	...
	if (!ptm_config) {
		dev_warn(dev, "No command line configuration set, using defaults\n");

		/* Telechips : To use CONFIG_MALI_DEFAULT_PTM_CONFIG in in-tree kernel */
		#ifdef CONFIG_MALI_DEFAULT_PTM_CONFIG
        		ptm_config=MALI_DEFAULT_PTM_CONFIG;
		#endif
	}

	ptm_assign_data = devm_kzalloc(dev, sizeof(struct mali_gpu_ptm_assign), GFP_KERNEL);
	if (!ptm_assign_data)
		return -ENOMEM;
	...
	if (!request_mem_region(res.start, remap_size, pdev->name)) {
		dev_err(dev, "req mem reg(%pa,%lu,%s) FAILED! Reg window unavailable", &res.start,
			remap_size, pdev->name);
		ret = -EIO;
		goto fail_resources;
	}
	...
	if (!base_addr) {
		dev_err(dev, "ioremap(%pa,%lu) FAILED! Can't remap register window", &res.start,
			remap_size);
		ret = -EIO;
		goto fail_ioremap;
	}
	...
	ret = check_ptm_version(dev, ioread32(base_addr + PTM_DEVICE_ID));
	if (ret)
		goto fail_getdata;

	if (!init_data(dev, &ptm_assign_data->assign, base_addr)) {
	...
	if (ptm_config && !get_data_from_command_line(
		dev, &ptm_assign_data->assign, ptm_config)) {
		ret = -ENODEV;
		goto fail_getdata;
	}

	if (!validate_data(dev, &ptm_assign_data->assign)) {
		ret = -ENODEV;
		goto fail_getdata;
	}

	print_config(dev, &ptm_assign_data->assign);

	pack_data_into_register_block(dev, &ptm_assign_data->assign, base_addr);
...
static int gpu_assign_device_suspend(struct device *dev)
{
/* Added by Telechips to support Suspend to RAM */
#if defined(CONFIG_TCC_MALI_VZ) && defined(CONFIG_TCC807X_CA55_SUB)
...
/**
 * gpu_assign_device_resume - Resume callback from the OS.
 *
 ...
 */
static int gpu_assign_device_resume(struct device *dev)
{
	struct mali_gpu_ptm_assign *ptm_assign_data;
#if defined(CONFIG_TCC_MALI_VZ) && defined(CONFIG_TCC807X_CA55_SUB)
	struct device_node *tcc_3d_cfg_np = NULL;
	...
	ptm_assign_data = dev_get_drvdata(dev);
	...
	pack_data_into_register_block(dev, &ptm_assign_data->assign,
									ptm_assign_data->base_addr);
 ...
 */
static const struct dev_pm_ops gpu_assign_pm_ops = {
	.suspend = gpu_assign_device_suspend,
	.resume = gpu_assign_device_resume,
};
...
MODULE_DESCRIPTION("Assign driver.");
