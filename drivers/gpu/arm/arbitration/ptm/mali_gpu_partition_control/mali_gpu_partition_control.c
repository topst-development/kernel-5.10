...
/*
 * (C) COPYRIGHT 2021-2023 ARM Limited or its affiliates. All rights reserved.
 */
 ...
 * @ptm_irq_mask: PTM_IRQ_MASK register value to store/restore in suspend/resume
 */
...
{
	struct vm_assign_ops *data;

	data = dev_get_drvdata(dev);
	if (likely(data))
		return container_of(data, struct mali_gpu_part_ctrl, assign_ops);

	dev_err(dev, "Partition Control interface missing.\n");
...
static int partition_control_reset(struct mali_gpu_part_ctrl *partition_ctrl)
{
	...
	iowrite32(PTM_RESET_SET_TRIGGER_VALUE, partition_ctrl->mem + PTM_RESET_SET_OFFSET);
	error = readx_poll_timeout(ioread32, partition_ctrl->mem + PTM_RESET_STATE_OFFSET,
				   reset_state, (reset_state & PTM_RESET_STATE_MASK),
				   REG_POLL_SLEEP_US, REG_POLL_RESET_TIMEOUT_US);

	if (error) {
		dev_err(partition_ctrl->dev, "Partition reset timed out.\n");
		return error;
	}
	...
	error = readx_poll_timeout(ioread32, partition_ctrl->mem + PTM_RESET_STATE_OFFSET,
				   ...
				   REG_POLL_SLEEP_US, REG_POLL_RESET_TIMEOUT_US);
	if (error) {
		dev_err(partition_ctrl->dev, "Partition reset clear timed out.\n");
		return error;
	}
	 ...
	 * the reset
	 */
	error = readx_poll_timeout(ioread32, partition_ctrl->mem + PTM_PARTITION_STATE_OFFSET,
				   part_state, ((part_state & PTM_PARTITION_STATE_LOCK_MASK) == 0),
				   REG_POLL_SLEEP_US, REG_POLL_TIMEOUT_US);

	if (error) {
		dev_err(partition_ctrl->dev, "Partition locked after reset! Error: %d\n", error);
		return error;
	}
 ...
 */
static int assign_partition_to_aw(struct device *dev, uint32_t aw_id)
{
	...
	if (aw_id > PTM_NUM_OF_AW - 1) {
		dev_err(dev, "Access Window ID out of range.\n");
		return -EINVAL;
	}
	...
	if (state & PTM_PARTITION_STATE_AW_ERROR_MASK) {
		dev_err(dev, "Partition is in error state after setting AW.\n");
		return -EIO;
	}
	...
	error = readx_poll_timeout(ioread32, partition_ctrl->mem + PTM_PARTITION_STATE_OFFSET,
				   ...
				   REG_POLL_SLEEP_US, REG_POLL_TIMEOUT_US);
	if (error) {
		dev_err(dev, "Partition ready timed out.\n");
		error = -EIO;
	}
 ...
 * * -EIO	- failed with hardware fault.
 */
static int unassign_partition(struct device *dev)
{
	struct mali_gpu_part_ctrl *partition_ctrl;
	int error;
	uint32_t state;
 ...
 * * -EIO	- failed with hardware fault.
 */
static int get_assigned_aw(struct device *dev, int32_t *aw_id)
{
	struct mali_gpu_part_ctrl *partition_ctrl;
	uint32_t val;
	int i;
...
static int pm_control_probe(struct platform_device *pdev)
{
	...
	partition_ctrl = devm_kzalloc(&pdev->dev, sizeof(struct mali_gpu_part_ctrl), GFP_KERNEL);

	if (!partition_ctrl)
		return -ENOMEM;

	if (!request_mem_region(reg_resrc->start, resource_size(reg_resrc), "mali_pm_control")) {
		dev_err(&pdev->dev, "Requesting Memory region failed\n");
		ret = -EIO;
		goto req_memory_fail;
	}

	partition_ctrl->mem = ioremap(reg_resrc->start, resource_size(reg_resrc));
	...
	ret = check_ptm_version(&pdev->dev, ioread32(partition_ctrl->mem + PTM_DEVICE_ID));
	if (ret)
...
/**
 * gpu_partition_ctrl_device_suspend - Suspend callback from the OS.
 *
 ...
 */
static int gpu_partition_ctrl_device_suspend(struct device *dev)
{
	struct mali_gpu_part_ctrl *partition_ctrl;
	...
	partition_ctrl->ptm_irq_mask = ioread32(partition_ctrl->mem + PTM_IRQ_MASK);

	return 0;
}

/**
 * gpu_partition_ctrl_device_resume - Resume callback from the OS.
 *
 ...
 * Return: A standard Linux error code
 */
static int gpu_partition_ctrl_device_resume(struct device *dev)
{
	struct mali_gpu_part_ctrl *partition_ctrl;
	...
	iowrite32(partition_ctrl->ptm_irq_mask, partition_ctrl->mem + PTM_IRQ_MASK);
 ...
 */
static const struct dev_pm_ops gpu_partition_ctrl_pm_ops = {
	.suspend = gpu_partition_ctrl_device_suspend,
	.resume = gpu_partition_ctrl_device_resume,
};

static const struct of_device_id pm_control_dt_match[] = { { .compatible = PTM_CONTROL_DT_NAME },
							   {} };

static struct platform_driver pm_control_driver = {
	.probe = pm_control_probe,
	.remove = pm_control_remove,
	.driver = {
		.name = "mali_pm_control",
		.of_match_table = pm_control_dt_match,
		.pm = &gpu_partition_ctrl_pm_ops,
	},
};
...
MODULE_ALIAS("mali-gpu-partition-control");
