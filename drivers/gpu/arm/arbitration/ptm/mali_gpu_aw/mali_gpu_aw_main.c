 ...
 *                      GPU request.
 * @aw_irq_mask:        PTM_AW_IRQ_MASK register value to store/restore in suspend/resume
 *
...
static void aw_read_incoming_msg(struct mali_gpu_ptm_aw *awdev)
{
	...
	error = ptm_msg_buff_write(&awdev->msg_handler.recv_msgs, MESSAGE_IDX_RX, message);
	if (error > 0)
		dev_dbg(awdev->msg_handler.dev, "Overriding incoming PTM message\n");
	else if (error < 0) {
		dev_err(awdev->msg_handler.dev, "AW message buffer write failed\n");
		return;
	}
	...
	queue_work(awdev->message_wq, &awdev->rx_msg_work);
}
 ...
 */
static inline const char *aw_state_str(enum ptm_aw_state state)
{
	switch (state) {
	case AW_INIT:
		return "AW_INIT";
 ...
 *
 */
static void aw_set_state(struct mali_gpu_ptm_aw *awdev, enum ptm_aw_state new_state)
{
	lockdep_assert_held(&awdev->prot_data.mutex);
	if (awdev->prot_data.state != new_state) {
		dev_dbg(awdev->dev, "%s: %s -> %s\n", __func__,
			aw_state_str(awdev->prot_data.state), aw_state_str(new_state));
 ...
 *
 */
static void aw_get_state(struct mali_gpu_ptm_aw *awdev, enum ptm_aw_state *curr_state)
{
	lockdep_assert_held(&awdev->prot_data.mutex);
 ...
 */
static int aw_get_init_data(struct mali_gpu_ptm_aw *awdev, struct init_msg *init_data)
{
...
static void send_message(struct mali_gpu_ptm_aw *awdev, uint64_t *message)
{
	...
	error = ptm_msg_buff_write(&msg_handler->send_msgs, 0, *message);
	if (error > 0)
		dev_dbg(msg_handler->dev, "Overriding outgoing PTM message for RG\n");
	else if (error < 0) {
		dev_err(msg_handler->dev, "AW message buffer write failed\n");
		return;
	}
 ...
 */
static void vm_arb_init(struct mali_gpu_ptm_aw *awdev, uint8_t ack, uint8_t version,
			bool gpu_request)
{
	...
	if (vm_arb_init_build_msg(ack, version, (uint8_t)gpu_request, &msg_payload))
		return;
 ...
 * Return: 0 if success, or a Linux error code.
 */
static int aw_respond_to_init(struct mali_gpu_ptm_aw *awdev, uint8_t recv_version)
{
	uint8_t sending_ack;
	...
	/*
	 * Note we don't change the state to HS_FAILED here since this is
	...
	if (recv_version < MIN_SUPPORTED_VERSION) {
		...
		dev_err(awdev->dev, "Protocol handshake failed with RG\n");
		err = -EPERM;
	} else {
 ...
 * Return: 0 if success, or a Linux error code
 */
static int aw_validate_init(struct mali_gpu_ptm_aw *awdev, uint8_t recv_version)
{
	enum ptm_aw_state state = AW_INIT;
...
static void update_max_config(struct mali_gpu_ptm_aw *awdev)
{
	...
	if (awdev->prot_data.max_config_req) {
		ret = aw_get_init_data(awdev, &init_data);
		if (ret < 0)
			return;
		...
		if (init_data.received) {
			...
			awdev->vm_drv_ops.arb_vm_max_config(awdev->vm_dev, init_data.max_l2_slices,
...
static bool aw_start_handshake(struct mali_gpu_ptm_aw *awdev)
{
	...
	err = aw_get_init_data(awdev, &init_data);
	if (err < 0)
		return false;
	...
	if (!init_data.received) {
		...
		err = aw_respond_to_init(awdev, init_data.version);
		if (!err)
			hs_complete = true;
	}
	return hs_complete;
...
static void device_attach_handler(struct work_struct *work)
{
	...
	if (!awdev->vm_dev || device_attach(awdev->vm_dev) != DEVICE_ATTACH_DRIVER_FOUND) {
		mutex_lock(&awdev->prot_data.mutex);
		aw_get_state(awdev, &state);
 ...
 */
static int process_protocol_handshake(struct mali_gpu_ptm_aw *awdev, enum ptm_aw_state state,
				      uint64_t *message)
{
	uint8_t recv_ack;
	uint8_t recv_version;
	...
	case AW_HS_INITIATED_NO_DEV:
		...
		break;
	case AW_HS_FAILED:
		dev_dbg(awdev->dev, "%s: Unexpected message from RG. Handshake failed.\n",
			__func__);
		return -EPERM;
	default:
		if (recv_ack == 0) {
	...
	if (state == AW_HS_INITIATED || state == AW_GPU_REQUESTED || state == AW_READY)
 ...
 */
static inline const char *msg_str(uint8_t msg_id)
{
	switch (msg_id) {
	case ARB_VM_INIT:
		return "ARB_VM_INIT";
...
static void process_queued_request(struct mali_gpu_ptm_aw *awdev)
{
	...
	case ARB_VM_GPU_LOST:
		...
		dev_warn(awdev->dev, "GPU lost occurred before GPU could be granted\n");
		break;
	}
}
...
static void message_handler(struct work_struct *work)
{
	...
	if (!test_bit(MESSAGE_IDX_RX, &awdev->msg_handler.recv_msgs.mask))
		goto cleanup_mutex;

	error = ptm_msg_buff_read(&awdev->msg_handler.recv_msgs, MESSAGE_IDX_RX, &message);
	if (error) {
		dev_info(awdev->dev, "Failed to get message (%d)\n", error);
		goto cleanup_mutex;
	}
...
/* LINUX_VERSION_CODE >= 5.4 */
	...
	case AW_GPU_REQUESTED_NO_DEV:
		if (gpu_granted) {
			...
			queue_work(awdev->device_attach_wq, &awdev->device_attach_work);
		}
		goto cleanup_mutex;
	case AW_READY:
		break;
	...
	case ARB_VM_GPU_GRANTED:
		...
		if (awdev->prot_data.last_freq != NO_FREQ)
			vm_drv_ops->arb_vm_update_freq(awdev->vm_dev, awdev->prot_data.last_freq);
		vm_drv_ops->arb_vm_gpu_granted(awdev->vm_dev);
		break;
	case ARB_VM_GPU_STOP:
		vm_drv_ops->arb_vm_gpu_stop(awdev->vm_dev);
...
static irqreturn_t aw_irq_handler(int irq, void *data)
{
	...
	if (irq == awdev->irqs.irq)
		irq_status = ioread32(awdev->regs.addr + PTM_AW_IRQ_STATUS);
	else {
...
static int initialize_communication(struct mali_gpu_ptm_aw *awdev)
{
	...
	irq_status = ioread32(awdev->regs.addr + PTM_AW_IRQ_RAWSTAT);
 ...
 */
static int register_vm_dev(struct arbiter_if_dev *arbif_dev, struct device *dev,
			   struct arbiter_if_arb_vm_ops *ops)
{
	...
	if (!ops->arb_vm_gpu_granted || !ops->arb_vm_gpu_lost || !ops->arb_vm_gpu_stop ||
	    !ops->arb_vm_max_config || !ops->arb_vm_update_freq) {
		pr_err("%s : NULL function pointers for vm ops.\n", __func__);
		return -EINVAL;
	}
...
static void gpu_stopped(struct arbiter_if_dev *arbif_dev, u8 gpu_required)
{
	...
	mutex_lock(&awdev->prot_data.mutex);
	aw_get_state(awdev, &state);
	awdev->prot_data.gpu_requested = gpu_required;
	switch (state) {
	case AW_HS_INITIATED:
		aw_set_state(awdev, AW_HS_INITIATED_NO_DEV);
		break;
	case AW_GPU_REQUESTED:
		...
		break;
	case AW_READY:
		if (!vm_arb_gpu_stopped_build_msg(gpu_required, &msg_payload))
			send_message(awdev, &msg_payload);
		break;
	default:
		break;
...
static int aw_probe(struct platform_device *pdev)
{
	...
	if (!request_mem_region(aw_reg_res->start, resource_size(aw_reg_res), dev_name(dev))) {
	...
	ptm_msg_handler_init(&awdev->msg_handler, dev, addr + PTM_AW_MESSAGE, MESSAGE_IDX_COUNT,
			     VM_TO_ARB_BUFF_SIZE, ARB_TO_VM_BUFF_SIZE);

	/* Check the partition manager ID. */
	err = check_ptm_version(dev, ioread32(addr + PTM_ID));
	if (err)
		goto cleanup;
	...
	awdev->irqs.irq = irq;
	awdev->irqs.flags = irqd_get_trigger_type(irq_get_irq_data(irq));
	...
	awdev->message_wq = alloc_workqueue("aw gpu message", WQ_HIGHPRI | WQ_UNBOUND, 1);
	...
	awdev->device_attach_wq = alloc_workqueue("aw device attach", WQ_HIGHPRI | WQ_UNBOUND, 1);
	...
	err = request_irq(awdev->irqs.irq, aw_irq_handler, awdev->irqs.flags | IRQF_SHARED,
			  dev_name(dev), awdev);
...
/**
 * gpu_aw_device_suspend - Suspend callback from the OS.
 *
 ...
 */
static int gpu_aw_device_suspend(struct device *dev)
{
	struct mali_gpu_ptm_aw *awdev;
	...
	awdev = dev_get_drvdata(dev);

	if (!awdev)
		return -ENODEV;

	awdev->aw_irq_mask = ioread32(awdev->regs.addr + PTM_AW_IRQ_MASK);

	return 0;
}

/**
 * gpu_aw_device_resume - Resume callback from the OS.
 *
 ...
 * Return: A standard Linux error code
 */
static int gpu_aw_device_resume(struct device *dev)
{
	struct mali_gpu_ptm_aw *awdev;
	...
	iowrite32(awdev->aw_irq_mask, awdev->regs.addr + PTM_AW_IRQ_MASK);
 ...
 */
static const struct dev_pm_ops gpu_aw_pm_ops = {
	.suspend = gpu_aw_device_suspend,
	.resume = gpu_aw_device_resume,
};

/*
 * aw_dt_match: Match the platform device with the Device Tree.
 */
static const struct of_device_id aw_dt_match[] = { { .compatible = AW_MESSAGE_DT_NAME }, {} };
