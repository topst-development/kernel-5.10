...
#define index_of(ptr, container) ((size_t)((ptr) - (container)))
...
struct mali_arb_sysfs {
	...
	int (*get_slice_assignment)(struct mali_arb *arb, uint8_t partition_index, uint32_t *buf);
	int (*set_slice_assignment)(struct mali_arb *arb, uint8_t partition_index, uint32_t slices,
				    uint32_t *old_slices);
	int (*get_aw_assignment)(struct mali_arb *arb, uint8_t partition_index, uint32_t *buf);
	int (*set_aw_assignment)(struct mali_arb *arb, uint8_t partition_index,
				 uint32_t access_windows, uint32_t *old_access_windows);
	...
	struct mali_arb_sysfs_partition_entry partitions[MALI_PTM_PARTITION_COUNT];
};

struct mali_arb_sysfs_entry_attr {
	struct attribute attr;
	ssize_t (*show)(struct kobject *kobj, char *buf);
	ssize_t (*store)(struct kobject *kobj, const char *buf, size_t size);
};
...
static int get_slice_partition(struct mali_arb_sysfs_slice_entry *entry)
{
	...
	for (i = 0; i < MALI_PTM_PARTITION_COUNT; ++i) {
		if (entry->root->get_slice_assignment(entry->root->arb, i, &slices))
			return -1;
...
static ssize_t mali_arb_sysfs_entry_attr_show(struct kobject *kobj, struct attribute *attr,
					      char *buf)
{
	int ret = -EIO;
	struct mali_arb_sysfs_entry_attr *eattr =
		container_of(attr, struct mali_arb_sysfs_entry_attr, attr);
...
static ssize_t mali_arb_sysfs_entry_attr_store(struct kobject *kobj, struct attribute *attr,
...
static void mali_arb_sysfs_slice_entry_release(struct kobject *kobj)
{
	struct mali_arb_sysfs_slice_entry *entry =
		container_of(kobj, struct mali_arb_sysfs_slice_entry, kobj);
	entry->root = NULL;
}

static void mali_arb_sysfs_partition_entry_release(struct kobject *kobj)
{
	struct mali_arb_sysfs_partition_entry *entry =
		container_of(kobj, struct mali_arb_sysfs_partition_entry, kobj);
	entry->root = NULL;
...
{
	enum mali_gpu_slice_tiler_type type = MALI_GPU_TILER_COMPACT;
	int err;
	bool is_large;
	...
	err = rg_ops->get_slice_tiler_type(entry->dev, index_of(entry, entry->root->slices), &type);
	if (err)
		return err;
...
static ssize_t slice_num_cores_show(struct kobject *kobj, char *buf)
{
	...
	err = rg_ops->get_slices_core_mask(entry->dev, &cores_mask, &mask_stride);
	if (err)
		return err;

	slice_ind = index_of(entry, entry->root->slices);

	cores = hweight64(cores_mask >> (mask_stride * slice_ind) & ((0x1 << mask_stride) - 1));
...
static struct attribute *mali_arb_sysfs_slice_attrs[] = {
	&slice_tiler_type_attr.attr, &slice_num_cores_attr.attr, &slice_partition_attr.attr,
	&slice_enabled_attr.attr,    &slice_powered_attr.attr,	 NULL,
};
#if (KERNEL_VERSION(5, 2, 0) <= LINUX_VERSION_CODE)
ATTRIBUTE_GROUPS(mali_arb_sysfs_slice);
#endif

static struct kobj_type mali_arb_slice_ktype = {
	.sysfs_ops = &mali_arb_sysfs_entry_ops,
#if (KERNEL_VERSION(5, 2, 0) <= LINUX_VERSION_CODE)
	.default_groups = mali_arb_sysfs_slice_groups,
#else
	.default_attrs = mali_arb_sysfs_slice_attrs,
#endif
...
static ssize_t partition_active_slices_show(struct kobject *kobj, char *buf)
{
	...
	const int err = entry->root->get_slice_assignment(entry->root->arb, p, &slices);
...
static ssize_t partition_active_slices_store(struct kobject *kobj, const char *buf, size_t size)
{
	struct mali_arb_sysfs_partition_entry *entry =
		container_of(kobj, struct mali_arb_sysfs_partition_entry, kobj);
	const size_t p = index_of(entry, entry->root->partitions);
	uint32_t old_slices = 0, slices = 0;
	int err, i;
	...
	err = entry->root->set_slice_assignment(entry->root->arb, p, slices, &old_slices);
	if (err)
		return err;
	...
	for (i = 0; i < MALI_PTM_SLICES_COUNT; ++i) {
		if ((old_slices & (1 << i)) && !(slices & (1 << i))) {
			/* Remove unused */
			sysfs_remove_link(entry->slices_kobj, entry->root->slices[i].kobj.name);
		} else if (!(old_slices & (1 << i)) && (slices & (1 << i))) {
			/* Add new */
			if (sysfs_create_link(entry->slices_kobj, &(entry->root->slices[i].kobj),
					      entry->root->slices[i].kobj.name))
				dev_dbg(entry->root->dev, "Cannot create slice%d SysFS link\n", i);
		}
	}
...
static ssize_t partition_assigned_aws_show(struct kobject *kobj, char *buf)
{
	...
	if (entry->root->get_aw_assignment(entry->root->arb, p, &aws))
		return scnprintf(buf, PAGE_SIZE, "0x0\n");

	return scnprintf(buf, PAGE_SIZE, "0x%X\n", aws);
}

static ssize_t partition_assigned_aws_store(struct kobject *kobj, const char *buf, size_t size)
{
	struct mali_arb_sysfs_partition_entry *entry =
		container_of(kobj, struct mali_arb_sysfs_partition_entry, kobj);
	const size_t p = index_of(entry, entry->root->partitions);
	uint32_t old_aws = 0, aws = 0;
	int err;
	...
	err = entry->root->set_aw_assignment(entry->root->arb, p, aws, &old_aws);
	if (err)
		return err;
...
static struct mali_arb_sysfs_entry_attr partition_active_slices_attr =
	__ATTR(active_slices, 0664, partition_active_slices_show, partition_active_slices_store);

static struct mali_arb_sysfs_entry_attr partition_assigned_aws_attr = __ATTR(
	assigned_access_windows, 0664, partition_assigned_aws_show, partition_assigned_aws_store);

static struct attribute *mali_arb_sysfs_partition_attrs[] = {
	&partition_active_slices_attr.attr,
	&partition_assigned_aws_attr.attr,
	NULL,
};
#if (KERNEL_VERSION(5, 2, 0) <= LINUX_VERSION_CODE)
ATTRIBUTE_GROUPS(mali_arb_sysfs_partition);
#endif

static struct kobj_type mali_arb_partition_ktype = {
	.sysfs_ops = &mali_arb_sysfs_entry_ops,
#if (KERNEL_VERSION(5, 2, 0) <= LINUX_VERSION_CODE)
	.default_groups = mali_arb_sysfs_partition_groups,
#else
	.default_attrs = mali_arb_sysfs_partition_attrs,
#endif
	.release = mali_arb_sysfs_partition_entry_release,
};

struct mali_arb_sysfs *mali_arb_sysfs_create_root(
	struct mali_arb *arb, struct device *dev,
	int (*get_slice_assignment)(struct mali_arb *, uint8_t, uint32_t *),
	int (*set_slice_assignment)(struct mali_arb *, uint8_t, uint32_t, uint32_t *),
	int (*get_aw_assignment)(struct mali_arb *, uint8_t, uint32_t *),
	int (*set_aw_assignment)(struct mali_arb *, uint8_t, uint32_t, uint32_t *))
{
	struct mali_arb_sysfs *root;
	...
	root->partitions_kobj = kobject_create_and_add("partitions", root->kobj);
...
int mali_arb_sysfs_add_slice(struct mali_arb_sysfs *data, uint8_t index, struct device *dev)
{
	struct mali_arb_sysfs_slice_entry *entry;
	int err;

	if (!data || (index >= MALI_PTM_SLICES_COUNT) || !dev || data->slices[index].dev)
		return -EINVAL;
	...
	entry->root = data;
	entry->dev = dev;
	err = kobject_init_and_add(&entry->kobj, &mali_arb_slice_ktype, data->slices_kobj,
				   "slice%u", index);
	if (err) {
				 ...
				 struct device *config_dev)
{
	...
	err = kobject_init_and_add(&entry->kobj, &mali_arb_partition_ktype, data->partitions_kobj,
				   "partition%u", index);
	if (err) {
		dev_dbg(data->dev, "Failed to init partition%u sysfs entry\n", index);
		goto clean_up;
	}

	entry->slices_kobj = kobject_create_and_add("slices", &entry->kobj);
	if (!entry->slices_kobj) {
		dev_dbg(data->dev, "Failed to init partition%u/slices sysfs entry\n", index);
		err = -ENOMEM;
		goto clean_up;
	}
