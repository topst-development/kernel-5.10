...
/* Maximum access window mask
 */
#define MAX_AW_MASK 0xFFFF

/* Shift used for dvfs.time busy/idle units of (1 << 8) ns
 * This gives a maximum period between utilization reports of 2^(32+8) ns
 ...
 */
enum mali_arb_partition_reassignment { ARB_REASSIGN_NONE, ARB_REASSIGN_AW, ARB_REASSIGN_SLICE };
 ...
 * @timer_expired: TRUE if timer has fired and needs to be handled
 * @timer_lock: Spinlock used to protect the #timer_expired variable
 * @last_state_transition: Time of last dvfs state transition
 * @last_dvfs_report: Time of last utilization report
 ...
 * @repart_if: Re-partition interfaces
 */
 ...
 * @pwr_priv: Arbiter power sub-module private data
 ...
 * @no_timeslice_yield_timeout: Max time in ms for a driver to stop before GPU lost handling
 *                              is invoked, when it is not timesliced with other drivers.
 */
struct mali_arb {
...
static void gpu_lost_complete_locked(struct mali_arb_priv_data *vm,
				     struct mali_arb_partition *partition, bool req_again);
 ...
 */
static int mali_gpu_get_powered_slices_mask(struct mali_arb *arb, uint32_t *slice_mask)
{
	int ret;
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_get_enabled_slices_mask(struct mali_arb *arb, uint32_t *slice_mask)
{
	int ret;
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_arbiter_poweron_slices(struct mali_arb *arb, uint32_t slice_mask)
{
	int i, ret = 0;
	...
	for (i = 0; i < MALI_PTM_SLICES_COUNT; i++) {
		if (slice_mask & (0x1 << i)) {
			if (arb->slices[i]->timer_running) {
				...
				cancel_work_sync(&arb->slices[i]->slice_timeout_work);
			}
		}
	}
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_arbiter_poweroff_slices(struct mali_arb *arb, uint32_t slice_mask)
{
	int i, ret = 0;
	uint32_t powered_mask = 0;
		...
		if (slice_mask & (0x1 << i)) {
			...
			if (powered_mask & (0x1 << i)) {
				hrtimer_start(&arb->slices[i]->tmr,
					      ms_to_ktime(arb->slice_power_off_wait_time),
					      HRTIMER_MODE_REL);
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_arbiter_nodelay_poweroff_slices(struct mali_arb *arb, uint32_t slice_mask)
{
	uint32_t powered_mask = 0;
	int res = 0;
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_arbiter_enable_slices(struct mali_arb *arb, uint32_t slice_mask)
{
	int i, ret = 0;
 ...
 * Return: 0 on success and non-zero on failure
 */
static int mali_gpu_arbiter_disable_slices(struct mali_arb *arb, uint32_t slice_mask)
{
	int ret = 0;
...
static void slice_timeout_worker(struct work_struct *data)
{
	...
	ret = mali_gpu_arbiter_nodelay_poweroff_slices(arb, (0x1 << this_slice->index));
	if (ret)
...
{
	return (access_window_mask && (access_window_mask & (access_window_mask - 1)));
}
 ...
 */
static void arb_dvfs_update(struct mali_arb_partition *partition, enum arb_state new_state)
{
	struct mali_arb *arb;
	...
	since_last = ktime_to_ns(ktime_sub(now, partition->last_state_transition));
 ...
 *                  the reassignment.
 *
 * As the sysfs calls need to be synchronous (so an error code can be returned),
 * we need to block this thread whilst the request completes.
 */
static void
stop_aw_for_partition_reassignment_locked(struct mali_arb_partition *partition,
					  enum mali_arb_partition_reassignment reassigning_state)
{
	...
	is_running = (partition->state == ARB_RUNNING || partition->state == ARB_SINGLE_REQ) &&
		     partition->slices;
	...
	if (needs_blocking) {
		...
		wait_event(partition->reassignment_wait, !partition->reassigning);
 ...
 */
static struct mali_arb_partition *locked_partition_from_vm_id(struct mali_arb *arb, u32 vm_id)
{
 ...
 */
static int get_slice_assignment(struct mali_arb *arb, uint8_t partition_index, u32 *buf)
{
	struct mali_arb_partition *partition;
 ...
 */
static int set_slice_assignment(struct mali_arb *arb, uint8_t partition_index, u32 slices,
				u32 *old_slices)
{
	struct mali_arb_partition *partition, *other_partition;
	struct part_cfg_ops *cfg_ops;
	u32 all_slices = 0, slice_mask = 0;
	int ret = 0;
	u32 s_index, p_index;
	struct vm_assign_ops *assign_ops;
	int32_t aw_id = 0;

	if (!arb || !arb->dev || !old_slices) {
		pr_err("%s : Invalid argument.\n", __func__);
		return -EINVAL;
	}

	if (partition_index >= MALI_PTM_PARTITION_COUNT) {
		dev_err(arb->dev, "Partition index out of range.\n");
		return -EINVAL;
	}

	/* Basic check that the mask only contains bits for the whole GPU */
	if (slices >= (1 << MALI_PTM_SLICES_COUNT)) {
		dev_err(arb->dev, "Slice mask out of range.\n");
		return -EINVAL;
	}

	partition = arb->partitions[partition_index];
	if (!partition) {
		dev_err(arb->dev, "No GPU/Partition built for the interface ID %d\n",
			partition_index);
		return -EINVAL;
	}

	if (!arb->rg_ops || !arb->rg_ops->get_slice_mask) {
		dev_err(arb->dev, "NULL function pointers for Resource Group.\n");
		return -EINVAL;
	}
	...
	*old_slices = partition->slices;
	...
	if (ret) {
		dev_err(arb->dev, "Get Slice Mask failed.\n");
		ret = -EIO;
		goto exit;
	}
	...
	stop_aw_for_partition_reassignment_locked(partition, ARB_REASSIGN_SLICE);

	/* Unlock the partition before re-partitioning */
	assign_ops = partition->vm_assign_if.ops;
	if (!assign_ops || !assign_ops->get_assigned_vm) {
		dev_err(arb->dev, "P%u: VM assign interface not available.\n", partition->index);
		ret = -ENODEV;
		goto exit;
	}
	...
	if (ret) {
		dev_err(arb->dev, "Failed to disable slices.\n");
		ret = -EIO;
		goto exit;
	}
	...
	if (ret) {
		dev_err(arb->dev, "Failed to poweroff slices.\n");
	...
	if (!cfg_ops || !cfg_ops->assign_slices) {
		dev_err(arb->dev, "P%u: Re-partition interface not available.\n", partition->index);
		ret = -ENODEV;
		goto exit;
	}
	ret = cfg_ops->assign_slices(partition->repart_if.dev, slices);
	if (ret) {
		dev_err(arb->dev, "Failed to assign slices.\n");
	...
	if (slices) {
		...
		if (ret) {
			dev_err(arb->dev, "Failed to poweron slices.\n");
			ret = -EIO;
			goto exit;
		}
		...
		if (ret) {
			dev_err(arb->dev, "Failed to enable slices.\n");
			ret = -EIO;
		}
	}
exit:
 ...
 * Return: 0 on success, or error code
 */
static int get_aw_assignment(struct mali_arb *arb, uint8_t partition_index, u32 *buf)
{
	struct mali_arb_partition *partition;
 ...
 */
static int set_aw_assignment(struct mali_arb *arb, uint8_t partition_index, u32 access_windows,
			     u32 *old_access_windows)
{
	struct mali_arb_partition *partition, *other_partition;
	int ret = 0;
	u32 current_aws = 0, aw_mask = 0;
	u32 aw, p;
	struct mali_arb_priv_data *cur_vm, *tmp_vm;
	bool poweron_required = false;

	if (!arb || !arb->dev || !old_access_windows) {
		pr_err("%s : Invalid argument.\n", __func__);
		return -EINVAL;
	}
	...
	*old_access_windows = current_aws;
	...
	if (ret) {
		dev_err(arb->dev, "Get Access Window Mask failed.\n");
		ret = -EIO;
		goto exit;
	}
	...
	stop_aw_for_partition_reassignment_locked(partition, ARB_REASSIGN_AW);
	 ...
	 */
	for (aw = 0; aw < MALI_PTM_ACCESS_WINDOW_COUNT; ++aw) {
		if ((*old_access_windows & (1 << aw)) && !(access_windows & (1 << aw))) {
			list_for_each_entry_safe(cur_vm, tmp_vm, &partition->req_list, entry) {
				if (cur_vm->info.id == aw) {
					list_del_init(&cur_vm->entry);
					list_add_tail(&cur_vm->entry, &arb->wait_list);
					break;
				}
			}
		}
	...
	if (poweron_required)
		mali_gpu_arbiter_poweron_slices(arb, partition->slices);
	/* Continue arbitration */
	run_next_vm_locked(partition);
...
static enum hrtimer_restart timer_isr(struct hrtimer *timer)
{
	...
	spin_lock(&partition->timer_lock);
	partition->timer_expired = true;
	queue_work(partition->wq, &partition->timeout_work);
	spin_unlock(&partition->timer_lock);
	return HRTIMER_NORESTART;
}
...
static void assign_to_vm(struct mali_arb_partition *partition, uint32_t vm_id)
{
	...
	if (ret) {
		dev_err(arb->dev, "P%u: Failed to assign VM%u to partition/GPU\n", partition->index,
			vm_id);
		goto drop_active;
	}
	return;
...
drop_active:
	if (partition->active_vm != NULL) {
		...
		list_move_tail(&partition->active_vm->entry, &partition->req_list);
		partition->active_vm = NULL;
	}
}
...
static void grant_gpu_to_next_locked(struct mali_arb_partition *partition)
{
	...
	partition->active_vm =
		list_first_entry(&partition->req_list, struct mali_arb_priv_data, entry);
	...
	if (partition->active_vm) {
		...
		partition->active_vm->info.ops.gpu_granted(partition->active_vm->vm,
							   arb->curr_gpu_freq);
#if IS_ENABLED(CONFIG_MALI_AGGRESSIVE_ARBITER)
		if (arb_lost_test_time_ms != 0)
			queue_delayed_work(partition->wq, &partition->aggr_lost_work,
					   usecs_to_jiffies(arb_lost_test_time_ms));
#endif /* CONFIG_MALI_AGGRESSIVE_ARBITER */
	}
}
 ...
 */
static inline void set_state_locked(struct mali_arb_partition *partition, enum arb_state new)
{
	struct mali_arb *arb;
 ...
 */
static void start_timer_locked(struct mali_arb_partition *partition, unsigned long timeout)
{
	...
	lockdep_assert_held(&partition->mutex);
	spin_lock_irqsave(&partition->timer_lock, irqflags);
	partition->timer_expired = false;
	spin_unlock_irqrestore(&partition->timer_lock, irqflags);
	hrtimer_start(&partition->tmr, ms_to_ktime(timeout), HRTIMER_MODE_REL);
...
static void run_next_vm_locked(struct mali_arb_partition *partition)
{
	...
	if (list_empty(&partition->req_list)) {
		if (partition->state != ARB_NO_REQ) {
			...
			if (arb->rg_ops)
				mali_gpu_arbiter_nodelay_poweroff_slices(arb, partition->slices);
			else
				mali_arbiter_power_disable(arb->pwr_priv);
		}
	...
	if (partition->state != ARB_NO_REQ && partition->slices) {
		...
		if (partition->state == ARB_RUNNING)
			start_timer_locked(partition, arb->request_timeout);
	}
}
...
static void gpu_stopped(struct mali_arb_priv_data *arb_vm, bool req_again)
{
	...
	if (req_again && !partition) {
		...
		dev_dbg(arb->dev, "Adding VM %u to partitionless wait list (after a restart)\n",
			arb_vm->info.id);
		list_add_tail(&arb_vm->entry, &arb->wait_list);
		return;
	} else if (!partition) {
		dev_warn(arb->dev, "GPU_STOPPED from VM %u not assigned to a partition\n",
			 arb_vm->info.id);
		return;
	}

	dev_dbg(arb->dev, "P%u: %u, req_again: %d\n", partition->index, arb_vm->info.id, req_again);
	...
	if (!list_empty(&arb_vm->entry)) {
		...
		if (arb_vm == partition->active_vm) {
			if (req_again)
				...
				list_move_tail(&arb_vm->entry, &partition->req_list);
			remove_active_vm(partition);
		 ...
		 * access the GPU.
		 */
		dev_dbg(arb->dev, "P%u - vm->entry is empty. requesting gpu for VM %u\n",
			partition->index, arb_vm->info.id);
		if (list_empty(&partition->req_list)) {
			if (arb->rg_ops)
				mali_gpu_arbiter_poweron_slices(arb, partition->slices);
			else
				mali_arbiter_power_enable(arb->pwr_priv);
		}
		list_add_tail(&arb_vm->entry, &partition->req_list);
		if (partition->state == ARB_NO_REQ || partition->state == ARB_SINGLE_REQ) {
...
static void gpu_request(struct mali_arb_priv_data *arb_vm)
{
	...
	if (!partition) {
		...
		list_add_tail(&arb_vm->entry, &arb->wait_list);
		dev_warn(arb->dev, "GPU_REQUEST from VM %u not assigned to a partition\n",
			 arb_vm->info.id);
		return;
	}
	...
	list_for_each_entry(cur_vm, &partition->req_list, entry) {
		if (cur_vm == arb_vm) {
			dev_dbg(arb->dev, "P%u: VM %u requested GPU when already requested\n",
				partition->index, arb_vm->info.id);
...
static void stop_gpu_locked(struct mali_arb_partition *partition)
{
	...
	int new_yield_timeout;

	arb = partition->arb;
	if (WARN_ON(!arb))
		return;

	new_yield_timeout = arb->yield_timeout;
	lockdep_assert_held(&partition->mutex);

	arb_vm = partition->active_vm;
	if (!arb_vm) {
		dev_warn(arb->dev, "P%u: GPU_STOP cannot be issued without an active VM.\n",
			 partition->index);
		return;
	}
	...
	if ((partition->reassigning == ARB_REASSIGN_SLICE) &&
		...
		new_yield_timeout = arb->no_timeslice_yield_timeout;
	}
...
static void handle_gpu_lost_locked(struct mali_arb_partition *partition)
{
	...
	arb_vm = partition->active_vm;
	if (!arb_vm) {
		dev_warn(arb->dev, "P%u: GPU_LOST cannot be issued without an active VM.\n",
			 partition->index);
		return;
	}
	...
	dev_warn(arb->dev, "P%u: GPU in VM%u took too long to yield.\n", partition->index,
		 arb_vm->info.id);

	list_del_init(&partition->active_vm->entry);
	remove_active_vm(partition);

	/* Send GPU lost before reassigning to another VM. */
...
static void timeout_worker(struct work_struct *data)
{
	...
	spin_lock_irqsave(&partition->timer_lock, irqflags);
	if (!partition->timer_expired) {
		/* Timer was stopped/restarted before worker ran */
		spin_unlock_irqrestore(&partition->timer_lock, irqflags);
		goto cleanup_mutex;
	}
	partition->timer_expired = false;
	...
	arb_vm = partition->active_vm;
	if (!arb_vm) {
		dev_dbg(arb->dev, "P%u: Timer cannot be pending without an active VM.\n",
			partition->index);
		goto cleanup_mutex;
	}
	switch (partition->state) {
	...
	case ARB_GPU_STOPPING:
		 ...
		 * VM to take longer.
		 */
		if (partition->reassigning || !list_is_singular(&partition->req_list))
			handle_gpu_lost_locked(partition);
		else
			dev_dbg(arb->dev, "GPU time is up but no VMs waiting.\n");
		break;
	default:
		break;
	}
 ...
 * * -EFAULT	- failed with software fault.
 */
static int reg_assign_if(struct mali_arb_data *arb_data, struct vm_assign_interface vm_assign)
{
	int ret = 0;
	struct mali_arb_partition *partition;
	struct mali_arb *arb;
	...
	if (unlikely(!vm_assign.ops->assign_vm || !vm_assign.ops->unassign_vm)) {
			 ...
			 struct mali_arb_partition **partition)
{
	...
	for (i = 0; i < MALI_PTM_PARTITION_COUNT; ++i) {
		...
		if (tmp_partition) {
			...
			if (!assign_ops || !assign_ops->get_assigned_vm) {
				dev_dbg(arb->dev, "P%u: get assigned VM not available.\n",
					tmp_partition->index);
				continue;
			}
			mutex_lock(&tmp_partition->mutex);
			err = assign_ops->get_assigned_vm(tmp_partition->vm_assign_if.dev, &aw_id);
			mutex_unlock(&tmp_partition->mutex);
			if (unlikely(err)) {
				dev_err(arb->dev, "P%u: get assigned VM failed.\n",
					tmp_partition->index);
				continue;
			}
			if (vm_id == (uint32_t)aw_id) {
 ...
 */
static int register_vm(struct mali_arb_data *arb_data, struct mali_vm_data *vm_info,
		       struct mali_vm_priv_data *vm, struct mali_arb_priv_data **arb_vm)
{
	...
	if (unlikely(vm_info->dev == NULL || vm_info->ops.gpu_granted == NULL ||
		     vm_info->ops.gpu_stop == NULL || vm_info->ops.gpu_lost == NULL)) {
		pr_err("%s : VM ops functions are invalid.\n", __func__);
		return -EINVAL;
	}
	...
	dev_dbg(arb->dev, "Registering VM %u\n", vm_info->id);

	priv_vm = devm_kzalloc(arb->dev, sizeof(struct mali_arb_priv_data), GFP_KERNEL);
	if (!priv_vm)
		return -ENOMEM;
	...
	reg_vm = devm_kzalloc(arb->dev, sizeof(struct registered_vm), GFP_KERNEL);
	if (!reg_vm)
		return -ENOMEM;
...
static void gpu_active(struct mali_arb_priv_data *arb_vm)
{
	...
	partition = locked_partition_from_vm_id(arb, arb_vm->info.id);
	if (!partition) {
		dev_warn(arb->dev, "GPU_ACTIVE from VM %u not assigned to a partition\n",
			 arb_vm->info.id);
		return;
	}
...
static void gpu_idle(struct mali_arb_priv_data *arb_vm)
{
	...
	if (!partition) {
		...
		dev_warn(arb->dev, "GPU_IDLE from VM %u not assigned to a partition\n",
			 arb_vm->info.id);
		return;
	}
	...
	if (arb_vm == partition->active_vm &&
	    (partition->state == ARB_RUNNING || partition->state == ARB_SINGLE_REQ)) {
...
/**
 * reset_utilisation_counters_locked() - Reset utilization counters
 * @partition: Partition for which counter will be reset
 * @t: Timestamp
 *
 * Called from get_utilisation function to reset the counters.
 */
static inline void reset_utilisation_counters_locked(struct mali_arb_partition *partition,
						     ktime_t t)
{
	lockdep_assert_held(&partition->mutex);
	partition->gpu_active_sum = 0;
	partition->gpu_switching_sum = 0;
	partition->last_state_transition = t;
	partition->last_dvfs_report = t;
}
 ...
 * deadlocks.
 */
static void get_utilisation(void *dev, u32 *gpu_busytime, u32 *gpu_totaltime)
{
	...
	*gpu_totaltime = 0;
	...
	/* Find the partition with the maximum utilization */
	last_percent = 0;
	for (i = 0; i < ARRAY_SIZE(arb->partitions); ++i) {
		...
		mutex_lock(&partition->mutex);
		now = ktime_get();
		time_since_last_report = ktime_to_ns(ktime_sub(now, partition->last_dvfs_report));
		time_since_last_transition =
			ktime_to_ns(ktime_sub(now, partition->last_state_transition));
		...
		case ARB_GPU_STOPPING: /* switching */
			partition->gpu_switching_sum += time_since_last_transition;
			break;
		default:
			break;
		}

		/* get GPU time and log utilization info */
		busytime = partition->gpu_active_sum >> ARBITER_TIME_SHIFT;
		totaltime = (time_since_last_report - partition->gpu_switching_sum) >>
			    ARBITER_TIME_SHIFT;

		/* Do not divide by zero... */
		if (totaltime == 0) {
			/* reset counters before continue */
			reset_utilisation_counters_locked(partition, now);
			mutex_unlock(&partition->mutex);
			continue;
		}

		percent = div_u64((u64)busytime * 100, totaltime);
		...
		dev_dbg(arb->dev, "Arbiter P%u reporting busy %u, total %u (~ %llu%%)\n",
			partition->index, busytime, totaltime, percent);
...
static void update_freq(void *dev, u32 new_freq)
{
	...
	for (; i <= num_partition; ++i) {
		 ...
		 * not the case.
		 */
		if (partition->active_vm && partition->state != ARB_GPU_STOPPING)
			partition->active_vm->info.ops.gpu_granted(partition->active_vm->vm,
								   arb->curr_gpu_freq);
 ...
 */
static int get_max_l2_slices(struct device *dev, uint32_t *max_l2_slices)
{
	...
	if (!rg_ops || !rg_ops->get_slice_mask) {
		dev_dbg(dev, "Setup does not support variable L2 slices number.\n");
		*max_l2_slices = 0;
		return 0;
	}
 ...
 * Return: 0 on success, standard linux error code otherwise
 */
static int get_max_core_mask(struct device *dev, uint32_t *max_core_mask)
{
	uint32_t slice_mask = 0;
	...
	rg_ops = dev_get_drvdata(dev);
	if (!rg_ops || !rg_ops->get_slice_mask || !rg_ops->get_slices_core_mask) {
		dev_dbg(dev, "Setup does not support variable Core Mask.\n");
		*max_core_mask = 0;
		return 0;
	}
	...
	err = rg_ops->get_slices_core_mask(dev, &slices_core_mask, &core_mask_stride);
	if (err)
		return err;
 ...
 */
static int get_max_config(struct mali_arb_priv_data *arb_vm, uint32_t *max_l2_slices,
 ...
 *
 */
static struct mali_arb_partition *build_partition(struct mali_arb *arb, u32 index)
{
	struct mali_arb_partition *partition;
	...
	spin_lock_init(&partition->timer_lock);
	partition->arb = arb;
	...
	partition->wq = alloc_workqueue("mali_arb_D%uP%u", WQ_UNBOUND, 0, arb->dev->id, index);
 ...
 *
 */
static void destroy_partition(struct mali_arb_partition *partition)
{
	if (!partition)
		return;
...
/* Telechips : 
 * Addition for setting assigned_access_windows and active_slices 
 * based on CONFIG_MALI_DEFAULT_PTM_CONFIG in in-tree kernel
 */
#ifdef CONFIG_MALI_DEFAULT_PTM_CONFIG	
...
static bool tcc_set_default_aw_slice(struct mali_arb *arb,const char *str, int partition_id, int resource_group_id)
{
	bool retval = false;
	u32 i = 0;
	u32 rg_cnt = 0, p_cnt = 0, slice_value = 0, aw_value = 0;
	u32 old_slices = 0, old_windows = 0;
	...
	if(resource_group_id < 0 || resource_group_id > MALI_PTM_RESOURCE_GROUP_COUNT) {
		dev_err(arb->dev,"Please check resource_group_id:%d\n", resource_group_id); 
		return false;
	}
		...
		if(rg_cnt == resource_group_id) {
			...
			while ((tok = get_token(&grp_tok, ":"))) {
				...
				case 'S':       /* a slice specifier? */
					++tok;
					if (!to_uint32(arb->dev, tok, &i, "Slice"))
						goto parse_fail;
					if (p_cnt == partition_id)
						slice_value |= (1<<i);   
					break;
				...
				case 'W':       /* an access window specifier? */
					++tok;
					if (!to_uint32(arb->dev, tok, &i, "Window"))
						goto parse_fail;
					if (p_cnt == partition_id+1)
						aw_value |= (1<<i);
					break;
				default:
					dev_err(arb->dev, "invalid token '%c'\n", *tok);
					goto parse_fail;
				}
			}
			/* To configure aw and slice for a partition, 
			 * you need to set ptm_config in the order S, P, W.
			 * For example, 
			 * MALI_DEFAULT_PTM_CONFIG="\",B:S0:P0:W0,A:S1:S2:P1:W1:S3:P2:W2\""
			 */
			dev_info(arb->dev,"%s RG:%d P:0x%x S:0x%x AW:0x%x \n", __func__,
				 resource_group_id, partition_id, slice_value, aw_value);
			if(set_slice_assignment(arb, partition_id, slice_value, &old_slices))
				goto parse_fail;
			if(set_aw_assignment(arb, partition_id, aw_value, &old_windows))
				goto parse_fail;
			break;
		}
		else
 ...
 * * -ENOMEM	- out of memory.
 */
static int update_arbiter_resources(struct mali_arb *arb, struct resource_interfaces res)
{
	struct mali_arb_partition *partition;
	int err;
	uint32_t i;
	...
	if (res.vm_assign) {
		struct mali_ptm_rg_ops *rg_ops;
		u32 slice_mask = 0, aw_mask = 0;

		rg_ops = dev_get_drvdata(arb->dev);
		if (!rg_ops) {
			dev_err(arb->dev, "Cannot get Resource Group Operations data\n");
			return -ENODEV;
		}

		if (!rg_ops->get_slice_mask || !rg_ops->get_aw_mask) {
			dev_err(arb->dev, "NULL function pointers in rg_ops\n");
			return -EINVAL;
		}
		...
		arb->sysfs = mali_arb_sysfs_create_root(arb, arb->dev, get_slice_assignment,
							set_slice_assignment, get_aw_assignment,
							set_aw_assignment);
		if (!arb->sysfs) {
		...
		for (i = 0; i < MALI_PTM_SLICES_COUNT; ++i) {
			...
			if (mali_arb_sysfs_add_slice(arb->sysfs, i, arb->dev)) {
				dev_err(arb->dev, "Failed to create slice %u SysFS entry\n", i);
				err = -EFAULT;
				goto clean_slices;
			}

			arb->slices[i] = kzalloc(sizeof(struct slice), GFP_KERNEL);
			if (!arb->slices[i]) {
				err = -ENOMEM;
				goto clean_slices;
			}
			...
			hrtimer_init(&arb->slices[i]->tmr, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			arb->slices[i]->tmr.function = slice_timer_isr;
			arb->slices[i]->timer_running = false;
			INIT_WORK(&arb->slices[i]->slice_timeout_work, slice_timeout_worker);
		}

		arb->slices_wq = alloc_ordered_workqueue("mali_arb_slices", WQ_HIGHPRI);
		if (!arb->slices_wq) {
		...
		for (i = 0; i < res.num_if; i++) {
			...
			err = mali_arb_sysfs_add_partition(arb->sysfs, partition_id,
							   partition->repart_if.dev);
			if (err) {
				dev_err(arb->dev, "P%u: Cannot create SysFS entry\n", partition_id);
				err = -EFAULT;
				goto clean_partitions;
			}

			/* Control module - It assumes that interface ID in
			 * re-partition and vm_assign are matching.
			 */
		...
		dev_info(arb->dev, "Arbiter running without Partition Manager\n");
		arb->sysfs = NULL;

		/* Non-PTM mode implicitly has one partition with one slice */
...
/* Telechips : To use CONFIG_MALI_DEFAULT_PTM_CONFIG in in-tree kernel */
#ifdef CONFIG_MALI_DEFAULT_PTM_CONFIG
	for (i = 0; i < res.num_if; i++) {
        uint32_t partition_id;

    	partition_id = res.repartition[i]->if_id;
		if(!tcc_set_default_aw_slice(arb, MALI_DEFAULT_PTM_CONFIG,partition_id,res.rg_index))
			dev_err(arb->dev, "tcc_set_default_aw_slice returned fail\n");
	}
#endif
	return 0;
...
/**
 * init_arbiter_data() - Initializes the Arbiter internal data.
 ...
 * Return: 0 on success, or error code
 */
static int init_arbiter_data(struct device *dev, struct mali_arb *arb)
{
	if (!arb || !dev) {
		pr_err("%s : Invalid argument.\n", __func__);
		return -EINVAL;
...
/** Initializes the Arbiter internal data.
 */
			...
			int no_timeslice_yield_timeout)
{
	struct mali_gpu_power_arbiter_cb_ops pwr_cb = {
		.arb_get_utilisation = get_utilisation,
		.update_freq = update_freq,
	};
	struct mali_arb *arb;
	int err;

	/* Allocate arbiter instance */
	arb = devm_kzalloc(dev, sizeof(struct mali_arb), GFP_KERNEL);
	if (!arb)
		return -ENOMEM;

	/* Init module params */
	arb->request_timeout = request_timeout;
	arb->yield_timeout = yield_timeout;
	arb->slice_power_off_wait_time = slice_power_off_wait_time;
	arb->no_timeslice_yield_timeout = no_timeslice_yield_timeout;


	/* Initialize arbiter data */
	err = init_arbiter_data(dev, arb);
	if (err)
		goto cleanup;

	/* Initialize arbiter resources */
	err = update_arbiter_resources(arb, res);
	if (err)
		goto cleanup;

	/* Initialize arbiter power sub-module */
	err = mali_arbiter_power_init(arb->dev, &arb->arb_data, pwr, pwr_cb, &arb->pwr_priv);
	if (err)
		goto cleanup;

	/* PTM specific initialization */
	if (pwr.dev) {
		/* After initializing the arbiter it is necessary to send the
		 * max config information to all VMs assigned to this resource group.
		 * Sending it automatically in the beginning helps to avoid round trip
		 * during the kbase initialization.
		 */
		arb->max_config_info.updated = false;
		err = calculate_max_config(arb);
		if (err)
			dev_dbg(dev, "Failed to calculate max config.\n");
...
/** Terminates the arbiter device and release its resources
 */
void arbiter_core_destroy(struct device *dev, struct mali_arb_data *arb_data)
{
	...
	/* Clean-up power sub-module */
	mali_arbiter_power_deinit(arb->pwr_priv);
