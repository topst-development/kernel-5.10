...
static void sdhci_tcc897x_dumpregs(struct sdhci_host *host)
{
	...
	pr_debug("[DEBUG][SDHC] DELAYCON: 0x%08x\n",
		 readl(tcc->chctrl_base + TCC897X_SDHC_DELAY_CON(ch)));
...
static void sdhci_tcc897x_reset(struct sdhci_host *host, u8 mask)
{
	sdhci_reset(host, mask);

	/* After sw reset, re-write the timeout value */
	if (mask & SDHCI_RESET_ALL) {
		sdhci_writeb(host, 0xE, SDHCI_TIMEOUT_CONTROL);
	}
}

static void sdhci_tcc807x_reset(struct sdhci_host *host, u8 mask)
{
	struct sdhci_tcc *tcc = to_tcc(host);
	const u32 vendor_dly[3][2] = { /* (CMD_DLY, DAT_DLY) */
	...
	/*
	 * RESET_ALL resets all host registers include vendor specific register.
	 * So, after RESET_ALL, the vendor register will be written again.
	 */
	if (mask == SDHCI_RESET_ALL) {
		vals = sdhci_readl(host, TCC_SDHC_VENDOR);

		/* For chip boundary issue */
		ch = tcc->controller_id;
		vals &= (~0x00003300u);
		vals |= (TCC_SDHC_VENDOR_DAT_DLY(vendor_dly[ch][1]) |
				TCC_SDHC_VENDOR_CMD_DLY(vendor_dly[ch][0]));

		/* Configure suto clock gate */
		if ((tcc->flags & TCC_SDHC_CLK_GATING) != 0u) {
			vals &= (~0x00000002u);
			vals |= 0x2u;
		}

		sdhci_writel(host, vals, TCC_SDHC_VENDOR);
	}
}
...
static int sdhci_tcc750x_parse_channel_configs(struct platform_device *pdev,
						struct sdhci_host *host)
{
	struct sdhci_tcc *tcc = to_tcc(host);
	struct device_node *np;
...
static void sdhci_tcc897x_set_channel_configs(struct sdhci_host *host)
{
	...
	/* Configure DELAY_CON */
	vals = readl(tcc->chctrl_base + TCC897X_SDHC_DELAY_CON(ch));
	if ((ch % 2u) != 0u) {
		vals &= (u32)0x0000FFFF;
		vals |= (TCC897X_SDHC_DELAY_CON_DEF << 16u);
	} else {
		vals &= (u32)0xFFFF0000;
		vals |= TCC897X_SDHC_DELAY_CON_DEF;
	}
	writel(vals, tcc->chctrl_base + TCC897X_SDHC_DELAY_CON(ch));

	/* clear CD/WP regitser */
	vals = readl(tcc->chctrl_base + TCC897X_SDHC_HOSTCFG);
	vals &= ~TCC897X_SDHC_HOSTCFG_CD(ch);
	vals &= ~TCC897X_SDHC_HOSTCFG_WP(ch);
	writel(vals, tcc->chctrl_base + TCC897X_SDHC_HOSTCFG);
...
static void sdhci_tcc_set_channel_configs_tap_v2(struct sdhci_host *host)
{
	...
	/* Configure CMD TAPDLY */
	vals = TCC_SDHC_MK_TAPDLY(0xF, tcc->cmd_tap);
	...
	/* Configure DATA TAPDLY */
	vals = TCC_SDHC_MK_TAPDLY(0xF, tcc->data_tap);
	for (i = 0u; i < 8u; i++) {
...
static void sdhci_tcn100x_set_channel_configs(struct sdhci_host *host)
{
	struct sdhci_tcc *tcc = to_tcc(host);
	...
	if (tcc->force_caps != 0UL) {
		...
		if ((tcc->soc_data->tcc_quirks & TCC_QUIRK_64BIT_DMA) == 0U) {
			writel(TCC_SDHC_CAPARG0_DEF,
				   tcc->chctrl_base + TCC_SDHC_CAPREG0);
		} else {
			writel((TCC_SDHC_CAPARG0_DEF | TCC_SDHC_64BIT_DMA),
				   tcc->chctrl_base + TCC_SDHC_CAPREG0);
		}
		writel(TCC_SDHC_CAPARG1_DEF,
		       tcc->chctrl_base + TCC_SDHC_CAPREG1);
...
static void sdhci_tcc750x_set_channel_configs(struct sdhci_host *host)
{
	struct sdhci_tcc *tcc = to_tcc(host);
...
static int sdhci_tcc750x_set_core_clock(struct sdhci_host *host)
{
	...
	if (is_tcc_support_hs400(host) != 0) {
		...
		writel(0x2, tcc->chctrl_base + TCC_SDHC_CORE_CLK_REG0);
		writel(0x1, tcc->chctrl_base + TCC_SDHC_CORE_CLK_REG1);
		...
		/* enable peri clock */
		clk_prepare_enable(pltfm_host->clk);
		writel(0x0, tcc->chctrl_base + TCC_SDHC_CORE_CLK_REG0);
...
tcc750x_err_pclk_disable:
	clk_disable_unprepare(pclk);
tcc750x_err_hclk_disable:
	clk_disable_unprepare(tcc->hclk);
tcc750x_err_pltfm_free:
...
static unsigned int sdhci_tcc_clk_get_max_clock(struct sdhci_host *host)
{
	...
	if (is_tcc_support_hs400(host) != 0) {
		...
		ret = clk_get_rate(pltfm_host->clk);
	}

	return ret;
}

static unsigned int sdhci_tcc897x_clk_get_max_clock(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
...
static void sdhci_tcc_set_uhs_signaling(struct sdhci_host *host, unsigned timing)
{
	u16 ctrl_2;
	...
	/*
	 * If 1.8v = 1, the signal is always output to the rising edge.
	 * Therefore, 1.8v = 0 is set so that the signal is output to
	 * the falling edge in a low-speed mode using 1.8v.
	 */
...
static const struct sdhci_ops sdhci_tcc_ops = {
	.get_max_clock = sdhci_tcc897x_clk_get_max_clock,
	.set_clock = sdhci_tcc_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.adma_write_desc = sdhci_tcc_adma_write_desc,
	.reset = sdhci_tcc897x_reset,
	.hw_reset = sdhci_tcc_hw_reset,
	.set_uhs_signaling = sdhci_tcc_set_uhs_signaling,
...
static const struct sdhci_ops sdhci_tcc750x_ops = {
	.get_max_clock = sdhci_tcc_clk_get_max_clock,
	.set_clock = sdhci_tcc_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.adma_write_desc = sdhci_tcc_adma_write_desc,
	.reset = sdhci_tcc_reset,
...
static const struct sdhci_ops sdhci_tcc807x_ops = {
	.get_max_clock = sdhci_tcc805x_clk_get_max_clock,
	.set_clock = sdhci_tcc_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.adma_write_desc = sdhci_tcc_adma_write_desc,
	.reset = sdhci_tcc807x_reset,
	.hw_reset = sdhci_tcc_hw_reset,
	.set_uhs_signaling = sdhci_tcc_set_uhs_signaling,
	.get_ro = sdhci_tcc_get_ro,
#ifdef CONFIG_SDHCI_TCC_USE_SW_TUNING
...
static const struct sdhci_ops sdhci_tcn100x_ops = {
	.get_max_clock = sdhci_tcc805x_clk_get_max_clock,
	.set_clock = sdhci_tcc_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.reset = sdhci_tcc807x_reset,
...
static const struct sdhci_pltfm_data sdhci_tcc750x_pdata = {
	.ops = &sdhci_tcc750x_ops,
	.quirks = SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_STOP_WITH_TC,
};

static const struct sdhci_pltfm_data sdhci_tcc807x_pdata = {
	.ops = &sdhci_tcc807x_ops,
	.quirks = SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_STOP_WITH_TC,
};

static const struct sdhci_pltfm_data sdhci_tcn100x_pdata = {
	.ops = &sdhci_tcn100x_ops,
	.quirks = SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_STOP_WITH_TC,
};
...
static const struct sdhci_tcc_soc_data soc_data_tcc750x = {
	.pdata = &sdhci_tcc750x_pdata,
	.parse_channel_configs = sdhci_tcc750x_parse_channel_configs,
	.set_channel_configs = sdhci_tcc750x_set_channel_configs,
	.set_core_clock = sdhci_tcc750x_set_core_clock,
#ifdef CONFIG_SDHCI_TCC_USE_SW_TUNING
...
static const struct sdhci_tcc_soc_data soc_data_tcc807x = {
	.pdata = &sdhci_tcc807x_pdata,
	.parse_channel_configs = sdhci_tcc805x_parse_channel_configs,
	.set_channel_configs = sdhci_tcc805x_set_channel_configs,
	.set_core_clock = sdhci_tcc805x_set_core_clock,
#ifdef CONFIG_SDHCI_TCC_USE_SW_TUNING
...
static const struct sdhci_tcc_soc_data soc_data_tcn100x = {
	.pdata = &sdhci_tcn100x_pdata,
	.parse_channel_configs = sdhci_tcc805x_parse_channel_configs,
	.set_channel_configs = sdhci_tcn100x_set_channel_configs,
	.set_core_clock = sdhci_tcc805x_set_core_clock,
	.set_channel_itap = NULL,
	.tcc_quirks = TCC_QUIRK_64BIT_DMA,
};
...
static const struct of_device_id sdhci_tcc_of_match_table[10] = {
	...
	{.compatible = "telechips,tcc750x-sdhci", .data = &soc_data_tcc750x},
	{.compatible = "telechips,tcc807x-sdhci", .data = &soc_data_tcc807x},
	{.compatible = "telechips,tcc897x-sdhci", .data = &soc_data_tcc897x},
	{.compatible = "telechips,tcc901x-sdhci", .data = &soc_data_tcc},
	{.compatible = "telechips,tcn100x-sdhci", .data = &soc_data_tcn100x},
	{}
};
					    ...
					    struct mmc_ios *ios)
{
	...
	vendor = sdhci_readl(host, TCC_SDHC_VENDOR);
	if (ios->enhanced_strobe) {
		(void)pr_info("[INFO] %s: Enable enhanced strobe\n",
				mmc_hostname(host->mmc));
	...
	sdhci_writel(host, vendor, TCC_SDHC_VENDOR);
}
...
static int sdhci_tcc_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		if (ret == -EPROBE_DEFER) {
			dev_info(&pdev->dev,
				"[INFO][SDHC] Deferred host driver. Please check moduele dependancy");
		} else {
			dev_err(&pdev->dev,
