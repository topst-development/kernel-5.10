...
static void tcc_sc_mmc_timeout_timer(struct timer_list *t)
{
...
static const struct of_device_id tcc_sc_mmc_of_match_table[3] = {
	{ .compatible = "telechips,tcc805x-sc-mmc", .data = NULL},
	{ .compatible = "telechips,tcc807x-sc-mmc", .data = NULL},
	{}
};
...
static s32 tcc_sc_mmc_probe(struct platform_device *pdev)
{
	...
	if (handle == NULL) {
		dev_err(&pdev->dev, "[ERROR][TCC_SC_MMC] Failed to get handle, deferring.\n");
		return -EPROBE_DEFER;
	}
