...
#if defined(CONFIG_ARCH_TELECHIPS)
	chan_id = pch->thread->id;

	reg_addr = pch->dmac->dmac_wrap + ch_extended_base + (chan_id * 4);
	wr_val = (dst >> 32) & 0xF;
	rd_val = (src >> 32) & 0xF;

	if (wr_val <= 0x6 && rd_val <= 0x6) {
		val = (wr_val << 16) | (rd_val);
		writel(val, reg_addr);

		/* Due to the characteristics of the IP design,
		 * the 4-bit Extended value must also be written to ch8.
		 */
		writel(val, pch->dmac->dmac_wrap + ch_extended_base + 0x20);
	} else {
		dev_err(pch->dmac->ddma.dev, "%s:%d Address out of range\n",
				__func__, __LINE__);
		return NULL;
	}
#endif
...
#if defined(CONFIG_ARCH_TELECHIPS)
	struct resource tcc_res;

	ret = dma_set_mask_and_coherent(&adev->dev, DMA_BIT_MASK(35));
#elif
...
#if defined(CONFIG_ARCH_TELECHIPS)
	ret = of_address_to_resource(adev->dev.of_node, 1, &tcc_res);
	pl330->dmac_wrap = devm_ioremap_resource(&adev->dev, &tcc_res);
	if (IS_ERR(pl330->dmac_wrap))
		return PTR_ERR(pl330->dmac_wrap);
#endif
