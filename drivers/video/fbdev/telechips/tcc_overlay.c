...
static int tcc_overlay_display_video_buffer(overlay_video_buffer_t buffer_cfg, struct overlay_drv_type *overlay_drv)
{
	...
	if (overlay_drv->sc.reg != NULL) {
		if (VIOC_CONFIG_Device_PlugState(overlay_drv->sc.id, &VIOC_PlugIn) == VIOC_DEVICE_CONNECTED) {
			(void)VIOC_CONFIG_PlugOut(overlay_drv->sc.id);
		}
	}
...
#endif
		VIOC_RDMA_SetImageAlphaSelect(overlay_drv->rdma[layer].reg, 1);
		VIOC_RDMA_SetImageAlphaEnable(overlay_drv->rdma[layer].reg, 1);
	...
	|| defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC901X)
		...
		(void)pr_warn("[WAR][OVERLAY] init layer rdma(0x%04X) VIOC_NO_COMPONENT\n",overlay_drv->rdma[layer].id);
	}
overlay_exit:
	return ret;
}
...
static int scaling_and_push_buffer(overlay_scaling_buffer_t buffer_cfg, struct overlay_drv_type *overlay_drv)
{
	unsigned int layer = 0, base0, base1, base2;
	struct panel_size_t panel;
	unsigned int output_width, output_height;
	...
	dprintk("%s: (0x%08x, 0x%08x, 0x%08x)(0x%x) (%dx%d) -> (%d,%d)(%dx%d) -> (%d,%d)(%dx%d)\n", __func__,
		buffer_cfg.addr, buffer_cfg.addr1, buffer_cfg.addr2,
		buffer_cfg.format, buffer_cfg.src_width, buffer_cfg.src_height,
		buffer_cfg.crop_cfg.sx, buffer_cfg.crop_cfg.sy, buffer_cfg.crop_cfg.width, buffer_cfg.crop_cfg.height,
		buffer_cfg.cfg.sx, buffer_cfg.cfg.sy, buffer_cfg.cfg.width, buffer_cfg.cfg.height);

	layer = overlay_drv->layer_n;

	if (VIOC_RDMA_IsVRDMA(overlay_drv->rdma[layer].id) == 0) {
		if ((buffer_cfg.format == (unsigned int)TCC_LCDC_IMG_FMT_444SEP) ||
			(buffer_cfg.format == (unsigned int)TCC_LCDC_IMG_FMT_YUV420SP) ||
			(buffer_cfg.format == (unsigned int)TCC_LCDC_IMG_FMT_YUV422SP) ||
			((buffer_cfg.format >= (unsigned int)TCC_LCDC_IMG_FMT_YUV420ITL0) &&
			(buffer_cfg.format <= (unsigned int)TCC_LCDC_IMG_FMT_YUV422ITL1))) {
			(void)pr_err("[ERR][OVERLAY] This format %d is not supported by RDMA id %d.\n",
				buffer_cfg.format, get_vioc_index(overlay_drv->rdma[layer].id));
	...
	if ((overlay_drv->rdma[layer].reg != NULL) && (overlay_drv->wmix.reg != NULL)) {
		...
		if (base1 == 0U) {
			base1 = GET_ADDR_YUV42X_spU(buffer_cfg.addr, buffer_cfg.src_width, buffer_cfg.src_height);
		}

		if (base2 == 0U) {
			if (buffer_cfg.format == (unsigned int)TCC_LCDC_IMG_FMT_444SEP) {
				base2 = (unsigned int)GET_ADDR_YUV42X_spU(base1, buffer_cfg.src_width, buffer_cfg.src_height);
			} else if (buffer_cfg.format == (unsigned int)TCC_LCDC_IMG_FMT_YUV422SP) {
				base2 = (unsigned int)GET_ADDR_YUV422_spV(base1, buffer_cfg.src_width, buffer_cfg.src_height);
			} else {
				base2 = (unsigned int)GET_ADDR_YUV420_spV(base1, buffer_cfg.src_width, buffer_cfg.src_height);
			}
		}

		if ((buffer_cfg.crop_cfg.sx + buffer_cfg.crop_cfg.width) > buffer_cfg.src_width) {
			buffer_cfg.crop_cfg.width = buffer_cfg.src_width - buffer_cfg.crop_cfg.sx;
			buffer_cfg.crop_cfg.height = buffer_cfg.src_height - buffer_cfg.crop_cfg.sy;
		}
	
		tcc_get_addr_yuv(buffer_cfg.format, buffer_cfg.addr, buffer_cfg.src_width, buffer_cfg.src_height, buffer_cfg.crop_cfg.sx,
					buffer_cfg.crop_cfg.sy, &base0, &base1, &base2);

		VIOC_RDMA_SetImageAlphaSelect(overlay_drv->rdma[layer].reg, 1);
		VIOC_RDMA_SetImageAlphaEnable(overlay_drv->rdma[layer].reg, 1);

		VIOC_RDMA_SetImageSize(overlay_drv->rdma[layer].reg, buffer_cfg.crop_cfg.width, buffer_cfg.crop_cfg.height);
		VIOC_RDMA_SetImageFormat(overlay_drv->rdma[layer].reg, buffer_cfg.format);
		VIOC_RDMA_SetImageOffset(overlay_drv->rdma[layer].reg, buffer_cfg.format, buffer_cfg.src_width);

		VIOC_RDMA_SetImageBase(overlay_drv->rdma[layer].reg, base0, base1, base2);
		VIOC_RDMA_SetImageRGBSwapMode(overlay_drv->rdma[layer].reg, 0);

		if (buffer_cfg.format >= VIOC_IMG_FMT_COMP) {
			VIOC_RDMA_SetImageY2RMode(overlay_drv->rdma[layer].reg, 2);
			VIOC_RDMA_SetImageY2REnable(overlay_drv->rdma[layer].reg, 1);
		} else {
			VIOC_RDMA_SetImageY2REnable(overlay_drv->rdma[layer].reg, 0);
		}

		if (overlay_drv->sc.reg != NULL) {
			if ((buffer_cfg.cfg.width <= 0U) || (buffer_cfg.cfg.height <= 0U)) {
			...
			if(panel.xres > buffer_cfg.cfg.sx) {
				output_width = ((panel.xres - buffer_cfg.cfg.sx) < buffer_cfg.cfg.width) ? (panel.xres - buffer_cfg.cfg.sx) : buffer_cfg.cfg.width;
			} else {
				output_width = buffer_cfg.cfg.width;
			}

			if(panel.yres > buffer_cfg.cfg.sy) {
				output_height = ((panel.yres - buffer_cfg.cfg.sy) < buffer_cfg.cfg.height) ? (panel.yres - buffer_cfg.cfg.sy) : buffer_cfg.cfg.height;
			} else {
				output_height = buffer_cfg.cfg.height;
			}

			VIOC_SC_SetDstSize(overlay_drv->sc.reg, output_width, output_height);
			VIOC_SC_SetOutSize(overlay_drv->sc.reg, output_width, output_height);
			VIOC_SC_SetOutPosition(overlay_drv->sc.reg, 0, 0);
			VIOC_CONFIG_PlugIn(overlay_drv->sc.id, overlay_drv->rdma[layer].id);
			VIOC_SC_SetUpdate(overlay_drv->sc.reg);
		}

		VIOC_WMIX_SetPosition(overlay_drv->wmix.reg, layer, buffer_cfg.cfg.sx, buffer_cfg.cfg.sy);
		VIOC_WMIX_SetUpdate(overlay_drv->wmix.reg);
...
static long tcc_overlay_ioctl(struct file *pfile, unsigned int cmd, unsigned long arg)
{
	...
	case OVERLAY_PUSH_VIDEO_BUFFER_SCALING:
		...
		break;
	case OVERLAY_SCALING_AND_PUSH_BUFFER:
		{
			overlay_scaling_buffer_t overBuffCfg;

			if ((bool)copy_from_user(
					&overBuffCfg, (overlay_scaling_buffer_t *)arg,
					sizeof(overlay_scaling_buffer_t))) {
				ret = -EFAULT;
			}

			if(ret != -EFAULT) {
				ret = scaling_and_push_buffer(overBuffCfg, overlay_drv);
			}
		}
		break;
...
static int tcc_overlay_probe(struct platform_device *pdev)
{
	...
	if (tmp_node != NULL) {
		for (i = 0; i < OVERLAY_LAYER_MAX; i++) {
			...
			if(ret < 0) {
				...
				overlay_drv->rdma[i].id = VIOC_NO_COMPONENT;
			} else {
				...
				dprintk("%s-%d :: fd_num(%d) => rdma[%d]: %d %d\n"
					, __func__, __LINE__, overlay_drv->fb_dd_num, i, index, overlay_drv->rdma[i].id);
	...
	if (tmp_node != NULL) {
		...
		if (ret < 0) {
			...
			overlay_drv->wmix.id = VIOC_NO_COMPONENT;
		} else {
	...
	if (tmp_node != NULL) {
		ret = of_property_read_u32_index(pdev->dev.of_node, "scalers", 1, &index);

		if (ret < 0) {
			overlay_drv->sc.reg = NULL;
			overlay_drv->sc.id = VIOC_NO_COMPONENT;
		} else {
	...
	if (tmp_node != NULL) {
		ret = of_property_read_u32_index(pdev->dev.of_node, "mc", 1, &index);

		if (ret < 0) {
			overlay_drv->mc.id = VIOC_NO_COMPONENT;
		} else {
			overlay_drv->mc.id = index;
			dprintk("%s-%d :: overlay%d: MC(0x%x)\n", __func__, __LINE__, overlay_drv->id, overlay_drv->mc.id);
		}
	} else {
		overlay_drv->mc.id = VIOC_NO_COMPONENT;
	}
