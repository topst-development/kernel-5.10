...
struct fb_dp_device {
	...
	unsigned int buf_idx;
	    ...
	    defined(CONFIG_TELECHIPS_FB_DPTXV14_PANEL)
	u32 lcdc_mux;
	struct videomode vm;
	#endif
};
...
static struct fb_vioc_waitq fb_waitq[FB_MAX];
...
static int fbX_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_UNBLANK:
		...
		ret = fbx_turn_on_resource(info);
		    ...
		    defined(CONFIG_TELECHIPS_FB_DPTXV14_PANEL)
		if (par->panel != NULL) {
			(void)fb_panel_set_pclk(par->panel, clk_get_rate(par->pdata.ddc_clock));
...
#if defined(CONFIG_TELECHIPS_FB_LVDS_PANEL) ||\
    defined(CONFIG_TELECHIPS_FB_DSI_PANEL) ||\
    defined(CONFIG_TELECHIPS_SIMPLE_FB_HDMI_SUPPORT) ||\
    defined(CONFIG_TELECHIPS_FB_DPTXV14_PANEL)
static int
