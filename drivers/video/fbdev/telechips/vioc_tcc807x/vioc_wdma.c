...
static struct device_node *ViocWdma_Vin_np;
static void __iomem *pWDMA_VIN_reg[VIOC_VIN_WDMA_MAX] = {0};
#endif
...
void __iomem *VIOC_WDMA_VIN_GetAddress(unsigned int vioc_id)
{
	unsigned int Num = get_vioc_index(vioc_id);
	void __iomem *ret = NULL;

	if ((Num >= VIOC_VIN_WDMA_MAX) || (pWDMA_VIN_reg[Num] == NULL)) {
		(void)pr_err("[ERR][WDMA] %s num:%d Max wdma num:%d\n",
			__func__, Num, VIOC_VIN_WDMA_MAX);
		ret = NULL;
	} else {
		ret = pWDMA_VIN_reg[Num];
	}

	return ret;
}
EXPORT_SYMBOL(VIOC_WDMA_VIN_GetAddress);
#endif
...
#if defined(CONFIG_ARCH_TCC750X)
	ViocWdma_Vin_np =
		of_find_compatible_node(NULL, NULL, "telechips,vioc_wdma_vin");
	if (ViocWdma_Vin_np == NULL) {
		/* Prevent KCS warning */
		(void)pr_info("[INF][WDMA] vioc-wdma-vin: disabled\n");
	} else {
		for (i = 0; i < VIOC_VIN_WDMA_MAX; i++) {
			pWDMA_VIN_reg[i] = (void __iomem *)of_iomap(
  				ViocWdma_Vin_np, (int)i);

			if (pWDMA_VIN_reg[i] != NULL) {
				/* Prevent KCS warning */
				(void)pr_info("[INF][WDMA] vioc-wdma-vin%d\n", i);
			}
		}
	}
#endif
