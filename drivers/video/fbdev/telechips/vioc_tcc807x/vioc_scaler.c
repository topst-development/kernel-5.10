...
void __iomem *VIOC_SC_GetAddress(unsigned int vioc_id)
{
	...
	if (num == get_vioc_index(VIOC_NO_COMPONENT)) {
		(void)pr_info("[INF][SC] %s is VIOC_NO_COMPONENT\n", __func__);
		ret = NULL;
	} else if ((num >= VIOC_SCALER_MAX) || (ViocSC_np == NULL)) {
		(void)pr_err("[ERR][SC] %s: SC num:%d, max num:%d\n", __func__, num, VIOC_SCALER_MAX);
		ret = NULL;
	} else {
		if (pScale[num] == NULL) {
			(void)pr_err("[ERR][SC] %s: SC%d.reg is NULL\n", __func__, num);
			ret = NULL;
		} else {
			ret = pScale[num];
		}
