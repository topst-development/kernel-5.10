...
static int panel_dsi_unprepare(struct fb_panel *panel)
{
	...
	dev_dbg(dsi->dev, "[DEBUG][%s] %s with %s\n",
		LOG_TAG, __func__, dsi->data->name);
	if (panel_tcc_pin_select_state(dsi->dsi_pins.p,
...
static int panel_dsi_enable(struct fb_panel *panel)
{
	...
	if (dsi->enabled == 0) {
		...
		if (dsi->blk_wait_time > 0u) {
			mdelay(dsi->blk_wait_time);
		}
		#if defined(CONFIG_TELECHIPS_FB_CTRL_BACKLIGHT)
		if (dsi->backlight != NULL) {
			dsi->backlight->props.state &= ~BL_CORE_FBBLANK;
...
static int panel_dsi_parse_pins(struct panel_dsi *dsi)
{
	...
	if (ret == 0) {
		...
		if (np != NULL) {
			...
			dev_info(dsi->dev, "[INFO][%s] %s: Use fb pinctrl backlight\n",
				 LOG_TAG, __func__);
		}
#else
		 dev_info(dsi->dev, "[INFO][%s] %s: Use gpio pinctrl backlight\n",
		  LOG_TAG, __func__);
#endif
	}
...
static int panel_dsi_parse_dt(struct platform_device *pDev, struct panel_dsi *dsi)
{
	...
	if (of_property_read_u32(dn, "blk-wait-time", &blk_wait_time) == 0u) {
		if (blk_wait_time > 0u) {
			dsi->blk_wait_time = blk_wait_time;
		}
	}
...
static int panel_dsi_probe(struct platform_device *pdev)
{
	...
	if (ret == 0) {
		...
		dev_info(dsi->dev, "[INFO][%s] %s with [%s], enabled : %d\n",
