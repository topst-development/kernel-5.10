...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	/* boot optimization
	 * reduce init time when touch IC probe
	 */
	usleep_range(200, 300);		/*  T3: > 100us (2ms)*/
	gpio_direction_output(ts->pdata->rst_gpio, 1);

	usleep_range(5000, 5500);		/*  T4: > 5ms */
#else
...
#ifndef CONFIG_ARCH_TELECHIPS
	/*
	 * Telechips doesn't run this code.
	 * We keep rst_gpio at output high for the stability of
	 * the signal level.
	 */
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	/*
	 * TCS: SDCL-1457
	 * If STYLUS is set, ID_INPUT_TABLET is set instead of ID_INPUT_TOUCHSCREEN.
	 * For ID_INPUT_TOUCHSCREEN, telechips doesn't set STYLUS.
	 */
	...
	/*
	 * TCS: SDCA-1309
	 * Android uses names to detect touch devices.
	 * So we divide names for Android.
	 */
	if (ts->client->addr == 0x4b) {
		ts->input_dev->name = "goodix-ts-0";
	} else if (ts->client->addr == 0x4d) {
		ts->input_dev->name = "goodix-ts-1";
	} else {
...
#ifdef CONFIG_ARCH_TELECHIPS
static int gtp_device_match_of_node(struct device *dev, const void *np)
{
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_ARCH_TELECHIPS
	/*
	 * Telechips EVB uses touch deivce through serdes.
	 * So, the touch driver should be probed later than the serdes driver.
	 */
	const struct device_node *np;
	struct device_node *tsc_serdes_np;
	struct device *tsc_serdes_dev = NULL;

	np = client->dev.of_node;
	tsc_serdes_np = of_parse_phandle(np, "tsc-serdes-handle", 0);
	if (tsc_serdes_np == NULL) {
		ret = -ENODEV;
	} else {
		ret = 0;
	}
	...
	if (ret == 0) {
		tsc_serdes_dev = bus_find_device(&i2c_bus_type,
						NULL,
						tsc_serdes_np,
						gtp_device_match_of_node);
		if (tsc_serdes_dev == NULL) {
			ret = -EPROBE_DEFER;
		}
	}

	of_node_put(tsc_serdes_np);

	if (ret == -EPROBE_DEFER) {
		return ret;
	} else if (ret == -ENODEV) {
		dev_warn(&client->dev,
			"failed to get serdes device\n");
	} else {
		/* In normal cases, no action is taken */
	}
#endif
...
#ifdef CONFIG_ARCH_TELECHIPS
	ts->tsc_serdes_link = NULL;

	if (tsc_serdes_dev != NULL) {
		ts->tsc_serdes_link = device_link_add(&client->dev,
						tsc_serdes_dev,
						(u32)DL_FLAG_STATELESS);
		if (ts->tsc_serdes_link == NULL) {
			dev_warn(&client->dev,
				"failed to register device link with tsc_serdes driver\n");
		} else {
			dev_info(&client->dev,
				"Added device link with tsc_serdes driver\n");
...
#ifdef CONFIG_ARCH_TELECHIPS
	if (ts->tsc_serdes_link != NULL) {
		device_link_del(ts->tsc_serdes_link);
	}
#endif
...
#ifdef CONFIG_ARCH_TELECHIPS
	/*
	 * If CONFIG_FB and CONFIG_HAS_EARLYSUSPEND is not defined,
	 * this function will return an uninitialized value.
	 * So we initialize 'ret' to zero.
	 */
	int ret = 0;
#else
	int ret;
