...
#ifdef CONFIG_ARCH_TELECHIPS
	struct device_link *tsc_serdes_link;
#endif
};
...
#ifdef CONFIG_ARCH_TELECHIPS
	if (data->client->addr == (u16)0x4b) {
		input_dev->name = "Atmel maXTouch Touchscreen 0";
	} else if (data->client->addr == (u16)0x4d) {
		input_dev->name = "Atmel maXTouch Touchscreen 1";
	} else if (data->client->addr == (u16)0x4f) {
		input_dev->name = "Atmel maXTouch Touchscreen 2";
	} else {
...
#ifdef CONFIG_ARCH_TELECHIPS
static int atmel_device_match_of_node(struct device *dev, const void *np)
{
...
#ifdef CONFIG_ARCH_TELECHIPS
	data->tsc_serdes_link = NULL;

	if (tsc_serdes_dev != NULL) {
		data->tsc_serdes_link = device_link_add(&client->dev,
						tsc_serdes_dev,
						(u32)DL_FLAG_STATELESS);
		if (data->tsc_serdes_link == NULL) {
			dev_warn(&client->dev,
...
#ifdef CONFIG_ARCH_TELECHIPS
	if (data->tsc_serdes_link != NULL) {
		device_link_del(data->tsc_serdes_link);
	}
#endif
...
static int __maybe_unused mxt_resume(struct device *dev)
{
	...
	if (data->reset_gpio) {
		ret = gpiod_direction_output(data->reset_gpio, 0);
		if (ret != 0) {
			return ret;
		}
	}

	for (i = 0; i < input_dev->mt->num_slots; i++) {
