...
struct device_attribute dev_attr_ovp;
...
struct tccmisc_data_t {
	...
	pid_t process_pid;
	unsigned int nr_wmix;
	unsigned int old_ovp;
	unsigned int new_ovp;
};
...
int tccmisc_ovp(struct tccmisc_data_t *pdata)
{
		int wmix_type;
		unsigned int get_ovp;
		void __iomem *wmix_reg;
		int ret = 0;

		/*
		 * check nr_wmix and ovp value
		 */
		wmix_type = VIOC_WMIX_GetMixerType(pdata->ovp_t.nr_wmix);

		if (wmix_type == VIOC_WMIX_TYPE_4TO2) {
			if (pdata->ovp_t.dir == 1 && pdata->ovp_t.ovp > 29) {
				pr_err("%s: error IOCTL_TCCMISC_OVP WMIX%d OVP(%d) is invalid\n", __func__, pdata->ovp_t.nr_wmix, pdata->ovp_t.ovp);
				ret = -EINVAL;
				goto exit;
			}
		} else if (wmix_type == VIOC_WMIX_TYPE_2TO2) {
			if (pdata->ovp_t.dir == 1 && pdata->ovp_t.ovp > 1) {
				pr_err("%s: error IOCTL_TCCMISC_OVP WMIX%d OVP(%d) is invalid\n", __func__, pdata->ovp_t.nr_wmix, pdata->ovp_t.ovp);
				ret = -EINVAL;
				goto exit;
			}
		} else {
			pr_err("%s: error IOCTL_TCCMISC_OVP WMIX%d is invalid\n", __func__, pdata->ovp_t.ovp);
			ret = -EINVAL;
			goto exit;
		}
		 ...
		 * Get & Set OVP
		 */
		wmix_reg = VIOC_WMIX_GetAddress(pdata->ovp_t.nr_wmix);

		if (pdata->ovp_t.dir == 0) {
			/* Get OVP */
			VIOC_WMIX_GetOverlayPriority(wmix_reg, &get_ovp);
			pr_info("IOCTL_TCCMISC_OVP: Get OVP %d in WMIX%d\n", get_ovp, pdata->ovp_t.nr_wmix);
		} else if (pdata->ovp_t.dir == 1) {
			/* Set OVP */
			VIOC_WMIX_GetOverlayPriority(wmix_reg, &pdata->old_ovp);
			VIOC_WMIX_SetOverlayPriority(wmix_reg, pdata->ovp_t.ovp);
			VIOC_WMIX_GetOverlayPriority(wmix_reg, &get_ovp);

			/* Save changed info */
			strcpy(pdata->process_comm, current->comm);
			pdata->process_pid = current->pid;
			pdata->new_ovp = get_ovp;
			pdata->nr_wmix = pdata->ovp_t.nr_wmix;

			pr_info("IOCTL_TCCMISC_OVP: Set OVP %d, Change OVP %d -> %d in WMIX%d by %s(pid %d)\n",
					pdata->ovp_t.ovp, pdata->old_ovp, pdata->new_ovp, pdata->ovp_t.nr_wmix, pdata->process_comm, pdata->process_pid);
		} else {
			pr_err("%s: error IOCTL_TCCMISC_OVP dir(%d) is invalid\n", __func__, pdata->ovp_t.dir);
			ret = -EINVAL;
			goto exit;
		}
	...
	unsigned long arg)
{
	...
	switch (cmd) {
	case IOCTL_TCCMISC_OVP:
	{
		if (copy_from_user(&pdata->ovp_t, (struct tccmisc_ovp_t *)arg, sizeof(struct tccmisc_ovp_t))) {
			pr_err("%s: error IOCTL_TCCMISC_OVP copy_from_user\n", __func__);
			ret = -EFAULT;
			goto exit;
		}

		ret = tccmisc_ovp(pdata);

		if (copy_to_user((struct tccmisc_ovp_t *)arg, &pdata->ovp_t, sizeof(struct tccmisc_user_t))) {
...
static ssize_t ovp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "Last change OVP %d->%d in WMIX%d by %s(pid %d)\n",
			pdata->old_ovp, pdata->new_ovp, pdata->nr_wmix,
			pdata->process_comm, pdata->process_pid);
}
DEVICE_ATTR_RO(ovp);
...
static int tccmisc_drv_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_ovp);
...
static int tccmisc_drv_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_ovp);
