...
static int tcc_scaler_run(const struct scaler_drv_type *scaler)
{
	...
	if (scaler->info->mapConv_info.m_CompressedY[0] != 0) {
		if (scaler->mc.id == VIOC_NO_COMPONENT) {
			...
			tca_map_convter_onoff(scaler->mc.id, 1U, 0U);
...
#endif
	{
		if (pSC_RDMABase == NULL) {
			(void)pr_err("[ERR][SCALER] %s(): scaler%d rdma reg empty.\n",__func__,get_vioc_index(scaler->sc.id));
			ret = -EINVAL;
		} else {
			...
			if ((scaler->info->src_fmt > VIOC_IMG_FMT_COMP) && (scaler->info->dest_fmt < VIOC_IMG_FMT_COMP)) {
				y2r = 1U;
				VIOC_RDMA_SetImageY2RMode(pSC_RDMABase,2U);
			} else {
				y2r = 0U;
			}
			...
			if (scaler->info->src_fmt < VIOC_IMG_FMT_COMP) {
				...
				VIOC_RDMA_SetImageRGBSwapMode(pSC_RDMABase, 0U);
			}
	...
	if (ret == 0) {
		...
		if (pSC_SCALERBase != NULL) {
	...
	#endif
			{
				...
				VIOC_SC_SetOutSize(
					...
					(scaler->info->dest_winBottom -
					scaler->info->dest_winTop));
				VIOC_SC_SetOutPosition(pSC_SCALERBase, 0U, 0U);
				(void)VIOC_CONFIG_PlugIn(scaler->sc.id, scaler->rdma.id);
				VIOC_SC_SetUpdate(pSC_SCALERBase);
			}
		}

		if (scaler->wmix_bypass == 0U && pSC_WMIXBase != NULL) {
			VIOC_WMIX_SetSize(
				pSC_WMIXBase,
		...
		if (pSC_WDMABase == NULL) {
			(void)pr_err("[ERR][SCALER] %s(): scaler%d wdma reg empty.\n",__func__,get_vioc_index(scaler->sc.id));
			ret = -EINVAL;
		} else {
			...
			if (scaler->info->dest_fmt < VIOC_IMG_FMT_COMP) {
				...
				VIOC_WDMA_SetImageRGBSwapMode(pSC_WDMABase, 0U);
			}
			...
			if ((scaler->info->src_fmt < VIOC_IMG_FMT_COMP) && (scaler->info->dest_fmt > VIOC_IMG_FMT_COMP)) {
				r2y = 1U;
				VIOC_WDMA_SetImageR2YMode(pSC_WDMABase, 2U);
			} else {
				r2y = 0U;
			}
			VIOC_WDMA_SetImageR2YEnable(pSC_WDMABase, r2y);
