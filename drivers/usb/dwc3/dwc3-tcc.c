...
static int32_t dwc3_tcc_get_child_dr_mode(struct device *dev, const char **str)
{
	struct fwnode_handle *child = device_get_next_child_node(dev, NULL);

	return fwnode_property_read_string(child, "role-switch-default-mode", str);
}
...
static int32_t dwc3_tcc_phy_register(struct dwc3_tcc_hcd *dwc3_tcc)
{
	...
	if (of_find_property(dwc3_tcc->dev->of_node, "usb-phy", NULL) != NULL) {
		...
		if (IS_ERR(dwc3_tcc->uphy)) {
			...
			if (ret == 0) {
				...
				ret = dwc3_tcc_get_child_dr_mode(dwc3_tcc->dev, &str);
