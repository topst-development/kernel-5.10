...
#if defined(CONFIG_USB_DWC2_TCC)
		for (ep = 0; ep < hsotg->num_of_eps; ep++) {
			epctrl = dwc2_readl(hsotg, DOEPCTL(ep));
			epctrl |= DXEPCTL_SNAK;
			dwc2_writel(hsotg, epctrl, DOEPCTL(ep));
		}
#endif /* defined(CONFIG_USB_DWC2_TCC) */
	}
...
#if defined(CONFIG_USB_DWC2_TCC)
	if (dwc2_readl(hsotg, GOTGINT) & GOTGINT_MVC) {
		dev_dbg(hsotg->dev, "[DEBUG][USB] There is a change in the value of at least one ACA pin value.\n");
		dwc2_writel(hsotg, GOTGINT_MVC, GOTGINT);
	}
#endif /* CONFIG_USB_DWC2_TCC */
