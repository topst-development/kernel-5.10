...
#if defined(CONFIG_ARCH_TELECHIPS) && defined(CONFIG_PM_SLEEP)
#include <linux/pm_runtime.h>
#endif
#if defined(CONFIG_PINCTRL_TCC_SCFW)
#if (defined(CONFIG_ARCH_TCC807X) || defined(CONFIG_ARCH_TCC805X) || defined(CONFIG_ARCH_TCC803X)) \
		     ...
		     struct ktermios *old)
{
	...
	baud = uart_get_baud_rate(port, termios, old, 0,
				  port->uartclk / clkdiv);
//telechips feature
	pr_info("[INFO][PL011][UART%02d] baud_rate %d, uart_clk %d\n",
...
{
	void __iomem *base;
#if (defined(CONFIG_ARCH_TCN100X) || defined(CONFIG_ARCH_TCC807X) || defined(CONFIG_ARCH_TCC805X) || \
		defined(CONFIG_ARCH_TCC803X)) && (!defined(CONFIG_TCC803X_CA7S))
	int ret;
#endif
...
#if (defined(CONFIG_ARCH_TCN100X) || defined(CONFIG_ARCH_TCC807X) || defined(CONFIG_ARCH_TCC805X) || \
		defined(CONFIG_ARCH_TCC803X)) && (!defined(CONFIG_TCC803X_CA7S))
	ret = of_property_read_u32_index(dev->of_node, "config-reg", 0, &uap->phy_config_reg);
	if (ret){
		printk(KERN_ERR "[ERROR][PL011] no configuration address \n");
		return ret;
        }

	ret = of_property_read_u32_index(dev->of_node, "config-reg", 1, &uap->phy_config_reg_size);
	if (ret){
		printk(KERN_ERR "[ERROR][PL011] no configuration address size \n");
		return ret;
        }

	ret = of_property_read_u32_index(dev->of_node, "config-reg", 2,	&uap->channel_num);
	if (ret){
		pr_err("[ERROR][PL011] no configuration address \n");
		return ret;
	}

	uap->config_reg = ioremap(uap->phy_config_reg, uap->phy_config_reg_size);
	if(uap->config_reg == NULL)
		return -ENOMEM;

#if defined(CONFIG_PINCTRL_TCC_SCFW)
        uap->sc_np = of_parse_phandle(dev->of_node, "sc-firmware", 0);
        if(uap->sc_np == NULL) {
                printk(KERN_ERR "[ERROR][PL011] no SC node\n");
        }
#endif	/* CONFIG_PINCTRL_TCC_SCFW */
#endif	/* CONFIG_ARCH_TELECHIPS */
...
{
#if (defined(CONFIG_ARCH_TCN100X) || defined(CONFIG_ARCH_TCC807X) || defined(CONFIG_ARCH_TCC805X) || defined(CONFIG_ARCH_TCC803X)) \
	&& (!defined(CONFIG_TCC803X_CA7S)) && (!defined(CONFIG_ARCH_TCC750X))
	struct device_node *np;
	int node_detect = 0;
	long cfg_num_len, i;
	...
	sc_fw_handle = tcc_sc_fw_get_handle(uap->sc_np);
#endif
	...
	if(node_detect != 0)
	{
		...
		if(pinctrl_name == NULL)
		{
			...
			if(string_temp == NULL)
			{
				...
				offset_reg = (uap->port.line*4u);
#else
				offset_reg = (uap->port.line/4u)*0x4u;
#endif

				for(i = 0; i < uap->channel_num; i++) {
#if defined(CONFIG_ARCH_TCN100X)
					shift = 0;
					tmp_portcfg = uap->phy_config_reg;
					portcfg_val = readl_relaxed(uap->config_reg + (i*0x4u));
					tmp_val = ((portcfg_val>>shift)&0xff);
#else
					shift = (i%4)*8;
					tmp_portcfg = uap->phy_config_reg+((i/4u)*0x4u);
					portcfg_val = readl_relaxed(uap->config_reg+
							((i/4u)*0x4u));
					tmp_val = ((portcfg_val>>shift)&0xff);
#endif

					if((uap->port.line!=i)&&(tmp_val==cfg_id)) {
#if defined(CONFIG_PINCTRL_TCC_SCFW)
						if(sc_fw_handle != NULL) {
							sc_fw_handle->ops.gpio_ops->request_gpio(
									sc_fw_handle, (uintptr_t)tmp_portcfg,
									shift, 8u, 0x3F);
						} else
#endif
						{
							...
							reg_val = 0x3F;
							writel_relaxed(
									reg_val,uap->config_reg+(i*0x4u));
#else
							reg_val = readl_relaxed(
									uap->config_reg+
									((i/4u)*0x4u));
							reg_val =
								reg_val&~((unsigned int)0xFF<<
										((i%4u)*8u));
							reg_val =
								reg_val|((unsigned int)0x3F<<
										((i%4u)*8u));
							writel_relaxed(
									reg_val,uap->config_reg+
									((i/4u)*0x4u));
#endif
						}
					}
				...
				if(sc_fw_handle == NULL) {
					...
					reg_val = readl_relaxed(uap->config_reg+offset_reg);
					reg_val = reg_val&~((unsigned int)0xFF<<((uap->port.line%4u)*8u));
					reg_val = reg_val|((unsigned int)cfg_id<<((uap->port.line%4u)*8u));

					writel_relaxed(reg_val, uap->config_reg+offset_reg);

				} else {

					sc_fw_handle->ops.gpio_ops->request_gpio(sc_fw_handle, uap->phy_config_reg+offset_reg, ((uap->port.line%4u)*8u), 8u, (unsigned int)cfg_id);
...
#if defined(CONFIG_ARCH_TCN100X)
				writel_relaxed((cfg_id & 0xFF), uap->config_reg+offset_reg);
#else
				reg_val = readl_relaxed(uap->config_reg+offset_reg);
