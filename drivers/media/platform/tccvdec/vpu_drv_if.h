...
struct vpu_drv_ops {
	void (*decode_done)(struct tcc_vpudec_ctx_t *ctx, struct tcc_vpudec_frame_t *fb);
	void (*decode_finish)(struct tcc_vpudec_ctx_t *ctx);
};
...
struct tcc_vpudec_ctx_t {
	...
	const struct vpu_drv_ops *ops;
	struct tcc_vpudec_frame_t last_decode_out;
};
