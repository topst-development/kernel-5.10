...
typedef struct vdec_t {
	...
	struct file *pfscaler_fd;

	const struct file_operations *pfdec_fd_fop;
	const struct file_operations *pfscaler_fd_fop;
...
vdec_handle_h vdec_alloc_instance()
{
    ...
    if(pInst != NULL)
    {
        ...
        pInst->pfdec_fd = NULL;
		pInst->pfscaler_fd = NULL;

		mutex_lock(&instance_mutex);

        pInst->pfdec_fd = filp_open(VPU_DECODER_DEVICE, O_RDWR, 0);
        pInst->pfscaler_fd = filp_open(SCALER_DEVICE, O_RDWR, 0);

        if (pInst->pfdec_fd < 0 || pInst->pfscaler_fd < 0)
        {
            tcvdec_err("/dev/vpu_vdec || /dev/scaler1 open error");
            ret = -1;
        }

		pInst->pfdec_fd_fop = pInst->pfdec_fd->f_op;
		pInst->pfscaler_fd_fop = pInst->pfscaler_fd->f_op;
...
void vdec_release_instance(vdec_handle_h handle)
{
    ...
    if(pInst)
    {
        ...
        if(pInst->pfscaler_fd)
        {
            if(filp_close(pInst->pfscaler_fd, NULL) < 0)
            {
                tcvdec_err("/dev/scaler1 close error");
            }
            pInst->pfscaler_fd = NULL;
        }
...
int vdec_decode(vdec_handle_h handle, struct tcc_codec_bs_t* bs, struct tcc_codec_decode_output_t* output)
{
	...
	else 
	{
		if(pInst->decode_info.output.out_info.display_status == VPU_DISP_STAT_SUCCESS) 
		{
			...
			if (vdec_scaler_process(pInst, pInst->decode_info.output, output) < 0) {
				tcvdec_err("[VDEC-%d] Scaler processing failed", pInst->vdec_id);
			}
		}
...
enum vpu_return_code vdec_drain(vdec_handle_h handle, struct tcc_codec_decode_output_t* output)
{
	...
	if(vpu_ret != VPU_RETCODE_SUCCESS) {
		...
		if(pInst->drain_info.output.out_info.display_status == VPU_DISP_STAT_SUCCESS) {
			...
			vdec_scaler_process(pInst, pInst->drain_info.output, output);
...
static int vdec_scaler_process(vdec_t *pInst, vdec_v3_decode_out_t vpu_output, struct tcc_codec_decode_output_t *output)
{
	...
	scaler_info.responsetype        = SCALER_POLLING;
	
	scaler_info.src_fmt             = SCLAER_COMPRESS_DATA;
	scaler_info.src_ImgWidth        = vpu_output.out_info.decoded_width - vpu_output.out_info.decoded_crop.left - vpu_output.out_info.decoded_crop.right;
	scaler_info.src_ImgHeight       = vpu_output.out_info.decoded_height - vpu_output.out_info.decoded_crop.bottom - vpu_output.out_info.decoded_crop.top;

	if(pInst->init_info.input.codec_id == VCODEC_ID_AVC) {
		scaler_info.src_fmt         = SCALER_YUV420_inter;
		scaler_info.src_Yaddr		= (unsigned long)vpu_output.display_out[VPU_PA][VPU_COMP_Y];
		scaler_info.src_Uaddr		= (unsigned long)vpu_output.display_out[VPU_PA][VPU_COMP_U];
		scaler_info.src_Vaddr		= (unsigned long)vpu_output.display_out[VPU_PA][VPU_COMP_V];
	}

	scaler_info.src_winLeft         = vpu_output.out_info.decoded_crop.left;
	scaler_info.src_winTop          = vpu_output.out_info.decoded_crop.top;
	scaler_info.src_winRight        = vpu_output.out_info.decoded_crop.right + scaler_info.src_ImgWidth;
	scaler_info.src_winBottom       = vpu_output.out_info.decoded_crop.bottom + scaler_info.src_ImgHeight;

	scaler_info.dest_Yaddr 			= (unsigned long)output->fb.pa[VPU_FRAMEBUFFER_Y];
	scaler_info.dest_Uaddr 			= (unsigned long)output->fb.pa[VPU_FRAMEBUFFER_Y];
	...
	scaler_info.dest_winLeft        = vpu_output.out_info.display_crop.left;
	scaler_info.dest_winTop         = vpu_output.out_info.display_crop.top;
	scaler_info.dest_winRight       = vpu_output.out_info.display_crop.right + scaler_info.dest_ImgWidth;
	scaler_info.dest_winBottom      = vpu_output.out_info.display_crop.bottom + scaler_info.dest_ImgHeight;

	/*
	tcvdec_info("[VDEC-%d] Source Crop Info %d:%d:%d:%d", 
		pInst->vdec_id, 
		vpu_output.out_info.decoded_crop.left, 
		vpu_output.out_info.decoded_crop.top, 
		vpu_output.out_info.decoded_crop.right, 
		vpu_output.out_info.decoded_crop.bottom);

	tcvdec_info("[VDEC-%d] Destination Crop Info %d:%d:%d:%d", 
		pInst->vdec_id, 
		vpu_output.out_info.display_crop.left, 
		vpu_output.out_info.display_crop.top, 
		vpu_output.out_info.display_crop.right,
		vpu_output.out_info.display_crop.bottom);

	tcvdec_info("[VDEC-%d] Destination resolution = %dx%d", pInst->vdec_id, scaler_info.dest_ImgWidth, scaler_info.dest_ImgHeight);
	tcvdec_info("[VDEC-%d] DMA_align resolution = %dx%d", pInst->vdec_id, vpu_output.out_info.dma_buf_align_width, vpu_output.out_info.dma_buf_align_height);
	...
	*/
	...
	if(pInst->init_info.input.codec_id == VCODEC_ID_HEVC || pInst->init_info.input.codec_id == VCODEC_ID_VP9) {
		...
		scaler_info.mapConv_info.m_CompressedCb[VPU_KVA] 	= vpu_output.out_info.disp_map_conv_info.compressed_cb[VPU_KVA];
		scaler_info.mapConv_info.m_FbcYOffsetAddr[VPU_PA] 	= vpu_output.out_info.disp_map_conv_info.fbc_y_offset_addr[VPU_PA];
		scaler_info.mapConv_info.m_FbcYOffsetAddr[VPU_KVA] 	= vpu_output.out_info.disp_map_conv_info.fbc_y_offset_addr[VPU_KVA];
		scaler_info.mapConv_info.m_FbcCOffsetAddr[VPU_PA] 	= vpu_output.out_info.disp_map_conv_info.fbc_c_offset_addr[VPU_PA];
		scaler_info.mapConv_info.m_FbcCOffsetAddr[VPU_KVA] 	= vpu_output.out_info.disp_map_conv_info.fbc_c_offset_addr[VPU_KVA];

		scaler_info.mapConv_info.m_uiCompressionTableLumaSize  		= vpu_output.out_info.disp_map_conv_info.compression_table_luma_size;
		scaler_info.mapConv_info.m_uiCompressionTableChromaSize 	= vpu_output.out_info.disp_map_conv_info.compression_table_chroma_size;
		scaler_info.mapConv_info.m_uiLumaStride  					= vpu_output.out_info.disp_map_conv_info.luma_stride;
		scaler_info.mapConv_info.m_uiChromaStride 					= vpu_output.out_info.disp_map_conv_info.chroma_stride;
		scaler_info.mapConv_info.m_uiLumaBitDepth 					= vpu_output.out_info.disp_map_conv_info.luma_bit_depth;
		scaler_info.mapConv_info.m_uiChromaBitDepth 				= vpu_output.out_info.disp_map_conv_info.chroma_bit_depth;
		scaler_info.mapConv_info.m_uiFrameEndian 					= vpu_output.out_info.disp_map_conv_info.frame_endian;

		for (i = 0; i < 17; i++) {
			scaler_info.mapConv_info.m_Reserved[i] = vpu_output.out_info.disp_map_conv_info.reserved[i];
		}
	}

	if (pInst->pfscaler_fd->f_op->unlocked_ioctl(pInst->pfscaler_fd, TCC_SCALER_IOCTRL_KERENL, (unsigned long)&scaler_info) < 0) {
		tcvdec_err("[VDEC-%d] scaler ioctl(0x%x) error!!", pInst->vdec_id, TCC_SCALER_IOCTRL_KERENL);
		return -EIO;
	}
