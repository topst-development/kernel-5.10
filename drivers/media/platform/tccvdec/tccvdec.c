...
static void tcc_vdec_escape_workqueue(struct tcc_vpudec_ctx_t *ctx);
...
/* vpu interface callbacks */
static void vdec_buf_done(struct tcc_vpudec_ctx_t *ctx, struct tcc_vpudec_frame_t *fb);
...
static const struct vpu_drv_ops tcc_vdec_vpu_ops = {
	.decode_done = vdec_buf_done,
	.decode_finish = tcc_vdec_escape_workqueue,
};
...
static void vdec_buf_done(struct tcc_vpudec_ctx_t *ctx, struct tcc_vpudec_frame_t *fb)
{
	...
	struct v4l2_m2m_buffer *m2m_buf = NULL;
	struct vb2_buffer *dst_vb;
	struct tcc_vdec_buffer *tdst_buf;
	struct tcc_vdec_ctx *vdec_ctx;
	struct v4l2_event ev;
	bool is_found = false;

    if (ctx == NULL || fb == NULL) {
        tcvdec_err("Invalid parameter: ctx=%p, fb=%p\n", ctx, fb);
        return;
    }

    vdec_ctx = (struct tcc_vdec_ctx *)ctx->priv;
    if (vdec_ctx == NULL) {
        tcvdec_err(" vdec_ctx is NULL\n");
        return;
    }

    if (!vdec_ctx->streaming) {
        tcvdec_err("Streaming has been stopped, ignoring buffer\n");
        return;
    }

	v4l2_m2m_for_each_dst_buf(vdec_ctx->fh.m2m_ctx, m2m_buf) {
		dst_buf = &m2m_buf->vb;

		if(dst_buf != NULL) {
			tdst_buf = to_tcvdec_buffer(dst_buf);
			if(tdst_buf->planes[0].phy_addr == fb->fb.pa[0]) {
				is_found = true;
				break;
			}
		}
	...
	if((is_found == false)) {
		tcvdec_err(" Not Found 0x%llx v4l2 buffer for decoded frame", (u64)fb->fb.pa[0]);
		return;
	}

	dst_vb = &dst_buf->vb2_buf;

	if(vdec_ctx->fmt_cap->num_planes == 1) {
		vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);			
		dst_vb->planes[0].data_offset = 0;
	} else if(vdec_ctx->fmt_cap->num_planes == 2) {
		vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);
		dst_vb->planes[0].data_offset = 0;
		vb2_set_plane_payload(dst_vb, 1, tdst_buf->planes[1].size);
		dst_vb->planes[1].data_offset = 0;
	} else if(vdec_ctx->fmt_cap->num_planes == 3) {
		vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);
		dst_vb->planes[0].data_offset = 0;
		vb2_set_plane_payload(dst_vb, 1, tdst_buf->planes[1].size);
		dst_vb->planes[1].data_offset = 0;
		vb2_set_plane_payload(dst_vb, 2, tdst_buf->planes[2].size);
		dst_vb->planes[2].data_offset = 0;
	}

	tdst_buf->vpu_idx = fb->displayIndex;
	tdst_buf->used = true;
	dst_buf->vb2_buf.timestamp = fb->timestamp;

	++vdec_ctx->used_buf_cnt;
	++vdec_ctx->output_frame_buf_cnt;
#ifdef DUMP_Y_FRAME
	dump_frame(vdec_ctx, dst_buf);
#endif
	v4l2_m2m_dst_buf_remove_by_buf(vdec_ctx->fh.m2m_ctx, dst_buf);
	v4l2_m2m_buf_done(dst_buf, VB2_BUF_STATE_DONE);
	tcvdec_dbg("Dequeue capture buffer: 0x%llx", (u64)tdst_buf->planes[0].phy_addr);
	if (dst_buf->flags & V4L2_BUF_FLAG_LAST) {
		tcvdec_err("Detect V4L2_BUF_FLAG_LAST");
		ev.type = V4L2_EVENT_EOS;
		v4l2_event_queue_fh(&vdec_ctx->fh, &ev);
	}
}
	...
	struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		pfmt[0].bytesperline = ALIGN(pixmp->width, ctx->align_width);
		pfmt[1].sizeimage = tcc_vdec_get_framesize_raw(ctx, pixmp->pixelformat, pixmp->width, pixmp->height, 1, 2);
		pfmt[1].bytesperline = ALIGN(pixmp->width, ctx->align_width);
	} else {
...
static int tcc_vdec_s_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		ctx->input_buf_size = pixmp->plane_fmt[0].sizeimage;
	}
...
*/
		...
		if(ctx->state == VPU_STATE_FREE) {
			ret = tccvpudec_if_init(ctx->vpu_drv, codec, ctx, &tcc_vdec_vpu_ops);
			if (ret)
				return ret;
...
static int tcc_vdec_decoder_cmd(struct file *file, void *fh, struct v4l2_decoder_cmd *cmd)
{
	...
	if (cmd->cmd == V4L2_DEC_CMD_STOP) {
		if(ctx->vpu_drv != NULL) {
			...
			if (!ctx->user_framebuffer_cnt) {
				if (!src_buf) {
					while(tccvpudec_if_drain(ctx->vpu_drv) >= 0);
				}
			}
			ctx->stopping = true;
...
static uint32_t get_framesize_raw_nv12(struct tcc_vdec_ctx *ctx, uint32_t width, uint32_t height, uint32_t plane_idx, uint32_t num_planes)
{
	uint32_t ret = 0;
	uint32_t y_stride, uv_stride, y_plane;
	uint32_t y_sclines, uv_sclines, uv_plane;
	uint32_t size;

	y_stride = ALIGN(width, ctx->align_width);
	uv_stride = ALIGN(width, ctx->align_width);
	y_sclines = ALIGN(height, ctx->align_width);
	uv_sclines = ALIGN(((height + 1) >> 1), ctx->align_width);
	...
	if(num_planes == 1) {
		ret = ALIGN(size, SZ_4K);
	} else if(num_planes == 2) {
		if(plane_idx == 0) {
			ret = ALIGN(y_plane, SZ_4K);
		} else if(plane_idx == 1) {
			ret = ALIGN(uv_plane, SZ_4K);
		} else {
			tcvdec_err("Invalid parameter. num_planes=%d <= %d ", num_planes, plane_idx);
			ret = 0;
		}
	} else {
		tcvdec_err("Invalid parameter. num_planes=%d >= 2 ", num_planes);
		ret = 0;
	}
	return ret;
}
	...
	uint32_t height, uint32_t plane_idx, uint32_t planes)
{
	switch (v4l2_fmt)
	{
		...
		case V4L2_PIX_FMT_NV21:
			return get_framesize_raw_nv12(ctx, width, height, plane_idx, planes);
		default:
			return 0;
	}
}
...
static int tcc_vdec_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	struct tcc_vdec_buffer *fb = NULL;
	int i;
	int fb_idx = 0;
	int fb_cnt = 0;
	struct tcc_codec_fb_t *fbs = NULL;
#ifdef DUMP_Y_FRAME
	mm_segment_t old_fs;
#endif
	if((ctx == NULL) || (q == NULL)) {
	...
	tcvdec_step("Count : %d", count);
	ctx->streaming = true;

	if(q->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE && ctx->state < VPU_STATE_FRAMEBUFFER) {
		fb_cnt = ctx->num_output_bufs;

		if(fb_cnt == 0) {
			return -EFAULT;
		}
		fbs = kmalloc(fb_cnt * sizeof(struct tcc_codec_fb_t), GFP_KERNEL);
		if(!fbs) {
			tcvdec_err("Failed to allocate memory for framebuffre info. cnt=%d", fb_cnt);
			return -ENOMEM;
		}
		memset(fbs, 0x0, fb_cnt * sizeof(struct tcc_codec_fb_t));

		list_for_each_entry(fb, &ctx->framebuffer_list, framebuffer_item) {
			for(i = 0; i < fb->m2m_buf.vb.vb2_buf.num_planes; i++) {
				fbs[fb_idx].dma_addr[i] = fb->planes[i].dma_addr;
				fbs[fb_idx].pa[i] = fb->planes[i].phy_addr;
				fbs[fb_idx].size[i] = fb->planes[i].size;
			}
			fbs[fb_idx].pa[2] = fbs[fb_idx].pa[1] + fbs[fb_idx].size[1] / 2;
			fb_idx++;
		}

		if((fb_idx== 0) || (fb_cnt == 0) || (fb_idx != fb_cnt)) {
			tcvdec_err(" Frame buffer count is wired. %d vs %d, cap_buf_count=%d num_output_bufs=%d", fb_idx, fb_cnt, ctx->cap_buf_count, ctx->num_output_bufs);
			ret = -EINVAL;
			goto err_buf_free;
		}
		if(0 != tccvpudec_if_register_fb(ctx->vpu_drv, fbs, fb_cnt)) {
			ret = -EFAULT;
			goto err_buf_free;
		}
...
static void tcc_vb2_buf_queue(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		 ...
		 if(ctx->stopping) {
			...
			if (tccvpudec_if_drain(ctx->vpu_drv) < 0)
			{
...
static int tcc_vdec_vb2_buf_init(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		tcvdec_step("dma : 0x%llx phy : 0x%llx size : %d", (u64)buf->planes[0].dma_addr, (u64)buf->planes[0].phy_addr, buf->planes[0].size);
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
		*num_planes = ctx->fmt_cap->num_planes;
		...
		for(i = 0; i < *num_planes; i++) {
			...
			if(sizes[i] > 0) {
				ctx->output_buf_size += sizes[i];
			}
		}
		*num_buffers = ctx->min_framebuffer_cnt + ctx->user_framebuffer_cnt;//TCCVDEC_MAX_CAPTURE_BUFFER_NUM;
...
static void tcc_vdec_escape_workqueue(struct tcc_vpudec_ctx_t *ctx)
{
	...
	tcvdec_step("escape workqueue");

	if (vdec_ctx->streaming) {
		vdec_ctx->streaming = false;
		vdec_ctx->stopping = true;


		while ((src_buf = v4l2_m2m_src_buf_remove(vdec_ctx->fh.m2m_ctx))) {
			v4l2_m2m_buf_done(src_buf, VB2_BUF_STATE_ERROR);
		}
		while ((dst_buf = v4l2_m2m_dst_buf_remove(vdec_ctx->fh.m2m_ctx))) {
			v4l2_m2m_buf_done(dst_buf, VB2_BUF_STATE_ERROR);
		}
	}
/* FixMe: When a SPECOUT or other unrecoverable condition occurs,
 *        there is currently no proper exit mechanism for the decode workqueue (tcvdec_worker).
 *
 *        Even after v4l2_m2m_job_finish() is called upon detecting SPECOUT (ret=9),
 *        the decode_work remains active and is likely to be rescheduled,
 *        causing repeated entries into the workqueue and unintended frame drops.
 *
 *        Currently, this function only flushes the buffer queues with VB2_BUF_STATE_ERROR
 *        and signals V4L2_EVENT_EOS, but the workqueue thread does not fully terminate.
 */
  	v4l2_event_queue_fh(&vdec_ctx->fh, &ev);
}
...
static void tcvdec_worker(struct work_struct *work)
{
	...
	struct tcc_vdec_buffer *tdst_buf;
	struct tcc_codec_bs_t input_bs;
	struct tcc_vpudec_frame_t frame;
	...
	if (v4l2_m2m_num_dst_bufs_ready(ctx->fh.m2m_ctx) < (ctx->num_output_bufs) - 2){
		msleep(TCCVDEC_WORKQUEUE_THROTTLE_MS);
		v4l2_m2m_job_finish(ctx->m2m_dev, ctx->fh.m2m_ctx);
		return;
	}
	...
	mutex_lock(&ctx->lock);
	ret = tccvpudec_if_decode(ctx->vpu_drv, &input_bs, &frame);
	mutex_unlock(&ctx->lock);
	...
	if (frame.status & TCC_VIDEO_CODEC_STATUS_BUF_FULL) {
		mutex_lock(&ctx->lock);
		clear_used_buffers(ctx);
		mutex_unlock(&ctx->lock);
	} else if ((frame.status & TCC_VIDEO_CODEC_STATUS_DECODED)){
		if (frame.decodedIndex >= 0) {
			tccvdecqueue_enqueue(&ctx->timestamp_queue, input_bs.timestamp, frame.decodedIndex, NULL);
		}
		if(ctx->streaming) {
			++ctx->output_bs_buf_cnt;
			v4l2_m2m_src_buf_remove_by_buf(ctx->fh.m2m_ctx, src_buf);
	...
	if (frame.status & TCC_VIDEO_CODEC_STATUS_DISPLAYABLE) {
		struct tccvdecqueue_entry *ts_entry;

		v4l2_m2m_for_each_dst_buf(ctx->fh.m2m_ctx, m2m_buf) {
			struct vb2_v4l2_buffer *tmp_buf = NULL;
			struct tcc_vdec_buffer *ttmp_buf = NULL;
			tmp_buf = &m2m_buf->vb;
	
			if(tmp_buf != NULL) {
				ttmp_buf = to_tcvdec_buffer(tmp_buf);
				if(ttmp_buf->planes[0].phy_addr == frame.fb.pa[0]) {
					tdst_buf = ttmp_buf;
					dst_buf = tmp_buf;
					is_found = true;
					break;
		...
		if(is_found == false) {
			tcvdec_step("Not Found 0x%llx v4l2 buffer for decoded frame, repeat work queue", (u64)frame.fb.pa[0]);
			//tccvpudec_if_buf_clear(ctx->vpu_drv, frame.displayIndex);
	   		v4l2_m2m_job_finish(ctx->m2m_dev, ctx->fh.m2m_ctx);
			return;
		}
		...
		if (ts_entry) {
			frame.timestamp = ts_entry->timestamp;
			kfree(ts_entry);
		} else {
			frame.timestamp = input_bs.timestamp;
		}
		tdst_buf->vpu_idx = frame.displayIndex;
		tdst_buf->used = true;
		dst_buf->vb2_buf.timestamp = frame.timestamp;
		dst_buf->flags |= src_buf->flags & V4L2_BUF_FLAG_LAST;
		dst_vb = &dst_buf->vb2_buf;

		if(ctx->fmt_cap->num_planes == 1) {
			vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);			
			dst_vb->planes[0].data_offset = 0;
		} else if(ctx->fmt_cap->num_planes == 2) {
			vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);
			dst_vb->planes[0].data_offset = 0;
			vb2_set_plane_payload(dst_vb, 1, tdst_buf->planes[1].size);
			dst_vb->planes[1].data_offset = 0;
		} else if(ctx->fmt_cap->num_planes == 3) {
			vb2_set_plane_payload(dst_vb, 0, tdst_buf->planes[0].size);
			dst_vb->planes[0].data_offset = 0;
			vb2_set_plane_payload(dst_vb, 1, tdst_buf->planes[1].size);
			dst_vb->planes[1].data_offset = 0;
		...
		tcvdec_step("Dequeue capture buffer: 0x%llx (%lld) idx : %d", (u64)tdst_buf->planes[0].phy_addr, frame.timestamp, tdst_buf->vpu_idx);
...
static int m2m_queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
{
	...
	/* FIXME: [2024-04-16] [YoungJun Shim]
	 * REASON :
	 * DMA_ATTR_NO_KERNEL_MAPPING is set to avoid Virtual Address mapping, thus reducing memory usage.
	 * DMA_ATTR_NO_KERNEL_MAPPING is disable for test.
	 * Enable DMA_ATTR_NO_KERNEL_MAPPING before release.
	 * ADDITIONAL INFO: DMA_ATTR_WRITE_COMBINE is set to enhance the write speed to the framebuffer in the VPU.
	*/
	dst_vq->dma_attrs = DMA_ATTR_WRITE_COMBINE /*| DMA_ATTR_NO_KERNEL_MAPPING */;
...
static int tcc_vdec_open(struct file *file)
{
	...
	ctx->pix_format = V4L2_PIX_FMT_H264;
