...
static int32_t tcc_isp_mcu_reset(struct tcc_isp_state *state, int reset)
{
	...
	uint32_t val = 0U;
	uint32_t swrst = CAM_SWRST0_ISP_RISCV_SWRST_MASK;
	int32_t ret = 0;
	...
	if (reset == ON) {
		...
		if (tcc_isp_mcu_enable_cnt == 0U) {
			/* reset state */
			val = tcc_isp_readl(cfg_base, CAM_SWRST0);
			val |= swrst;
			tcc_isp_writel(val, cfg_base, CAM_SWRST0);

			logi(&(state->pdev->dev), "reset (cnt is %d)\n",
			     tcc_isp_mcu_enable_cnt);
		} else {
			/* skip reset state */
			logi(&(state->pdev->dev), "reset skip (cnt is %d)\n",
			     tcc_isp_mcu_enable_cnt);
		}
	} else {
		tcc_isp_mcu_enable_cnt++;
		...
		if (tcc_isp_mcu_enable_cnt == 1U) {
			...
			logi(&(state->pdev->dev), "reset release (cnt is %d)\n",
			     tcc_isp_mcu_enable_cnt);
		} else {
				  ...
				  const struct firmware *fw)
{
	...
	if (state->fw_load == 0) {
		...
		/* skip loading */
		logi(&(state->pdev->dev), "ISP MCU F/W is already loaded\n");
	}
