...
static void tcc_isp_core_set_rccb(const struct tcc_isp_state *state)
{
	...
	val = 0U;
	val |= ((ISP_RCCB_CTL_RBE_ENABLE << ISP_RCCB_CTL_RBE_SHIFT) |
...
static void tcc_isp_core_set_rccc(const struct tcc_isp_state *state)
{
	...
	val |= ((pixel_order << ISP_RCCC_CTL_RCIO_SHIFT) |
		((uint32_t)1U << ISP_RCCC_CTL_RCE_SHIFT));
...
static void tcc_isp_core_set_rgbir(const struct tcc_isp_state *state)
{
	...
	logi(&(state->pdev->dev), "input size(%d x %d), rgb-ir order(%s)\n",
	...
	val |= ((state->rgbir_order << ISP_RGBIR_CTL_RIBP_SHIFT) |
		(ISP_RGBIR_CTL_RSE_MASK | ISP_RGBIR_CTL_RWE_MASK) |
		(output_order << ISP_RGBIR_CTL_ROBP_SHIFT) |
		((uint32_t)1U << ISP_RGBIR_CTL_RE_SHIFT) |
...
static void tcc_isp_core_set_demosaic_mode(const struct tcc_isp_state *state)
{
	...
	logi(&(state->pdev->dev), "demosaic %s\n",
...
static void tcc_isp_core_set_deblank(const struct tcc_isp_state *state)
{
	...
	val &= ~(ISP_DEBLANK_ONOFF_MASK | ISP_DEBLANK_VSEL_MASK |
		 ISP_DEBLANK_IB_SEL_MASK);

	val |= ((ISP_DEBLANK_OFF << ISP_DEBLANK_ONOFF_SHIFT) |
		(ISP_DEBLANK_VS_BYPASS << ISP_DEBLANK_VSEL_SHIFT) |
		(ISP_DEBLANK_IB_12BIT_INPUT << ISP_DEBLANK_IB_SEL_SHIFT));
#if 0 /* for test CD1-8333*/
	val &= ~(ISP_DEBLANK_FRONTPORCH_CONFIGURE_MASK |
		 ISP_DEBLANK_VBLANK_CONFIGURE_MASK);
	/* TODO: why does it(set margin) need? */
	val |= ((15U << ISP_DEBLANK_FRONTPORCH_CONFIGURE_SHIFT) |
		(3U << ISP_DEBLANK_VBLANK_CONFIGURE_SHIFT));
#else
	/* TODO: why does it(set margin) need? */
	val |= ((5U << ISP_DEBLANK_FRONTPORCH_CONFIGURE_SHIFT) |
		(3U << ISP_DEBLANK_VBLANK_CONFIGURE_SHIFT));
#endif
					    ...
					    uint32_t reg, uint32_t val)
{
	...
	if (ret >= 0) {
		...
		if (state->tune_core.setting[cmd_cnt].reg == REG_ISP_CTL) {
...
static void tcc_isp_core_set_up_mode(const struct tcc_isp_state *state)
{
	 ...
	 */
	logi(&(state->pdev->dev), "up_sel0, 2(0x%x, 0x%x) is %s\n", up_sel0,
	     up_sel1,
	 ...
	 */
	logi(&(state->pdev->dev), "up_mode0(0x%x) is %s\n", up_mode0,
...
static void tcc_isp_core_set_rgbir_sync(const struct tcc_isp_state *state)
{
	...
	logi(&(state->pdev->dev), "input size(%d x %d)\n", w, h);
