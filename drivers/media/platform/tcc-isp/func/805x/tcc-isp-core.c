					      ...
					      uint32_t reg, uint32_t *val)
{
	...
	switch (reg) {
	case REG_ISP_SOFT_RESET:
		ret = true;
		logd(&(state->pdev->dev),
		     "ISP_SOFT_RESET(0x%x) of btset will be ignored\n", reg);
		break;
	case REG_ISP_MCU_CTL:
		ret = true;
		logd(&(state->pdev->dev),
		     "ISP_MCU_CTL(0x%x) of btset will be ignored\n", reg);
		break;
	case REG_ISP_HDR_CTL:
		...
		break;
	case REG_ISP_IMG_WIN_CTL:
		ret = false;
		mask = (IMG_WIN_CTL_DEBLANK_EN_ENABLE << IMG_WIN_CTL_DEBLANK_EN_SHIFT);
		*val &= ~mask;
		break;
	default:
		ret = false;
...
static void tcc_isp_core_set_up_mode(const struct tcc_isp_state *state)
{
	 ...
	 */
	logd(&(state->pdev->dev), "up_sel1, 2(0x%x, 0x%x) is %s\n", up_sel1,
	     up_sel2,
	 ...
	 * 1: group sync
	 */
	logd(&(state->pdev->dev), "up_mode2(0x%x) is %s\n", up_mode2,
...
static void tcc_isp_core_set_output(const struct tcc_isp_state *state)
{
	...
	win_ctl |=
		(IMG_WIN_CTL_DEBLANK_EN_DISABLE << IMG_WIN_CTL_DEBLANK_EN_SHIFT);
...
void tcc_isp_core_set_deblank_on(const struct tcc_isp_state *state)
{
	...
	val = tcc_isp_readl(state->isp_base, REG_ISP_IMG_WIN_CTL);

	val |= (IMG_WIN_CTL_DEBLANK_EN_ENABLE << IMG_WIN_CTL_DEBLANK_EN_SHIFT);

	tcc_isp_writel(val, state->isp_base, REG_ISP_IMG_WIN_CTL);
...
int32_t tcc_isp_core_init(const struct tcc_isp_state *state, uint32_t enable)
{
	...
	if (enable != 0U) {
		...
		if (tcc_isp_mcu_is_all_isp_stopped() == TCC_ISP_STOPPED) {
			...
			logd(&(state->pdev->dev), "skip isp reset\n");
		}
	}
