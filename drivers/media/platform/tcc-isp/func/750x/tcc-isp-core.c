...
static void tcc_isp_core_set_deblank(const struct tcc_isp_state *state)
{
	...
	val &= ~(ISP_DEBLANK_ONOFF_MASK |
		 ISP_DEBLANK_VSEL_MASK |
		 ISP_DEBLANK_IB_SEL_MASK |
		 ISP_DEBLANK_FRONTPORCH_CONFIGURE_MASK |
		 ISP_DEBLANK_VBLANK_CONFIGURE_MASK);

	val |= ((ISP_DEBLANK_OFF << ISP_DEBLANK_ONOFF_SHIFT) |
		(ISP_DEBLANK_VS_BYPASS << ISP_DEBLANK_VSEL_SHIFT) |
		(ISP_DEBLANK_IB_BYPASS << ISP_DEBLANK_IB_SEL_SHIFT));

	/* CD750XL-717 */
	val |= ((0xFU << ISP_DEBLANK_FRONTPORCH_CONFIGURE_SHIFT) |
...
static void tcc_isp_core_set_3dnr(const struct tcc_isp_state *state)
{
	...
	logi(&(state->pdev->dev), "patch(%d, %d/%d x %d)\n", pxs, pys, pw, ph);
