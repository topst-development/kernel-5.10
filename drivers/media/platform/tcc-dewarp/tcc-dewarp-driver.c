				 ...
				 const struct platform_device *p_pdev)
{
	...
	ret_call = tcc_dewarp_v4l2_init_queue(&vstream->queue);
	if (ret_call != 0) {
		/* fail */
...
static ssize_t dewarping_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	const struct tcc_dewarp_stream *vstream = NULL;
...
static ssize_t dewarping_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	struct tcc_dewarp_stream *vstream = NULL;
	...
	if (ret == 0) {
		/* update */
		vstream->dewarping = val;
	}

	return (ret > 0) ? (ssize_t)ret : clamp_t(s64, count, 0, INT_MAX);
}

static DEVICE_ATTR_RW(dewarping);

static struct attribute *tcc_dewarp_attrs[] = {
	&dev_attr_vs_stabilization.attr,
	&dev_attr_timestamp.attr,
	&dev_attr_dewarping.attr,
	NULL,
};
...
#if IS_ENABLED(CONFIG_VIDEO_TCC_SVDW)
static int tcc_dewarp_svdw_start_streaming(void *p_data)
{
	...
	atomic_set(&dwrp_dev->svdw_dwp_unit.svdw_mode, TCC_SVDW_USER_STITCHER);
	return 0;
}
...
static int tcc_dewarp_svdw_stop_streaming(void *p_data)
{
	...
	atomic_set(&dwrp_dev->svdw_dwp_unit.svdw_mode, TCC_SVDW_USER_NONE);
	return 0;
}
...
static int tcc_dewarp_svdw_g_subdev(void *p_data, struct v4l2_subdev **sd)
{
	struct tcc_dewarp_stream *stream;
	struct tcc_dewarp_device *dwrp_dev;
	struct video_device *vdev;
	...
	*sd = stream->tdev->tsubdev.sd;

	return (*sd == NULL) ? -EINVAL : 0;
}
...
const struct tcc_svdw_ops svdw_ops_tbl = {
	...
	.g_subdev = tcc_dewarp_svdw_g_subdev,
};
