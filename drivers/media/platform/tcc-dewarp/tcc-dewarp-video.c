...
/* this spinlock is defined in the tcc-cap-media driver */
...
int tcc_dewarp_video_s_dewarp_params(struct tcc_dewarp_stream *vstream,
						struct dewarp_params *params)
{
	...
	if(!IS_ERR_OR_NULL(params)) {
		memcpy(&odw_params->params, params, sizeof(struct dewarp_params));
	} else {
		loge(p_dev, "dewarp_params is NULL\n");
		ret = -1;
	}

	dewarp_odw_set_cam_matrix(odw_params->id, odw_params->cam_mat);
	dewarp_odw_set_forward_dist_coeff(odw_params->id, odw_params->dist_coeff_fwds);
	dewarp_odw_set_backward_dist_coeff(odw_params->id, odw_params->dist_coeff_bwds);
	dewarp_odw_set_forward_homography(odw_params->id, odw_params->homography_fwd);
	dewarp_odw_set_backward_homography(odw_params->id, odw_params->homography_bwd);
...
#if defined(CONFIG_ARCH_TCC750X)
		ret_call = fwnode_property_read_u32(fwnode, "ir-enable", &ir_enable);
		if (ret_call == 0) {
			vstream->dewarp_wrap.odw_params.in.ir_enable = ir_enable;
		}
#endif //CONFIG_ARCH_TCC750X
	}
...
static int tcc_dewarp_set_path(struct tcc_dewarp_stream *vstream)
{
	...
	if (!list_empty(&p_queue->buf_list)) {
		vstream->curr_buf = list_first_entry(
			&p_queue->buf_list, struct tcc_dewarp_buffer, entry);
	}
	spin_unlock_irqrestore(&p_queue->slock, flags);

	if (vstream->curr_buf != NULL) {
		vb = &vstream->curr_buf->buf.vb2_buf;
		(void)memset(dma_addrs, 0, sizeof(dma_addrs));

		if (vstream->dewarp_flags == 0U) {
			if (vstream->dewarping == 1U) {
				in->is_dewarp = 1U;
				in->is_bypass = 0U;
			} else {
				in->is_dewarp = 0U;
		...
		ret = tcc_dewarp_parse_params(vstream);
		if (ret < 0) {
		...
		case (unsigned int)DEWARP_COLOR_FMT_RGB:
			...
			break;
#if defined(CONFIG_ARCH_TCC750X)
		case (unsigned int)DEWARP_COLOR_FMT_RGB_IR:
			out->address[0] = dma_addrs[0];
			out->strides[0] = out->width * 4;
			if (in->ir_enable) {
				out->address[1] = dma_addrs[1];
				out->strides[1] = out->width;
			}
			break;
#endif //CONFIG_ARCH_TCC750X
		...
		dewarp_odw_set_output_address(odw_params->id, out->address,
					      in->ir_enable, in->is_dewarp);
		dewarp_odw_set_output_stride(odw_params->id, out->strides, in->is_dewarp);
...
#if defined(CONFIG_ARCH_TCC750X)
		/* This code is a workaround for the 750x chip bug.
		 * The ch1 resolution depends on the dewarp_width_0 and dewarp_height_0 settings.
		 * Therefore, the resolution of ch0 and ch1 must be the same.
		 * In this case, an update to odw ch0 should be performed.
		 */
		if (odw_params->id == 1U) {
			dewarp_odw_update(0);
		}
...
static irqreturn_t tcc_dewarp_odw_isr(int irq, void *data)
{
	...
	struct vb2_buffer *vb = NULL;
	u32 dma_addrs[MAX_PLANES];
	struct tcc_dewarp_buffer *done_buf = NULL;
	u32 buffer_count = 0U;
	struct dewarp_odw_params *odw_params = NULL;
	u32 idxpln = 0;
	...
	odw_params = &vstream->dewarp_wrap.odw_params;
	...
	/* check if the queued buffer is under 3 */
	buffer_count = list_get_entry_count(vstream, &vstream->queue.buf_list);
	if (buffer_count < 3U) {
		loge(p_dev, "Queued buffer is under 3(%d)\n", buffer_count);

		goto odw_update;
	}

	if ((vstream->curr_buf != NULL) && (vstream->next_buf == NULL)) {
		/* The first ISR after start stream */
		vstream->next_buf = list_next_entry(vstream->curr_buf, entry);
	} else {
		done_buf = vstream->curr_buf;
		vstream->curr_buf = vstream->next_buf;
		vstream->next_buf = list_next_entry(vstream->curr_buf, entry);
	}
	...
	tcc_dewarp_print_dma_addrs(vstream, vb, dma_addrs);
	for (idxpln = 0; idxpln < vb->num_planes; idxpln++) {
		odw_params->out.address[idxpln] = dma_addrs[idxpln];
	}

	tcc_dewarp_get_and_update_time(vstream, &vstream->ts_prev,
				       &vstream->ts_next, vstream->timestamp);

	if (!IS_ERR_OR_NULL(done_buf)) {
		/* fill the buffer info */
		done_buf->buf.vb2_buf.timestamp = clamp_t(
			u64, timespec64_to_ns(&vstream->ts_next), 0UL, S64_MAX);
		done_buf->buf.field = (u32)V4L2_FIELD_NONE;
		vstream->sequence = clamp(vstream->sequence + 1U, 0U, UINT_MAX);
		done_buf->buf.sequence = vstream->sequence;

		/* dequeue done_buf from the incoming buffer list */
		list_del(&done_buf->entry);

		/* queue the done_buf to the outgoing buffer list */
		vb2_buffer_done(&done_buf->buf.vb2_buf,
				VB2_BUF_STATE_DONE);
	}

	dewarp_odw_set_output_address(odw_params->id,
				      odw_params->out.address,
				      odw_params->in.ir_enable,
				      odw_params->in.is_dewarp);
...
static int tcc_dewarp_stop_stream(struct tcc_dewarp_stream *vstream)
{
	...
	odw_params->stream_enable = 0U;

	if (vstream->dewarp_flags == 0U) {
		odw_params->in.is_dewarp = 0U;
		odw_params->in.is_bypass = 0U;
	}
...
static void tcc_dewarp_init_buffer_pointers(struct tcc_dewarp_stream *vstream)
{
	struct tcc_dewarp_queue *p_queue = NULL;
	...
	if (!list_empty(&p_queue->buf_list)) {
		vstream->curr_buf = list_first_entry(&p_queue->buf_list, struct tcc_dewarp_buffer, entry);
	}
}

int tcc_dewarp_video_streamon_concurrent(struct tcc_dewarp_stream *vstream)
{
	const struct device *p_dev = NULL;
	int ret_call = 0;
	int ret = 0;
	...
	tcc_dewarp_init_buffer_pointers(vstream);
	ret_call = tcc_dewarp_request_irq(vstream);
	if (ret_call < 0) {
		loge(p_dev, "tcc_dewarp_request_irq, ret: %d\n",
			ret_call);
...
EXPORT_SYMBOL_GPL(tcc_dewarp_video_streamon_concurrent);
...
int tcc_dewarp_video_streamoff_concurrent(struct tcc_dewarp_stream *vstream)
{
	const struct device *p_dev = NULL;
	int ret_call = 0;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(tcc_dewarp_video_streamoff_concurrent);
