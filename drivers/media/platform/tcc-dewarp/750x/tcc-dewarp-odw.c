...
EXPORT_SYMBOL(dewarp_odw_get_ip_version);
...
EXPORT_SYMBOL(dewarp_odw_get_dummy_data_status);
...
EXPORT_SYMBOL(dewarp_odw_set_dummy_data);
...
void dewarp_odw_set_input_format(u32 id, u32 format, u32 ir_enable)
{
	...
	value = (__raw_readl(pODW_reg + offset) &
		 ~(ODW_IMAGE_FORMAT_IR_ENABLE_MASK) &
		 ~(ODW_IMAGE_FORMAT_INPUT_FORMAT_MASK));
	value |= (ir_enable << ODW_IMAGE_FORMAT_IR_ENABLE_SHIFT);
...
void dewarp_odw_set_output_format(u32 id, u32 format)
{
	...
	value = (__raw_readl(pODW_reg + offset) &
		 ~(ODW_IMAGE_FORMAT_OUTPUT_FORMAT_MASK));
	value |= (format << ODW_IMAGE_FORMAT_OUTPUT_FORMAT_SHIFT);
				   ...
				   u32 is_dewarp)
{
	switch (id) {
	case 0:
		__raw_writel(addresses[0], pODW_reg + ODW_OUTPUT_ADDRESS_0_0);
		__raw_writel(addresses[1], pODW_reg + ODW_OUTPUT_ADDRESS_1_0);
		__raw_writel(addresses[2], pODW_reg + ODW_OUTPUT_ADDRESS_2_0);
		break;
	case 1:
		__raw_writel(addresses[0], pODW_reg + ODW_OUTPUT_ADDRESS_0_1);
		__raw_writel(addresses[1], pODW_reg + ODW_OUTPUT_ADDRESS_1_1);
		__raw_writel(addresses[2], pODW_reg + ODW_OUTPUT_ADDRESS_2_1);
		break;
	default:
		break;
	}
...
void dewarp_odw_set_output_stride(uint id, uint strides[], u32 is_dewarp)
{
	u32 values[3] = {
		0,
	};
	...
	case 0:
		...
		values[2] |= (strides[2] << 8U);

		__raw_writel(values[0], pODW_reg + ODW_OUTPUT_STRIDE_0_0);
		__raw_writel(values[1], pODW_reg + ODW_OUTPUT_STRIDE_1_0);
		__raw_writel(values[2], pODW_reg + ODW_OUTPUT_STRIDE_2_0);
		break;
	case 1:
		values[0] |= (strides[0] << 8U);
		values[1] |= (strides[1] << 8U);

		__raw_writel(values[0], pODW_reg + ODW_OUTPUT_STRIDE_0_1);
		__raw_writel(values[1], pODW_reg + ODW_OUTPUT_STRIDE_1_1);
		break;
	default:
		break;
	}
...
EXPORT_SYMBOL_GPL(dewarp_odw_set_filter);
...
void dewarp_odw_set_background_color(u32 id, u32 color)
{
	...
	switch (id) {
	case 0:
		offset = ODW_DEFAULT_COLOR_0;
		shift = ODW_DEFAULT_COLOR_0_SHIFT;
		break;
	case 1:
		offset = ODW_DEFAULT_COLOR_1;
		shift = ODW_DEFAULT_COLOR_1_SHIFT;
		break;
	default:
		break;
	}

	__raw_writel((color << shift), pODW_reg + offset);
}
...
void dewarp_odw_set_axi_config(u32 id, u32 max_ros, u32 max_wos, u32 max_rburst,
			       u32 max_wburst)
{
	/* max_ros, max_rbust used in 807x */
	u32 value = 0;

	value |= (max_wos << ODW_AXI_CONFIG_MAX_OS_SHIFT) &
		 ODW_AXI_CONFIG_MAX_OS_MASK;

	value |= (max_wburst << ODW_AXI_CONFIG_MAX_BURST_SHIFT) &
		 ODW_AXI_CONFIG_MAX_BURST_MASK;
...
void dewarp_odw_set_dewarp_fifo_timeout_cnt(u32 id, u32 dewarp_fifo_timeout_cnt)
{
	...
	switch (id) {
	case 0:
		offset = ODW_OUTPUT_FIFO_TIMEOUT_THRES_0;
		break;
	case 1:
		offset = ODW_OUTPUT_FIFO_TIMEOUT_THRES_1;
		break;
	default:
		break;
	}
...
void dewarp_odw_set_dewarp_buffer_wait_cnt(u32 id, u32 dewarp_buffer_wait_cnt)
{
	...
	switch (id) {
	case 0:
		offset = ODW_OUTPUT_BUFFER_WAIT_TIME_0;
		break;
	case 1:
		offset = ODW_OUTPUT_BUFFER_WAIT_TIME_1;
		break;
	default:
		break;
	}
...
void dewarp_odw_set_buffer_capacity(u32 id, u32 buffer_capacity)
{
	...
	value = (buffer_capacity & ODW_BUFFER_CAPACITY_MASK);
			    ...
			    const struct firmware *fw)
{
	...
	char *regval_str = NULL;
	u32 id;

	p_dev = stream_to_device(vstream);

	id = vstream->dewarp_wrap.odw_params.id;
	...
	while ((p = strsep(&line, "\n")) != NULL) {
		...
		if (strncmp(p, "CH", 2U) == 0) {
			...
			if (id == ch) {
				if (offset_str && regval_str) {
					offset = (u32)simple_strtol(offset_str,
								    NULL, 0);
					regval = (u32)simple_strtol(regval_str,
								    NULL, 0);
