...
/*
 *      tcc-dewarp-v4l2.c  --  Telechips On-the-Fly Dewarp Path Driver
 *
...
static void tcc_dewarp_queue_return_buffers(
...
static int tcc_dewarp_queue_setup(struct vb2_queue *vq,
...
{
	const struct tcc_dewarp_queue *p_queue = NULL;
	int ret = 0;

	p_queue = (const struct tcc_dewarp_queue *)vb2_get_drv_priv(
		vb->vb2_queue);

	if (unlikely((p_queue->flags & (u32)TCC_DEWARP_QUEUE_DISCONNECTED) !=
		     0U)) {
...
static void tcc_dewarp_buf_queue(struct vb2_buffer *vb)
{
	...
	if (likely((p_queue->flags & (u32)TCC_DEWARP_QUEUE_DISCONNECTED) ==
		   0U)) {
			       ...
			       bool drop_corrupted)
{
	...
	if (ret != 0) {
		...
		queue->flags = drop_corrupted ?
				       (u32)TCC_DEWARP_QUEUE_DROP_CORRUPTED :
				       0U;
 ...
 */
static int tcc_dewarp_fop_open(struct file *pfile)
{
	struct tcc_dewarp_stream *vstream = NULL;
	struct tcc_dewarp_fh *p_handle = NULL;
	int ret = 0;

	vstream = (struct tcc_dewarp_stream *)video_drvdata(pfile);

	/* Create the device handle. */

	p_handle = kzalloc(sizeof(*p_handle), GFP_KERNEL);
	if (p_handle == NULL) {
		/* failure of allocating memory */
		ret = -ENOMEM;
	} else {
		vstream->sequence = 0;
		v4l2_fh_init(&p_handle->vfh, &vstream->tdev->vdev);
		v4l2_fh_add(&p_handle->vfh);
		p_handle->vstream = vstream;
		p_handle->state = TCC_DEWARP_HANDLE_PASSIVE;
		pfile->private_data = p_handle;

#ifdef CONFIG_VIDEO_TCC_SVDW
		atomic_dec(&tcc_svdw_dewarp_cnt);
#endif
	}
...
static int tcc_dewarp_fop_release(struct file *pfile)
{
	struct tcc_dewarp_fh *p_handle = NULL;
	struct tcc_dewarp_stream *vstream = NULL;

	p_handle = (struct tcc_dewarp_fh *)pfile->private_data;
	vstream = p_handle->vstream;

	vb2_queue_release(&vstream->queue.queue);

	/* Release the file handle. */
	v4l2_fh_del(&p_handle->vfh);
	v4l2_fh_exit(&p_handle->vfh);
	kfree(p_handle);
	pfile->private_data = NULL;

#ifdef CONFIG_VIDEO_TCC_SVDW
	atomic_inc(&tcc_svdw_dewarp_cnt);
#endif
	return 0;
}

static int tcc_dewarp_fop_mmap(struct file *pfile, struct vm_area_struct *vma)
{
	const struct tcc_dewarp_fh *p_handle = NULL;
	struct tcc_dewarp_stream *vstream = NULL;
...
static __poll_t tcc_dewarp_fop_poll(struct file *pfile, poll_table *wait)
{
	const struct tcc_dewarp_fh *p_handle = NULL;
	struct tcc_dewarp_stream *vstream = NULL;
...
const struct v4l2_file_operations tcc_dewarp_fops = {
	.owner = THIS_MODULE,
	.open = tcc_dewarp_fop_open,
	.release = tcc_dewarp_fop_release,
	.unlocked_ioctl = video_ioctl2,
	.mmap = tcc_dewarp_fop_mmap,
	.poll = tcc_dewarp_fop_poll,
};
				     ...
				     struct v4l2_capability *cap)
{
	...
	p_handle = (struct tcc_dewarp_fh *)fh;
...
static long tcc_dewarp_ioctl_default(struct file *pfile, void *fh,
				     bool valid_prio, u32 cmd, void *arg)
{
	const struct tcc_dewarp_fh *p_handle = NULL;
	struct tcc_dewarp_stream *vstream = NULL;
	int ret = 0;
	...
	case (u32)VIDIOC_CHECK_PATH_STATUS:
		...
		tcc_dewarp_video_check_path_status(vstream, (u32 *)arg);
		break;

	case (u32)VIDIOC_S_HANDOVER:

		ret = tcc_dewarp_video_s_handover(vstream, (u32 *)arg);
		break;
