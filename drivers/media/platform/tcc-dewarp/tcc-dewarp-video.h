...
enum tcc_dewarp_queue_state {
	TCC_DEWARP_QUEUE_DISCONNECTED = 1, /* 1 << 0 */
	TCC_DEWARP_QUEUE_DROP_CORRUPTED = 2, /* 1 << 1 */
};

enum tcc_dewarp_handle_state {
	TCC_DEWARP_HANDLE_PASSIVE,
	TCC_DEWARP_HANDLE_ACTIVE,
};
...
struct tcc_dewarp_stream {
	...
	struct tcc_dewarp_buffer *prev_buf;
...
struct tcc_dewarp_fh {
	struct v4l2_fh vfh;
	struct tcc_dewarp_stream *vstream;

	enum tcc_dewarp_handle_state state;
};
...
extern void
tcc_dewarp_video_check_path_status(const struct tcc_dewarp_stream *vstream,
				   u32 *status);
extern int tcc_dewarp_video_s_handover(struct tcc_dewarp_stream *vstream,
				       const u32 *flag);
#endif
