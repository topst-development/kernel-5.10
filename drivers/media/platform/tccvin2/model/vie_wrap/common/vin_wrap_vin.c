...
#define VIN_WRAP_VIN_IREQ_UPD_MASK 0x00000001U
#define VIN_WRAP_VIN_IREQ_EOF_MASK 0x00000002U
#define VIN_WRAP_VIN_IREQ_VS_MASK 0x00000004U
#define VIN_WRAP_VIN_IREQ_INVS_MASK 0x00000008U

static struct device_node *VinWrapVin_np;
static void __iomem *pVIN_reg[VIN_WRAP_VIN_MAX] = { 0 };

/* VIN UV Swap Setting
 *	- VIN_CTRL
 *		. [29] = UV swap
 */
void VIN_WRAP_VIN_SetUVSwap(void __iomem *reg, unsigned int uv_swap)
{
	u32 val;

	val = (__raw_readl(reg + VIN_CTRL) & ~(VIN_CTRL_UVS_MASK));
	val |= ((uv_swap & 0x1U) << VIN_CTRL_UVS_SHIFT);
 ...
 *		. [ 8] = pxclk_pol
 */
void VIN_WRAP_VIN_SetSyncPolarity(void __iomem *reg, unsigned int hs_active_low,
 ...
 *		. [ 1] = conv_en
 */
void VIN_WRAP_VIN_SetCtrl(void __iomem *reg, unsigned int conv_en,
 ...
 *		. [ 2] = intl_en
 */
void VIN_WRAP_VIN_SetInterlaceMode(void __iomem *reg, unsigned int intl_en,
 ...
 *		. [31] = cap_en
 */
void VIN_WRAP_VIN_SetCaptureModeEnable(void __iomem *reg, unsigned int cap_en)
{
	u32 val;
 ...
 *		. [24] = skip
 */
void VIN_WRAP_VIN_SetFrameSkipNumber(void __iomem *reg, unsigned int skip)
{
	u32 val;
 ...
 *		. [ 0] = vin_en
 */
void VIN_WRAP_VIN_SetEnable(void __iomem *reg, unsigned int vin_en)
{
	u32 val;
 ...
 *		. return [ 0]
 */
unsigned int VIN_WRAP_VIN_IsEnable(const void __iomem *reg)
{
 ...
 *		. [15: 0] = width
 */
void VIN_WRAP_VIN_SetImageSize(void __iomem *reg, unsigned int width,
			       unsigned int height)
{
	u32 val;
 ...
 */
void VIN_WRAP_VIN_SetImageOffset(void __iomem *reg, unsigned int offs_width,
 ...
 *		. [15: 0] = width
 */
void VIN_WRAP_VIN_SetImageCropSize(void __iomem *reg, unsigned int width,
				   unsigned int height)
{
	u32 val;
 ...
 *		. [15: 0] = offs_width
 */
void VIN_WRAP_VIN_SetImageCropOffset(void __iomem *reg, unsigned int offs_width,
				     unsigned int offs_height)
{
	u32 val;
 ...
 *		. [ 7: 5] = y2r_mode
 */
void VIN_WRAP_VIN_SetY2RMode(void __iomem *reg, unsigned int y2r_mode)
{
	u32 val;
 ...
 *		. [ 4] = y2r_en
 */
void VIN_WRAP_VIN_SetY2REnable(void __iomem *reg, unsigned int y2r_en)
{
	u32 val;
 ...
 */
void VIN_WRAP_VIN_SetLUT(void __iomem *reg, const unsigned int *pLUT)
{
 ...
 *		. [ 0] = lut0_en
 */
void VIN_WRAP_VIN_SetLUTEnable(void __iomem *reg, unsigned int lut0_en,
 ...
 *		. [14] = se
 */
void VIN_WRAP_VIN_SetSEEnable(void __iomem *reg, unsigned int se)
{
	u32 val;
 ...
 *		. [16] = fvs
 */
void VIN_WRAP_VIN_SetFlushBufferEnable(void __iomem *reg, unsigned int fvs)
{
	u32 val;
 ...
 *	- VIN_INT
 *		. [19:16] = mask and set
 *	Not Used, Please use the alternative functions in vin_wrap_intr.c
 */
void VIN_WRAP_VIN_SetIreqMask(void __iomem *reg, unsigned int mask,
			      unsigned int set)
{
	/*
 ...
 *	Not Used, Please use the alternative functions in vin_wrap_intr.c
 */
void VIN_WRAP_VIN_GetStatus(const void __iomem *reg, unsigned int *status)
{
	*status = __raw_readl(reg + VIN_INT) & VIN_WRAP_VIN_INT_MASK;
}
 ...
 */
void VIN_WRAP_VIN_ClearAllMonitorCounters(void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetHSyncMax(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetHSyncCounter(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetDataEnableMax(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetDataEnableCounter(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetLineCountMax(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetLineCountCounter(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetVSyncMax(const void __iomem *reg)
{
 ...
 *	* The monitoring registers are used for debugging only.
 */
unsigned int VIN_WRAP_VIN_GetVSyncCounter(const void __iomem *reg)
{
 ...
 */
void __iomem *VIN_WRAP_VIN_GetAddress(unsigned int Num)
{
	void __iomem *ret = NULL;

	Num = get_vin_wrap_index(Num);
	if ((Num >= VIN_WRAP_VIN_MAX) || (pVIN_reg[Num] == NULL)) {
 ...
 */
void vin_wrap_vin_init(void)
{
	unsigned int i = 0;

	VinWrapVin_np =
		of_find_compatible_node(NULL, NULL, "telechips,vin_wrap_vin");
	if (VinWrapVin_np == NULL) {
		/* Prevent KCS warning */
		(void)pr_info("[INF][VIN] vin-wrap-vin: disabled\n");
	} else {
		for (i = 0; i < VIN_WRAP_VIN_MAX; i++) {
			pVIN_reg[i] =
				(void __iomem *)of_iomap(VinWrapVin_np, (int)i);

			if (pVIN_reg[i] != NULL) {
				/* Prevent KCS warning */
				(void)pr_info("[INF][VIN] vin-wrap-vin%d\n", i);
			}
		}
	}
}
