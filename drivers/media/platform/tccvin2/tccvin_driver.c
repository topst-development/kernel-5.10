...
DEFINE_SPINLOCK(cam_mux_cfg_lock);
...
device_to_tccvin_device(struct device *p_dev)
{
...
device_to_tccvin_stream(struct device *p_dev)
{
	struct tccvin_device *tdev = NULL;

	tdev = device_to_tccvin_device(p_dev);
 ...
 * is released.
 */
static void tccvin_delete(struct kref *p_ref)
{
	const struct tccvin_device *tdev = NULL;
	const struct tccvin_stream *vstream = NULL;

	tdev = container_of(p_ref, struct tccvin_device, ref);
...
static void tccvin_release(struct video_device *vdev)
{
	...
	const struct tccvin_stream *vstream = NULL;
	struct tccvin_device *p_tdev = NULL;
	int ret = 0;
	...
	ret = kref_put(&p_tdev->ref, tccvin_delete);
	if (ret == 1) {
				   ...
				   const struct platform_device *p_pdev)
{
	...
	ret_call = tccvin_video_init(vstream, p_pdev->dev.of_node);
	if (ret_call < 0) {
		loge(p_dev, "tccvin_video_init, ret: %d\n", ret_call);
		ret = -1;
	}
...
static int tccvin_init_device_data(struct tccvin_device *p_tdev,
				   struct platform_device *p_pdev)
{
	...
	(void)scnprintf((char *)p_tdev->name, 32, p_pdev->name);
	logi(p_dev, "id: %d, name: %s\n", p_tdev->id, p_tdev->name);

	kref_init(&p_tdev->ref);

	vstream->tdev = p_tdev;
	ret_call = tccvin_init_stream_data(vstream, p_pdev);
	if (ret_call < 0) {
		loge(p_dev, "tccvin_init_stream_data, ret: %d\n", ret_call);
		ret = -1;
	}
...
static int
tccvin_register_video_device_to_v4l2_device(struct tccvin_device *p_tdev)
{
	const struct device *p_dev = NULL;
	const struct tccvin_stream *vstream = NULL;
...
static int tccvin_register_video_device(struct tccvin_device *p_tdev,
					const struct platform_device *p_pdev)
{
	...
	ret_call = tccvin_register_video_device_to_v4l2_device(p_tdev);
	if (ret_call != 0) {
		loge(p_dev,
		     "tccvin_register_video_device_to_v4l2_device, ret: %d\n",
		     ret_call);
		ret = ret_call;
	}
	...
	if (ret_call != 0) {
		/* fail */
		loge(p_dev, "tccvin_v4l2_init_queue, ret: %d\n", ret_call);
		ret = -1;
	}

	/* We already hold a reference to p_dev->udev. The video device will be
	...
	vdev->queue = &p_tdev->vstream.queue.queue;
	...
	p_tdev->pad.flags = MEDIA_PAD_FL_SINK;
	ret_call =
		media_entity_pads_init(&p_tdev->vdev.entity, 1U, &p_tdev->pad);
	if (ret_call < 0) {
...
static void tccvin_unregister_video_device(const struct tccvin_device *tdev)
{
	const struct device *p_dev = NULL;
	const struct tccvin_stream *vstream = NULL;
			       ...
			       struct platform_device *pdev)
{
	...
	if (ret_call != 0) {
		loge(p_dev, "tccvin_init_device_data, ret: %d\n", ret_call);
		ret = -1;
	} else {
		ret_call = tccvin_register_video_device(tdev, pdev);
		if (ret_call != 0) {
			loge(p_dev, "tccvin_register_video_device, ret: %d\n",
			     ret_call);
			ret = -1;
		}
	}
...
static void tccvin_remove_device(struct tccvin_device *tdev)
{
	...
	if (ret_call != 0) {
		...
		loge(p_dev, "tccvin_video_deinit, ret: %d\n", ret_call);
	}
}
...
static int tccvin_add_asd(struct tccvin_device *tdev)
{
	struct device *p_dev = NULL;
	const struct tccvin_stream *vstream = NULL;
	struct tccvin_subdev *tsubdev = NULL;
	...
	ret_call = v4l2_async_notifier_parse_fwnode_endpoints(
		p_dev, &tsubdev->notifier, sizeof(struct v4l2_async_subdev),
		NULL);
	if (ret_call != 0) {
		loge(p_dev,
...
static int tccvin_register_asd_nf(struct tccvin_device *tdev)
{
	const struct device *p_dev = NULL;
	const struct tccvin_stream *vstream = NULL;
	struct tccvin_subdev *tsubdev = NULL;
...
static int tccvin_probe_subdevs(struct tccvin_device *tdev,
				const struct device_node *root_node)
{
	...
	ret_call = tccvin_add_asd(tdev);
	if (ret_call != 0) {
		loge(p_dev, "tccvin_add_asd, ret: %d\n", ret_call);
		ret = -1;
	} else {
		ret_call = tccvin_register_asd_nf(tdev);
		if (ret_call != 0) {
			loge(p_dev, "tccvin_register_asd_nf, ret: %d\n",
			     ret_call);
			ret = -1;
		}
	}
	...
	if (ret == 0) {
		...
		if (val != 0U) {
			...
			if (ret_call != 0) {
				...
				loge(dev,
				     "tccvin_video_trigger_recovery, ret: %d\n",
				     ret_call);
			}
		}
...
static int tccvin_probe(struct platform_device *pdev)
{
	...
	if (tdev == NULL) {
		...
		if (ret_call != 0) {
			loge(p_dev, "tccvin_probe_device, ret: %d\n", ret_call);
			ret = -1;
		} else {
			(void)tccvin_v4l2_init_format(tdev);

			ret_call =
				tccvin_probe_subdevs(tdev, pdev->dev.of_node);
			if (ret_call != 0) {
				loge(p_dev, "tccvin_probe_subdevs, ret: %d\n",
				     ret_call);
				ret = -1;
			}
		}
		...
		tccvin_video_declare_coherent_dma_memory(&tdev->vstream);
	}
