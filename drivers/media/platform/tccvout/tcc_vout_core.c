...
static signed int pmap_get_info(const char *name, struct pmap *mem)
{
	...
	if (tccmisc_pmap(&pmap) < 0) {
		...
		(void)strncpy(mem->name, pmap.name, (TCC_PMAP_NAME_LEN - 1U));
		mem->base = pmap.base;
		mem->size = pmap.size;
...
int vout_set_m2m_path(int deintl_default, struct tcc_vout_device *vout)
{
	...
	if (dev_np != NULL) {
		...
		if ((get_vioc_type(vioc->mc.id) == get_vioc_type(VIOC_MC)) && (get_vioc_index(vioc->mc.id) < VIOC_MC_MAX)) {
			dprintk("[M2M] MC < id = %d >\n", get_vioc_index(vioc->mc.id));
		} else {
			vioc->mc.id = VIOC_NO_COMPONENT;
			dprintk("[M2M] MC not used.\n");
		}
	} else {
		vioc->mc.id = VIOC_NO_COMPONENT;
		dprintk("[M2M] MC not define\n");
	}
#endif
	ret = 0;
err_exit:
