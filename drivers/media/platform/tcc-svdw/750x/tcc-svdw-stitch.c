...
s32 svdw_set_ireq_mask(const struct device *pdev, u32 id, u32 set)
{
	/*
			    ...
			    const struct firmware *fw)
{
	...
	if (fw_data == NULL) {
		loge(p_dev, "Failed to allocate memory to load the firmware.\n");
		return;
	}
	...
	while ((p = strsep((char **)&fw_data, " \n")) != NULL) {
		...
		if (strncmp(p, "CH", 2U) == 0) {
			...
			if (step == 1U) {
				...
				logd(p_dev, "regval = 0x%08x, offset = 0x%08x\n",
				     regval, offset);
			}
		}
	}
