...
#define MAX96701_PAD_SINK	(0U)
#define MAX96701_PAD_SRC	(1U)
#define MAX96701_PAD_NUM	(2U)

static const uint64_t max96701_pad_flag[MAX96701_PAD_NUM] = {
	MEDIA_PAD_FL_SINK,
	MEDIA_PAD_FL_SOURCE,
};

static const struct v4l2_mbus_framefmt max96701_mbus_frmfmt_default = {
	.width = 1,
	.height = 1,
...
struct max96701 {
	...
	struct media_pad		pads[MAX96701_PAD_NUM];
...
static int max96701_nf_bound(struct v4l2_async_notifier *nf,
			     struct v4l2_subdev *sd,
			     struct v4l2_async_subdev *asd)
{
	struct max96701 *dev;
	...
	dev = container_of(nf, struct max96701, nf);
...
static void max96701_nf_unbind(struct v4l2_async_notifier *nf,
			       struct v4l2_subdev *sd,
			       struct v4l2_async_subdev *asd)
{
...
static const struct v4l2_async_notifier_operations max96701_nf_ops = {
	.bound = max96701_nf_bound,
	.unbind = max96701_nf_unbind,
};
 ...
 */
static int32_t max96701_init_cfg(struct v4l2_subdev *sd,
 ...
 */
static int max96701_registered(struct v4l2_subdev *sd)
{
	int ret = 0;
...
static const struct v4l2_subdev_internal_ops max96701_internal_ops = {
	.registered = max96701_registered,
};
...
static int max96701_add_asd(struct max96701 *dev)
{
...
int max96701_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev->sd.internal_ops = &max96701_internal_ops;
	...
	if (ret >= 0) {
		/* init pads */
		for (idx = 0U; idx < MAX96701_PAD_NUM; idx++) {
			dev->pads[idx].index = idx;
			dev->pads[idx].flags = max96701_pad_flag[idx];
			dev->fmt = max96701_mbus_frmfmt_default;
		}
		media_entity_pads_init(&dev->sd.entity, MAX96701_PAD_NUM,
				       dev->pads);

		/* init notifier */
		v4l2_async_notifier_init(&dev->nf);
		dev->nf.ops = &max96701_nf_ops;
	}

	/* add async subdevs */
	if (ret >= 0) {
		ret = max96701_add_asd(dev);
		if (ret < 0) {
