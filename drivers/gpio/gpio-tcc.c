...
#define GPIO_EINT_MUX	0x640U
#else
#define GPIO_EINT_MUX	0x280U
#endif

#if defined(CONFIG_ARCH_TCN100X)
#define GPIO_IRQ_BASE 	443U
#define GPIO_BIT_SET	0x4U
#define GPIO_BIT_CLEAR	0x8U
#define GPIO_OEN	0x10U
#else
#define GPIO_IRQ_BASE 	0x0U
#define GPIO_BIT_SET	0x8U
#define GPIO_BIT_CLEAR	0xCU
#define GPIO_OEN	0x4U
#endif
...
#elif defined(CONFIG_ARCH_TCN100X)
		*idx_sel_reg = hwirq - GPIO_IRQ_BASE;
		*idx_sel_plc = 0;
#else
	if (hwirq > 15U) {
		hwirq -= 16U;
...
#elif defined(CONFIG_ARCH_TCN100X)
	idx_sel_reg = port->irq_port_map[cnt][0];
	idx_sel_plc = 0;
#else
...
static int tcc_get_direction(struct gpio_chip *gc, unsigned int gpio_offset)
{
	...
	gpio_add_wrap_ul(port->base, gpio_gr->reg_offset, &reg);
	gpio_add_wrap_ul(reg, GPIO_OEN, &reg);
	...
	if (ret == 0) {
		...
		if (port->irq_port_map == (void *)0) {
			...
			for (i = 0U; i < (*irq_num / 2U); i++) {
				...
				port->irq_port_map[i][0] -= GPIO_IRQ_BASE;
				pr_debug("[GPIO][DEBUG] %s: interrupt port mux num : %d\n",
...
#if defined(CONFIG_TCC803X_CA7S)
			idx_sel_reg = port->irq_port_map[i][0] / 2U;
			idx_sel_plc = port->irq_port_map[i][0] % 2U;
#elif defined(CONFIG_ARCH_TCN100X)
			idx_sel_reg = port->irq_port_map[i][0];
			idx_sel_plc = 0;
#else
