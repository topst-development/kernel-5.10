...
#if !defined(TRACE_TCC_SC_FW_H) || defined(TRACE_HEADER_MULTI_READ)
#define TRACE_TCC_SC_FW_H
...
TRACE_EVENT(tcc_sc_fw_start_mmc_req,
	...
	TP_fast_assign(
		...
		__entry->rel_wr = cmd ? cmd->rel_wr : 0;
	...
	TP_printk(
		"start mmc_req struct tcc_sc_fw_mmc_cmd[%p]: cmd_opcode=%u cmd_arg=0x%x cmd_flags=0x%x cmd_part_num=%u cmd_rel_wr=%u struct tcc_sc_fw_mmc_data[%p]: blocks=%u block_size=%u blk_addr=%u data_flags=0x%x sg_len=%u sg_count=%u sg=%p",
		  __entry->cmd,
		  __entry->cmd_opcode, __entry->cmd_arg,
		  __entry->cmd_flags, __entry->cmd_part_num,
		  __entry->rel_wr, __entry->data,
...
TRACE_EVENT(tcc_sc_fw_start_xfer,
	...
	TP_printk(
		"start xfer[%p]: cmd_len %u data_len %u cmd[0] 0x%08x cmd[1] 0x%08x",
...
TRACE_EVENT(tcc_sc_fw_done_xfer,
	...
	TP_printk(
		"done xfer[%p]: cmd_len %u data_len %u cmd[0] 0x%08x cmd[1] 0x%08x",
		  __entry->xfer, __entry->cmd_len,  __entry->data_len,
		  __entry->cmd0, __entry->cmd1)
);
...
TRACE_EVENT(tcc_sc_fw_rx_invalid_message,
	...
	TP_PROTO(struct tcc_mbox_msg *msg),

	TP_ARGS(msg),

	TP_STRUCT__entry(
		__field(struct tcc_mbox_msg *,	msg)
		__field(u32,			cmd_len)
		__field(u32,			data_len)
		__field(u32,			cmd0)
		__field(u32,			cmd1)
	...
	TP_printk(
		"invalid rx_msg[%p]: cmd_len %u data_len %u cmd[0] 0x%08x cmd[1] 0x%08x",
...
TRACE_EVENT(tcc_sc_fw_xfer_status,
	...
	TP_printk(
		"Status %u xfer[%p]: TX(%u, %u): cmd[0] 0x%08x cmd[1] 0x%08x RX(%u, %u): cmd[0] 0x%08x cmd[1] 0x%08x",
		  __entry->status, __entry->xfer,
		  __entry->tx_cmd_len, __entry->tx_dat_len,
		  __entry->tx_cmd0, __entry->tx_cmd1,
		  __entry->rx_cmd_len, __entry->rx_dat_len,
		  __entry->rx_cmd0, __entry->rx_cmd1)
);
