...
typedef struct {
	...
	// in case of encoder type, used to decide
	//  whether applying clock limitation or not.
...
typedef struct dec_user_info_t {
	...
	// If this is set, content is jpeg only file (ex. **.jpg)
	unsigned int  m_bStillJpeg;

	// 0: Original Size,
	// 1: 1/2 Scaling Down,
	// 2: 1/4 Scaling Down,
	// 3: 1/8 Scaling Down
	unsigned int  jpg_ScaleRatio;
} dec_user_info_t;
...
#define V_DEC_ALLOC_MEMORY_EX           31
...
#define VPU_SET_MEM_ALLOC_MODE_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_SET_MEM_ALLOC_MODE)
#define VPU_FORCE_CLK_CLOSE_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_FORCE_CLK_CLOSE)
#define VPU_CHECK_CODEC_STATUS_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_CHECK_CODEC_STATUS)
#define VPU_GET_INSTANCE_IDX_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_GET_INSTANCE_IDX)
#define VPU_CLEAR_INSTANCE_IDX_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_CLEAR_INSTANCE_IDX)
#define VPU_USE_WAIT_LIST_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_USE_WAIT_LIST)
#define VPU_SET_RENDERED_FRAMEBUFFER_KERNEL  (VPU_KERNEL_OP_BASE \
					+ VPU_SET_RENDERED_FRAMEBUFFER)
#define VPU_GET_RENDERED_FRAMEBUFFER_KERNEL  (VPU_KERNEL_OP_BASE \
					+ VPU_GET_RENDERED_FRAMEBUFFER)
#define VPU_ENABLE_JPU_CLOCK_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_ENABLE_JPU_CLOCK)
#define VPU_DISABLE_JPU_CLOCK_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_DISABLE_JPU_CLOCK)
#define VPU_CHECK_INSTANCE_AVAILABLE_KERNEL (VPU_KERNEL_OP_BASE \
					+ VPU_CHECK_INSTANCE_AVAILABLE)

#define VPU_GET_FREEMEM_SIZE_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_GET_FREEMEM_SIZE)
#define VPU_HW_RESET_KERNEL		(VPU_KERNEL_OP_BASE + VPU_HW_RESET)
#define VPU_TRY_FORCE_CLOSE_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_TRY_FORCE_CLOSE)
#define VPU_TRY_CLK_RESTORE_KERNEL	(VPU_KERNEL_OP_BASE \
					+ VPU_TRY_CLK_RESTORE)
...
#define DEVICE_INITIALIZE_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ DEVICE_INITIALIZE)
#define V_DEC_INIT_KERNEL		(V_DEC_KERNEL_OP_BASE + V_DEC_INIT)
#define V_DEC_SEQ_HEADER_KERNEL		(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_SEQ_HEADER)
#define V_DEC_GET_INFO_KERNEL		(V_DEC_KERNEL_OP_BASE + V_DEC_GET_INFO)
#define V_DEC_REG_FRAME_BUFFER_KERNEL	(V_DEC_KERNEL_OP_BASE \
...
#define V_DEC_FLUSH_OUTPUT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_FLUSH_OUTPUT)
#define V_GET_RING_BUFFER_STATUS_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_GET_RING_BUFFER_STATUS)
#define V_FILL_RING_BUFFER_AUTO_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_FILL_RING_BUFFER_AUTO)
#define V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_KERNEL \
				(V_DEC_KERNEL_OP_BASE \
				+ V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY)
#define V_DEC_CLOSE_KERNEL		(V_DEC_KERNEL_OP_BASE + V_DEC_CLOSE)

#define V_DEC_ALLOC_MEMORY_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_ALLOC_MEMORY)
#define V_DEC_GENERAL_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_GENERAL_RESULT)
#define V_DEC_INIT_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_INIT_RESULT)
#define V_DEC_SEQ_HEADER_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_SEQ_HEADER_RESULT)
#define V_DEC_DECODE_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_DECODE_RESULT)
#define V_DEC_UPDATE_RINGBUF_WP_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_UPDATE_RINGBUF_WP)

#define V_GET_RING_BUFFER_STATUS_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_GET_RING_BUFFER_STATUS_RESULT)
#define V_FILL_RING_BUFFER_AUTO_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_FILL_RING_BUFFER_AUTO_RESULT)
#define V_DEC_UPDATE_RINGBUF_WP_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_UPDATE_RINGBUF_WP_RESULT)
#define V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT_KERNEL \
			(V_DEC_KERNEL_OP_BASE \
			+ V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT)

#define V_GET_VPU_VERSION_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_GET_VPU_VERSION)
#define V_GET_VPU_VERSION_RESULT_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_GET_VPU_VERSION_RESULT)

#define V_DEC_SWRESET_KERNEL		(V_DEC_KERNEL_OP_BASE + V_DEC_SWRESET)
#define V_DEC_FLUSH_OUTPUT_RESULT_KERNEL  (V_DEC_KERNEL_OP_BASE \
					+ V_DEC_FLUSH_OUTPUT_RESULT)
#define V_DEC_FREE_MEMORY_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_FREE_MEMORY)

#define V_DEC_GET_OUTPUT_INFO_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_GET_OUTPUT_INFO)
#define V_DEC_GET_OUTPUT_INFO_RESULT_KERNEL  (V_DEC_KERNEL_OP_BASE \
					+ V_DEC_GET_OUTPUT_INFO_RESULT)

#define V_DEC_TRY_OPEN_DEV_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_TRY_OPEN_DEV)
#define V_DEC_TRY_CLOSE_DEV_KERNEL	(V_DEC_KERNEL_OP_BASE \
					+ V_DEC_TRY_CLOSE_DEV)
