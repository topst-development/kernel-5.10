...
struct dewarp_params {
	unsigned int	cam_mat[5];
	unsigned int	dist_coeff_fwds[4];
	unsigned int	dist_coeff_bwds[4];
	unsigned int	homography_fwd[9];
	unsigned int	homography_bwd[9];
};
...
#define VIDIOC_S_DEWARP_PARAMS		\
		_IOWR('V', BASE_VIDIOC_PRIVATE+21, struct dewarp_params)
