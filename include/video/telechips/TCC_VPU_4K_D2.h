...
//-----------------------------------------------------
typedef struct vp9_color_info_t {
	/*
	 * 0	UNKNOWN
	 * 1	BT.601 (SDTV)
	 * 2	BT.709 (HDTV)
...
typedef struct vpu_4K_D2_dec_initial_info_t {
	...
	unsigned int m_uiRtlDate;
	unsigned int m_uiRtlRevision;

	//When using register_framebuffer3,
	// the size information for each buffer can be obtained through the following values.
	unsigned int m_uiBufSizeLinearLuma;
	unsigned int m_uiBufSizeLinearChroma;
	unsigned int m_uiBufSizeCompressedLuma;
	unsigned int m_uiBufSizeCompressedChroma;
	unsigned int m_uiBufSizeMVCol;
	unsigned int m_uiBufSizeFBCYTable;
	unsigned int m_uiBufSizeFBCCTable;

	// VP9 Only. VP9 color information, including color space and color range.
...
#define WAVE5_AFBC_ENABLE		(1<<5) // AFBC Enable
	...
	int   (*m_Interrupt)(unsigned int reason);
