...
typedef long long disp_addr_t;   //!< handle - 64bit
#else
typedef long disp_addr_t;        //!< handle - 32bit
#endif
...
typedef struct vp9_compressed_info_t {
	disp_addr_t m_CompressedY[2];
	disp_addr_t m_CompressedCb[2];

	// physical[0] and virtual[1] address of Luma Compression Table
	//  for compressed frame buffer.
	disp_addr_t m_CompressionTableLuma[2];
	disp_addr_t m_CompressionTableChroma[2];
...
typedef struct hevc_MapConv_info_t {
	disp_addr_t	m_CompressedY[2];
	disp_addr_t	m_CompressedCb[2];

	disp_addr_t	m_FbcYOffsetAddr[2];
	disp_addr_t	m_FbcCOffsetAddr[2];
...
typedef struct TCC_PLATFORM_PRIVATE_PMEM_INFO {
	...
	// To check if decoded data is flushed Frame buffer
...
typedef enum CONTENT_TYPES_ {
	TYPE_IMAGE = 0,
	TYPE_VIDEO = 1,
} CONTENT_TYPES;

typedef enum DISP_BIT_ {
	BIT_8  =  8,
	BIT_10 = 10,
	BIT_16 = 16,
} DISP_BIT;

typedef enum DEC_FLAGS_ {
...
typedef struct StrideInfo_ {
	int iY;
	int iCbCr;
} StrideInfo;

typedef struct SubTitleInfo_ {
	int iSubEnable;
	int iSubAddr;
	int iSubIdx;
	int iSubWidth;
...
typedef struct tcc_video_out_info_ {
