...
#define HEVCDEC_API_VERSION "4.0"
...
#define WAVE4_SIZE_BIT_WORK         (WAVE4_MAX_CODE_BUF_SIZE \
					+ WAVE4_STACK_SIZE \
					+ WAVE4_TEMPBUF_SIZE \
					+ WAVE4_SEC_AXI_BUF_SIZE)
#define WAVE4_WORK_CODE_BUF_SIZE (WAVE4_SIZE_BIT_WORK \
				+ (WAVE4_WORKBUF_SIZE*HEVC_MAX_NUM_INSTANCE))
...
/**
 @brief Structure for HEVCDEC_GET_VERSION command
 */
typedef struct hevc_dec_get_version_t {
	char *pszHeaderApiVersion; /**< [inp] Set the value to HEVCDEC_GET_VERSION to check API compatibility, or NULL to skip */
	char szGetVersion[32];	   /**< [out] Returns TCC_VPU_4K_D2 version */
	char szGetBuildDate[32];   /**< [out] Returns TCC_VPU_4K_D2 build date */
} hevc_dec_get_version_t;
...
typedef struct hevc_hrd_param_t {
	...
	unsigned int   nal_bit_rate_value_minus1[HEVC_MAX_NUM_SUB_LAYER]
						[HEVC_MAX_CPB_CNT];
	unsigned int   nal_cpb_size_value_minus1[HEVC_MAX_NUM_SUB_LAYER]
						[HEVC_MAX_CPB_CNT];
	...
	unsigned int   vcl_bit_rate_value_minus1[HEVC_MAX_NUM_SUB_LAYER]
						[HEVC_MAX_CPB_CNT];
...
typedef struct hevc_film_grain_characteristics_t {
	...
	unsigned char
		comp_model_present_flag[HEVC_MAX_NUM_FILM_GRAIN_COMPONENT];
...
typedef struct hevc_dec_input_t {
	...
	int m_iSkipFrameMode;           // Skip Frame Mode
					...
					// 2: Skip B picture
					//    skip if nal_ref_idc==0 in H.264
...
typedef struct hevc_dec_output_info_t {
	...
	wave4_pic_crop_t m_DecodedCropInfo;
	// Cropping information of output frame.
	wave4_pic_crop_t m_DisplayCropInfo;
	...
	hevc_dec_MapConv_info_t m_DispMapConvInfo;
	// current compressed frame info. for map converter
	hevc_dec_MapConv_info_t m_CurrMapConvInfo;
	// previous compressed frame info. for map converter
	hevc_dec_MapConv_info_t m_PrevMapConvInfo;

	unsigned int m_Reserved[41];
 ...
 * \return
 *      If successful, TCC_HEVC_DEC returns 0 or plus.
