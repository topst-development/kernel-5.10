...
struct tcc_sc_fw_xfer {
	struct tcc_mbox_msg tx_mssg;
	u32 tx_cmd_buf_len;
	u32 tx_data_buf_len;
	struct tcc_mbox_msg rx_mssg;
	u32 rx_cmd_buf_len;
	u32 rx_data_buf_len;
	...
	u8	status;
#define TCC_SC_FW_XFER_STAT_IDLE	(0x0U)
#define TCC_SC_FW_XFER_STAT_TX_PEND	(0x1U)
#define TCC_SC_FW_XFER_STAT_TX_START	(0x2U)
#define TCC_SC_FW_XFER_STAT_RX_PEND	(0x3U)
#define TCC_SC_FW_XFER_STAT_HALT	(0x4U)
...
struct tcc_sc_fw_mmc_ops {
	void* (*request_command)(const struct tcc_sc_fw_handle *handle,
		...
		void (*mmc_complete)(void *, void *), void *args);
...
struct tcc_sc_fw_tfuse_ops {
	s32 (*get_tfuse)(const struct tcc_sc_fw_handle *handle,
...
struct tcc_sc_fw_ops {
	...
	struct tcc_sc_fw_tfuse_ops *tfuse_ops;
};
