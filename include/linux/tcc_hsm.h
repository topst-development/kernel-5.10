...
struct tcc_hsm_ioctl_set_key_from_otp_param {
	uint32_t keyIndex;
	uint32_t keyType;
	uint32_t keyMode;
	uint32_t keySize;
...
struct tcc_hsm_ioctl_run_sha_param {
	uint32_t flag;
	uint32_t srcSize;
	uint8_t *srcAddr;
	uint8_t *dig;
...
struct tcc_hsm_ioctl_otp_image_param {
	uint8_t *buf;
	uint32_t size;
};
