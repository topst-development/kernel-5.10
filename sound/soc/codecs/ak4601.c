 ...
 *   Taro Okamoto       21/05/13        2.2  Kernel 5.4
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*
 *
...
static int setSDMaster(struct snd_soc_component *component,int nSDNo,int nMaster);


static int get_sd1_ms(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
...
struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(component, AK4601_000_SYSTEM_CLOCK_SETTING_1, 0xE0, value);
	...
	snd_soc_component_update_bits(component, AK4601_000_SYSTEM_CLOCK_SETTING_1, 0x1F, n);
...
static int setSDMaster(struct snd_soc_component *component,int nSDNo,int nMaster)
{
	...
	snd_soc_component_update_bits(component, AK4601_003_SYNC_DOMAIN_1_SETTING_1 + (2 * nSDNo), 0xF0, value);

	return(0);
}

#ifdef TCC_CODEC_SAMPLING_FREQ
static int setCodecFSMode(struct snd_soc_component *component,int nSDNo)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	unsigned int value;
	...
	snd_soc_component_update_bits(component, AK4601_001_SYSTEM_CLOCK_SETTING_2, 0x1f, value);

	return 0;
}
#endif//TCC_CODEC_SAMPLING_FREQ

static int setSDClock(struct snd_soc_component *component,int nSDNo)
{
	...
	int sdv, bdv;
		...
		akdbgprt("\tsdv bits=%x\n", sdv);
		snd_soc_component_update_bits(component, addr, 0x0F, (sdv | ((bdv & 0x100) >> 5) ));// BDV[8],SDV[2:0]

		addr++;

		snd_soc_component_update_bits(component, addr, 0xFF, ( bdv & 0xFF)); //BDIV[7:0]
...
static int set_sd1_fs(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
	...
	if (currMode < 8){
		if ( ak4601->SDfs[0] != currMode ) {
			...
			setSDMaster(component, 0, ak4601->Master[0]);
		}
	}
	else {
...
static int set_sd2_fs(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
	...
	if (currMode < 8){
		if ( ak4601->SDfs[1] != currMode ) {
			...
			setSDMaster(component, 1, ak4601->Master[1]);
		}
	}
	else {
		akdbgprt(" [AK4601] %s Invalid Value selected!\n",__FUNCTION__);
...
static int set_sdio3_word_len(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
	...
	if (currMode < 4){
		...
		snd_soc_component_update_bits( component, AK4601_052_SDIN3_DIGITAL_INPUT_FORMAT,  0x03, ak4601->DIDL3);
		snd_soc_component_update_bits( component, AK4601_057_SDOUT3_DIGITAL_OUTPUT_FORMAT,0x03, ak4601->DIDL3);
	}
	else {
		akdbgprt(" [AK4601] %s Invalid Value selected!\n",__FUNCTION__);
		return -EINVAL;
...
static int set_sdin3_slot(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
	...
	if (currMode < 4){
		...
		snd_soc_component_update_bits( component, AK4601_052_SDIN3_DIGITAL_INPUT_FORMAT, 0x30, currMode );
	}
	else {
		akdbgprt(" [AK4601] %s Invalid Value selected!\n",__FUNCTION__);
		return -EINVAL;
...
static int set_sdout3_slot(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value  *ucontrol)
{
	...
	if (currMode < 4){
		...
		snd_soc_component_update_bits( component, AK4601_057_SDOUT3_DIGITAL_OUTPUT_FORMAT, 0x30, currMode << 4 );
	}
	else {
		akdbgprt(" [AK4601] %s Invalid Value selected!\n",__FUNCTION__);
		return -EINVAL;
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
		case SND_SOC_DAPM_PRE_PMU:
			...
			snd_soc_component_update_bits(component, AK4601_001_SYSTEM_CLOCK_SETTING_2, 0x80, 0x80);		// CKRESETN bit = 1
			mdelay(10);
			snd_soc_component_update_bits(component, AK4601_08C_RESET_CONTROL, 0x01, 0x01);				// HRESETN bit = 1
			break;

		case SND_SOC_DAPM_POST_PMD:
			akdbgprt("\t[AK4601] SND_SOC_DAPM_POST_PMD\n");
			snd_soc_component_update_bits(component, AK4601_08C_RESET_CONTROL, 0x01, 0x00);         // HRESETN bit = 0
			snd_soc_component_update_bits(component, AK4601_001_SYSTEM_CLOCK_SETTING_2, 0x80, 0x00);// CKRESETN bit = 0
			break;
	}
	return 0;
}
...
static int ak4601_MicBias2(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	...
	//int ret;
	unsigned int MicBias1;

	akdbgprt("\t[AK4601] %s\n",__FUNCTION__);

	switch( event ){
		case SND_SOC_DAPM_POST_PMU:
			//ret = snd_soc_component_read(component, AK4601_002_MIC_BIAS_POWER_MANAGEMENT, &MicBias1);
			MicBias1 = snd_soc_component_read(component, AK4601_002_MIC_BIAS_POWER_MANAGEMENT);
		...
		struct snd_soc_dai *dai)
{
	...
	setSDClock(component, nPortNo);
#ifdef TCC_CODEC_SAMPLING_FREQ
	setCodecFSMode(component, nPortNo);
#endif//TCC_CODEC_SAMPLING_FREQ
...
static int ak4601_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	/* set format */
	setSDMaster(component, nPortNo, msnbit);
...
static int ak4601_set_bias_level(struct snd_soc_component *component,
...
static int ak4601_set_dai_mute(struct snd_soc_dai *dai, int mute, int direction)
{
#ifdef TCC_USE_MUTE_GPIO
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(dai->component);
#endif//TCC_USE_MUTE_GPIO
	akdbgprt("\t[AK4601] direction(%d) %s(%d)\n", direction, __FUNCTION__,__LINE__);
...
#define AK4601_RATES			(SNDRV_PCM_RATE_8000_192000 | SNDRV_PCM_RATE_KNOT)
...
static struct snd_soc_dai_ops ak4601_dai_ops = {
	...
	.mute_stream = ak4601_set_dai_mute,
#ifdef TCC_SET_TDM_SLOT
...
static int ak4601_write_spidmy(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	unsigned char tx[4];
	int wlen;
...
static int ak4601_init_reg_for_subcore(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	int i;
	unsigned int value;
	...
	if ( ak4601->pdn_gpio > 0 ) {
		gpio_set_value(ak4601->pdn_gpio, GPO_PDN_LOW);
		mdelay(1);
		gpio_set_value(ak4601->pdn_gpio, GPO_PDN_HIGH);
		mdelay(1);
	}

#ifndef AK4601_I2C_IF
	ak4601_write_spidmy(component);
#endif

	//ret = snd_soc_component_read(component, AK4601_075_VOL1LCH_DIGITAL_VOLUME, &value);
	//if (ret < 0) {
	//	akdbgprt("\t[AK4601] %s(%d), ret=%d\n", __func__, __LINE__, ret);
	//	return ret;
	//}

	value = snd_soc_component_read(component, AK4601_075_VOL1LCH_DIGITAL_VOLUME);
	if(value == 0x18) {
	...
	ak4601_set_bias_level(component, SND_SOC_BIAS_STANDBY);
	...
	snd_soc_component_update_bits( component, AK4601_016_SYNC_DOMAIN_SELECT_6, 0x77, ((ak4601->EXBCK[0]<<4) + (ak4601->EXBCK[1])));
	snd_soc_component_update_bits( component, AK4601_017_SYNC_DOMAIN_SELECT_7, 0x70, ak4601->EXBCK[2]<<4 );


	akdbgprt("\t[AK4601] %s\n addr=%03X data=%02X",__FUNCTION__, AK4601_05E_OUTPUT_PORT_ENABLE_SETTING, 0x38);

	snd_soc_component_update_bits( component, AK4601_05E_OUTPUT_PORT_ENABLE_SETTING, 0x38, 0x38 );//SDOUT1~3 OutputEnable=1

	snd_soc_component_update_bits( component, AK4601_013_SYNC_DOMAIN_SELECT_3, 0x07, 0x01);
	snd_soc_component_update_bits( component, AK4601_014_SYNC_DOMAIN_SELECT_4, 0x77, 0x11);

	snd_soc_component_update_bits( component, AK4601_011_SYNC_DOMAIN_SELECT_1, 0x12, 0x12 );//LR/BICKx Output SDx

	//'CODEC Sync Domain' 'SD1', 'ADC1 Sync Domain' 'SD1'
	snd_soc_component_update_bits( component, AK4601_020_SYNC_DOMAIN_SELECT_16, 0x77, 0x11);
	//'MixerB Sync Domain' 'SD1'
	snd_soc_component_update_bits( component, AK4601_01F_SYNC_DOMAIN_SELECT_15, 0x07, 0x01);
	//'LRCK2/SDOUT3 Pin BICK2/SDIN3 Pin Setting' 'SDIN3,SDOUT3 Enable'
	snd_soc_component_update_bits( component, AK4601_010_PIN_SETTING, 0x01, 0x01);
	//'ADCM MUX' 'AINM'
	//'AIN1 Lch MUX' 'IN1P_N'
	//'AIN1 Rch MUX' 'IN2P_N'
	snd_soc_component_update_bits( component, AK4601_06B_ANALOG_INPUT_SELECT_SETTING, 0x1C, 0x10);
	//'MicBias1 MUX' 'MicBias1'
	//'MicBias2 MUX' 'MicBias2'
	snd_soc_component_update_bits( component, AK4601_002_MIC_BIAS_POWER_MANAGEMENT, 0xFF, 0x03);
	//'ADC1 Digital Volume L' 255
	snd_soc_component_update_bits( component, AK4601_064_ADC1LCH_DIGITAL_VOLUME, 0xFF, 0x00);
	//'ADC1 Digital Volume R' 0
	snd_soc_component_update_bits( component, AK4601_065_ADC1RCH_DIGITAL_VOLUME, 0xFF, 0xFF);
	//'MixerB Ch1 Source Selector' 'ADC1'
	snd_soc_component_update_bits( component, AK4601_047_MIXER_B_CH1_INPUT_DATA_SELECT, 0x3F, 0x15);
	//'MixerB Ch2 Source Selector' 'ADCM'
	snd_soc_component_update_bits( component, AK4601_048_MIXER_B_CH2_INPUT_DATA_SELECT, 0x3F, 0x17);
	//'MixerB Input2 Data Change' 'Swap'
	snd_soc_component_update_bits( component, AK4601_061_MIXER_B_SETTING, 0x0c, 0x0c);
	//'SDOUT1 Source Selector' 'MixerB'
	snd_soc_component_update_bits( component, AK4601_021_SDOUT1_TDM_SLOT1_2_DATA_SELECT, 0x3F, 0x19);
	//'SDOUT1 Output Enable' on
	snd_soc_component_update_bits( component, AK4601_05E_OUTPUT_PORT_ENABLE_SETTING, 0x20, 0x20);
	//'ADC1 Mute' off
	//'ADCM Mute' off
	snd_soc_component_update_bits( component, AK4601_06C_ADC_MUTE_AND_HPF_CONTROL, 0x50, 0x00);
	//'DAC1 Source Selector' 'SDIN1'
	//'DAC2 Source Selector' 'SDIN1B'
	//'DAC3 Source Selector' 'SDIN1C'
	snd_soc_component_update_bits( component, AK4601_035_DAC1_INPUT_DATA_SELECT, 0x3F, 0x01);
//	snd_soc_component_update_bits( component, AK4601_036_DAC2_INPUT_DATA_SELECT, 0x3F, 0x02);
//	snd_soc_component_update_bits( component, AK4601_037_DAC3_INPUT_DATA_SELECT, 0x3F, 0x03);
	//'DAC2 Source Selector' 'SDIN2'
	//'DAC3 Source Selector' 'SDIN1'
	snd_soc_component_update_bits( component, AK4601_036_DAC2_INPUT_DATA_SELECT, 0x3F, 0x09);
	snd_soc_component_update_bits( component, AK4601_037_DAC3_INPUT_DATA_SELECT, 0x3F, 0x01);
#ifdef TCC_ALWAYS_PMDAC_ON
	snd_soc_component_update_bits( component, AK4601_08A_POWER_MANAGEMENT1, 0x07, 0x07 );//DAC1,2,3 On
#endif//TCC_ALWAYS_PMDAC_ON
	//'DAC1 Mute' off
	//'DAC2 Mute' off
	//'DAC3 Mute' off
	snd_soc_component_update_bits( component, AK4601_073_DAC_MUTE_AND_FILTER_SETTING, 0x70, 0x00);
#ifdef TCC_USE_MUTE_GPIO
	ak4601->playback_active = 0;
#endif//TCC_USE_MUTE_GPIO
	return 0;
...
#else
static int ak4601_init_reg(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	int i;
	unsigned int value;
	...
	//ret = snd_soc_component_read(component,AK4601_075_VOL1LCH_DIGITAL_VOLUME, &value);
	value = snd_soc_component_read(component,AK4601_075_VOL1LCH_DIGITAL_VOLUME);
	printk("[AK4601] %s Check Connection...%s\n", __FUNCTION__, value == 0x18 ? "SUCCESS" : "FAILUE");
	...
	snd_soc_component_update_bits( component, AK4601_013_SYNC_DOMAIN_SELECT_3, 0x07, 0x01);
	snd_soc_component_update_bits( component, AK4601_014_SYNC_DOMAIN_SELECT_4, 0x77, 0x23);
...
static int ak4601_probe(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	int ret = 0;
	...
	if ( ak4601->pdn_gpio != -1 ) {
		ret = gpio_request(ak4601->pdn_gpio, "ak4601 pdn");
		gpio_direction_output(ak4601->pdn_gpio, 0);
	}
#if	defined(CONFIG_TCC803X_CA7S) || defined(CONFIG_SND_SOC_AK4601_SUBCORE)
	ak4601_init_reg_for_subcore(component);
#else
	ak4601_init_reg(component);
#endif//defined(CONFIG_TCC803X_CA7S) || defined(CONFIG_SND_SOC_AK4601_SUBCORE)
...
static void ak4601_remove(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);

	akdbgprt("\t[AK4601] %s(%d)\n",__FUNCTION__,__LINE__);

	ak4601_set_bias_level(component, SND_SOC_BIAS_OFF);
	if ( ak4601->pdn_gpio > 0 ) {
		gpio_set_value(ak4601->pdn_gpio, GPO_PDN_LOW);
		msleep(1);
...
static int ak4601_hw_read_register(struct snd_soc_component *component, unsigned int reg)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
	unsigned char tx[3], rx[1];
	int	wlen, rlen;
...
static int ak4601_suspend(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
...
// Only for debug [[
{
	...
	for ( i = regs ; i <= rege ; i++ ){
		value = ak4601_hw_read_register(component, i);
		printk("***AK4601 Addr,Reg=(%03x, %02x)\n", i, value);
	}
	printk("\t[AK4601] %s(%d) hw reg dump <===\n",__FUNCTION__,__LINE__);
}
...
static int ak4601_resume(struct snd_soc_component *component)
{
	struct ak4601_priv *ak4601 = snd_soc_component_get_drvdata(component);
#ifdef TCC_SUSPEND_TO_RAM
	int ret;
...
#ifdef TCC_USE_MUTE_GPIO
	if (ak4601->cmute_gpio >= 0) {
		if (gpio_request_one(ak4601->cmute_gpio,
					(ak4601->cmute_gpio_flags ? GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW),
					"cmute-gpios")) {
			ak4601->cmute_gpio = -1;
			printk("[AK4601] request cmute gpio(%d) failed\n", ak4601->cmute_gpio);
		}
		gpio_direction_output(ak4601->cmute_gpio, 0);
		gpio_set_value(ak4601->cmute_gpio, ak4601->cmute_gpio_flags); //cmute on
		akdbgprt("\t[AK4601] SND_SOC_BIAS_ON cmute_gpio unmute\n");
	}
	if (ak4601->amute_gpio >= 0) {
		if (gpio_request_one(ak4601->amute_gpio,
					(ak4601->amute_gpio_flags ? GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW),
					"amute-gpios")) {
			ak4601->amute_gpio = -1;
			printk("[AK4601] request amute gpio(%d) failed\n", ak4601->amute_gpio);
		}
		gpio_direction_output(ak4601->amute_gpio, 0);
		gpio_set_value(ak4601->amute_gpio, ak4601->amute_gpio_flags); //amute on
		akdbgprt("\t[AK4601] SND_SOC_BIAS_ON amute_gpio unmute\n");
	}
#endif//TCC_USE_MUTE_GPIO
...
struct snd_soc_component_driver soc_component_dev_ak4601 = {
	.probe = ak4601_probe,
                            ...
                            const struct i2c_device_id *id)
{
	...
	ret = devm_snd_soc_register_component(&i2c->dev,
			&soc_component_dev_ak4601, &ak4601_dai[0], ARRAY_SIZE(ak4601_dai));
	if (ret < 0){
		devm_kfree(&i2c->dev, ak4601);
