...
#undef spdif_dai_func_dbg
#if 0
#define spdif_dai_func_dbg_enable
#define spdif_dai_func_dbg(a...) \
	(void) pr_info("[FUNC_DEBUG][SPDIF]" a)
#else
#define spdif_dai_func_dbg(a...)
#endif
...
{
#ifdef spdif_dai_func_dbg_enable
	struct tcc_spdif_t *spdif =
	    (struct tcc_spdif_t *)snd_soc_dai_get_drvdata(dai);
#else
	unused(dai);
	...
	spdif_dai_func_dbg("[%d] +- %s - active : %d -+\n", spdif->blk_no, __func__, snd_soc_dai_active(dai));
	...
	int clk_div)
{
	...
	spdif_dai_func_dbg("[%d] ++ %s - div_id:%d, div:%d ++\n",
					spdif->blk_no, __func__, div_id, clk_div);
...
static uint32_t tcc_spdif_is_active(
	struct snd_soc_component *comp,
	const char *set_change)
{
	const struct tcc_spdif_t *spdif =
		(struct tcc_spdif_t *)snd_soc_component_get_drvdata(comp);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *compo =
	    snd_soc_kcontrol_component(kcontrol);
	struct tcc_spdif_t *spdif =
	    (struct tcc_spdif_t *)snd_soc_component_get_drvdata(compo);
	int ret = 0;

	if (tcc_spdif_is_active(compo, __func__) > 0U) {
		ret = -EINVAL;
	} else {
...
static const struct snd_kcontrol_new tcc_spdif_snd_controls[] = {
	SOC_ENUM_EXT(("Data Format"), (spdif_data_format_enum[0]),
		     (get_spdif_data_format), (set_spdif_data_format)),
	SOC_ENUM_EXT(("Copyright"), (spdif_copyright_enum[0]),
		     (get_spdif_copyright), (set_spdif_copyright)),
	SOC_SINGLE_EXT(("MCLK DIV"), 0, 0, 16, 0,
			 (get_spdif_ratio), (set_spdif_ratio))
};

int tcc_spdif_open(struct snd_soc_component *component,
		struct snd_pcm_substream *substream)
{
...
static const struct snd_soc_component_driver tcc_spdif_component_drv = {
	.name = "tcc-spdif",
	.controls = tcc_spdif_snd_controls,
	.num_controls =
	    (uint32_t) (TCC_AUDIO_ARRAY_SIZE(tcc_spdif_snd_controls)),
   	.open = tcc_spdif_open,
	...
	if (IS_ERR((void *)spdif->reg)) {
		...
		if ((IS_ERR(spdif->pclk)) ||
			...
			if (IS_ERR((void *)spdif->pcfg_reg)) {
				spdif->pcfg_reg = NULL;
				spdif_dai_err("pcfg_reg is NULL\n");
				ret = -EINVAL;
			} else {
