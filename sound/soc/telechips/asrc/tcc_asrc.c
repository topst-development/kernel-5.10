	...
	enum tcc_asrc_component_t comp_asrc)
{
	...
	val = readl(asrc_reg + TCC_ASRC_INIT_OFFSET) | ui_lshift(1U, (uint32_t)comp_asrc);
	writel(val, asrc_reg + TCC_ASRC_INIT_OFFSET);

	val &= ~ui_lshift(1U, (uint32_t)comp_asrc);
	writel(val, asrc_reg + TCC_ASRC_INIT_OFFSET);
}
	...
	enum tcc_asrc_path_t inport_path)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_IP0_PATH_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_IP1_PATH_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_IP2_PATH_MASK
		: ~TCC_ASRC_IP3_PATH_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_IP0_PATH(inport_path)
		: (asrc_ch == 1U) ? TCC_ASRC_IP1_PATH(inport_path)
		: (asrc_ch == 2U) ? TCC_ASRC_IP2_PATH(inport_path)
		: TCC_ASRC_IP3_PATH(inport_path);
	...
	enum tcc_asrc_path_t outport_path)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_OP0_PATH_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_OP1_PATH_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_OP2_PATH_MASK
		: ~TCC_ASRC_OP3_PATH_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_OP0_PATH(outport_path)
		: (asrc_ch == 1U) ? TCC_ASRC_OP1_PATH(outport_path)
		: (asrc_ch == 2U) ? TCC_ASRC_OP2_PATH(outport_path)
		: TCC_ASRC_OP3_PATH(outport_path);
	...
	uint32_t ratio_shift22)
{
	...
	if (ratio_shift22 < 0x400000U) {
		if (ratio_shift22 != 0U) {
			zero_init_sz = (32U * 0x400000U) / ratio_shift22;
		} else {
			zero_init_sz = 0xffU;
		}
	}
	//asrc_info("zero_init_sz : %d(0x%x)\n", zero_init_sz, zero_init_sz);

	val = readl(asrc_reg + TCC_ASRC_SRC_INIT_ZERO_SZ_OFFSET);

	val &= (asrc_ch == 0U) ? ~TCC_ASRC_SRC_INIT_ZERO_SIZE0_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_SRC_INIT_ZERO_SIZE1_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_SRC_INIT_ZERO_SIZE2_MASK
		: ~TCC_ASRC_SRC_INIT_ZERO_SIZE3_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_SRC_INIT_ZERO_SIZE0(zero_init_sz)
		: (asrc_ch == 1U) ? TCC_ASRC_SRC_INIT_ZERO_SIZE1(zero_init_sz)
		: (asrc_ch == 2U) ? TCC_ASRC_SRC_INIT_ZERO_SIZE2(zero_init_sz)
	...
	uint32_t lvl)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_SRC_OPT_BUF_LVL0_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_SRC_OPT_BUF_LVL1_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_SRC_OPT_BUF_LVL2_MASK
		: ~TCC_ASRC_SRC_OPT_BUF_LVL3_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_SRC_OPT_BUF_LVL0(lvl)
		: (asrc_ch == 1U) ? TCC_ASRC_SRC_OPT_BUF_LVL1(lvl)
		: (asrc_ch == 2U) ? TCC_ASRC_SRC_OPT_BUF_LVL2(lvl)
	...
	uint32_t ratio_shift22)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_SRC_RATE0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_SRC_RATE1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_SRC_RATE2_OFFSET
	...
	uint32_t cnt)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_PERIOD_SYNC_CNT0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_PERIOD_SYNC_CNT1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_PERIOD_SYNC_CNT2_OFFSET
	...
	enum tcc_asrc_ip_op_timing_t op_timing)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_IP0_TIMING_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_IP1_TIMING_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_IP2_TIMING_MASK
		: ~TCC_ASRC_IP3_TIMING_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_IP0_TIMING(op_timing)
		: (asrc_ch == 1U) ? TCC_ASRC_IP1_TIMING(op_timing)
		: (asrc_ch == 2U) ? TCC_ASRC_IP2_TIMING(op_timing)
		: TCC_ASRC_IP3_TIMING(op_timing);
	...
	enum tcc_asrc_ip_op_timing_t ip_timing)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_OP0_TIMING_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_OP1_TIMING_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_OP2_TIMING_MASK
		: ~TCC_ASRC_OP3_TIMING_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_OP0_TIMING(ip_timing)
		: (asrc_ch == 1U) ? TCC_ASRC_OP1_TIMING(ip_timing)
		: (asrc_ch == 2U) ? TCC_ASRC_OP2_TIMING(ip_timing)
		: TCC_ASRC_OP3_TIMING(ip_timing);
	...
	uint32_t gain)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_VOL_GAIN0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_VOL_GAIN1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_VOL_GAIN2_OFFSET
	...
	uint32_t gain)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_VOL_RAMP_GAIN0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_VOL_RAMP_GAIN1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_VOL_RAMP_GAIN2_OFFSET
	...
	uint32_t dn_wait)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_VOL_RAMP_DN_CFG0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_VOL_RAMP_DN_CFG1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_VOL_RAMP_DN_CFG2_OFFSET
			: TCC_ASRC_VOL_RAMP_DN_CFG3_OFFSET;

	val = TCC_ASRC_VOL_RAMP_TIME(dn_time) | TCC_ASRC_VOL_RAMP_WAIT(dn_wait);
	...
	uint32_t up_wait)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_VOL_RAMP_UP_CFG0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_VOL_RAMP_UP_CFG1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_VOL_RAMP_UP_CFG2_OFFSET
			: TCC_ASRC_VOL_RAMP_UP_CFG3_OFFSET;

	val = TCC_ASRC_VOL_RAMP_TIME(up_time) | TCC_ASRC_VOL_RAMP_WAIT(up_wait);
	...
	bool enable)
{
	...
	if (enable) {
		val |= (asrc_ch == 0U) ? TCC_ASRC_VOL_EN0
			: (asrc_ch == 1U) ? TCC_ASRC_VOL_EN1
			: (asrc_ch == 2U) ? TCC_ASRC_VOL_EN2
			: TCC_ASRC_VOL_EN3;
	} else {
		val &= (asrc_ch == 0U) ? ~TCC_ASRC_VOL_EN0
			: (asrc_ch == 1U) ? ~TCC_ASRC_VOL_EN1
	...
	bool enable)
{
	...
	if (enable) {
		val |= (asrc_ch == 0U) ? TCC_ASRC_RAMP_EN0
			: (asrc_ch == 1U) ? TCC_ASRC_RAMP_EN1
			: (asrc_ch == 2U) ? TCC_ASRC_RAMP_EN2
			: TCC_ASRC_RAMP_EN3;
	} else {
		val &= (asrc_ch == 0U) ? ~TCC_ASRC_RAMP_EN0
			: (asrc_ch == 1U) ? ~TCC_ASRC_RAMP_EN1
	...
	uint32_t threshold)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_IN0_CTRL_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_FIFO_IN1_CTRL_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_FIFO_IN2_CTRL_OFFSET
	...
	uint32_t threshold)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_OUT0_CTRL_OFFSET
		: (asrc_ch == 1U) ? TCC_ASRC_FIFO_OUT1_CTRL_OFFSET
		: (asrc_ch == 2U) ? TCC_ASRC_FIFO_OUT2_CTRL_OFFSET
	...
	uint32_t asrc_ch)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_IN0_DATA_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_FIFO_IN1_DATA_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_FIFO_IN2_DATA_OFFSET
			: TCC_ASRC_FIFO_IN3_DATA_OFFSET;

	return ui_add(asrc_reg_phys, offset);
}
	...
	uint32_t asrc_ch)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_OUT0_DATA_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_FIFO_OUT1_DATA_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_FIFO_OUT2_DATA_OFFSET
...
uint32_t tcc_asrc_get_fifo_in_status(
	const void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t offset;

	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_IN0_STATUS_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_FIFO_IN1_STATUS_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_FIFO_IN2_STATUS_OFFSET
	...
	uint32_t asrc_ch)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_OUT0_STATUS_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_FIFO_OUT1_STATUS_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_FIFO_OUT2_STATUS_OFFSET
	...
	uint32_t asrc_ch)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_SRC_STATUS0_OFFSET
			: (asrc_ch == 1U) ? TCC_ASRC_SRC_STATUS1_OFFSET
			: (asrc_ch == 2U) ? TCC_ASRC_SRC_STATUS2_OFFSET
	...
	enum tcc_asrc_clksel_t clksel)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_IP0_CLKSEL_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_IP1_CLKSEL_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_IP2_CLKSEL_MASK
		: ~TCC_ASRC_IP3_CLKSEL_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_IP0_CLKSEL(clksel)
		: (asrc_ch == 1U) ? TCC_ASRC_IP1_CLKSEL(clksel)
		: (asrc_ch == 2U) ? TCC_ASRC_IP2_CLKSEL(clksel)
	...
	enum tcc_asrc_clksel_t clksel)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_OP0_CLKSEL_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_OP1_CLKSEL_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_OP2_CLKSEL_MASK
		: ~TCC_ASRC_OP3_CLKSEL_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_OP0_CLKSEL(clksel)
		: (asrc_ch == 1U) ? TCC_ASRC_OP1_CLKSEL(clksel)
		: (asrc_ch == 2U) ? TCC_ASRC_OP2_CLKSEL(clksel)
	...
	enum tcc_asrc_ip_route_t route)
{
	...
	val &= (asrc_ch == 0U) ? ~TCC_ASRC_IP0_ROUTE_MASK
		: (asrc_ch == 1U) ? ~TCC_ASRC_IP1_ROUTE_MASK
		: (asrc_ch == 2U) ? ~TCC_ASRC_IP2_ROUTE_MASK
		: ~TCC_ASRC_IP3_ROUTE_MASK;

	val |= (asrc_ch == 0U) ? TCC_ASRC_IP0_ROUTE(route)
		: (asrc_ch == 1U) ? TCC_ASRC_IP1_ROUTE(route)
		: (asrc_ch == 2U) ? TCC_ASRC_IP2_ROUTE(route)
	...
	enum tcc_asrc_op_route_t route)
{
	...
	val &= (mcaudio_ch == 0U) ? ~TCC_ASRC_OP0_ROUTE_MASK
		: (mcaudio_ch == 1U) ? ~TCC_ASRC_OP1_ROUTE_MASK
		: (mcaudio_ch == 2U) ? ~TCC_ASRC_OP2_ROUTE_MASK
		: ~TCC_ASRC_OP3_ROUTE_MASK;

	val |= (mcaudio_ch == 0U) ? TCC_ASRC_OP0_ROUTE(route)
		: (mcaudio_ch == 1U) ? TCC_ASRC_OP1_ROUTE(route)
		: (mcaudio_ch == 2U) ? TCC_ASRC_OP2_ROUTE(route)
	...
	bool en_swap)
{
	...
	val &= (mcaudio_ch == 0U) ? ~TCC_ASRC_IP0_FMT_MASK
		: (mcaudio_ch == 1U) ? ~TCC_ASRC_IP1_FMT_MASK
		: (mcaudio_ch == 2U) ? ~TCC_ASRC_IP2_FMT_MASK
		: ~TCC_ASRC_IP3_FMT_MASK;

	val |= (mcaudio_ch == 0U) ? TCC_ASRC_IP0_FMT(fmt)
		: (mcaudio_ch == 1U) ? TCC_ASRC_IP1_FMT(fmt)
		: (mcaudio_ch == 2U) ? TCC_ASRC_IP2_FMT(fmt)
		: TCC_ASRC_IP3_FMT(fmt);

	if (en_swap) {
		val |= (mcaudio_ch == 0U) ? TCC_ASRC_IP0_SWAP
			: (mcaudio_ch == 1U) ? TCC_ASRC_IP1_SWAP
			: (mcaudio_ch == 2U) ? TCC_ASRC_IP2_SWAP
			: TCC_ASRC_IP3_SWAP;
	} else {
		val &= (mcaudio_ch == 0U) ? ~TCC_ASRC_IP0_SWAP
			: (mcaudio_ch == 1U) ? ~TCC_ASRC_IP1_SWAP
	...
	bool en_swap)
{
	...
	val &= (mcaudio_ch == 0U) ? ~TCC_ASRC_OP0_FMT_MASK
		: (mcaudio_ch == 1U) ? ~TCC_ASRC_OP1_FMT_MASK
		: (mcaudio_ch == 2U) ? ~TCC_ASRC_OP2_FMT_MASK
		: ~TCC_ASRC_OP3_FMT_MASK;

	val |= (mcaudio_ch == 0U) ? TCC_ASRC_OP0_FMT(fmt)
		: (mcaudio_ch == 1U) ? TCC_ASRC_OP1_FMT(fmt)
		: (mcaudio_ch == 2U) ? TCC_ASRC_OP2_FMT(fmt)
		: TCC_ASRC_OP3_FMT(fmt);

	if (en_swap) {
		val |= (mcaudio_ch == 0U) ? TCC_ASRC_OP0_SWAP
			: (mcaudio_ch == 1U) ? TCC_ASRC_OP1_SWAP
			: (mcaudio_ch == 2U) ? TCC_ASRC_OP2_SWAP
			: TCC_ASRC_OP3_SWAP;
	} else {
		val &= (mcaudio_ch == 0U) ? ~TCC_ASRC_OP0_SWAP
			: (mcaudio_ch == 1U) ? ~TCC_ASRC_OP1_SWAP
...
#if defined(CONFIG_ARCH_TCC805X) || defined(CONFIG_ARCH_TCC806X)
void tcc_asrc_irq_zero_fifo_in_read_complete_enable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
	...
	val |= (asrc_ch == 0U) ? TCC_ASRC_IRQ_FIFO_IN0_COMPLETE :
		(asrc_ch == 1U) ? TCC_ASRC_IRQ_FIFO_IN1_COMPLETE :
		(asrc_ch == 2U) ? TCC_ASRC_IRQ_FIFO_IN2_COMPLETE :
		TCC_ASRC_IRQ_FIFO_IN3_COMPLETE;

	writel(val, asrc_reg + offset);
}

void tcc_asrc_irq0_fifo_in_read_complete_disable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
...
#if 0//TBD
void tcc_asrc_irq0_fifo_out_read_complete_enable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
	...
	val |= (asrc_ch == 0U) ? TCC_ASRC_IRQ_FIFO_OUT0_COMPLETE :
		(asrc_ch == 1U) ? TCC_ASRC_IRQ_FIFO_OUT1_COMPLETE :
		(asrc_ch == 2U) ? TCC_ASRC_IRQ_FIFO_OUT2_COMPLETE :
		TCC_ASRC_IRQ_FIFO_OUT3_COMPLETE;

	writel(val, asrc_reg + offset);
}

void tcc_asrc_irq0_fifo_out_read_complete_disable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
...
void tcc_asrc_irq1_ringbuf_empty_enable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
	...
	val |= (asrc_ch == 0U) ? TCC_ASRC_IRQ_PAIR0_EMPTY :
		(asrc_ch == 1U) ? TCC_ASRC_IRQ_PAIR1_EMPTY :
		(asrc_ch == 2U) ? TCC_ASRC_IRQ_PAIR2_EMPTY :
		TCC_ASRC_IRQ_PAIR3_EMPTY;

	writel(val, asrc_reg + offset);
}

void tcc_asrc_irq1_ringbuf_empty_disable(
	void __iomem *asrc_reg,
	uint32_t asrc_ch)
{
	uint32_t val;
...
void tcc_asrc_set_fifo_in_read_cnt_threshold(
	void __iomem *asrc_reg,
	uint32_t asrc_ch,
	uint32_t buffer_size)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_IN0_CTRL1_OFFSET :
		(asrc_ch == 1U) ? TCC_ASRC_FIFO_IN1_CTRL1_OFFSET :
		(asrc_ch == 2U) ? TCC_ASRC_FIFO_IN2_CTRL1_OFFSET :
		TCC_ASRC_FIFO_IN3_CTRL1_OFFSET;

	val = readl(asrc_reg + offset);

	val &= ~TCC_ASRC_FIFO_RD_CNT_THR_MASK;
	val |= TCC_ASRC_FIFO_RD_CNT_THR(ui_sub(buffer_size, 1U));

	writel(val, asrc_reg + offset);
}

void tcc_asrc_clear_fifo_in_read_cnt(void __iomem *asrc_reg, uint32_t asrc_ch)
{
	uint32_t val;
	uint32_t offset;
...
void tcc_asrc_set_fifo_out_read_cnt_threshold(
	void __iomem *asrc_reg,
	uint32_t asrc_ch,
	uint32_t buffer_size)
{
	...
	offset = (asrc_ch == 0U) ? TCC_ASRC_FIFO_OUT0_CTRL1_OFFSET :
		(asrc_ch == 1U) ? TCC_ASRC_FIFO_OUT1_CTRL1_OFFSET :
		(asrc_ch == 2U) ? TCC_ASRC_FIFO_OUT2_CTRL1_OFFSET :
		TCC_ASRC_FIFO_OUT3_CTRL1_OFFSET;
...
void tcc_asrc_clear_fifo_out_read_cnt(void __iomem *asrc_reg, uint32_t asrc_ch)
{
	uint32_t val;
	uint32_t offset;
