...
#ifndef TCC_ASRC_RULE_H
#define TCC_ASRC_RULE_H
...
//CERT C INT31-C unsigned int to signed long (Unsigned to Signed)
static inline int ui_to_sl(uint32_t in)
{
	long out = 0;
	if (in <= sl_to_ui(TCC_AUDIO_LONG_MAX)) {
		out = (long)in;
	}
	return out;
}
...
#define asrc_info(a...) (void) pr_info("[INFO]" a)
#define asrc_dbg(a...) (void) pr_info("[DEBUG]" a)
#define asrc_err(a...) (void) pr_err("[ERROR]" a)

#define ASRC_INT_MAX TCC_AUDIO_INT_MAX
#define ASRC_INT_MIN TCC_AUDIO_INT_MIN
#define ASRC_LONG_MAX_UL (~0UL>>1)

#define TCC_ASRC_ARRAY_SIZE(a) \
        (uint32_t)(sizeof(a) / sizeof((a)[0]))



//CERT C INT31-C unsigned int to enum tcc_asrc_drv_bitwidth_t (Unsigned to ENUM)
static inline enum tcc_asrc_drv_bitwidth_t ui_to_enum_tcc_asrc_drv_bitwidth_t(
		uint32_t in)
{
	enum tcc_asrc_drv_bitwidth_t out = TCC_ASRC_16BIT;
	if (in <= si_to_ui(ASRC_INT_MAX)) {
		out = (enum tcc_asrc_drv_bitwidth_t)in;
	}
	return out;
}

//CERT C INT02-C unsigned int to enum tcc_peri_id_t (Unsigned to ENUM)
static inline enum tcc_peri_id_t ui_to_enum_tcc_peri_id_t(
		uint32_t in)
{
	enum tcc_peri_id_t out = PERI_ID_P0_TX;
	if (in <= si_to_ui(ASRC_INT_MAX)) {
		out = (enum tcc_peri_id_t)in;
	}
	return out;
}


//CERT C INT36-C ulong to (struct soc_mixer_control *) (Integer to Pointer)
static inline struct soc_mixer_control * ul_to_ptr_soc_mixer_control(
		unsigned long in)
{
	volatile uintptr_t iptr = in; 

	struct soc_mixer_control *ptr = (struct soc_mixer_control *)iptr;

	return ptr;
}


#endif /*TCC_ASRC_RULE_H*/
