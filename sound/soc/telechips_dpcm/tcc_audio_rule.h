...
#define TCC_AUDIO_LONG_MIN		(-TCC_AUDIO_LONG_MAX-1)
#else /* 32bit */
#define TCC_AUDIO_LONG_MAX		TCC_AUDIO_INT_MAX
#define TCC_AUDIO_LONG_MIN		TCC_AUDIO_INT_MIN
#endif
...
//CERT C INT31-C unsigned long to singed long
static inline int ul_to_sl(unsigned long in)
{
	int out = 0;
...
static inline unsigned long ul_add(unsigned long ul_a, unsigned long ul_b)
{
	unsigned long ret = 0;
	if((ULONG_MAX - ul_a) < ul_b){
		tcc_audio_err("Unsigned long + operaion is wrap!!!");
	} else {
		ret = ul_a + ul_b;
	}
	return ret;
...
//CERT C INT30-C (Addition)
static inline int si_add(int si_a, int si_b)
{
	int ret = 0;
	if(si_a >= 0){
		if((TCC_AUDIO_INT_MAX - si_a) < si_b){
			tcc_audio_err("Signed int + operaion is wrap!!!");
		} else {
			ret = si_a + si_b;
		}
	}else{
		if((TCC_AUDIO_INT_MIN - si_a) > si_b){
			tcc_audio_err("Signed int + operaion is wrap!!!");
		} else {
			ret = si_a + si_b;
		}
...
//CERT C INT30-C (Addition)
static inline long sl_add(long sl_a, long sl_b)
{
	long ret = 0;
	if(sl_a >= 0){
		if((TCC_AUDIO_LONG_MAX - sl_a) < sl_b){
			tcc_audio_err("Signed long + operaion is wrap!!!");
		} else {
			ret = sl_a + sl_b;
		}
	}else{
		if((TCC_AUDIO_LONG_MIN - sl_a) > sl_b){
			tcc_audio_err("Signed long + operaion is wrap!!!");
		} else {
			ret = sl_a + sl_b;
		}
...
static inline unsigned long long ull_sub(unsigned long long ull_a, unsigned long long  ull_b)
{
...
static inline unsigned long long si_to_ull(int in)
{
...
//CERT C INT02-C ull to uint (Unsigned, Loss of Precision)
static inline int32_t ull_to_si(unsigned long long in)
{
	int out = 0;
	if (in <= si_to_ull(TCC_AUDIO_INT_MAX)) {
		out = (int)in;
	}
	return out;
}
